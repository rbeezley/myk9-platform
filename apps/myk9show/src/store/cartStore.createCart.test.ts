...
import { CART_OPEN_FAILED_MESSAGE, resetEnsureCartInFlight } from './cartStore.ensureCart';
...
describe('cartStore.ensureCart', () => {
  ...
  it('clears the in-flight entry after a failure so a retry actually re-runs', async () => {
    ...
    expect(retried).toEqual({ kind: 'ready', cart: { id: 'cart-existing', items: [] } });
    expect(loadActiveCart).toHaveBeenCalledTimes(2);
  });

  it('returns failed when nothing exists to recover and the create itself fails', async () => {
  ...
  it('reports failed — never a bare nothing — when the conflict branch finds nothing to recover', async () => {
    behaviour.insertError = {
      code: '23505',
      message:
        'duplicate key value violates unique constraint "entry_carts_active_show_exhibitor_unique_idx"',
    ...
    // This is the exact shape that hung the class step for two review rounds:
    // `createCart` resolves null from its 23505 branch WITHOUT setting an
    // error, so a caller reading `cart === null && error === null` had nothing
    // to render and nothing to retry. The opener must still answer `failed`.
    const created = await useCartStore.getState().createCart(SHOW_ID, EXHIBITOR_ID);
