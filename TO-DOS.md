...
- **Build print template system for run orders, score sheets, and results reports** — Secretary dashboard needs a Print action on pipeline cards to generate printable documents. This is a larger feature requiring a template system that doesn't exist yet. **Problem:** The Print button was removed from ClassPipelineCard (commit `170aaba`) because there was no implementation. Secretaries need to print run orders (list of entries in running order), score sheets (blank scoring forms for judges), check-in sheets, and results reports (final placements/scores). These are standard documents at every dog show trial. **Files:** `apps/myk9show/src/features/pipeline/components/ClassPipelineCard.tsx` (where Print button will be re-added), `apps/myk9q/src/pages/reports/` (myK9Q has existing report implementations to reference), `apps/myk9q/src/pages/scoresheets/` (myK9Q scoresheet templates for inspiration). **Solution:** Design a print template system: (1) survey myK9Q's existing check-in sheets, score sheets, and result reports for layout/data patterns, (2) decide rendering approach (CSS `@media print`, PDF generation via browser, or server-side PDF), (3) create reusable print templates in a shared location, (4) add Print action to ClassPipelineCard that opens print preview, (5) support at minimum: run order, blank score sheet, results report. Consider `@media print` CSS as simplest approach — generate an HTML view and use `window.print()`.
...
- **Verify Live indicator color contrast meets WCAG AA** — `text-green-400` used for "Live" text on dark backgrounds may not meet 4.5:1 contrast ratio. **Problem:** WCAG 1.4.3 requires 4.5:1 for normal text, 3:1 for large text. `text-green-400` (#4ade80) on typical dark backgrounds (~#1a1a2e) is approximately 6.5:1 which likely passes, but on lighter dark backgrounds it may not. Also used on stat chip icons. **Files:** `apps/myk9show/src/features/pipeline/components/PipelineDashboard.tsx:95,106`, `apps/myk9show/src/features/pipeline/components/ClassPipelineColumn.tsx:58`, `apps/myk9show/src/features/pipeline/components/ShowContextRow.tsx:75`, `apps/myk9show/src/features/pipeline/components/TrialContextRow.tsx:80`. **Solution:** Test contrast in browser dev tools against actual rendered background colors. If below 4.5:1, bump to `text-green-300` (#86efac) or ensure background provides sufficient contrast.
