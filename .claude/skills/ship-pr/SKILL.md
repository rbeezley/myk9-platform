...
**Observed Codex workaround (#2136, 2026-09-08).** An escalated detached attempt disappeared without a verdict; its cause was not confirmed. Restarting with the supervising shell kept open produced five completed reviews. This is a verified workaround, not proof of a root-cause fix. Before restarting, use the current wrapper's `--wait 1 "$PR_NUMBER"` against the existing attempt: exit 3 means it is alive, so keep waiting without detaching again; exit 0/1 is a completed verdict to handle, not a reason to restart. For exit 2, inspect this PR's `.logs/claude-review-<pr>.pid` and the referenced PID/token with `kill -0` and `ps -o command= -p <pid>`; a missing or reused PID confirms that attempt ended. If liveness is uncertain or another operator owns the attempt, coordinate with that operator instead of restarting. A fresh `--detach` overwrites the existing attempt's status/output files. A still-live old child can then write its verdict into the replacement's files; never detach again while that child is alive, even if a status file looks completed.

Once the earlier attempt is confirmed ended and a replacement is needed, start the block below using Codex **`exec_command` with `yield_time_ms: 1000` and escalated permissions**. Keep the returned `session_id`, and poll it with `write_stdin` in short calls. The one-second yield returns control without ending the supervising shell; it is not a one-second process timeout. The harness must support a live session of at least 30 minutes without a hard command timeout. If it cannot, use the operator-terminal route instead; do not launch this loop under a foreground timeout. Replace the worktree placeholder below with the verified absolute PR worktree root. The block resolves its own PR number because shell variables do not persist between tool calls. Supervision is bounded to 30 minutes:

```bash
cd "<absolute PR worktree root>" || exit 2
PR_NUMBER="$(gh pr view --json number --jq .number)" || exit 2
review_deadline=$(( $(date +%s) + 1800 ))
...
while true; do
  ...
  if [ "$(date +%s)" -ge "$review_deadline" ]; then
    echo 'Supervision deadline reached; no verdict. Report this attempt and coordinate with the operator before any restart.' >&2
    exit 3
  fi
done
```

Keep the supervising tool session alive; do not run only the detach command and close its shell. The loop waits on the same attempt and exits on its actual result; it never restarts a failed review automatically. Exit 2 and deadline exit 3 remain **no verdict**. At the deadline, stop automated retries and report the exact PR and log path; the supervisor ending does not establish whether the child survived, so re-check its liveness with the operator before any later attempt. If this supervised attempt also dies, report it and use the operator-terminal route above. Do not kill another task's reviewer or substitute same-harness review evidence.
