...
- [ ] **Split/rewrite `apps/myk9show/src/test/e2e/cross-role-workflows.spec.ts`** — Rechecked 2026-05-13: all 13 scenarios fail across entry lifecycle, realtime, search, notifications, offline, accessibility, and error handling. Failures include route aborts, obsolete test IDs (`search-input`, `entry-status-badge`, notification/error controls), and an invalid Playwright assertion (`toHaveCount.toBeGreaterThan`). Do not promote; split into focused current role/workflow specs.
...
- [ ] **Retire or rewrite `apps/myk9show/src/test/e2e/registration/errorHandlingAndRecovery.spec.ts`** — Rechecked 2026-05-13: all 9 scenarios still fail. The suite targets a fake `/shows/show-123/register` flow plus obsolete test IDs (`network-error-boundary`, `dog-card-1`, `search-input`, synthetic retry/conflict controls) that are not present in app code. Do not promote; either replace with focused current UI/service coverage or delete after preserving any unique requirements.
- [ ] **Repair `apps/myk9show/src/test/e2e/registration/exhibitorSelfRegistration.spec.ts`** — Rechecked 2026-05-13: passes alone as current “Online show entry is coming soon” placeholder coverage. Do not promote as a Nightly gate until it covers a real exhibitor registration journey again.
...
- [ ] **Retire or rewrite `apps/myk9show/src/test/e2e/registration/performanceAndCaching.spec.ts`** — Rechecked 2026-05-13: all 10 scenarios fail. The suite targets the obsolete fake `/shows/show-123/register` flow plus synthetic test IDs (`registration-step-dog-selection`, `search-input`, `bulk-selection-toggle`, `dog-card-*`) that are not present in current registration UI. Replace with current instrumentation or delete after preserving unique performance requirements.
- [ ] **Convert `apps/myk9show/src/test/e2e/registration/phase3-2-multi-class-entries.spec.ts` to store/unit coverage** — Rechecked 2026-05-13: useful entry-store scenarios exist, but the Playwright wrapper leaks replicated cache state across reruns and the bulk case takes ~36s for 12 entries. Do not promote to Nightly as E2E; preserve scenarios as Vitest/store tests or split only a tiny stable smoke.
- [ ] **Convert `apps/myk9show/src/test/e2e/registration/phase3-4-entry-limits-waitlists.spec.ts` to service/unit coverage** — Rechecked 2026-05-13: all 7 scenarios fail because browser `page.evaluate` cannot resolve TS aliases such as `@/services/entries/EntryLimitChecker` and `@/store/entryStore`. This is service/store coverage wrapped in Playwright; convert to Vitest or split out only a real UI smoke before promotion.
...
- [ ] **Repair `apps/myk9show/src/test/e2e/registration/secretaryNewUsers.spec.ts`** — Rechecked 2026-05-13: passes alone as current-availability documentation, but do not promote until the secretary account exposes a real new-exhibitor/new-dog creation path.
...
- [ ] **Rewrite `apps/myk9show/src/test/e2e/scoring/scoringWorkflow.spec.ts` against current ringside UI** — Rechecked 2026-05-13: all 8 scenarios fail on obsolete myK9Show scoring/dispute test IDs (`class-name`, `scoring-type-*`, `standard-course-time`, `offline-mode-toggle`, dispute controls). Do not promote; verify whether current coverage belongs in myK9Q first.
...
- [ ] **Repair `apps/myk9show/src/test/e2e/secretary-entry-walk.spec.ts`** — Rechecked 2026-05-13: passes alone, but still logs duplicate-entry `409` and local CORS failure for `send-registration-email`. Do not promote until the flow is clean or those failures are explicitly mocked/expected.
- [ ] **Split/rewrite `apps/myk9show/src/test/e2e/show/showManagement.spec.ts`** — Rechecked 2026-05-13: all 10 scenarios fail on obsolete all-in-one show-management test IDs (`create-new-show-button`, `show-card-1`, `show-trials-tab`, settings/capacity/announcement/analytics controls). Keep out of Nightly; replace with focused current wizard/detail/admin specs.
...
- [ ] **Split/rewrite `apps/myk9show/src/test/e2e/unified-shows-workflows.spec.ts`** — Rechecked 2026-05-13: 22 failed, 2 passed. Guest/exhibitor/secretary/judge/admin/multi-role/tab/loading flows are stale against current routes, auth landing, Base UI tab attributes, and empty/loading states. Only narrow responsive checks passed; do not promote the broad suite. Extract current public shows/tab smoke coverage separately if still useful.
