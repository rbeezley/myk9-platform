...
describe('TabBar', () => {
  it('should render all tabs', () => {
    render(<TabBar tabs={sampleTabs} activeTab="pending" onTabChange={() => {}} />);
  ...
  it('should not display count when count is undefined', () => {
    render(
      <TabBar tabs={[{ id: 'test', label: 'Test' }]} activeTab="test" onTabChange={() => {}} />
    );
  ...
  it('should call onTabChange with tab id when clicked', async () => {
    ...
    render(<TabBar tabs={sampleTabs} activeTab="pending" onTabChange={onTabChange} />);
    await user.click(screen.getByText('Completed'));
    expect(onTabChange).toHaveBeenCalledWith('completed');
  });

  it('should render tab icons when provided', () => {
    const tabs: Tab[] = [{ id: 'tab1', label: 'Tab 1', icon: <span data-testid="icon1">I</span> }];
