...
- Make notification-preference ownership and SMS consent server-authoritative: authenticated clients mutate only through caller-derived RPCs, while SMS evidence remains service-role-only.
...
- `supabase/migrations`: MYK9-191 hardens notification-preference RLS/grants, adds caller-derived preference RPCs, consent write tokens, and a service-only opt-in rate limit; MYK9-193 later adds its delivery marker.
