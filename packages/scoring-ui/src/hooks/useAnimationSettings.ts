/**
 * Animation Settings Hooks
 *
 * Provides animation configuration based on device capabilities,
 * user settings, and current performance metrics.
 *
 * These hooks are designed to be app-agnostic. Apps provide their
 * animation settings through the AnimationSettingsProvider interface.
 */

import { useState, useEffect, useMemo } from 'react';
import { getDeviceTier, type DeviceTier } from '@myk9/core';

/**
 * Animation-related settings that apps can provide
 */
export interface AnimationSettingsInput {
  /** Enable animations (null = auto-detect based on device) */
  enableAnimations: boolean | null;
  /** Enable blur effects (null = auto-detect based on device) */
  enableBlur: boolean | null;
  /** Enable shadow effects (null = auto-detect based on device) */
  enableShadows: boolean | null;
}

/**
 * Provider interface for animation settings
 * Apps implement this to connect their settings store
 */
export interface AnimationSettingsProvider {
  /** Get current animation settings */
  getSettings: () => AnimationSettingsInput;
  /** Optional localStorage key for user performance mode override */
  storageKey?: string;
}
...
const DEFAULT_SETTINGS: AnimationSettingsInput = {
 ...
 *
 * @param provider - Optional settings provider from the app
 */
export function useAnimationSettings(provider?: AnimationSettingsProvider): AnimationConfig {
  const settings = provider?.getSettings() ?? DEFAULT_SETTINGS;
  const [deviceTier, setDeviceTier] = useState<DeviceTier>('medium');
  const [currentFps, setCurrentFps] = useState(60);

  useEffect(() => {
    // Detect device tier
    getDeviceTier(provider?.storageKey).then(setDeviceTier);
    ...
    const MEASUREMENT_DURATION = 2000;
...
export function useAnimationDuration(
  ...
  const config = useAnimationSettings(provider);
 ...
 */
export function useSpringConfig(provider?: AnimationSettingsProvider) {
  const config = useAnimationSettings(provider);
 ...
 */
export function useAnimationClasses(provider?: AnimationSettingsProvider): string {
  const config = useAnimationSettings(provider);
...
/**
 * Create an animation settings provider from a Zustand-style store
 *
 * @example
 * ```typescript
 * // In myK9Q
 ...
 *
 * const provider = createAnimationSettingsProvider(
 ...
 * ```
 */
export function createAnimationSettingsProvider(
  getSettings: () => AnimationSettingsInput,
  storageKey?: string
