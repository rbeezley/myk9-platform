...
- [x] 1.1 Drift check: `git diff --stat 929240192..HEAD -- apps/myk9show/src/pages/secretary/ClassManagementPage.tsx`; if changed, compare current-state excerpts in the source plan to the live code and STOP on mismatch
- [x] 1.2 Confirm whether the judge dropdown optimistically updates local
      state ahead of `onSuccess` (determines whether a revert-on-error is
      needed beyond the toast)

## 2. Judge-assign error surfacing

- [x] 2.1 Mock `sonner` in `ClassManagementPage.judges.test.tsx`
      (`vi.hoisted` + `vi.mock('sonner', ...)`)
- [x] 2.2 Write failing test: `upsertClassJudgeAssignmentMock` rejects → assert
      `toast.error` called (red against current code, no `onError`)
- [x] 2.3 Add `import { toast } from 'sonner';` to `ClassManagementPage.tsx`
- [x] 2.4 Add `onError: () => toast.error('Failed to assign judge. Please try again.')`
      to `assignJudgeMutation`
- [x] 2.5 If 1.2 found optimistic local state, add the revert-on-error logic
- [x] 2.6 Run the test from 2.2 green

## 3. Bulk operation error surfacing + double-fire guard

- [x] 3.1 Write failing test: bulk delete's `deleteClassMutation` rejects →
      assert `toast.error` called (red)
- [x] 3.2 Write failing test: invoking a bulk handler while a bulk mutation
      `isPending` is a no-op — no additional mutations fire (red)
- [x] 3.3 Add per-call `onError` toasts to `handleBulkStatusChange`'s and
      `handleBulkDelete`'s `.mutate()` calls
- [x] 3.4 Add `bulkBusy = updateClassMutation.isPending || deleteClassMutation.isPending`
      and early-return `if (bulkBusy) return;` at the top of both bulk
      handlers
- [x] 3.5 Disable the bulk action buttons while `bulkBusy`
- [x] 3.6 Run the tests from 3.1–3.2 green
