...
vi.mock('@/components/dogs/AddEditRegistrationDialog', () => ({
  AddEditRegistrationDialog: ({ open, onSave, onOpenChange }: { open: boolean; onSave: (data: unknown) => void; onOpenChange: (open: boolean) => void }) => {
    if (!open) return null;
    return (
...
describe('AddDogDialog', () => {
  const mockUsers = [
    ...
    personFactory.build({ id: 'person-2', firstName: 'Jane', lastName: 'Smith', email: 'jane@example.com' })
  ];
  ...
  beforeEach(() => {
    ...
    (useUserStore as ReturnType<typeof vi.fn>).mockImplementation((selector: (state: unknown) => unknown) => {
      const state = {
        people: mockUsers,
        addUser: vi.fn(),
        updateUser: vi.fn(),
