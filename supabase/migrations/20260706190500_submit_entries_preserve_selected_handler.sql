-- Preserve per-entry handler_id supplied by secretary/on-behalf registration.
--
-- The prior submit_show_entries implementation resolved auth.uid() once and
-- wrote that caller's people.id to every inserted entry. For mail-in entries,
-- that made the signed-in secretary appear as the handler in Entry Management
-- even when the wizard selected a different exhibitor/handler. This keeps the
-- existing caller fallback for legacy clients but honors each entry's
-- handler_id when supplied.
