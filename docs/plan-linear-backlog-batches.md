...
| 1G | [MYK9-226](https://linear.app/myk9-platform/issue/MYK9-226) (Medium, evidence PR [#1734](https://github.com/rbeezley/myk9-platform/pull/1734) open) | Current main does not reproduce the payable judge-day-full/no-waitlist line. The fail-closed submit-time refresh landed after the four Aug 18 test-mode incidents in PR #1700, so no production change is justified; retain Error alerts | OpenSpec reproduction and verification evidence only |
...
**Batch 1 progress (2026-08-21):** Lane 1A is complete and MYK9-54 is Done. Its deployed build is PR #1727 / merge commit `b3f4cdd67cd487b24e2f579a94da25bde346bae0`; the private evidence ledger contains desktop and tablet controlled-failure and recovered-state screenshots with checksums recorded in Linear. This satisfies Lane 2D's MYK9-54 dependency; Lane 2D may now run in its required MYK9-232 → MYK9-230 → MYK9-231 order. Lane 1D's implementation merged in PR #1730 / `f3bed32435b5e657f13beb8308b05ab4dd13e624`; its 10-capture authenticated staging matrix passed, but MYK9-57 remains In Progress until Lane 1C merges and the shared `/admin/permissions` closure replay is rerun. Lanes 1B, 1C, 1E, and 1F are implemented in PRs #1736, #1733, #1735, and #1737. Lane 1G's evidence-only PR #1734 records that current main no longer reproduces MYK9-226 and that a new code fix would be unsupported. All five linked Linear issues are In Progress pending CI/review/merge; MYK9-163 and MYK9-225 also retain explicit post-merge browser evidence gates.
