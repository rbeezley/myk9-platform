 ...
 * Renders against the REAL cart store, with the reasons `getClassEntryWindow`
 * produces, so the wording under test is the wording the store records.
 */
...
describe('ClosedClassRemovedNotice', () => {
  ...
  it('names each removed class and its reason, then dismisses', () => {
      ...
      droppedClosedClassItems: [
        {
          itemId: 'item-1',
          dogName: 'Rover',
          className: 'Novice Exterior',
          reason: 'This class was cancelled',
        },
        {
          itemId: 'item-2',
          dogName: 'Rover',
          className: 'Novice Buried',
    ...
    expect(
      screen.getByText('Rover in Novice Exterior: this class was cancelled.')
    ).toBeInTheDocument();
    expect(screen.getByText('Rover in Novice Buried: this class has started.')).toBeInTheDocument();
