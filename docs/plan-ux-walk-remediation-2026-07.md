...
- [ ] **0.G — Entries read-path statement timeout (S2, UI-matrix Critical). [UI-matrix]** During the matrix run an `entries` read query hit Postgres `statement timeout` → HTTP 500 on 7 secretary surfaces, surfacing a raw vendor toast ("canceling statement due to statement timeout"). Investigate the query (missing index? unbounded join? shared-dev contention?) and whether it shares a root cause with 0.A's write-path timeout and the staging OCC/CPU storm. Verdict gates 5.A (toast copy) and 2.C(6) (partial-failure rendering, S3).

### Testing (Phase 0)

- [ ] 0.A, 0.D, and 0.G close with their remaining questions answered in this section; each confirmed code defect gets its failing-then-passing regression test in the dependent phase task (4.H write-guard test; 1.C cart-reconciliation tests; 4.A enrollment no-409 test).

### Exit criteria

All seven verdicts final; dependent tasks (1.C, 1.F, 2.C, 2.G, 4.A, 4.H, 5.A) re-checked against verdicts before implementation.
