...
> **Remediation status (2026-07-10, PR #1256, openspec: account-page-ux-remediation):**
> **Fixed** — theme apply-on-save (single source of truth incl. System mode; legacy settingsStore kept in sync), font size (rem-root scaling, Small removed to preserve the 14px text-xs floor), Devices/mock-device removal (plus dead PreferencesDialog + preferences-local SyncStatusIndicator), sync/cache strategy radios, presence toggles, toast auto-dismiss (4s), flash banner overlay (no nav reflow), honest install-app copy, delete type-to-confirm + real delete call with surfaced server errors, inline password validation.
> **Deferred** — self-service delete RPC (server-side; spun off as its own task — the old button was a fake delete that only signed out), push-toggle permission feedback, email-change flow clarity, General-tab consolidation, impact-badge explanations, re-auth before password change, Export/Import testing.
