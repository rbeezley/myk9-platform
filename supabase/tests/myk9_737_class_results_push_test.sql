...
LANGUAGE sql AS $$
  SELECT EXISTS (SELECT 1 FROM jsonb_array_elements(p_health -> 'sample') s
                 WHERE s ->> 'class_name' = p_class AND s ->> 'status' = p_status)
$$;
...
-- The retry cron's liveness facts: whether it is scheduled and active, its
-- last successful run, and its last status. The 15-minute threshold is the
-- TS check's (classResultsPushChecks.test.ts); this pins the facts it reads.
DO $$
DECLARE
  v_jobid bigint;
  ...
  v_retry jsonb;
BEGIN
  SELECT jobid INTO v_jobid FROM cron.job WHERE jobname = 'class-results-push-retry';
  IF v_jobid IS NULL THEN
    -- The CI database has no push_webhook_secret, so the migration did not
    -- schedule it; schedule it inside this rolled-back transaction.
    v_jobid := cron.schedule('class-results-push-retry', '*/5 * * * *', 'select 1');
    v_created := true;
  END IF;

  INSERT INTO cron.job_run_details (jobid, runid, job_pid, database, username, command, status, return_message, start_time, end_time)
  VALUES (v_jobid, 737000001, 0, current_database(), current_user, 'select 1', 'succeeded', '1 row',
          now() - interval '20 minutes 1 second', now() - interval '20 minutes');

  SET LOCAL ROLE service_role;
  v_retry := public.class_results_push_health() -> 'retry_job';
  RESET ROLE;
  IF (v_retry ->> 'scheduled')::boolean IS NOT TRUE
     OR (v_retry ->> 'active')::boolean IS NOT TRUE
     OR (v_retry ->> 'last_success_at')::timestamptz > now() - interval '19 minutes'
     OR v_retry ->> 'last_status' IS DISTINCT FROM 'succeeded' THEN
    RAISE EXCEPTION 'FAIL retry_job facts are wrong for a job last run 20 minutes ago: %', v_retry;
  END IF;

  INSERT INTO cron.job_run_details (jobid, runid, job_pid, database, username, command, status, return_message, start_time, end_time)
  VALUES (v_jobid, 737000002, 0, current_database(), current_user, 'select 1', 'failed', 'ERROR: boom',
          now() - interval '1 minute', now() - interval '59 seconds');
  SET LOCAL ROLE service_role;
  v_retry := public.class_results_push_health() -> 'retry_job';
  RESET ROLE;
  IF v_retry ->> 'last_status' IS DISTINCT FROM 'failed'
     OR v_retry ->> 'last_message' NOT LIKE 'ERROR: boom%'
     OR (v_retry ->> 'last_success_at')::timestamptz > now() - interval '19 minutes' THEN
    RAISE EXCEPTION 'FAIL retry_job did not report the failed latest run: %', v_retry;
  END IF;

  -- Leave the wiring arm's view of cron.job as the migration left it.
  DELETE FROM cron.job_run_details WHERE runid IN (737000001, 737000002);
  IF v_created THEN
