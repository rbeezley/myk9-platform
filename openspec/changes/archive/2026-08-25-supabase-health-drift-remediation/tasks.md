...
- [x] 4.4 Obtain explicit merge approval, merge the PR, and confirm the exact merge SHA on current `main`.
...
- [x] 5.4 Obtain one Linear batch approval, then attach closure evidence to exact issues/findings without creating duplicates.

## 6. Archive And Cleanup

- [x] 6.1 Update the audit evidence ledger and automation memory only after each proof gate passes.
- [x] 6.2 Archive the OpenSpec change after the implementation PR is merged and all required deployment evidence is recorded.
- [x] 6.3 Sync `main`, prune merged branches, and remove the feature worktree as the final cleanup step.
