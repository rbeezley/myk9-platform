...
- Model the other actors that write to a class while it is being scored: a steward or secretary checking dogs in, and a secretary editing the class itself. Five distinct paths take the class-row lock, and modelling only exhibitor check-in understates concurrency on the one row that serializes. These actors must target classes that are concurrently being scored, since the contention is the property under test.
