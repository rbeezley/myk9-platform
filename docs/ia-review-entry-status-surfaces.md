...
**Broader inventory:** a grep for entry-state render sites (`entry_status`/`entryStatus`/`withdrawn`/`refund`/`payment_status`) surfaces **~25 components** that display per-entry state (At-Show pages, Trial entries table, TV display, dog activity tab, move-up/pull/waitlist tabs, entry receipt/edit dialogs, scoring/results cards). The full list is the Phase B sweep target; it is _not_ reproduced here because the IA finding is about the shared logic they all call, not their individual placement.

**Orphan routes:** none confirmed among the three core surfaces — all have incoming nav (see Step 4). Two _candidate_ legacy pages flagged during the sweep (`ResultEntryDashboard`, scoring `EntryPanel`) need verification before any action; out of scope for this review.
...
**Verdict:** Completable, but the two exhibitor surfaces **disagreed on the same fact** — the worst IA failure mode (the user can't trust either). Both specific symptoms are now fixed (#735/#800/#822), but the _structural_ cause remains (Step 4).
...
**Verdict:** Completable. The secretary surface is the **only** one reading ground-truth refund columns; the exhibitor surfaces _infer_ refund from `payment_status` (Step 4, F3).
...
**Conclusion:** the IA _structure_ passes the mental-model test. The model breaks only on **consistency of the rendered fact**, which is a data-layer problem wearing an IA costume.

---

## Step 4: Duplication & Orphan Scan

The duplication is **not** task-level (you don't do the same job in three places — each surface serves a different role/scope). It is **logic-level**: the same display fact is _derived_ in multiple independent places.
...
**Top to fix next:** F1 + F4 (Critical), then F2 + F3 (High) — all four resolve with the _same_ move (one shared selector).
...
| **C** _(optional)_ | Decide the page-vs-tab question (F5): keep the tab as a full render, or make it a summary that deep-links to `/exhibitor/entries?show=` per CLAUDE.md "fast path = link" | Owner decision | One documented canonical surface per scope                                                                                                        | 0–1      |
