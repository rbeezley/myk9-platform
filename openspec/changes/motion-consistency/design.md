...
**D4 — Ringside toast: Tailwind-on-markup (ringside idiom), parent-owned timer. [UPDATED at implementation]** Implementation confirmed ringside's `styles/index.css` deliberately emits **only `@tailwind base`** (no component/utility layer — a duplicate utility layer once clobbered the host's responsive classes by source order), so every ringside component styles itself with Tailwind utilities on the markup, generated by the host's Tailwind scan of `packages/ringside/src` (verified in the host `content` globs). Both legacy semantic-class leaves (`.success-toast`, `.floating-done-button`) were left **unstyled** by that migration — neither class has a rule anywhere. The idiomatic, footgun-proof fix is therefore **Tailwind utilities on the toast markup**, not a bespoke `.success-toast` rule: `fixed inset-x-0 mx-auto w-fit` bottom-center, `bg-success`/`text-success-foreground` (status vocabulary per DESIGN.md), fade + rise via `transition duration-enter ease-enter` toggling `opacity`/`translate-y` (element stays mounted so it animates both IN and OUT), reduced-motion via the `motion-reduce:transition-none` variant. This dogfoods the Phase-2 tokens. The #432 footgun D4 originally hedged against is already mitigated (ringside is in the content globs). The auto-dismiss timer is a small `useAutoDismiss` hook (owns a `useRef` timeout): clears on unmount, resets on rapid consecutive saves — replacing the fire-and-forget `setTimeout(() => setShowSuccessMessage(false), 2000)` in `CombinedEntryListPage`. Acceptance is the rendered toast in preview/`/at-show`, not the diff. (`FloatingDoneButton`'s identical unstyled bug is noted but out of this change's scope.)
...
**D6 — Reorder motion with a hard virtualization guard. [RESOLVED at implementation]** Concrete target = `components/classes/ClassResultsTable/EntryCardGrid.tsx`, a **non-virtualized** card grid with stable `entryId` keys — each card wrapped in `motion.div layout="position"` (position-only FLIP, `duration-layout` 350ms + ease-enter curve), gated on Framer's `useReducedMotion` via the pure, unit-tested `reorderLayoutMode()` helper (`false` when reduced-motion → instant reposition). The results **table** view (`DndTableView`/`SortableRow`) is deliberately NOT given Framer `layout` (unreliable on `<tr>`), and virtualized lists (react-window: `VirtualScrollList`, `EntitySidebar`, `DogSelectionStepEnhanced`) are excluded by the guard. **Ringside reorder: verified `@dnd-kit` already provides it** — `SortableEntryCard` applies `useSortable`'s `transform` + `transition` to the node (no change needed, and `packages/ringside` stays framer-free per the non-goal). Broad data-driven reorder animation on the results table + ringside non-drag reorder is intentionally left as a separate incremental follow-up (the plan marks Phase 5 reorder as incremental).
