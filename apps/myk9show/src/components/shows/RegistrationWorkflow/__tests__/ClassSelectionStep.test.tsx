...
describe('ClassSelectionStep — add-only entry actions (6.4)', () => {
  ...
  it('(i) a cart that failed to open says so and offers a retry, instead of inert chips (MYK9-581)', async () => {
    ...
    //
    // `error: null` is the load-bearing half: the store's error flag is NOT
    // set on every failing path (`createCart`'s 23505 branch resolves null
    // without one), so the alert has to come from the opener's own result or it
    // is back to a silent hang.
    const ensureCart = vi
      .fn()
      .mockResolvedValue({ kind: 'failed', error: 'entries reconcile read failed' });
    mockCartStore({ cart: null, error: null, ensureCart });
