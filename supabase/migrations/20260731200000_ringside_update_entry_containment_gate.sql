-- MYK9-115: RS429 admission-control gate in ringside_update_entry.
-- Design: docs/superpowers/specs/2026-07-30-ringside-occ-admission-control-design.md
-- Plan:   docs/plan-ringside-occ-admission-control.md (Task 2)
--
-- While public.ringside_containment.state = 'contained', a CONFLICTING call sleeps
-- backpressure_ms and raises SQLSTATE RS429 ('Ringside scoring contained; retries
-- paused') instead of 40001. The authoritative current version is still in DETAIL,
-- so the contract that clients depend on is unchanged; HINT carries retry_after=60.
-- Version-CORRECT calls never take this path — a judge whose device is in step with
-- the server keeps scoring at full speed no matter what the breaker is doing. That
-- blast-radius property is the whole reason the gate lives at the conflict sites and
-- nowhere else. Do not hoist it to the top of the function.
--
-- CATCH-UP MIGRATION — see the header of 20260731190000_ringside_containment_state.sql.
-- This function body already exists in the live project; the plan's SQL was applied
-- directly without committing the file. `create or replace` makes re-applying a no-op.
--
-- Rebuilt from 20260712101000_authorize_ringside_occ_conflicts.sql, the LATEST
-- migration defining this function (verified with
--   grep -l "CREATE OR REPLACE FUNCTION public.ringside_update_entry" supabase/migrations/
-- ) and diffed against the live pg_get_functiondef output. Rebuilding from an older
-- file silently reverts the authorization work in 20260712101000 and the OCC precheck
-- ordering in 20260711150000 — neither of which typecheck or unit tests can catch.
--
-- Grants on ringside_update_entry itself are deliberately unchanged by this work.
...
BEGIN
  ...
  IF v_updated_id IS NULL THEN
    ...
    IF FOUND THEN
      PERFORM nextval('public.ringside_conflict_seq');
      -- Same containment gate at the late TOCTOU conflict site (mirrors step 5).
      SELECT * INTO v_containment FROM public.ringside_containment;
      IF FOUND AND v_containment.state = 'contained' THEN
        PERFORM pg_sleep(v_containment.backpressure_ms / 1000.0);
        RAISE EXCEPTION 'Ringside scoring contained; retries paused'
