...
| --- | --- | --- | --- | --- | --- | --- |
| Generated Supabase types are divergent, not just duplicated | `packages/supabase/src/database.types.ts`, `packages/supabase/src/types/database.types.ts`, `packages/supabase/src/types.ts`, `apps/myk9show/src/types/supabase.ts` | P1 | Line counts differed: 5,204 / 7,742 / 7,550 / 9,052; hashes differed. App-local type had surfaces missing from package-exported type. Package export and app client imported different files. Type generation path/docs disagreed. | Phase-2 confirmed; Wave B slice 1 implemented | `packages/supabase/src/types/database.types.ts` is now the canonical generated file; the other three files are compatibility re-exports; package exports include generated helper aliases and `Constants`; generation docs/scripts point to the canonical path. | Duplication removed for this slice; remaining Wave B duplication clusters stay open below. |
| Replication/PostgREST read-shape duplication | `entries/reads.ts`, `classes/reads.ts`, `dogs/reads.ts`, `trials/reads.ts` | P2 | Each maintained replicated read, manual join mapping, and PostgREST select fallback; existing helpers only covered fallback/map construction. | Phase-2 confirmed; Wave B read-shape parity slice implemented | Narrow helpers now cover replicated lookup loading, immutable sort parity, and `{ data, error }` fallback envelopes; focused tests pin high-risk ordering parity. | Avoid building a broad ORM; continue watching future replicated reads for drift. |
