...
- [ ] 2.2 Verify watchdog schedule ordering, exact alert fields, query plan/index use, deduplication, recurrence after resolution, and rollback SQL with focused source tests plus a rolled-back database transaction when available. (Source checks and non-executing live `EXPLAIN` are complete; the approval-gated write proof must run as the `cron.job.username` owner, not `service_role`, and the first scheduled run must be checked in `cron.job_run_details`.)
...
- [ ] 10.0a Add failing focused tests for the portable Resend helper covering first-attempt success, 429 plus `Retry-After`, 503 exponential fallback, network exception recovery, immediate non-408 4xx return, three-attempt exhaustion, HTTP-date parsing, 2,000 ms clamping, hashed fallback idempotency, and explicit-key preservation; record RED before implementation.
- [ ] 10.0b Implement byte-identical `resendEmail.ts` helpers in both Supabase deployment roots and make the focused helper tests GREEN without logging request bodies, authorization headers, recipient addresses, Auth token hashes, or generated idempotency material.
...
- [ ] 10.0d Add a repository source-contract test that proves the helper mirrors are byte-identical, every direct sender imports a helper, and no production file outside the two helpers contains a raw `fetch` to the Resend emails endpoint. Add or update focused caller tests for final-failure logging/status and no duplicate business action after retry.
- [ ] 10.0e Run focused Vitest suites, app and repository typecheck/lint, both Supabase-root bundle checks, OpenSpec strict validation, and a second-opinion Edge/security review; stop a hanging runner after 60 seconds and record any limitation.
- [ ] 10.0f [SHARED-SYSTEM GATE] After PR review/merge and explicit approval, deploy every affected Edge Function with `--no-verify-jwt`, then run one controlled transient-failure smoke or provider-supported test plus ordinary auth/registration/alert smokes without sending bulk production mail.
