...
- **2026-07-06 update:** Reproduced on isolated `origin/main` `2ed8f0ba8c009961a5540e4591137079281aecba` with `PLAYWRIGHT_BASE_URL=http://127.0.0.1:6107`. Phase 1 Vitest passed after bootstrapping the isolated worktree (`18/18`). The exact Phase 2 active Playwright command failed within budget with `48 passed, 4 failed, 1 skipped (3.9m, --retries=0)`. Initial failures were `cross-role-workflows.spec.ts` waiting for stale `Browse Shows` link copy while the current exhibitor sidebar exposes `Find Shows`; `registration/exhibitorSelfRegistration.spec.ts` expecting immediate `/cart` navigation even though the visible cart button already showed `1`; `secretary-entry-walk.spec.ts` using stale payment/receipt copy and a non-late mail-in path after the seeded show's entry close; and `uat/secretary/disposable-entry.spec.ts` losing a Base UI menu item during click after re-render. Low-risk test repairs updated those four surfaces. Focused proof passed with `--retries=0`: `cross-role-workflows.spec.ts --grep "exhibitor can land"` `1/1`, `exhibitorSelfRegistration.spec.ts` `1/1`, `secretary-entry-walk.spec.ts` `1/1`, and `disposable-entry.spec.ts` `1/1`. Standalone Phase 3 route-health also passed all role groups (`6/6`, public/exhibitor/secretary/judge/club-admin/admin). Keep open because the exact Phase 2 command was not rerun after the focused repairs; a first multi-spec focused replay also exposed a new public `/shows/browse` navigation timeout before the repaired specs completed.
