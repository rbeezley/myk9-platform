...
test.describe('Cross-Browser Performance Tests', () => {
  ...
  test.describe('Page Load Performance', () => {
    ['chromium', 'firefox', 'webkit'].forEach(browserName => {
      test(`homepage load performance in ${browserName}`, async ({
        page,
        ...
        const [response] = await Promise.all([
          page.waitForResponse(
            response => response.url().includes('localhost:5173') && response.status() === 200
          ),
        ...
        const threshold =
          browserThresholds[browserName as keyof typeof browserThresholds] ||
    ...
    test('dashboard load performance comparison', async ({ page, browserName }) => {
      ...
      const maxLoadTime = browserName === 'webkit' ? 5000 : browserName === 'firefox' ? 4000 : 3000;
  ...
  test.describe('JavaScript Execution Performance', () => {
    test('form interaction performance', async ({ page, browserName }) => {
      ...
      await expect(page.locator('[data-testid="dog-call-name"]')).toHaveValue(
        'Performance Test Dog'
      );
    ...
    test('table rendering performance', async ({ page, browserName }) => {
      ...
      const maxRenderTime =
        browserName === 'webkit' ? 2500 : browserName === 'firefox' ? 2000 : 1500;
  ...
  test.describe('Network Performance', () => {
    ...
    test('concurrent request handling', async ({ page, browserName }) => {
      ...
      const maxConcurrentTime =
        browserName === 'webkit' ? 4000 : browserName === 'firefox' ? 3000 : 2500;
