...
State the computed ISO week and chosen registry at the top of the report, put the registry in the walk show's name (`ZZ Walk <run token> <registry> - teardown me`), and add a registry column to the coverage table so a registry that was not walked is a visible gap. Every task below runs on that registry's walk show; the registry-specific checks are marked in tasks 1, 4 and 7. MYK9-448 and MYK9-447 are the shape of defect this rotation exists to catch: a registry's form missing from its own show's catalog, and a registry-specific field never set.
...
7. **Waitlist and classes** — capacity cards, class judge display. **Registry:** the classes offered on this week's walk show must be that registry's own — AKC Scent Work: Container, Interior, Exterior, Buried, Handler Discrimination, Detective × Novice, Advanced, Excellent, Master; UKC Nosework: Container, Interior, Exterior, Vehicle, Handler Discrimination × Novice, Advanced, Superior, Master, Elite; ASCA Scent Detection: Container, Interior, Exterior, Vehicle × Novice, Open, Advanced, Excellent. An AKC element (Buried, Detective) or AKC level offered on a UKC or ASCA show is a P1.
...
- **MYK9-448** (Done) — on a UKC week, the UKC Nosework Trial Report must be in the catalog on the UKC walk show. **MYK9-447** (Done) — on a UKC week, the downloaded UKC entry form must tick the permanent-registration box for a permanently registered dog. Both closed; confirm they have not recurred on the week that can see them.
