-- =============================================================================
-- MYK9-245: keep an exhibitor's own paid entry visible after a show is dropped.
--
-- The show soft-delete cascades `deleted_at` to its entries.  The authenticated
-- entry-results view previously removed every tombstone, which left My Payments
-- with a charge whose corresponding entry had disappeared.  The existing
-- `is_own_entry` handler/owner arms are intentionally unchanged; only those
-- callers may see their own tombstoned row.  Managers, judges, stewards, and
-- ringside claims retain the prior live-entry predicate.
--
-- `deleted_at` is the cancellation marker consumed by the exhibitor entry
-- surface.  Payment/refund columns remain subject to the existing
-- `can_view_admin` mask, so the visible row reconciles the existing charge
-- without adding a duplicate payment UI or exposing another user's entry.
-- =============================================================================
...
COMMENT ON VIEW public.view_authenticated_entry_results IS
  ...
  'Soft-deleted entries remain visible only to the owning exhibitor so paid '
  'entries can reconcile against My Payments; the deleted_at value marks the '
  'entry cancelled. The view remains security_invoker = false.';
