...
describe('TrialConfigurationStep existing snapshot state', () => {
  ...
  it('uses the latest existing-trial snapshot when creating a trial', async () => {
    ...
    expect(wizardState.addTrial).toHaveBeenCalledWith(
      expect.objectContaining({ name: 'Saturday Trial 2', autoGeneratedName: true })
    );
  });

  it('updates an auto-generated name when its trial moves to another day', async () => {
    const user = userEvent.setup();
    wizardState.trials = [
      {
        id: 'draft-trial',
    ...
    render(
      <TrialConfigurationStep
        existingTrials={[{ name: 'Sunday Custom Trial', trialDate: '2026-08-02' }]}
      />
    );
    await user.click(
