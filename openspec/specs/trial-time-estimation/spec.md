# trial-time-estimation Specification

## Purpose

How the class-selection surfaces estimate judging time. The estimate is only honest once a trial has entries, so it is hidden until current entry counts are loaded and nonzero, and it is computed from those counts (expected entries x the template's minutes per run) so it moves as entries are added or withdrawn.
