    ...
    <div className="bg-blue-50 border border-blue-100 rounded-lg p-4 mb-3 flex flex-col relative">
      <div className="flex justify-between items-start">
        <div>
          ...
          <div className="text-sm text-gray-500 mb-1">
            {formatDateMMDDYYYY(show.startDate)} • {show.location}
          </div>
        </div>
      ...
      <div className="flex flex-wrap gap-2 mt-2 mb-1">
        {show.events.map((event, i) => (
          <span
            key={i}
            className={`px-2 py-0.5 rounded text-xs font-semibold ${event === 'Conformation' ? 'bg-blue-100 text-blue-800' : event === 'Obedience' ? 'bg-purple-100 text-purple-800' : 'bg-gray-100 text-gray-700'}`}
          >
            {event}
          </span>
        ))}
