...
describe('DogsService', () => {
  ...
  describe('getAll', () => {
    ...
    it('should handle Supabase data source', async () => {
      ...
      // Service transforms rows via mapDogRowToDog; assert key fields only
      expect(dogs[0]).toEqual(
        expect.objectContaining({ id: '1', name: 'Test Dog', breed: 'Test Breed' })
      );
  ...
  describe('getById', () => {
    ...
    it('should handle Supabase data source', async () => {
      ...
      // Service transforms rows; assert key fields only
      expect(dog).toEqual(
        expect.objectContaining({ id: '1', name: 'Test Dog', breed: 'Test Breed' })
      );
      expect(mockSupabase.from).toHaveBeenCalledWith('dogs');
  ...
  describe('create', () => {
    ...
    it('should throw error when Supabase create fails', async () => {
      ...
      await expect(DogsService.create(newDogData)).rejects.toThrow(
        'Failed to create dog: Validation error'
      );
    });
  });
  ...
  describe('update', () => {
    ...
    it('should throw error for non-existent dog in mock data', async () => {
      await expect(DogsService.update('non-existent', updateData)).rejects.toThrow('Dog not found');
    });
    ...
    it('should handle Supabase data source', async () => {
      ...
      // Service transforms rows; assert key fields only
      expect(dog).toEqual(
        expect.objectContaining({ id: '1', name: updateData.name, breed: 'Original Breed' })
      );
      expect(mockSupabase.from).toHaveBeenCalledWith('dogs');
    });
    ...
    it('should throw error when Supabase update fails', async () => {
      ...
      await expect(DogsService.update('1', updateData)).rejects.toThrow(
        'Failed to update dog: Update failed'
      );
    });
  });
  ...
  describe('getByOwnerId', () => {
    ...
    it('should handle Supabase data source', async () => {
      ...
      // Service transforms owner_id → ownerId via mapDogRowToDog; assert key fields only
      expect(dogs[0]).toEqual(
        expect.objectContaining({ id: '1', name: 'Dog 1', ownerId: 'owner-1' })
      );
      expect(dogs[1]).toEqual(
        expect.objectContaining({ id: '2', name: 'Dog 2', ownerId: 'owner-1' })
      );
      expect(mockSupabase.from).toHaveBeenCalledWith('dogs');
    });
    ...
    it('should throw error when Supabase getByOwnerId fails', async () => {
      ...
      await expect(DogsService.getByOwnerId('owner-1')).rejects.toThrow(
        'Failed to fetch dogs by owner: Query failed'
      );
    });
  });
