...
| Entry Form Template | `docs/AKC-forms/SW-EntryForm.pdf` | Reports -> AKC Scent Work Entry Form | Not wired | The app renders an in-app printable entry form using richer entry data, but does not yet fill the official AKC PDF. Decide whether official output should be one PDF per dog, one combined packet, or both. |
| Judge's Score Sheet | `docs/AKC-forms/SW-Scoresheet.pdf` | Reports -> Score Sheet | Not wired | The app renders an operational score sheet, but does not yet fill the official AKC score sheet PDF. Audit PDF fields and decide whether output is class-scoped, judge-scoped, or trial-scoped. |
...
1. **Entry Form Template official PDF fill.** Highest secretary value before the show and closest to the old Access/mySWT workflow. Use the existing `useEntryFormData` data shape as the domain source, but decide packet behavior before wiring UI.
2. **Judge's Score Sheet official PDF fill.** Important for show-day paper fallback. Start with one selected class because that matches existing Reports filters and limits PDF complexity.
