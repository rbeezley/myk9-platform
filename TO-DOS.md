...
- [x] **Add per-show branding (club logo, accent color, cover image)** — Implemented in PR #7 (2026-03-14). Wired existing branding infrastructure end-to-end: Branding tab on club profile (admin-only, accent color picker, logo/cover, live preview, Save/Discard), club→show branding fallback in queries/mapper, accent color bars on all show cards/lists, PublicShowView uses ShowBrandedHero. 8 tests added (mapper fallback + BrandingTab). **Spec:** `docs/superpowers/specs/2026-03-13-club-branding-design.md`. Show-level branding overrides deferred to future work.
...
- [x] **Integrate Resend for transactional exhibitor emails** — Implemented in PR #5 (commit `6286cf14`). Created 3 Edge Functions (`send-auth-email`, `send-registration-email`, `resend-webhook`), migration 061 (`email_log` table) + 062 (insert policy), email status UI in entry management with resend button, signup confirmation screen. Reminder, announcement, and results emails deferred to future work.
