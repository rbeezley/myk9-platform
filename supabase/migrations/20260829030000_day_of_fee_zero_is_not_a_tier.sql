-- =============================================================================
-- A blank Day-of-Show Fee is not a $0 tier (audit finding F33).
--
-- `submit_show_entries` is authoritative for what an entry costs. Its fee
-- expression took `day_of_show_fee` whenever the show had started, and COALESCE
-- only skips NULL -- but leaving the field blank in the creation wizard stores
-- "0.00". So from the start date onward every entry was written with
-- entry_fee = 0 and the enrollment with total_amount = 0.
--
-- The client had already been corrected, which made this worse rather than
-- visible: the Payment step and the receipt showed $30.00 while the database
-- recorded nothing. The fee-mismatch guard below could not catch it either --
-- it raises only when the client sends LESS than the server requires, so
-- 3000 < 0 is false and the correct client figure was silently discarded.
--
-- Downstream this is the Financial Report, closeout reconciliation ("At-show
-- collected $0.00") and any payout maths.
--
-- Rebuilt from the LIVE definition (pg_get_functiondef) rather than from an
-- older migration file, so no intervening change is reverted. The only edit is
-- the fee expression.
-- =============================================================================
...
BEGIN
  ...
  LOOP
    ...
    v_server_fee := COALESCE(
      CASE
        WHEN v_show_start IS NOT NULL
             AND CURRENT_DATE >= v_show_start
