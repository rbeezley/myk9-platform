...
25. **Decided:** Central-table check-in remains show-day work rather than a permanent Entry Management column. Promote the existing Show Desk `People at show` capability into a prominent `Check-in desk` entry point with focused search, person-level `Check in all eligible` behavior, individual Entry/Class exceptions, and attention deep links. Entry Management links to that canonical surface instead of duplicating its controls.
26. **Decided:** The staff Check-in desk supports Secretaries and show-scoped Stewards through the existing staff check-in mutation path. Exhibitor self-check-in remains on `/at-show` and uses the ownership-restricted self-check-in path. A shared unattended kiosk is a separate future design because it requires a locked presentation and privacy-safe identity lookup rather than access to the staff roster.
27. **Decided:** `Check in all eligible` checks every accepted Entry handled by that person for the current show day across all of that day's Trials. The person can be expanded first to exclude or handle an individual Entry/Class; future-day and otherwise ineligible Entries are not changed.
28. **Decided:** The desktop canonical queue has only Selection, Registration, Entries, Review, Payment, and Next action columns. Registration combines Exhibitor, confirmation, and submitted time; Entries summarizes Dogs and Entry/Class counts. Per-Entry Handler and detailed Class data remain searchable and appear in the focused registration rather than permanent columns.
29. **Decided:** The focused registration keeps one stable hierarchy: Registration header, Primary work, Entries, Payment, then Communication and history. The active queue or deep link opens and emphasizes the relevant section and affected Entries without rearranging the pane.
30. **Decided:** Group focused-registration Entries by Dog. Each collapsible Dog header shows Call name, show-wide Armband number, and Entry count; each child Entry row shows Trial/date, Class, its own Handler, status, and next action. The group containing an active search match or attention item opens automatically.
31. **Decided:** Each child Entry row uses one clickable status control, similar to Linear, that offers only valid transitions. Required reasons or notes appear after a transition is chosen. A separate primary action appears only for work beyond a status change; edit, payment, Armband, removal, and other secondary operations remain in overflow.
32. **Decided:** Validate the redesign with an interactive desktop/tablet prototype before production changes. Deliver production work as coordinated slices: shared floating selection toolbar, promoted Check-in desk with authorized Steward access, then the Entry Management cockpit and deletion of superseded legacy views.
33. **Decided after prototype review:** Use Variant A, the balanced queue-and-focused-registration cockpit, as the production direction because it matches Show Desk and gives Secretaries one consistent interaction model. When available content width cannot support both panes, preserve the focused registration through an explicit queue-to-full-width-detail transition rather than silently hiding it.
...
The production cockpit implements these resolved decisions. Verification evidence lives in `openspec/changes/entry-management-cockpit/tasks.md`.
