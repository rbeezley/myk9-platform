...
2. **A read path for removed people comes first.** `people_select` is `deleted_at IS NULL AND auth.uid() IS NOT NULL` (migration `20260602010000`), so `/people/:id` cannot load a soft-deleted person for any role.

   **No new RPC was needed.** `get_deleted_people()` (migration `20260616140000`) already exists — admin-gated, `SECURITY DEFINER`, `REVOKE`d from PUBLIC, granted to `authenticated` — and returns `SETOF public.people`. It is what Deleted Items already lists from. The record page reads one row out of it rather than adding a second definer function answering the same question, so **Phase C carries no migration and needs no deploy.** If the removed set ever stops being small, `get_deleted_person(uuid)` is the drop-in and only `getDeletedUserById` changes.
...
- Every phase is client-side against existing services. Phase C was expected to carry a migration; it did not — the admin-gated read it needed was already deployed as `get_deleted_people()` (see its scope).
...
| 2026-08-02 | Phase C implemented. The plan's central Phase C assumption was wrong: `get_deleted_people()` (20260616140000) already provided the admin-gated read, so no migration, no deploy, and no grant audit were required. Phase A's removed-row profile link and Phase B's inert removed row are both reverted — their reason (the page could only bounce) no longer holds. | This session |
