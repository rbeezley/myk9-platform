...
**In-flight check:** `pnpm qa:inflight` — exit 1 means an open PR, another worktree, or a recent unmerged branch already touches what this branch changes. Stale local-branch inventory is counted in the output and can be expanded with `--verbose`; exit 2 means it could not decide (no paths, or `gh`/git failed) and is not a pass. Do not open a second PR on it; coordinate first (see `/commit` Step 0).
