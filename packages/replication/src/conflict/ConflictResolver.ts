...
export class ConflictResolver {
    ...
    resolveLWW<T extends { updated_at?: string; id?: string }>(
        ...
        if (localTime > remoteTime) {
            this.logger.log(`[ConflictResolver] LWW: local wins (${localTimestamp} > ${remoteTimestamp})`);
            return {
        ...
        if (remoteTime > localTime) {
            this.logger.log(`[ConflictResolver] LWW: remote wins (${remoteTimestamp} > ${localTimestamp})`);
            return {
                resolvedEntity: remote,
                strategy: 'last-write-wins',
                hadConflict: true,
        ...
        if (localTimestamp !== remoteTimestamp) {
            ...
            if (stringComparison > 0) {
                return { resolvedEntity: local, strategy: 'last-write-wins', hadConflict: true, automatic: true };
            }
            if (stringComparison < 0) {
                return { resolvedEntity: remote, strategy: 'last-write-wins', hadConflict: true, automatic: true };
            }
        }
    ...
    resolveServerAuthoritative<T>(_local: T, remote: T): ConflictResolutionResult<T> {
        return { resolvedEntity: remote, strategy: 'server-authoritative', hadConflict: true, automatic: true };
    }

    resolveClientAuthoritative<T>(local: T, _remote: T): ConflictResolutionResult<T> {
        return { resolvedEntity: local, strategy: 'client-authoritative', hadConflict: true, automatic: true };
    }
    ...
    resolveFieldLevel<T extends Record<string, any>>(
        ...
        for (const field of authority.clientFields) {
            if (field in local) {
                ...
                if (localValue !== remoteValue && !this.isUndefinedOrNull(localValue) && !this.isUndefinedOrNull(remoteValue)) {
    ...
    resolve<T extends Record<string, any>>(
        ...
        switch (strategy) {
            case 'last-write-wins': return this.resolveLWW(local, remote);
            case 'server-authoritative': return this.resolveServerAuthoritative(local, remote);
            case 'client-authoritative': return this.resolveClientAuthoritative(local, remote);
            ...
            default: return this.resolveLWW(local, remote);
        }
    }
   ...
   */
    ...
    findConflictingFields<T extends Record<string, any>>(
        ...
        for (const key of allKeys) {
            ...
            if (!this.isUndefinedOrNull(localValue) && !this.isUndefinedOrNull(remoteValue) && localValue !== remoteValue) {
