...
- **The demo show** in this prompt is `dededede-...010`; the walk reads it and never mutates it. Its 63-entry classes (the PDF calibration show below) exist only while `supabase/seed-load-fixture.sql` is applied on top of `supabase/seed-demo.sql` (MYK9-558); a plain reseed leaves the lean set, where the demo show has 12 entries and no class holds more than a handful. Check before calibrating: `select count(*) from entries where show_id = 'dededede-0000-0000-0000-000000000010'` returns 516 with the load fixture and 12 without.
