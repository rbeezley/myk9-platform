...
export const StatusCell: React.FC<Omit<EditableCellProps, 'onKeyDown'>> = ({
  ...
  return (
    <div className="space-y-1">
      ...
      {errors.filter(err => err.field === 'status').map((error, errIndex) => (
...
export const TimeCell: React.FC<EditableCellProps> = ({
  ...
  const updateTimeComponent = (component: 'minutes' | 'seconds' | 'hundredths', newValue: string) => {
  ...
  return (
    <div className="space-y-1">
      ...
      {errors.filter(err => err.field === 'time').map((error, errIndex) => (
        <div key={errIndex} className="text-xs text-red-600">
          {error.message}
        </div>
      ))}
  ...
  return (
    <div className="space-y-1">
      <Input
        ...
        data-field="score"
      />
      {errors.filter(err => err.field === 'score').map((error, errIndex) => (
        <div key={errIndex} className="text-xs text-red-600">
          {error.message}
        </div>
      ))}
...
export const PlacementCell: React.FC<EditableCellProps> = ({
  ...
  return (
    <div className="space-y-1">
      ...
      />
      {errors.filter(err => err.field === 'placement').map((error, errIndex) => (
        <div key={errIndex} className="text-xs text-red-600">
          {error.message}
        </div>
      ))}
