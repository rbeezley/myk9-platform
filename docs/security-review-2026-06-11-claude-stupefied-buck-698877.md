...
`is_show_manager()` = `is_site_admin() OR is_trial_secretary() OR has_role('club_admin')` — all three sub-helpers are `SECURITY DEFINER STABLE`, check `auth.uid()`, and respect `is_active = true AND (expires_at IS NULL OR expires_at > now())`. So **suspended/expired staff roles do not grant access**, and there is no privilege-escalation surface (the function only reads role state; it grants nothing and takes no user input). Uncorrelated `(SELECT …)` call sites are InitPlans — a positive side effect for performance, neutral for security.
