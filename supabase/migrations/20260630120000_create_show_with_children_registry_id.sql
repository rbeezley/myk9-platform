-- create_show_with_children: persist trials.registry_id.
--
-- The atomic online show-creation path (saveShowAtomicOnline → this RPC) is the
-- PRIMARY new-show path, but its trials INSERT never listed registry_id, so every
-- online-created trial fell to the column default 'AKC' — a UKC/ASCA show's trials
-- were silently AKC. (The offline / edit-mode store path was fixed separately in
-- the write-path Phase 1 client change; this closes the atomic-RPC gap.)
--
-- Fix: add registry_id to the trials INSERT, reading p_trials[].registry_id (which
-- the client now derives from the show's organization) and defaulting to 'AKC' so
-- an absent value never violates the NOT-NULL column. Only the trials INSERT
-- changes; the rest of the function is reproduced verbatim (CREATE OR REPLACE
-- requires the full body). Signature unchanged; EXECUTE grant re-asserted.
...
COMMENT ON FUNCTION public.create_show_with_children(jsonb, jsonb, jsonb, uuid[]) IS
  'Atomically creates a show, trials (incl. registry_id derived from the show organization), classes, and CLASS-LEVEL judge assignments (one judge_assignments row per class that carries p_classes[].judge_id). Accepts p_show.style for premium experience style.';
