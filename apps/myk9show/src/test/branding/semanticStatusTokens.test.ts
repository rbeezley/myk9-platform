...
describe('semantic status token foundation', () => {
  ...
  describe('index.css — light values in :root', () => {
    ...
    it('defines --warning as an RGB triplet that passes WCAG AA as text', () => {
      // amber-700 (180 83 9): ~5.0:1 as text on card, ~5.0:1 white-on-bg.
      // amber-600 (217 119 6) failed AA as small text (~3.2:1) — see a11y smoke
      // secretary-dashboard fix (text-warning on UpcomingCard).
    ...
    it('defines --destructive as an RGB triplet that passes WCAG AA as text', () => {
      // red-600 (220 38 38): 4.83:1 as text on card AND white-on-bg; unified
      // with dark mode. red-500 (239 68 68) failed AA both ways (~3.76:1) —
      // see a11y smoke secretary-dashboard fix (text-destructive).
