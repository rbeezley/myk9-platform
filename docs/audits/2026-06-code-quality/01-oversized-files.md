...
| --- | --- | --- | --- | --- | --- | --- |
| `packages/replication/src/MutationManager.ts`, `packages/replication/src/core/ReplicatedTable.ts`, `apps/myk9show/src/providers/ReplicationSyncProvider.tsx`, sync/offline helpers | P2 | Multiple concerns | Queueing, backup, OCC, upload, conflict, and toast/provider orchestration are mixed. | Phase-2 confirmed; Wave C slices 1-3 implemented; see `09-phase-2-verification.md` | Mutation ordering, queue capacity, localStorage backup, retry-state, OCC empty-update, and conflict row-state helpers are extracted with focused tests; continue with table/provider helpers. | High launch priority: offline-first reliability. |
