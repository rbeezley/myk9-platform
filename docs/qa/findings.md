...
- **Notes:** Closed 2026-06-18. Root cause was the public landing header overflowing at 375px: the persistent `Sign in` ghost link extended past the viewport when `Browse shows` stayed visible for mobile discovery. Fixed by giving the sign-in link a scoped `l-signin-btn` class and hiding it below 480px while preserving the mobile `Browse shows` path. Route-health now includes overflow-source diagnostics, which identified `Sign in` as the overflowing element. Proof: `route-health-by-role.spec.ts --grep "Route health: public"` passed on exported isolated port `5857` (`1 passed, 14.3s`), and full standalone route-health later passed the public group with `0px` overflow.
