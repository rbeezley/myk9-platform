...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  // Get user with roles - for development, use mock users
  const userWithRoles = React.useMemo((): UserWithRoles | null => {
    ...
    // In development, try to match by email to mock users
    const mockUser = Object.values(MOCK_USERS).find(u => u.email === auth.user?.email);
    if (mockUser) {
      return mockUser;
    }
    
    // Default to exhibitor role for unknown users
    return {
      ...auth.user,
