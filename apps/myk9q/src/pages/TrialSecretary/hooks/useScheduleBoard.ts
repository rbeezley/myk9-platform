  ...
  useEffect(() => {
    async function loadScheduleData() {
      ...
      try {
        ...
        const [rolesResult, volunteersResult, classAssignResult, generalAssignResult] =
          await Promise.all([
        ...
        if (hasSupabaseData) {
          ...
          if (stored) {
            ...
            interface LegacyGeneralAssignment {
              id: string;
              roleId: string;
              volunteerId?: string;
              volunteerIds?: string[];
            ...
            const localGeneralAssignments = (state.generalAssignments || []).map(
              (a: LegacyGeneralAssignment) => {
            ...
            migrateToSupabase(
              ...
              localRoles,
              localVolunteers,
              localClassAssignments,
              localGeneralAssignments
            )
              .then(() => {
    ...
    async (id: string, updates: Partial<Volunteer>) => {
      ...
      withSyncTracking(async () => {
        try {
          ...
          if (updates.exhibitorId !== undefined)
            dbUpdates.exhibitor_id = updates.exhibitorId || null;
          if (updates.enteredClassIds !== undefined)
            dbUpdates.entered_class_ids = updates.enteredClassIds;
