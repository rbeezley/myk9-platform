...
- [ ] **Eliminate redundant `sport_type` column** — `trialType` (human label like "Scent Work") and `sportType` (DB code like "akc-scent-work") encode the same information. `deriveSportType(org, trialType)` can convert on the fly. Refactor scoring pages to derive at render time instead of reading `trial.sportType`, then drop the `sport_type` column. Touches: `ScoresheetPage`, `SecretaryScoringPage`, `ReplicatedTrialsTable`, `trialStore`, `wizardStore`.
