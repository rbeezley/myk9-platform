...
class NavigationPerformanceTracker {
  ...
  endNavigation(route: string, isFromCache: boolean = false) {
    ...
    if (import.meta.env.DEV) {
      ...
      logger.debug(
        `${color} ${status} Navigation to ${route}: ${loadTime.toFixed(2)}ms`,
        'hooks',
        {}
      );
      ...
      if (loadTime > 5000) {
        logger.warn(
          `🐌 Slow navigation detected: ${loadTime.toFixed(2)}ms to ${route}`,
          'hooks',
          {}
        );
      }
  ...
  getAverageLoadTime(route?: string): number {
    const relevantMetrics = route ? this.metrics.filter(m => m.route === route) : this.metrics;
