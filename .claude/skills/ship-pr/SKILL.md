...
**A replacement needs coordination first.** Before any new detach, identify the original attempt's PR, owning task/operator, and worktree. Poll it from that **original worktree** with a wrapper that includes #2132's liveness checks and `--wait 1`. If that branch predates #2132, invoke the absolute path to the wrapper in a checkout updated to main, while keeping `workdir` set to the original worktree: the default state directory follows the working directory, not the script path. Alternatively set `CLAUDE_REVIEW_STATE_DIR` explicitly to the original attempt's absolute `.logs` directory. Another worktree's missing state files prove nothing. Exit 3 means keep waiting, and exit 0/1 is a verdict to handle. Exit 2 is no verdict, not permission to restart. Have the original owner confirm the child has ended before starting a replacement; if process checks are denied or uncertain, stop and ask Richard to inspect it. A successful `kill -0` with empty/denied `ps` output is not evidence of death. A fresh detach overwrites the attempt's files, and an old live child could write its verdict into the replacement's state.

At the 25-minute wait limit, report exit 3 as **no verdict**, with the PR and original log path, and stop automated retries. Ending the supervising session does not prove the child survived or died; coordinate with the owner before any later attempt. Never kill another task's reviewer or substitute same-harness review evidence.
