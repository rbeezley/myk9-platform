# entry-confirmation-idempotency Specification

## Purpose
At-most-one confirmation email per entry across the webhook send path and the scheduled send-confirmation-email sender: the webhook stamps confirmation_email_* with the scheduled sender's semantics so its audience query excludes already-confirmed entries; failed sends leave the scheduled sender as the retry path. Introduced by money-path-hardening-remainder (MP-13).
## Requirements
