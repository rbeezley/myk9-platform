...
- **Enhance Entry Management page with trial/class filters + inline scoring** — Instead of unifying three separate entry tables, make the existing Entry Management page (`/secretary/entries`) the secretary's command center. Add cascading trial/class filter dropdowns. When filtered to a class, enable inline scoring (reuse `useClassResults` hook and scoring columns from ClassResultsTable). When filtered to a trial, show read-only roster with scoring progress. Detail page entry tabs remain unchanged for browsing/discovery. Design: `docs/plans/2026-03-29-entry-management-scoring-design.md`.
