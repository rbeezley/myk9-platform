...
export class StorageEstimator {
  ...
  estimateRoleStorage(
    role: UserRole,
    customScopes?: SyncScope
  ): { sizeKB: number; sizeMB: number } {
  ...
  async getStorageBreakdown(): Promise<
    Record<string, { count: number; sizeKB: number; sizeMB: number }>
  > {
    // eslint-disable-next-line @typescript-eslint/no-require-imports
  ...
  async getOptimizationRecommendations(): Promise<string[]> {
    ...
    const sortedBySize = Object.entries(breakdown).sort(([, a], [, b]) => b.sizeMB - a.sizeMB);
