...
| MYK9-563                     | Split: 563 keeps items 2, 3, 5, 6, 7 (all in `entries/search.ts`, the four hooks, two receipt components). New child issues: item 1 (offline-durable `personId`, touches AuthContext) and item 4 (`useShowEntriesForUser` divergence). |
