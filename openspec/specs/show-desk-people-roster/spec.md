...
Capture the Show Desk `People at show` roster behavior for staff lookup,
direct class-row check-in, and deep links into canonical messaging and entry
management surfaces.
...
The system SHALL keep the People at show roster entry facts consistent with the
Show Desk entry, Show Map, and closeout summaries for the same show scope.
