-- =============================================================================
-- Migration 20260704201000: keep at-show co-owner access queue-only.
--
-- Follow-up to 20260704200000_at_show_exhibitor_queue_read.sql. That migration
-- correctly admitted co-owners to show queue rows via is_show_exhibitor, but it
-- also changed is_own_entry to include dog co-owners. Since can_view_admin is
-- `can_manage OR is_own_entry`, that temporarily widened payment/refund/Stripe/
-- confirmation-email visibility beyond the registrant/handler + primary owner.
--
-- Fix: keep co-owners in the show-exhibitor row gate for at-show awareness, but
-- keep is_own_entry/admin visibility to handler + primary owner only. Keep least
-- privilege:
...
--   * Co-owner-at-show rows expose operational queue identity/status, not payment,
--     refund, Stripe, comp, confirmation email, judge notes, or unreleased scores,
--     unless the co-owner is also the handler, manager, or primary owner.
-- =============================================================================
