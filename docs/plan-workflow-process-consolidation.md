...
| `verify` vs `verify-plan`                                                             | Similar names but different purposes — `verify` exercises runtime behavior of a code change; `verify-plan` checks an implementation _plan's_ completeness before code is written. Naming is close enough to risk a wrong pick, but not a content duplicate — no merge needed, just watch for it.                   |
...
| `codebase-health` (project) vs `engineering:tech-debt` (plugin) | Was deferred in Phase 3a. Project skill already consolidated three former skills (code-quality-audit, hotspots, improve) and is myK9-specific (churn hotspots, static-debt drift for _this_ repo). Plugin is namespaced (`engineering:tech-debt`), so no bare-name collision. | **KEEP both.** A project-specific health skill outranks a generic plugin for this repo's audits, and the namespace prevents invocation ambiguity. No merge. |
...
## Phase 6 — Single-source skill trees + reconciled shipping rules — DONE 2026-09-05

Codex's 2026-09-05 process review found `ship-pr`, `cleanup`, `commit`, `harden`, `audit-pages` and `qa-feature` diverged between `.claude/skills/` (Claude Code) and `.agents/skills/` (Codex), each side carrying improvements the other lacked, and `ship-pr` encoding the exact lapse PLAYBOOK § 4 bans (a same-harness subagent as the review gate, which is how #2040 merged mid-review). The Phase 3a note "don't symlink-dedupe" is retired: Codex resolves skill directories through symlinks (verified with a `codex exec` probe), and the repo already used them the other way for eight skills.

1. Every skill present in more than one of `.claude/skills`, `.agents/skills`, `.codex/skills` is now ONE real directory plus symlinks; `apps/myk9show/src/test/ci/skillTrees.test.ts` enumerates the trees from disk and fails on a second real copy or a broken link.
