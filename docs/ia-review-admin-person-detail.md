...
**Duplicate-purpose routes:** none at the _route_ level. The duplication in this surface is **route vs. non-route** — a person's detail exists both as a page (`/people/:id`) and as a panel (`UserEditPanel`), which is exactly the "modal/inline duplication" case the methodology flags.
...
**Verdict:** Completable with friction. Suspension is a _lifecycle_ action wearing a _profile field_ costume.
...
`/people/:id` cannot answer, because it cannot load them at all: `people_select` is
`deleted_at IS NULL AND auth.uid() IS NOT NULL` (migration `20260602010000`), so a
soft-deleted person is invisible to **every** role, site admin included. The page
hangs or bounces rather than saying "this person was removed."

**Verdict:** Broken. Corrected during Phase A implementation — the original audit
guessed the failure mode was a missing banner; it is a missing read path.
...
| -------------------------- | ------------------------------------------------------------------------------------------------------ |
| `/admin/users`             | Find, bulk ops, Change (via panel), Remove, _partial_ Lifecycle (suspend, hidden inside the edit form) |
...
**Key structural fact:** the profile page and the roster are _not_ two implementations of an editor. `UserDetailsDialogs.tsx:66` mounts the **same `UserEditPanel`**. There is one editor and two shells. The genuine gap is the **read** view: the panel shows form fields; the page shows dogs, clubs, judge cards, invitation state.
...
| ---------------- | ------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Open a person    | Row click → panel; menu "Edit user" → panel; menu "Open profile page" → `/people/:id` | Split by _intent_: row click = **read** (`/people/:id`), menu "Edit user" = **write** (panel). Keep both; stop pointing two affordances at the same panel. |
...
| F3  | `/people/:id` cannot load a soft-deleted person at all (`people_select` is `deleted_at IS NULL`), so no surface shows a removed person's record | 2    | 3         | 5        | 4                | **12** | **Critical**                   |
...
| F8  | Three paths to role assignment (panel, dialog, `/admin/permissions/users`)                                                                      | 4    | 3         | 3        | 4                | 10     | High _(deferred — own review)_ |
...
| **C** | F3 + F5 — one delete dialog for both shells; a **read path for removed people** (admin-gated RPC, mirroring `restore_person`) plus the removed-state banner with Restore on `/people/:id` | Phase B merged | Both shells open `AdminDeleteUserDialog`; a soft-deleted person's page loads, states it, and offers Restore | 1–2      |
...
3. **F3** — no surface in the app can show a removed person's record: `people_select` hides `deleted_at IS NOT NULL` rows from every role, site admin included. Phase C needs a read path before it can need a banner. **Critical**
