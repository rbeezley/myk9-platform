...
- [ ] **Show creation wizard cannot choose premium list style** — Found during the 2026-05-10 premium style show QA walk. Premium style is only editable after creation in the show edit Experience tab, so secretaries must create the show, reopen it, then set the style. Needs a style selector in the show wizard that writes `shows.landing_style` consistently with `ShowEditPremiumTab.tsx`, defaulting to `default` when unset.
- [ ] **Fallback AKC Scent Work template rules fetch 400s** — During premium fixture creation, the wizard logged failed requests for `sport_class_rules?sport_template_id=eq.akc-scent-work-official-2024`. Shows/classes still save, but scoring rule metadata may be missing when the local fallback template id is absent in Supabase. Audit `sport_templates`, `sport_class_rules`, and the local AKC fallback path before changing data or code.
- [ ] **Show overview judges card can render `?` after wizard assignment** — Monogram fixture classes were assigned to Test Judge, but the show overview Judges summary displayed `?`. Trace `create_show_with_children` / class judge assignment persistence through the show-detail overview query so secretary-facing setup status shows the actual judge name.
