-- Migration 065: Update RBAC functions to respect user_roles.is_active
-- Without this, soft-deactivated roles still grant permissions via RPC functions.

-- Recreate get_user_permissions with is_active filter
...
BEGIN
  ...
  WHERE pe.auth_user_id = get_user_permissions.user_id
    ...
    AND (
      ...
      OR (filter_scope_type = 'global' AND ur.show_id IS NULL AND ur.club_id IS NULL)
    );
END;
$$;

-- Recreate user_has_permission with is_active filter
  ...
  SELECT EXISTS (
    ...
    WHERE pe.auth_user_id = user_has_permission.user_id
      ...
      AND (
        ...
        OR (permission_name LIKE '%:create' AND p.code = REPLACE(permission_name, ':create', ':manage'))
        OR (permission_name LIKE '%:read' AND p.code = REPLACE(permission_name, ':read', ':manage'))
        OR (permission_name LIKE '%:update' AND p.code = REPLACE(permission_name, ':update', ':manage'))
        OR (permission_name LIKE '%:delete' AND p.code = REPLACE(permission_name, ':delete', ':manage'))
      )
      AND (
        scope_type IS NULL
...
-- Recreate get_user_roles to read is_active from column instead of computing from expires_at
...
-- Recreate get_effective_permissions with is_active filter
