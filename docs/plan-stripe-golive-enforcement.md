...
**Implementation status (2026-07-13):** merged in [#1314](https://github.com/rbeezley/myk9-platform/pull/1314), migration deployed, functions live, and controlled Stripe test-mode smoke passed. `20260713110000_waitlist_offer_payment_guard.sql` is applied; `stripe-payment-link` is ACTIVE v13 (16:04 UTC) and `decline-waitlist-offer` ACTIVE v1. Both reject unauthenticated no-op POSTs with 401. A controlled `cs_test_` Checkout payment reconciled entry/link/offer/order and a separate controlled decline closed its link without an order or payment intent; only the paid claim consumed judge-day capacity. The dynamic-method follow-up merged in [#1317](https://github.com/rbeezley/myk9-platform/pull/1317), was deployed to v13, and a fresh sandbox Checkout visibly offered Apple Pay, Link, Klarna, Amazon Pay, Cash App Pay, and Bank alongside card; a Link-saved `4242` test card reconciled the promoted entry, paid link, accepted offer, and succeeded order.
