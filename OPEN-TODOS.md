...
- [ ] **Show-map tree extensions for show-day use** — Discrete enhancements to `ShowMapTab` so it can serve as the spine of the Today and Wrap-up tabs. Each sub-item is independently shippable.
  ...
  - [ ] **Class completion guardrails** — Before class-status quick actions become the primary wrap-up path, hide or confirm `Mark Class Complete` when unresolved scoring/signature work remains. The v1 action intentionally uses the routine one-tap mutation path; this follow-up decides whether incomplete score progress, unsigned sheets, or registry wrap-up attention should block or warn.
