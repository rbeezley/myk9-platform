...
Comprehensive Postgres performance optimization guide for developers using Supabase and Postgres. Contains performance rules across 11 categories, prioritized by impact from critical (query performance, connection management) to incremental (advanced features). Includes Supabase-specific patterns for Realtime, Storage, Edge Functions, and Auth, plus migration best practices and RLS testing/debugging guides. Each rule includes detailed explanations, incorrect vs. correct SQL examples, query plan analysis, and specific performance metrics to guide automated optimization and code generation.
...
9. [Migrations](#migrations) - **HIGH** *(New in v2.0)*
   - 9.1 [Zero-Downtime Schema Migrations](#91-zero-downtime-schema-migrations)
   - 9.2 [Data Backfill Strategies](#92-data-backfill-strategies)
   - 9.3 [Migration Rollback Patterns](#93-migration-rollback-patterns)

10. [Supabase Patterns](#supabase-patterns) - **HIGH** *(New in v2.0)*
    - 10.1 [Realtime Subscription Optimization](#101-realtime-subscription-optimization)
    - 10.2 [Storage Bucket Policies](#102-storage-bucket-policies)
    - 10.3 [Edge Functions Database Access](#103-edge-functions-database-access)
    - 10.4 [Auth Triggers for User Data](#104-auth-triggers-for-user-data)

11. [RLS Testing & Debugging](#rls-testing--debugging) - **HIGH** *(New in v2.0)*
    - 11.1 [Testing RLS Policies Locally](#111-testing-rls-policies-locally)
    - 11.2 [Debugging RLS Policies](#112-debugging-rls-policies)
    - 11.3 [Common RLS Pitfalls](#113-common-rls-pitfalls)
...
Zero-downtime schema changes, data backfills, and rollback strategies for production databases.

### 9.1 Zero-Downtime Schema Migrations
...
```sql
-- Updates all rows in one transaction - will timeout or lock table
update orders set shipping_address_id = (
  select id from addresses where addresses.user_id = orders.user_id
);
```
...
```sql
create or replace function backfill_shipping_addresses(batch_size int default 1000)
...
begin
  loop
    ...
    update orders o set shipping_address_id = batch.address_id
  ...
  select count(*) into remaining from orders where shipping_address_id is null;
  return next;
end;
$$;

-- Run backfill (can be interrupted and resumed)
select * from backfill_shipping_addresses(1000);
```
...
### 9.3 Migration Rollback Patterns
...
-- Create sync trigger during transition
...
$$;
create trigger sync_names before update on users for each row execute function sync_user_names();

-- Phase 2: Deploy application using full_name

-- Phase 3: Contract (remove old after stable)
...
Supabase-specific patterns for Realtime, Storage, Edge Functions, and Auth integration.

### 10.1 Realtime Subscription Optimization
...
```sql
-- Ensure realtime is enabled and indexed
...
```sql
-- Bucket with no policies = no access, or overly permissive = security risk
create policy "Anyone can do anything" on storage.objects for all using (true);
```
...
create policy "Users manage own avatar" on storage.objects for update, delete
to authenticated
...
### 10.3 Edge Functions Database Access
...
Deno.serve(async (req) => {
  // Reuse the client
  const { data } = await supabase.from('users').select('id, email').limit(100);
  return new Response(JSON.stringify(data));
});

// For direct Postgres, use transaction mode pooler (port 6543)
...
### 10.4 Auth Triggers for User Data

**Impact: HIGH (Proper user data initialization, maintain data integrity)**

**Incorrect (manual sync with race conditions):**

```typescript
const { data: authData } = await supabase.auth.signUp({ email, password });
// Race condition: what if this fails? User exists but profile doesn't
await supabase.from('profiles').insert({ id: authData.user.id, email });
```
...
## 11. RLS Testing & Debugging

**Impact: HIGH**

Testing, debugging, and avoiding common pitfalls with Row-Level Security policies.

### 11.1 Testing RLS Policies Locally
...
select * from orders;  -- Test user's view
select test_reset();
```
...
```sql
select tablename, policyname, permissive, roles, cmd, qual as using_expr, with_check
...
```sql
-- Step 1: Check data exists (as superuser)
reset role;
select count(*) from orders where user_id = 'target-user-id';

-- Step 2: Check auth context
select current_user, (select auth.uid()) as auth_uid;

-- Step 3: Manually evaluate policy
select o.*, (o.user_id = auth.uid()) as passes_policy
from orders o where o.user_id = 'target-user-id';

-- Step 4: Check for restrictive policies
select * from pg_policies where tablename = 'orders' and permissive = 'RESTRICTIVE';
```
...
### 11.3 Common RLS Pitfalls

**Impact: CRITICAL (Avoid security vulnerabilities and performance issues)**

**Pitfall 1: RLS not enabled**
```sql
-- Policies exist but RLS not enabled = WIDE OPEN!
alter table user_data enable row level security;
alter table user_data force row level security;
```

**Pitfall 2: Missing policy for command**
```sql
-- Only SELECT policy exists, INSERT fails for everyone
create policy "Users insert own" on orders for insert with check (user_id = auth.uid());
```

**Pitfall 3: Function called per row (performance)**
```sql
-- WRONG: auth.uid() called for EVERY row
using (user_id = auth.uid());

-- CORRECT: Wrap in subquery to cache
using (user_id = (select auth.uid()));
```

**Pitfall 4: Data leak through related tables**
```sql
-- orders has RLS, but order_items doesn't - attacker can JOIN!
...
**Pitfall 5: USING vs WITH CHECK confusion**
```sql
-- USING: filters existing rows (SELECT, UPDATE existing, DELETE)
-- WITH CHECK: validates new data (INSERT, UPDATE new values)
create policy "correct_insert" on orders for insert with check (user_id = auth.uid());
```

**RLS Security Checklist:**
- [ ] RLS enabled on all sensitive tables
- [ ] RLS forced (for table owner enforcement)
- [ ] Policies for all needed CRUD operations
- [ ] Related tables also have RLS
- [ ] Subqueries wrapped for caching
- [ ] Indexes on policy columns
- [ ] WITH CHECK used for INSERT/UPDATE
