...
async function sweepGroup(page: Page, group: SweepGroup, theme: 'light' | 'dark') {
  ...
  for (const route of group.routes) {
    ...
    const summary = probe
      ? `contrast=${probe.totals.contrast} targets=${probe.totals.targets} names=${probe.totals.names} measured=${probe.measured} unmeasurable=${probe.unmeasurable} sanity=${probe.sanity.blackOnWhite}/${probe.sanity.whiteOnWhite}/${probe.sanity.greyOnWhite}`
      : `error=${error}`;
    // The console IS the live progress view for a findings run.
