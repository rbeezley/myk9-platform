...
export class OfflineManager {
  ...
  private setupEventListeners(): void {
    ...
    window.addEventListener('storage', () => this.updateStorageEstimate());
    window.addEventListener('beforeunload', () => savePendingOperations(this.pendingOperations));
  }

  private handleOnline(): void {
    logger.debug('Browser online event', 'offline');
    this.doConnectivityCheck().then(online => {
      if (online) this.updateNetworkState(true);
    });
  }
  ...
  private handleVisibilityChange(): void {
    if (document.visibilityState === 'visible') {
      this.doConnectivityCheck().then(online => {
        ...
        if (online && this.hasPendingOperations()) this.syncPendingOperations();
      });
    }
  }
  ...
  private startMonitoring(): void {
    ...
    if (this.config.enableAutoSync) this.startAutoSync();
  }

  private startNetworkMonitoring(): void {
    if (this.pingTimer) clearInterval(this.pingTimer);
    this.pingTimer = setInterval(async () => {
      const online = await this.doConnectivityCheck();
      this.updateNetworkState(online);
    }, this.config.pingInterval);
  }

  private async doConnectivityCheck(): Promise<boolean> {
    return checkConnectivity(this.config.pingUrl, this.config.pingTimeout);
  }

  private startStorageMonitoring(): void {
    if (this.storageMonitorTimer) clearInterval(this.storageMonitorTimer);
    this.storageMonitorTimer = setInterval(() => this.updateStorageEstimate(), 60000);
    this.updateStorageEstimate();
  }

  private async updateStorageEstimate(): Promise<void> {
    try {
      const est = await buildStorageEstimate(this.config.maxOfflineStorage);
      this.state.storageUsed = est.used;
      this.state.storageAvailable = est.available;
      if (est.percentage > this.config.storageWarningThreshold) {
        this.emitEvent({
          type: 'storage-warning',
          timestamp: new Date(),
          details: { storageEstimate: est },
        });
      }
    } catch (error) {
  ...
  private updateNetworkState(isOnline: boolean): void {
    ...
    if (isOnline) {
      ...
      if (!wasOnline) this.handleConnectionRestored();
    } else {
  ...
  private async handleConnectionRestored(): Promise<void> {
    ...
    if (this.offlineStartTime) {
      ...
      this.metrics.totalOfflineTime += duration;
    ...
    if (this.config.enableAutoSync) await this.syncPendingOperations();
  }
  ...
  private startAutoSync(): void {
    ...
    this.syncTimer = setInterval(() => {
      if (this.state.isOnline && this.hasPendingOperations()) this.syncPendingOperations();
    }, interval);
  }
  ...
  // ── Queue & draft operations (public API) ────────────────────────
  ...
  createDraft(entityType: string, entityId: string, data: Record<string, unknown>): string {
    if (!this.config.enableDraftMode) throw new Error('Draft mode is disabled');
    ...
    saveDrafts(this.draftOperations);
    ...
    try {
      ...
      for (const item of batch) {
        try {
          ...
          if (isConflictError(error as Error)) await this.handleSyncConflict(item);
        }
      }
      ...
      this.emitEvent({
        type: 'sync-completed',
        timestamp: new Date(),
        details: { syncedItems: batch.length, remainingItems: syncQueue.size() },
      });
    } catch (error) {
      ...
      errorMonitor.captureError(error as Error, {
        additionalData: { pendingOperations: pendingItems.length, isOnline: this.state.isOnline },
      });
    } finally {
  ...
  private async handleSyncConflict(item: SyncQueueItem): Promise<void> {
    ...
    if (conflict) {
      ...
      this.emitEvent({
        ...
        timestamp: new Date(),
        details: { conflictId: conflict.id, entityType: item.entityType, entityId: item.entityId },
      });
    }
  }
    ...
    if (listeners) {
      listeners.forEach(listener => {
        try {
          ...
          logger.error(
            'Error in offline event listener',
            'offline',
  ...
  async forceSyncNow(): Promise<void> {
    if (!this.state.isOnline) throw new Error('Cannot sync while offline');
  ...
  updateConfig(updates: Partial<OfflineManagerConfig>): void {
    ...
    if (updates.pingInterval) this.startNetworkMonitoring();
    if (updates.syncInterval && this.config.enableAutoSync) this.startAutoSync();
  }
