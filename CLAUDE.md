...
3. **Before merging**: scrutiny scales with risk — `pnpm qa:review-tier --base origin/main` prints the floor (and the triggering file) for `independent` / `adversarial` / `none`; full path list in `docs/PLAYBOOK.md` § 4, authoritative source `scripts/qa/review-tier.ts`. `scripts/qa/post-review-gate.sh` is the only writer of a `Review gate:` line. **`independent`** — the OTHER harness reviews (Codex for Claude-authored, Claude for Codex-authored; same-harness never suffices), via `pnpm qa:codex-review` (`scripts/qa/codex-review.sh`: `--base origin/main`; exit 2 `GATE DID NOT RUN` on abort, 1 on findings, 0 only when clean; Codex runs the Claude half detached, `claude-review.sh --detach` + `--wait 240`, never under `timeout`). Verdict: a PR comment's FIRST line, exactly `Review gate: <codex|claude> reviewed <base>..<head> — no findings` or `… — <N> findings, all addressed`; `.github/workflows/review-gate.yml` turns it into the required `Review gate` status pinned to that head SHA — any later push turns it red until a new review is recorded. A review that finishes after the merge is an audit, not a gate. **`adversarial`** — 2+ same-harness lenses told to find bugs, not approve; verdict `<N> lenses, all findings addressed` (`<N>` >= 2). (#1536: two clean subagent rounds missed a P1 Codex caught) — weaker than `independent`. **`none`** — verdict exactly `low-risk paths, CI green`, no log. **`owner` override** — harness unavailable: an OWNER/MEMBER may defer scrutiny with verdict `override, floor was independent`/`override, floor was adversarial` (claimed floor checked against the real one; a mismatch is refused), plus `Override reason: <harness> unavailable — <detail>` and `Deferred re-review: MYK9-<n>` — see [`docs/PLAYBOOK.md`](docs/PLAYBOOK.md) § 4. **Convergence — when to stop patching.** The gate re-runs on every new head with no stop condition. Stop and restructure at the SECOND finding on the same path across rounds, or when findings describe code the PREVIOUS fix introduced. Both mean the design is wrong: ask whether to delete the feature before guarding it (LESSONS `discriminator-branches`). Read severity, not count (#2180, #2210). Orchestrated changes ship one PR per batch and review after the structural batch, not only at the end ([`docs/lessons/README.md#orchestrated-change-rounds`](docs/lessons/README.md#orchestrated-change-rounds)). On stopping, post the restructure proposal instead of another patch.
