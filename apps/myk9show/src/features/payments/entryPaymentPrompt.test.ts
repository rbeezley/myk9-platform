...
const pending = (paymentMethod: PaymentMethod | undefined, totalFee = 30) =>
...
describe('getEntryPaymentPrompt (4.C — cash/check is a status, not a debt)', () => {
  it('online card balance → the Finish Payment action', () => {
    expect(pending('credit_card')).toEqual({ kind: 'finish-online' });
  });

  it('unknown method defaults to the online action so a real balance is never hidden', () => {
  ...
  it.each(['secretary_paid', 'group_payment', 'waived'] as const)(
