...
- [ ] **PREREQ: Add `data-node-id` + `data-node-type` attributes to show-map rows** — `ShowMapStructureTable.tsx` renders plain `<li>`+`<div>` rows with no semantic attributes. This blocks the row-action menu work (can't identify which node was right-clicked), blocks Playwright row-level selectors, and prevents the Attention filter from being scriptable. Cheap, isolated, must land before "inline row actions" work begins. Add `data-node-id={node.id}` and `data-node-type={node.type}` to the outer row `<div>` for show/trial/class/entry node types.
- [ ] **Unify attention computation across dashboard + show-map** — The dashboard's "Needs Attention" strip lists Heritage as "81 entries pending review", but the same show on the Show List tab shows "0 Need Attention." Two divergent implementations of the same concept against the same data. **Must be consolidated before the Next Best Action card or smart Recommended menu ship**, otherwise we add a fourth and fifth divergent attention surface. Extract a single `getAttention(state)` (or richer `getRankedActions(scope, state)` per the shared-priority-function todo above) and route all surfaces through it.
- [ ] **Entry count inconsistency: tabs say "Entries 80", Show List tile says "81 Entries"** — Same Heritage show, same screen, two different numbers. Investigate whether the discrepancy is filter-related (one count includes a soft-deleted/cancelled entry, the other doesn't) or a stale-cache issue. Trust-eroding for the secretary.
...
- [ ] **Rename "Show List" UI label back to "Show Map"** — Code, file names, types, and tests all use "ShowMap" (`ShowMapTab.tsx`, `showMapTree.ts`, `ShowMapStructureTable.tsx`, `showMapTypes.ts`). The UI tab label was changed to "Show List" at some point, which diverges from the code's vocabulary and undersells the hierarchical structure (`Map` matches the show → trials → classes → entries mental model better than `List`). Trivial change in the show-detail tabs component; align UI text with code so the team uses one consistent term.
- [ ] **Remove `apps/myk9show/secretary-walk.mjs`** — Throwaway script created for the 2026-05-16 live walk. Not part of the app. Delete (and re-run from a worktree-local plan doc if needed in the future) before merging anything related to this brainstorm.
