...
- [x] **Warm up light mode color palette** — Done: 6 commits (`f38639b7`..`f2c312bd`). Updated 11 CSS variables in `index.css` and 7 in `design-tokens.css` to subtle warm cream (`--background: #f5f2ed`, `--card: #faf8f4`). Replaced 63 `bg-white` Tailwind classes across 38 component files with semantic `bg-card`/`bg-background`/`bg-popover`. Swept `#fff` in 11 CSS files (most were mask patterns — kept; 2 surface backgrounds replaced). Print components (`PrintableReport`, `PrintManager`) and `.high-contrast` class left intentionally white. All 3530 tests pass, typecheck/lint clean. Spec: `docs/superpowers/specs/2026-03-23-soften-light-mode-design.md`.
