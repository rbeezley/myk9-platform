...
- **Phase 5: Exhibitor check-in** — Reuse myK9Q's check-in patterns (status state machine, self-check-in cascade, optimistic update). Extract `EntryStatus` + `statusConfig` to `@myk9/core`. Build `CheckInStatusBadge`, `CheckInStatusMenu`, `useCheckInMutation`, `useSelfCheckinEnabled` in myK9Show. Wire into `NextUpCard` and `ClassTimelineCard`. **Plan:** `docs/plans/exhibitor-dashboard-redesign.md`.
