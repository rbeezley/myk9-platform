...
- [x] 8.2.6 **Divergence audit of the two update mappers.** `mapDogInputToUpdate` (Supabase) and `mapPartialDogInputToReplicated` (IndexedDB) are fed from the same `Partial<DogInput>` and write to two stores. Three divergences were found across three review rounds — null-vs-omit, clear-vs-skip, trim-vs-no-trim — so the pair was audited field by field rather than waiting for a fourth.
  ...
  | `status` / `deceased` / `deceasedDate`                                                    | writes all three           | _was_ writing nothing         | **FIXED in a follow-up** — `status` + `deceasedDate` are carried locally; `deceased` stays derived                            |
  | `weight`, `height`                                                                        | `0` _was_ becoming null    | `0` _was_ becoming `"0"`      | **FIXED in a follow-up** — one `measurementForWrite` rule: `0` clears on both                                                 |
  | `sex`                                                                                     | `''` _was_ written through | `''` _was_ becoming undefined | **FIXED in a follow-up** — one `blankToNull` rule: `''` clears on both                                                        |

  **Answer to the question asked: yes, there is a fourth — `status`.** `mapDogToDogInput` includes `status`, and `UserDetailsTabs` calls `updateDog` with it, so marking a dog retired or deceased reached Supabase while the local replicated row kept its old status until a refetch overwrote it. It was user-visible and genuine, but it had nothing to do with dog identity and fixing it here would have widened an already-large change; raised separately rather than folded in, and fixed there along with the `weight`/`height` `0` and `sex` `''` cases.
