 ...
 * Timer Store
 *
 * Zustand store for managing multi-area timers in scent work scoring.
 * Supports starting, stopping, pausing, and tracking multiple timer areas.
 */

import { create } from 'zustand';
import { devtools } from 'zustand/middleware';
import type { TimerArea } from '../types';
 ...
 */
export function createTimerStore(enableDevtools = false) {
      ...
      (set, get) => ({
        ...
        removeArea: (areaId) => {
          set((state) => ({
            ...
            activeAreaId:
              state.activeAreaId === areaId ? null : state.activeAreaId,
          }));
        },
        ...
        stopTimer: (areaId) => {
          ...
          set((state) => {
            const updatedAreas = state.areas.map((area) => {
              if (area.id === areaId && area.isRunning) {
                const elapsed = area.startTime
                  ? now - area.startTime + area.elapsedTime
                  : area.elapsedTime;
                return {
                  ...area,
                  endTime: now,
            ...
            return {
              areas: updatedAreas,
              isAnyTimerRunning: isAnyRunning,
              globalElapsedTime: isAnyRunning
                ? state.globalElapsedTime
                : now - (state.globalStartTime || now),
            };
          });
        },
        ...
        resetTimer: (areaId) => {
          set((state) => ({
            ...
            alertPlayed: new Set(
              [...state.alertPlayed].filter((id) => id !== areaId)
            ),
          }));
        },
