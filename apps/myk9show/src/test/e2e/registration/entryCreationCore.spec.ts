...
test.describe('Phase 3.1: Core Entry Creation Functionality', () => {
  ...
  test('should create entry via entryStore API', async ({ page }) => {
    ...
    expect(entryCreated.entryFee).toBe(25.00);
  ...
  test('should update entry status through workflow states', async ({ page }) => {
    ...
    const statusProgression = await page.evaluate(async () => {
      ...
      // Update to submitted
      const submitted = await entryStore.updateStatus(entryId, 'submitted', 'test-user', 'Registration submitted');
      ...
      results.push({ step: 'payment', paymentStatus: withPayment?.registrationData?.paymentStatus });
      ...
      // Update to confirmed
      const confirmed = await entryStore.updateStatus(entryId, 'confirmed', 'secretary-user', 'Entry confirmed by show secretary');
  ...
  test('should handle payment processing workflow', async ({ page }) => {
    ...
    const paymentWorkflow = await page.evaluate(async () => {
      ...
      const paidEntry = await entryStore.updateStatus(entryId, 'paid', 'payment-system', 'Payment confirmed');
  ...
  test('should maintain entry audit trail', async ({ page }) => {
    ...
    const auditTrail = await page.evaluate(async () => {
      ...
      await entryStore.updateStatus(entryId, 'submitted', 'exhibitor-user', 'Entry submitted by exhibitor');
      await entryStore.updateStatus(entryId, 'paid', 'payment-system', 'Payment confirmed via credit card');
      await entryStore.updateStatus(entryId, 'confirmed', 'secretary-user', 'Entry confirmed by show secretary');
      ...
      return {
        ...
        statusProgression: finalEntry?.statusHistory?.map(h => ({ 
          status: h.status, 
          userId: h.userId, 
          reason: h.reason 
        })) || []
  ...
  test('should handle bulk entry operations', async ({ page }) => {
    console.log('=== Testing Bulk Entry Operations ===');
    
    await page.goto('/');
    await testSetup.waitForLoading();
    
    const bulkOperations = await page.evaluate(async () => {
      const { useEntryStore } = await import('/src/store/entryStore.ts');
      const entryStore = useEntryStore.getState();
      ...
      return {
        ...
        entriesUpdated: updatedEntries.filter(e => e?.status === 'submitted').length,
        entryStatuses: updatedEntries.map(e => e?.status)
      };
    });
    
    // Verify bulk operations
    expect(bulkOperations.entriesCreated).toBe(2);
    expect(bulkOperations.entriesUpdated).toBe(2);
    expect(bulkOperations.entryStatuses).toEqual(['submitted', 'submitted']);
    
    console.log('✓ Bulk entry operations working correctly:', bulkOperations);
  });
  ...
  test('should integrate with show statistics', async ({ page }) => {
      ...
      const entries = await entryStore.createMultipleEntries([
        {
          ...
          classId: 'novice-standard',
          dogId: 'dog-1',
          registrationData: { submittedAt: new Date().toISOString(), handler: 'Handler 1', entryFee: 25.00, paymentStatus: 'paid' }
        },
        {
          showId,
          classId: 'open-standard', 
          dogId: 'dog-2',
          registrationData: { submittedAt: new Date().toISOString(), handler: 'Handler 2', entryFee: 30.00, paymentStatus: 'paid' }
        },
        {
          showId,
          classId: 'novice-jumpers',
          dogId: 'dog-3', 
          registrationData: { submittedAt: new Date().toISOString(), handler: 'Handler 3', entryFee: 25.00, paymentStatus: 'pending' }
        }
      ]);
    ...
    expect(statistics.totalRevenue).toBe(55.00); // 25 + 30 from paid entries
