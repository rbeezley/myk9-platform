...
const ProfileHeader: React.FC<ProfileHeaderProps> = ({
  ...
  return (
    <Card
      className={`relative flex flex-col items-center justify-center p-6 ${cardClassName || ''}`}
    >
      {/* 3-dot menu */}
      {!hideMenu && (
      ...
      >
        <Avatar
          key={`${name}-${photoUrl || 'no-photo'}`}
          className="w-32 h-32 md:w-40 md:h-40 border-2 border-gray-200 group-hover:opacity-75 transition-opacity ring-1 ring-border"
        >
      ...
      {roles && roles.length > 0 ? (
        ...
        <span className="text-sm text-muted-foreground bg-muted px-3 py-1 rounded-full mb-3">
          Member
        </span>
      )}
    </Card>
