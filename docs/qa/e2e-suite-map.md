...
| `apps/myk9show/src/test/e2e/registration/handlerNameSpace.spec.ts`          | Handler-name field accepts space/hyphen/apostrophe in a real browser (MYK9-567). Asserts in-page state only: every cart and entry write is mocked and `submit_show_entries` is refused, so nothing is persisted. |
