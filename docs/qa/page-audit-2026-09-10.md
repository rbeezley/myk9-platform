...
| F-21 | exhibitor | `/shows/:showId/register` step 2                                     | P2  | `Interior Advanced` and `Interior Advanced Preliminary` render identical `Advanced` chips: label built from level+section, never the name. Wrong-entry risk at $30/class                            | linear MYK9-489 | Chips distinguish distinct classes                                            |
| F-22 | public    | show premium offered-classes                                         | P2  | MY BUG, shipped in d4cb4941c: level-keyed grouping merged two classes sharing element+level. Confirmed live signed out — `Advanced Preliminary` absent                                              | FIXED 4259a1eb9 | Red-then-green test; signed-out browser check still outstanding               |
