...
- Make the existing Command Palette context-aware: show current-surface navigation and the first allowlisted mutation, “Check in selected entries,” only when applicable, via the `MYK9-47` shared action registry (merged, PR #1376).
...
- The `MYK9-47` shared action registry dependency is satisfied (merged in PR #1376). The initial command mutation does not expand beyond eligible selected-entry check-in.
- Is sequenced after `operational-views-and-display-presets` (MYK9-48) and `show-attention-and-context-navigation` (MYK9-50): class/entry queries resolve to preset-filtered owner-list URLs defined by MYK9-48, and destination routing reuses the route builders consolidated by MYK9-50.
