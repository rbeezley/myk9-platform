 ...
 */
export function JudgeClassInterface({ classId: propClassId, className }: JudgeClassInterfaceProps) {
  ...
  const { entries, results } = useMemo(() => {
    if (!rawEntries || !classConfig)
      return { entries: [] as EntryWithResult[], results: new Map<string, ScentWorkResult>() };
  ...
  const handleSaveResult = useCallback(
    async (result: ScentWorkResult): Promise<ValidationResult> => {
      try {
        ...
        {currentView === 'navigation' ? (
          <ResultEntryNavigation
            ...
            classInfo={{
              ...
              classNumber:
                ((rawClassData as Record<string, unknown>)?.class_number as string) || undefined,
            }}
