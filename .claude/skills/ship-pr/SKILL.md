...
For this workaround, use Codex `exec_command` with escalated permissions, the **verified PR worktree as `workdir`**, and `yield_time_ms: 1000`. Inside that command, first run `PR_NUMBER="$(gh pr view --json number --jq .number)" || exit 2`; variables from an earlier tool call do not persist. Use **two separate wrapper invocations in that one shell**: first `bash scripts/qa/claude-review.sh --detach --post "$PR_NUMBER"`; only if it succeeds, run `bash scripts/qa/claude-review.sh --wait 1500 "$PR_NUMBER"` (25 minutes). Never combine `--detach` and `--wait` in one invocation: the wait branch takes precedence, skips the head check, and can return a prior head's saved exit code without starting a review. That stale result is not evidence for the current head. Do not end the shell between the two invocations. Poll the returned `session_id` with short `write_stdin` calls. The initial yield returns control without terminating the process. Use this only on a harness that supports a live session for at least 30 minutes without a hard command timeout; otherwise use Richard's terminal. Do not add an unbounded shell loop or an automatic restart.

**A replacement needs coordination first.** Before any new detach, identify the original attempt's PR, owning task/operator, and worktree. First establish which wrapper started that attempt. **Do not run the current wrapper's liveness probe against a pre-#2132 attempt or an attempt with missing/unknown PID metadata**: older detaches never wrote a `.pid` file, and the newer probe can overwrite a live legacy attempt's status with exit 2. Leave those legacy/unknown state files untouched and have the original owner inspect the process in their original terminal; absent metadata proves nothing. For an attempt known to have been started with #2132's PID/token tracking, poll from its **original worktree** with that compatible wrapper and `--wait 1`. Another worktree's missing state files prove nothing. Exit 3 from this one-second triage probe means the wrapper considers the attempt potentially alive; when `ps` is denied it cannot exclude PID reuse, so this is not certain proof of reviewer identity. Keep the owner involved if identity is uncertain; it does not reset or extend the supervised workaround's 25-minute limit. Exit 0/1 is a verdict to handle. Exit 2 is no verdict, not permission to restart. Have the original owner confirm the child has ended before starting a replacement; if process checks are denied or uncertain, stop and ask Richard to inspect it. A successful `kill -0` with empty/denied `ps` output is not evidence of death. A fresh detach overwrites the attempt's files, and an old live child could write its verdict into the replacement's state.

For the supervised workaround only, at its 25-minute wait limit, report exit 3 as **no verdict**, with the PR and original log path, and stop automated retries. Ending the supervising session does not prove the child survived or died; coordinate with the owner before any later attempt. Never kill another task's reviewer or substitute same-harness review evidence.
