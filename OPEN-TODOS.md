...
- [ ] **Execute myK9Show + myK9Q unification plan** — Absorb myK9Q's ringside UI into myK9Show as `/at-show` backed by a shared `packages/ringside`, unify the messaging pipeline (inbox + push + suppression), and eventually delete `apps/myk9q`. Phases 0–6 (foundation → at-show route → auto-routing for entered exhibitors → unified messaging → retire duplicated surfaces → branding/copy → app deletion). Full plan in [docs/plans/2026-05-17-unify-myk9show-myk9q.md](docs/plans/2026-05-17-unify-myk9show-myk9q.md). Context in TO-DOS.md § "Platform Unification — myK9Show + myK9Q — 2026-05-25 10:39".
  ...
  - [ ] **Phase 3 acceptance: real-device push tap verification** — Before enabling the Phase 4B live sunset/redirect, run a real browser/device pilot with an actual push subscription and confirm tapping the notification opens the correct inbox thread. The 2026-05-31 synthetic transport pilot could not cover this because staging had zero existing push subscriptions. PR #479 fixed the stale-client service-worker tap bug and merged. 2026-06-02: real-device testing then showed account taps landing on `/at-show` instead of the message thread; fixed by making `send-targeted-message`'s push `actionUrl` identity-aware (account `user_id` → `/messages/:showId`, passcode → `/at-show`) — see plan changelog "Phase 3b push-tap routing fix". **Still pending:** deploy `send-targeted-message` to staging and re-run the real-device tap to confirm account taps open the inbox thread. Phase 4A code cleanup and dormant Phase 4B prep may proceed, but do not redirect/sunset the standalone myK9Q deployment until this item is complete.
  ...
  - [ ] **Account push by entry (no favoriting) — deploy `send-targeted-message`** — 2026-06-02 testing showed signed-in exhibitors weren't notified unless they favorited an armband (a passcode-era requirement). `send-targeted-message` now also fans out push to entered account exhibitors' own `push_subscriptions` (resolved by `auth_user_id`, deduped against ringside sessions) so they're notified by their entry → `/messages`. Unit-tested; see plan changelog "Phase 3b account push by entry". **Pending:** deploy the function to staging + verify.
  ...
  - [x] **Fix Message Show recipient dropdown not opening in Show Desk sheet** — RESOLVED 2026-06-02 (PR #493): migrated `Sheet` from Radix to Base UI. Radix's modal Dialog set `pointer-events:none` on `<body>`, which swallowed clicks on the Base UI `<Select>` popup portaled to the body — so the `Recipient` `<Select>` in `MessageShowComposer` could not expand, locking the secretary to the default "Everyone in show" and blocking the Class / Checked-in targeted paths. This was *why* every test send hit the (then-broken) all-show announcement lane. Found 2026-06-02 via real-browser walk.
  - [x] **Fix Show Desk quick broadcast post failure** — RESOLVED 2026-06-02 (migration `20260602153923_guard_notify_announcement_push_missing_config.sql`). **Root cause (confirmed against live DB):** the `on_announcement_insert_push` trigger fires `AFTER INSERT ... WHEN priority IN ('high','urgent')` and ran `notify_announcement_push()`, which used the *single-argument* `current_setting('app.settings.supabase_url')` / `service_role_key`. The single-arg form RAISES when the GUC is unset, and those GUCs are unset on this project (the codebase standardized on `app.settings.edge_function_base_url`, not the legacy `supabase_url`). So a `Send push alert` broadcast (priority `high`) aborted the INSERT → `Could not send show message`; a quiet send (priority `normal`) skipped the trigger and worked. This directly answered the open question — yes, high/urgent announcements were blocked differently than normal ones. **Fix:** mirror the guard already applied to `notify_chat_message` (`20260531144257`): two-arg `current_setting` (returns NULL, no raise), skip the webhook with a NOTICE when config is absent, and target `edge_function_base_url || '/push-trigger-announcement'`. Announcement now saves and shows in-app via realtime; push fires once the GUC is configured. Coverage: `announcementPushFunctionContract.test.ts` guard assertions. **Delivery enablement 2026-06-02:** the GUCs can't be set (ALTER DATABASE SET is unavailable to the postgres role on this project), so migration `20260602161813_notify_announcement_push_from_vault.sql` re-sources the trigger from `vault.decrypted_secrets` (keeps the guard-and-skip semantics — never raises). **Requires seeding two Vault secrets to actually deliver push:** `edge_function_base_url` (= `https://sojmvhhwsjxmfistvzbe.supabase.co/functions/v1`) and `service_role_key`.
  - [ ] **Bring `notify_chat_message` push to Vault parity** — `notify_chat_message` (migration `20260531144257`) still reads `current_setting('app.settings.edge_function_base_url', true)` / `service_role_key`, which are unset → chat push silently skips, same as the announcement trigger did. Re-source it from `vault.decrypted_secrets` (same two secrets) once seeded. **Caveat to resolve first:** `send-targeted-message` already pushes class/checked_in messages *directly* and also inserts into `show_messages`, which fires this trigger — confirm whether that double-pushes before enabling, and scope the trigger to non-targeted inserts if so.
  - [ ] **Widen push-trigger-announcement audience beyond dog owner** — `push-trigger-announcement` resolves the all-show audience from `dog.owner` (owner_id) only, so a co-owner or handler entered in the show but not the owner gets no announcement push. `send-targeted-message` already resolves owner + co_owner + handler; align the announcement audience query (and consider the same for the chat trigger). Discovered alongside the `keys`-column fix 2026-06-02.
  - [ ] **Fix Show Desk class broadcast selector labels** — During the 2026-06-01 Phase 4B real-device push verification, the Class broadcast dropdown rendered class UUIDs instead of dog-show labels. The selector should show element, level, and section where applicable, e.g. `Container · Novice A & B`, not `10e39f5f-...`. Fix the class option mapping that feeds `ClassBroadcastCard` and add coverage for the rendered label.
  - [ ] **Verify and fix My Entries summary counts on stable staging** — During the 2026-06-01 Phase 4B real-device push verification, the exhibitor `/exhibitor/entries` summary cards showed broken counts (`0` active entries / upcoming shows while dog cards showed upcoming entries). The screenshot was on the stale `eolf80hbs` preview origin, so first re-check the same account on stable staging after latest `main`; if still broken, fix the `MyEntriesPage` summary derivation so Active Entries / Upcoming Shows / Past Shows align with the rendered entries and dog cards.
  - [ ] **Fix Show Access Codes regenerate confirm in nested Show Desk sheet** — During the 2026-05-31 real-device push pilot, the `Generate new codes` CTA opened the confirmation dialog but clicking the confirm `Generate` button appeared to do nothing from the Show Desk tools sheet. The same `regenerate_show_passcodes` RPC succeeded when run under the signed-in secretary auth context, so investigate the browser UI path: nested `AlertDialog` inside `Sheet`, request/error surfacing, and whether the confirm action fires. Add focused coverage for `ShowAccessCodesCard` rendered inside the Show Desk tools sheet.
...
- [ ] **[P2] Fix multi-dog discount applied to single-dog entries (BUG-EX-03)** — "10% multi-dog discount (3+ dogs)" applied to a 1-dog, 1-class registration; appears keyed to dogs *owned* (4) not *entered* (1). Trace the fee calculator's dog-count source. (Step 5.2)
- [ ] **[P2] Stop exhibitor check-in writing staff/scoring fields (BUG-EX-13)** — My Entries check-in dialog calls secretary `updateCheckInStatus` (`apps/myk9show/src/services/database/entries/secretary.ts:321`) which also sets `is_in_ring`/`ring_entry_time`/`judge_notes`; route exhibitor self-check-in through an owner-scoped path (`self_checkin_entry` RPC) + review RLS. (Step 7)
- [ ] **[P2] Verify `/dogs` "My Dogs" count (9) vs dashboard (4) (BUG-EX-12)** — confirm whether `/dogs` is owner-scoped or a global browse; fix the count/scope mismatch if real. (Step 6)
- [ ] **[P2] Fix premium landing show-date off-by-one (BUG-EX-04)** — Headline detail/footer show Jun 11–13 vs browse card + receipt Jun 12–14 (UTC-parse off-by-one in the premium landing date renderer). (Step 4)
- [ ] **[P2] Fix premature "CONFIRMED / FEES RECEIVED" on confirmation step (BUG-EX-10)** — shown before "Complete Registration" and while payment is deferred ("collected later"). (Step 5.3)
- [ ] **[P2] Fix Browse Shows count inconsistencies + "My Entries" tab 0 (BUG-EX-07)** — "All Shows (5)" vs "Browse All 9" vs "5 of 9"; My Entries tab shows 0 despite entries. `useBrowseShowsData`/`useBrowseShowsFilters`. (Step 3)
- [ ] **[Minor] Add accessible names to class checkboxes + dog chips (BUG-EX-05)** — Base UI `span[role=checkbox]` has no accessible name; dog chips lack `aria-pressed`. (Step 5.1)
- [ ] **[Minor] Explain disabled "Next" on payment step (BUG-EX-08)** — no inline reason until a payment method is selected (only a blank "Selected Payment Method:"). (Step 5.2)
- [ ] **[Minor] Humanize trial-type label in registration wizard (BUG-EX-06)** — shows raw enum "scent_work" not "Scent Work" (reuse `formatTrialTypeLabel`). (Step 5.1)
- [ ] **[Minor] Fix CheckInStatusDialog DOM nesting (BUG-EX-14)** — renders `<div>` inside `DialogDescription`'s `<p>` (validateDOMNesting/hydration error). (Step 7)
