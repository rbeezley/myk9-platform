...
## Night Before Checklist + Secretary Task Board - 2026-03-04 12:33 (updated 2026-03-11)

- **Build "Show Readiness" checklist with kanban task board** — Secretary needs a clear checklist view the night before a trial to confirm everything is handled, plus a general-purpose task board for ad hoc show prep tasks. **Problem:** INTENT.md specifies "Night before the trial: Everything is handled — A clear checklist view, green checks, not a wall of data." The current Mission Control dashboard is purely a pipeline/Kanban view with no readiness overview. There's no way for a secretary to glance at a summary and know all preparation is complete. myK9Q already has a kanban task board (To Do / In Progress / Done) in Secretary Tools that works well — port this concept to myK9Show. **Files:** `apps/myk9show/src/features/pipeline/components/PipelineDashboard.tsx` (current dashboard — could add toggle or panel), `docs/INTENT.md` (defines the "That was easy" intent and night-before scenario), `apps/myk9show/src/features/pipeline/hooks/useMissionControlData.ts` (data source — would need readiness queries), `apps/myk9q/src/pages/TrialSecretary/components/KanbanBoard.tsx` (reference implementation — 3-column drag-and-drop), `apps/myk9q/src/pages/TrialSecretary/hooks/useKanbanBoard.ts` (reference — localStorage persistence scoped by license key), `apps/myk9q/src/pages/TrialSecretary/types.ts` (KanbanTask, KanbanState types). **Solution:** Combine two features: (1) Auto-generated readiness checklist (judges confirmed, entries finalized, run orders set, classes created, venue/ring assignments) with green checks and clear indicators. (2) Kanban task board for custom secretary tasks (drag-and-drop, priority, due date, assignee — port from myK9Q). Could live as tabs or sections within a dedicated `/secretary/readiness` route or as part of a broader "Secretary Tools" page in myK9Show.
...
## Port myK9Q Secretary Tools to myK9Show - 2026-03-11 09:15

**IMPORTANT: Shared-first approach.** These features overlap significantly between myK9Q and myK9Show. Extract shared types, hooks, and logic into workspace packages (`@myk9/core`, or new packages like `@myk9/secretary`) rather than duplicating code. myK9Q should then consume the shared packages too, replacing its local implementations. Follow the same pattern as `@myk9/scoring-ui` (shared logic, app-specific thin wrappers for UI).

- **Build volunteer schedule / assignment board in myK9Show** — Port myK9Q's volunteer scheduling system (drag-and-drop assignment of volunteers to class roles and general duties) into myK9Show. **Problem:** myK9Show has no volunteer management. Secretaries need to assign stewards (gate, timer, ring) per class and general duties (hospitality, equipment, ring setup) before the show. myK9Q has a working implementation with Supabase persistence + offline fallback. **Files:** `apps/myk9q/src/pages/TrialSecretary/components/ScheduleBoard.tsx` (reference — class-based steward scheduling with drag-and-drop), `apps/myk9q/src/pages/TrialSecretary/components/VolunteerPool.tsx` (reference — unassigned volunteer pool), `apps/myk9q/src/pages/TrialSecretary/components/VolunteerDialog.tsx` (reference — add/edit volunteer), `apps/myk9q/src/pages/TrialSecretary/components/RoleConfigDialog.tsx` (reference — manage schedule roles), `apps/myk9q/src/pages/TrialSecretary/hooks/useScheduleBoard.ts` (reference — Supabase persistence with localStorage fallback), `apps/myk9q/src/pages/TrialSecretary/hooks/useScheduleFiltering.ts` (reference — multi-select filtering), `apps/myk9q/src/pages/TrialSecretary/types.ts` (ScheduleRole, Volunteer, ClassAssignment, GeneralAssignment types). **Solution:** Extract shared types (ScheduleRole, Volunteer, assignments) and data hooks into a shared package. Build Tailwind/shadcn UI in myK9Show, then migrate myK9Q to consume the same shared types/hooks. Use Supabase persistence (myK9Q keeps offline fallback, myK9Show is online-first).

- [x] **Add show settings page for results visibility and self check-in controls** — Port myK9Q's per-class results visibility and self check-in toggle settings into myK9Show. **Problem:** myK9Show has no way for secretaries to control when exhibitors can see results (hidden, name only, results, full results) or whether exhibitors can self check-in. myK9Q has these controls in its Secretary Tools Settings tab. **Files:** `apps/myk9q/src/pages/TrialSecretary/components/ResultsControlTab.tsx` (reference — per-class results release + self check-in toggles), `apps/myk9show/src/features/pipeline/components/PipelineDashboard.tsx` (current secretary dashboard — could add settings section). **Solution:** Extract shared visibility preset types and self check-in config to `@myk9/core` (similar to how `CheckInStatus` was already extracted). Build settings UI in myK9Show, migrate myK9Q to import shared types. Ties into the existing check-in system already built for Phase 5 of the exhibitor dashboard (`@myk9/core` CheckInStatus type, `useCheckInMutation`, `useSelfCheckinEnabled`).

## Add Resend Email Integration for Exhibitor Emails - 2026-03-13 13:18

- **Integrate Resend for transactional exhibitor emails** — Add Resend email service to send automated emails to exhibitors for key events in the show lifecycle. **Problem:** No email infrastructure exists — exhibitors receive no confirmation when they register, no reminders before shows, no notification when new shows are posted, and no results summary after competing. This is a critical communication gap for the platform. **Files:** `supabase/functions/` (new Edge Functions for email sending, same pattern as `stripe-checkout`), `apps/myk9show/src/services/` (email trigger points), `packages/core/` (shared email types/templates). **Solution:** Use Resend (free tier: 3K emails/month, paid: $20/mo for 50K). Create a `@myk9/email` package or add to `@myk9/core` with React Email templates (TSX). Supabase Edge Functions handle sending (same pattern as Stripe). Four email types needed:
  - **Confirmation emails** — Sent after successful registration/entry submission. Include confirmation number, dogs entered, classes, payment summary.
  - **Reminder emails** — Sent 1 week and 1 day before show date. Include show details, entry summary, venue/directions, check-in instructions.
  - **New show announcement emails** — Sent to exhibitors who opt in when a new show is published in their region or for their breed/organization. Requires notification preferences.
  - **Results emails** — Sent after show completion with scores, placements, qualifying legs, and career impact summary.
