...
| `apps/myk9show/src/test/e2e/theme-mode-selection-border.spec.ts`            | MYK9-591: Theme Mode card selection border (peer-data-[checked], a Base UI Radio.Root attribute) via rendered `border-color`. Mutates and restores the shared e2e-admin's persisted `user_preferences.mode`.                                   |
