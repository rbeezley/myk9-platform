...
- [ ] **Execute myK9Show + myK9Q unification plan** — Absorb myK9Q's ringside UI into myK9Show as `/at-show` backed by a shared `packages/ringside`, unify the messaging pipeline (inbox + push + suppression), and eventually delete `apps/myk9q`. Phases 0–6 (foundation → at-show route → auto-routing for entered exhibitors → unified messaging → retire duplicated surfaces → branding/copy → app deletion). Full plan in [docs/plans/2026-05-17-unify-myk9show-myk9q.md](docs/plans/2026-05-17-unify-myk9show-myk9q.md). Context in TO-DOS.md § "Platform Unification — myK9Show + myK9Q — 2026-05-25 10:39".
  ...
  - [ ] **Fix "Show today" banner reachability** — `ShowTodayBanner` (the Phase 2 entry point into `/at-show` for entered exhibitors) is mounted only on `Home` (`/`), but `HomeRedirect` immediately redirects any authenticated user to their dashboard (`getDashboardRoute` → `/exhibitor/entries`), so a signed-in exhibitor **never renders Home and never sees the banner**. Its data (`get_account_today_entries`) is auth-gated, so it only has content for the very users who can't reach it. Fix: render `ShowTodayBanner` on the dashboard the exhibitor actually lands on (`MyEntriesPage`), or keep entered-today exhibitors on `Home`. Found 2026-06-02 via real-browser walk.
  - [ ] **Fix Message Show recipient dropdown not opening in Show Desk sheet** — The `Recipient` `<Select>` in `MessageShowComposer` (rendered inside the Show Desk tools sheet) does not expand, locking the secretary to the default "Everyone in show" and blocking the Class / Checked-in targeted paths entirely. Same nested-overlay-in-`Sheet` family as the access-codes-confirm bug (line below); DevTools Issues also shows a CSP `script-src 'self'` "eval blocked" error worth ruling in/out. Found 2026-06-02 via real-browser walk; this is *why* every test send hit the (broken) all-show announcement lane.
  - [ ] **Fix Show Desk quick broadcast post failure** — During the 2026-06-01 Phase 4B real-device push verification, Show Desk Quick broadcast with `Send push alert` checked failed with the toast `Could not post broadcast` on stable staging. (2026-06-02: confirmed the Message Show composer's "Everyone in show" recipient routes to this same announcement lane (`postAnnouncement` → `push-trigger-announcement`) and fails identically with `Could not send show message` — it is NOT the `send-targeted-message` path. The broken pipeline is the announcement write/push.) Investigate the announcement write path from `QuickBroadcastCard` / `useWorkbenchAnnouncementPost`: handler execution, surfaced error details, RLS/author role mismatch, and whether high-priority announcements are blocked differently than normal announcements. Add focused coverage that proves the failure state exposes a useful error and that a secretary can post a push-alert broadcast.
