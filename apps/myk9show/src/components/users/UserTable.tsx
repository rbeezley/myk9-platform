...
const PersonRow: React.FC<{
  ...
  const dogCount = useMemo(() => person.dogs?.length || 0, [person.dogs]);
  ...
  const handleManageQualifications = useCallback(
    () => onManageQualifications?.(person),
    [onManageQualifications, person]
  );
...
const UserTable: React.FC<PeopleTableProps> = ({
  people,
  onView,
  onEdit,
  onDelete,
