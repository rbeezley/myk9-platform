...
- **Golden path sign-off:** re-walk both golden paths end-to-end against the scorecard's pass thresholds and update the secretary/exhibitor rows in `docs/goals/fall-2026-launch-readiness-scorecard.md` with the earned status and evidence link. This is the audit's exit criterion. (The scorecard's third golden path — Admin — is deliberately out of this audit's scope; it gets a functional walkthrough, not a UX audit, at the program-level scorecard close-out tracked in OPEN-TODOS.)
