...
export function useRegistrationConflicts(_showId: string, registrationId?: string) {
  ...
  const { getRegistration } = useShowRegistrationStore();
  ...
  const checkRegistrationConflicts = useCallback(
    async (
      ...
      try {
        ...
        if (JSON.stringify(localDogs.sort()) !== JSON.stringify(serverDogs.sort())) {
          const conflictId = checkForConflicts(
            ...
            {
              ...(serverData.lastModifiedByUserId !== undefined && {
        ...
        if (
          localRegistrationData.paymentMethod &&
  ...
  const checkClassSelectionConflicts = useCallback(
    async (localClassSelections: ClassSelectionData[], serverRegistration?: ShowRegistration) => {
      if (!registrationId) return null;
      ...
      const localString = JSON.stringify(
        localClassSelections.sort((a, b) => a.dogId.localeCompare(b.dogId))
      );
      const serverString = JSON.stringify(
        serverClassSelections.sort((a, b) => a.dogId.localeCompare(b.dogId))
      );
  ...
  const resolveRegistrationConflict = useCallback(
    async (
      conflictId: string,
      ...
      try {
        const conflictResolution = await resolveConflict(
          conflictId,
          strategy,
