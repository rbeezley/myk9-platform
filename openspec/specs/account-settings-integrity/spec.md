# account-settings-integrity Specification

## Purpose

Guarantees that every control on the Account page (`/account`) observably does what it claims for the exhibitor audience: theme and font settings apply on selection and persist, no placeholder or mock-backed controls are rendered, save feedback is transient and never shifts layout, install-app messaging reflects real installability, and destructive account actions require explicit typed confirmation with server rejections surfaced in user-readable form. Born from the 2026-07-10 exhibitor UX audit (docs/ux-audits/account-page-exhibitor-2026-07-10.md) and the account-page-ux-remediation change.
