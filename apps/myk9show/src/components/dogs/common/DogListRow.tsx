    ...
    <div className="flex items-center bg-white dark:bg-card rounded-lg shadow-sm border px-4 py-3 mb-2">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1">
          ...
          {dog.registrations?.[0]?.breed && <span className="text-sm text-muted-foreground ml-2">Breed: <span className="font-medium text-foreground">{dog.registrations[0].breed}</span></span>}
        </div>
        <div className="flex flex-wrap items-center gap-3 text-sm text-muted-foreground mb-1">
          <span>Owner: <span className="text-blue-700 dark:text-blue-300 font-medium">{owner ? `${owner.firstName} ${owner.lastName}` : 'N/A'}</span></span>
          <span>Gender: {dog.gender === 'Male' ? <span className="text-blue-600">♂</span> : <span className="text-pink-600">♀</span>} {dog.gender}</span>
          <span>Age: {calculateAge(dog?.dateOfBirth || "")} {+calculateAge(dog?.dateOfBirth || "") === 1 ? 'year' : 'years'}</span>
        </div>
        <div className="flex flex-wrap items-center gap-2 text-xs">
          {dog.registrations && dog.registrations.map((reg) => (
