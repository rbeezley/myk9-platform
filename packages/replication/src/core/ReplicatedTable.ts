...
export abstract class ReplicatedTable<T extends { id: string }> {
  ...
  /**
   * Optional SECURITY DEFINER RPC routing for a repair re-queue (see
   * requeueOrphanedDirtyRows). Subclasses whose writes must go through an RPC
   * (e.g. ringside entries) override this so a repaired mutation takes the same
   * authorized path as a normal write. Default: direct UPDATE.
   */
  protected buildRepairRpc(
    _row: T,
    _payload: Record<string, unknown>
  ):
    | { name: string; fields?: Record<string, unknown>; args?: Record<string, unknown>; expectRowId?: boolean }
    | undefined {
    return undefined;
  }

  /**
   * Repair the crash window between `set(id, data, true)` and `queueMutation`:
   * a crash in between leaves a dirty row with NO pending mutation, so it shows
   * as saved but never uploads. This scans THIS table's dirty rows and re-queues
   * an UPDATE for any that have no pending mutation, so a stranded score still
   * syncs (July 2026 audit finding M6). Skips conflicted rows (left for user
   * reconciliation) and tables that can't rebuild a payload (RPC/delta-only).
   *
   * Safe to call at startup after restoreMutationsFromLocalStorage(). Returns
   * the number of rows re-queued.
   */
  async requeueOrphanedDirtyRows(): Promise<number> {
    if (!this.mutationManager) return 0;
    const db = await this.init();
    const all = (await db.getAll(REPLICATION_STORES.REPLICATED_TABLES)) as ReplicatedRow<T>[];
    ...
    for (const row of dirty) {
      ...
      const payload = this.rebuildUpdatePayload(row.data);
      if (!payload) continue; // RPC/delta-only table can't rebuild a full row
      ...
      await this.queueMutation(
        ...
        payload,
        undefined,
        this.buildRepairRpc(row.data, payload)
      );
      requeued++;
      this.logger.warn(
        `[${this.tableName}] Re-queued orphaned dirty row ${row.id} (dirty but no pending mutation)`
      );
    }

    if (requeued > 0) {
      this.logger.log(`[${this.tableName}] Repaired ${requeued} orphaned dirty row(s)`);
    }
    return requeued;
  }
