...
> **Status:** Active — Batch 0 complete; Batch 1 merged with only MYK9-226's hosted evidence gate still open; Batch 2 unblocked and active in a separate conversation
...
| 1G | [MYK9-226](https://linear.app/myk9-platform/issue/MYK9-226) (Medium, **In Progress**, evidence PR [#1734](https://github.com/rbeezley/myk9-platform/pull/1734) merged as `7052c7ad3`) | Source and page-level tests are fail-closed, and the submit-time refresh landed after the four Aug 18 incidents, but mocked boundaries do not satisfy the required hosted real-flow gate. Keep the issue open until actual wizard/cart hydration is replayed before Stripe; a disposable shared cart fixture requires separate database-mutation approval | OpenSpec source/page-test evidence; hosted replay pending |
...
**Batch 1 progress (2026-08-21):** All seven implementation/evidence PRs merged. MYK9-54, MYK9-57, MYK9-163, MYK9-209, MYK9-212, and MYK9-225 are Done with their required verification recorded in Linear. MYK9-225's corrected hosted replay passed 2/2 after its trace established that the first harness had forced both the initial request cycle and the user-triggered Retry cycle to fail. MYK9-226 alone remains In Progress because its source/page-test evidence does not replace the required hosted wizard/cart hydration replay. MYK9-212 and MYK9-209 no longer block Batch 2A; MYK9-54 no longer blocks Lane 2D.
...
> **Execution status — 2026-08-21:** Batch 2 is active.
>
> - **Prerequisites:** MYK9-209 merged in [PR #1735](https://github.com/rbeezley/myk9-platform/pull/1735) with green required CI and is Done. MYK9-212's production change merged in [PR #1737](https://github.com/rbeezley/myk9-platform/pull/1737), but its full-suite CI exposed a missing `useMyPaymentYears` mobile-test mock. MYK9-212 was honestly reopened to In Progress; the test-only repair is [PR #1741](https://github.com/rbeezley/myk9-platform/pull/1741) and must merge with green required CI before the evidence gate is closed. The production query shape is on `main`, so Lane 2A may proceed without stacking on the repair branch.
> - **Lane 2A:** MYK9-215 is implemented on `codex/myk9-215-order-receipts`. Initial independent review found three money-accuracy blockers: the card-wide My Shows receipt remained reachable for split registrations, partially replicated orders could print a full charged amount with incomplete lines, and receipt payment status came from the grouped registration instead of the exact order. Fixes and mounted coverage are in progress; do not open/merge the PR until re-review clears them. MYK9-216 and MYK9-217 remain serialized behind 215.
> - **Lane 2B:** MYK9-191 is [PR #1739](https://github.com/rbeezley/myk9-platform/pull/1739), tracked by OpenSpec change `sms-ring-alert-delivery`. Security review drove owner-derived preference RPCs, RLS/ACL hardening, tokenized exactly-one writes, compare-and-clear compensation, rate limiting, provider timeout, and channel-state separation. The first CI run exercised the migrated database and found a SQL argument cast plus two edge-test inventory omissions; follow-up `bd1e4387f` fixed both. The rerun's SQL job is green; full test shards remain the merge gate. Vercel Hobby preview rate-limit failures are non-blocking under the repository runbook. Do not start MYK9-192 until 191 merges; no SMS deploy, secret write, or handset proof is authorized.
> - **Lane 2C:** Investigation and the no-code recommendation are recorded on MYK9-204. Keep `payment_method_types: ['card']`; add neither `payment_method_configuration` nor a redundant API-version pin. The proven control is the pruned per-environment Stripe Dashboard configuration. The MYK9-11 runbook step already exists on `main`. MYK9-204 remains In Progress until a fresh desktop checkout proves card plus eligible Apple/Google Pay only, with no Link/Klarna/bank.
> - **Shared-system gates:** PR creation and Linear updates were approved for this batch. PR merges, Supabase migration/function deployment, Twilio secret writes, Stripe Dashboard changes, and external/provider actions still require their own explicit approval.
