...
>
> **DB PUSH DONE:** migration applied to staging as **20260621171500_ringside_update_entry.sql** (renamed from 120000 — collided with a co-resident agent's already-applied version; Supabase tracks by timestamp). Function verified live (signature + grants). Secretary club-scope OK (auditor Finding 8).
>
> **ACCOUNT-JUDGE PATH VERIFIED LIVE 2026-06-24.** Both prior blockers are cleared: (a) `judge_assignments` now has 10 Heartland rows (seed §11 applied); (b) client `viaRpc` routing is on `main` (deployed). Verified via JWT-impersonation of `e2e-judge@test.myk9.com` (uid `a88ed65e…`) in a rolled-back txn against staging: the judge reads **18** Heartland entries through `view_authenticated_entry_results` with **scores visible + payment/PII NULL** (the `20260621190000` read view's column cascade), and `ringside_update_entry` authorizes the judge write (returns new version, OCC honored). The companion **read** path is `20260621190000_ringside_entry_read_for_staff.sql`. Remaining: a literal browser round-trip walk (low-risk; `atShowJudgeScoring.spec.ts` covers wiring), and the **passcode** identity (no `auth.uid()`) — tracked in [`plan-ringside-entries-read-authz.md`](plan-ringside-entries-read-authz.md).
