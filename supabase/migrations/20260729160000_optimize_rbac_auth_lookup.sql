-- Use the auth identity already denormalized and indexed on user_roles by
-- migration 156. This removes the repeated people join from the four RPCs
-- that build the signed-in user's access context.
--
-- Rollback: recreate these functions from
-- 065_rbac_functions_filter_is_active.sql, then reapply the authenticated-only
-- revokes from 20260613100000_security_revoke_anon_grants.sql.

DO $$
BEGIN
  IF to_regclass('public.user_roles_auth_user_id_idx') IS NULL THEN
    RAISE EXCEPTION
      'Required index public.user_roles_auth_user_id_idx is missing';
  END IF;
END;
$$;
