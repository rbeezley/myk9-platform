...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `supabase/functions/validate-passcode/index.ts:251` | Nationals/Regular source hardcoded to `Regular` | file-as-todo | P2 | Affects `competition_type`; related DB function has same gap. | Phase-2 confirmed | Add one OPEN-TODOS item for real Nationals discriminator. | Consolidate with migration item. |
...
| `supabase/migrations/20260526200000_fix_class_completion_nationals_source.sql:41` | DB placement uses `v_is_nationals=false` | file-as-todo | P2 | `recalculate_class_placements` receives hardcoded false. | Phase-2 confirmed | Same consolidated Nationals discriminator todo. | Do not edit migration directly unless superseded. |
...
| `apps/myk9show/supabase/functions/stripe-connect-onboard/index.ts:135` | Stripe Accounts v2 migration note | file-as-todo | P2 | Existing Stripe go-live items do not cover this exact API migration. | Not included in initial Phase-2 pass; pending | Fold into Stripe go-live checklist. | Payment launch risk. |
...
| `apps/myk9show/src/hooks/useDogStoreCompat.ts:98` | Dog and registrations not atomic | file-as-todo | P2 | Dog create can succeed before registration sync. | Phase-2 confirmed | Add OPEN-TODOS item for `create_dog_with_children` RPC. | Reliability gap. |
...
| `apps/myk9show/src/utils/show-management-tracking.ts:133` | Site admin treated as club admin pending real membership | file-as-todo | P2 | Scope accuracy matters for role semantics. | Not included in initial Phase-2 pass; pending liveness verification | Verify liveness, then use RBAC scopes or delete utility. | Avoid misleading relationship state. |
