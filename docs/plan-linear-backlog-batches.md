...
> **Status:** Active — Batch 0 complete; Batch 1 merged with two follow-up evidence/fix gates still open; Batch 2 unblocked and active in a separate conversation
...
| 1B | [MYK9-225](https://linear.app/myk9-platform/issue/MYK9-225) (P2, **In Progress**, PR [#1736](https://github.com/rbeezley/myk9-platform/pull/1736) merged as `028c7d4d4`) | Durable failure state passed hosted desktop/tablet replay: unknown counts, meaningful fallback copy, no false empty state, and a keyboard-focusable 44px Retry. Retry did not issue a fresh request, so recovery failed and a focused follow-up remains required | `pages/admin/SupportInboxPage.tsx`, its test, `features/support/useSupportTickets.ts` |
...
**Batch 1 progress (2026-08-21):** All seven implementation/evidence PRs merged. MYK9-54, MYK9-57, MYK9-163, MYK9-209, and MYK9-212 are Done with their required verification recorded in Linear. MYK9-225 remains In Progress: its durable hosted error state passed at both required viewports, but Retry did not reissue the query or recover, so it needs a focused follow-up. MYK9-226 also remains In Progress because its source/page-test evidence does not replace the required hosted wizard/cart hydration replay. MYK9-212 and MYK9-209 no longer block Batch 2A; MYK9-54 no longer blocks Lane 2D.
