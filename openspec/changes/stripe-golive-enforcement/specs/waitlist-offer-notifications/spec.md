...
The system SHALL dispatch the offer notification when a waitlist row transitions to `offered`,
whether cron, a secretary, or another authorized server path performed the promotion.
...
Offer and reminder notifications SHALL deep-link to My Shows/My Entries with the waitlist-offer id
and SHALL NOT expose a raw long-lived payment URL as the only recovery path.
...
The system SHALL persist one notification-event record per waitlist row, offer cycle, and event type
so normal cron retries or duplicate webhooks converge on retryable pending/processing/sent/failed
state instead of creating repeated promotion, halfway reminder, or expiry events.
...
The waitlist notification dispatcher SHALL require the dedicated configured secret, reject a
service-role bearer as a substitute, validate event payloads, and report delivery failures without
rolling back a committed promotion.
