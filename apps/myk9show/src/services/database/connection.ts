...
export class DatabaseService extends Dexie implements DatabaseAPI {
  ...
  async bulkUpdate<T>(
    ...
    const table = this.table(store);
    await Promise.all(updates.map(update => table.update(update.id, update.data)));
  }
      ...
      if (filter.orderBy && this.hasIndex(store, filter.orderBy)) {
        ...
        if (filter.where) {
          results = results.filter(item =>
            this.matchesFilter(item as Record<string, unknown>, filter.where!)
          );
        }
  ...
  private trackQueryPerformance(
    ...
    if (duration > 100) {
      ...
      logger.warn('Slow query detected', 'database', {
        query: this.performanceMetrics.lastSlowQuery,
      });
    }
  }
    ...
    for (const key of keys) {
      try {
        ...
        if (remainingKeys.length > 0) {
          const remainingFilter = Object.fromEntries(remainingKeys.map(k => [k, where[k]]));
          return collection.filter(item =>
            this.matchesFilter(item as Record<string, unknown>, remainingFilter)
          );
        }
  ...
  async runMigrations(): Promise<void> {
    try {
      ...
      logger.info('Database migrated successfully', 'database', {
  ...
  async clearAllStores(): Promise<void> {
    await Promise.all(Object.keys(STORES).map(storeName => this.clearStore(storeName)));
  }
