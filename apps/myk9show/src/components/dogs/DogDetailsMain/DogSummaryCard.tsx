...
const DogSummaryCard: React.FC<DogSummaryCardProps> = ({ dog }) => {
  const { stats } = usePerformanceStatistics(dog.id);
  const { progressBySport } = useTitleProgress(dog.id);

  const competitionCount = stats?.summary.totalCompetitions ?? null;

  // Count earned, non-superseded titles across all sports
  const titlesEarnedCount = Object.values(progressBySport)
    .flat()
      ...
      <div className="relative space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          >
            ...
            <span className="text-lg font-semibold text-foreground">{competitionCount ?? 0}</span>
          </div>
          ...
          >
            ...
            <span className="text-lg font-semibold text-foreground">{titlesEarnedCount}</span>
          </div>
