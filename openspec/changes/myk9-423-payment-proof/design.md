...
- Begin with no cart shell and submitted unpaid entries; include an unrelated entry and an outsider entry ID in the URL to prove exact scope and ownership filtering. Existing cart-store tests cover stale paid-line reconciliation. Derive the fee-card target through existing payment logic.
