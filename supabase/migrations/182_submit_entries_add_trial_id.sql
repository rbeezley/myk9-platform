-- Fix submit_show_entries: write trial_id on each entry row so entries can be
-- grouped by trial. The RPC already joins classes → trials for fee lookup but
-- never stored the trial_id on the inserted entry.
...
BEGIN
  ...
  LOOP
    ...
    -- 5d. Insert entry with handler_id and trial_id
