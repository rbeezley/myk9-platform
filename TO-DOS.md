...
- [ ] **Secretary show-level statistics** — Add a "Show Stats" tab on ShowDetailsPage visible to secretaries/admins showing aggregate stats across all exhibitors for that show. New `useShowStats(showId)` hook fetches all entries (no dog filter) from `view_entry_with_results`. Reuses existing shared analytics components (StatsSummaryCards, ResultDistributionChart, DogBreakdownCards, FastestTimesTable). Per-dog breakdown shows how each dog performed at the show. No new visualization components needed — only a data hook and tab wiring.
- [ ] **Judge class-level statistics** — Add stats for judges showing performance across classes they judged. New `useJudgeClassStats(judgeId, showId)` hook fetches entries for classes where the judge is assigned, via `classes` join on `view_entry_with_results`. Could live on Judge Dashboard or as a role-gated tab on ShowDetailsPage. Reuses shared analytics components. Shows qualification rates, time distributions, and fastest dogs for the judge's classes.
- [ ] **Add analytics section usage tracking** — Add lightweight tracking events to the four future-premium analytics sections (QualificationTrendChart, DogBreakdownCards, FastestTimesTable, lifetime analytics page) so we can measure which sections get the most engagement before gating them. Track page/section views (not clicks within). Use existing `LoggingService` or a simple analytics event. Data informs whether the free/premium line is drawn correctly. Should be in place before production launch so we collect data during the free trial period.
- [ ] **Gate premium analytics behind subscription** — Wrap four sections with `FeatureGate` so free users see summary cards + result distribution (basic "how did I do"), while premium sections are blurred/locked with an upgrade prompt. Sections to gate: (1) **Lifetime analytics page** — cross-show trends and historical performance on `/analytics`; free users see summary cards and pie chart, everything below is gated. (2) **QualificationTrendChart** — the "am I improving?" trend line is the highest-value visualization. (3) **Per-dog breakdown (DogBreakdownCards)** — especially once comparative stats (breed averages) are added. (4) **Fastest times leaderboard (FastestTimesTable)** — competitive cross-show rankings. **Keep free:** show-scoped "My Stats" tab (you entered a show, you see your results), summary cards, result distribution pie chart. **Rollout:** Platform-wide free trial for 3–6 months after production launch. After that, new users get a 30-day trial (check `user.created_at + 30 days > now OR has paid tier`). 30 days gives enough time to enter 1–2 shows and see stats populate; loss aversion on the trend line drives conversion. Don't exceed 30 days — longer trials feel like free features being taken away. **Implementation:** Extend `FeatureGate`/`useSubscriptionGate` with a time-based trial check alongside the existing tier check. **Files:** `AnalyticsPage.tsx` (wrap individual sections, not the whole page), `FeatureGate` component + `useSubscriptionGate` hook already exist in the codebase.
