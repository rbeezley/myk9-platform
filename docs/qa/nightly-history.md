...
- **Fixes made:** docs only (`docs/qa/findings.md`, `docs/qa/nightly-history.md`). No product/test code changed: the Wave 1 failures were contaminated by cross-agent contention (untrusted), and the repo convention is to verify fixes from a clean tree — which was not available because a second agent's WIP filled the working tree mid-run (`App.tsx`, `SmartSignInPage.*`, `AtShowAccessGate.*`, `atShowRoutes.tsx`, ringside `SortableEntryCard.*`) and the shared `codex/nightly-qa-2026-05-30` branch was being rewritten repeatedly (HEAD moved `91122807` → `4d5c2d63` → `98694c87` → `dd41af00`, wiping a first docs commit). The legacy-sign-in-selector test repair is being handled by that agent in PR #448. This nightly's docs were therefore committed from an isolated `git worktree` on branch `codex/nightly-qa-2026-05-30-report` off `origin/main`.
