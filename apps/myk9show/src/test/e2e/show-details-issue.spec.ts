...
test.describe('Show Details Issue Investigation', () => {
  test('should investigate Browse Shows page and Show Details navigation', async ({ page }) => {
    ...
    const isOnBrowseShows =
      finalUrl.includes('browse-shows') || pageContent?.includes('Browse Shows');
    ...
    if (isOnBrowseShows) {
      ...
      const showElements = await page
        .locator('[data-testid*="show"], .show-card, .card, a[href*="show"], a[href*="/shows/"]')
        .count();
      const detailsButtons = await page
        .locator('text=/view.*details/i, text=/details/i, button:has-text("Details")')
        .count();
      ...
      if (showElements > 0 || detailsButtons > 0) {
        ...
        if (detailsButtons > 0) {
          ...
          clickTarget = page
            .locator(
              '[data-testid*="show"], .show-card, .card, a[href*="show"], a[href*="/shows/"]'
            )
            .first();
        ...
        if (clickTarget) {
          ...
          if (detailsIssues.hasNoShowsAvailable) {
            ...
            const errorElements = await page
              .locator('text=/no.*show.*available/i, text=/no.*show/i')
              .all();
            ...
            console.log(
              '  - Contains show ID:',
              detailsUrl.includes('/show/') || detailsUrl.includes('/shows/')
            );
            console.log('  - URL pattern:', detailsUrl.replace('http://127.0.0.1:5174', ''));
          } else if (Object.values(detailsIssues).some(Boolean)) {
            console.log(
              '⚠️ Show Details page has some issues but not the specific "no shows available" error'
            );
          } else {
      ...
      const browseShowsLink = await page
        .locator('a[href*="browse"], text=/browse.*show/i, text=/show/i')
        .count();
    ...
    networkRequests
      .filter(req => req.includes('show') || req.includes('api'))
      .slice(0, 5)
