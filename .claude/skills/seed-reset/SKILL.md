...
| Seed aborts: `paid or refunded entr(ies) sit on a show, trial, class or dog this reseed deletes` | A walk or manual test left a paid/refunded entry on a demo, sibling or load show, or on a seeded dog. The seed refuses rather than destroying a money row and its `entry_status_history` (MYK9-526) | The error names the exact count and the first 10 entry ids; for the rest, read the `stray` CTE in the guard near the top of section 0 of `seed-demo.sql` and run it as a SELECT. **Remove** those entries — soft-deleting them does NOT clear the abort, because the guard has no `deleted_at` filter (deliberately: a soft-deleted row still cascades). Never widen the guard to get past it |
