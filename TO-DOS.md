...
- [ ] **Detective still shows "Unknown" in trial classes table Level column** — The `formatClassTitle` fix was applied to ClassDetailsPage and ClassCompactHeader, but `TrialClassesTable.tsx` renders `{classItem.level}` directly without filtering out Detective's "Unknown" level. **Files:** `apps/myk9show/src/components/trials/TrialDetail/TrialClassesTable.tsx:320` (level cell rendering). **Fix:** Use `shouldShowLevel` or inline check to hide level when element is Detective.
