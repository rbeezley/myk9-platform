...
- [x] 10.0f [EXPANDED] [SHARED-SYSTEM GATE] After PR review/merge and explicit approval, record the current version of every affected Edge Function, deploy each with `--no-verify-jwt`, then run one controlled transient-failure smoke or provider-supported test plus ordinary auth/registration/alert smokes without sending bulk production mail. Verify retry telemetry contains only attempt/status/delay. Roll back by redeploying the recorded previous versions and repeating ordinary smokes if any function regresses. Completed 2026-07-12: all 12 affected functions deployed ACTIVE; fail-closed smokes passed; one post-deploy password reset recorded delivered; deterministic 429/503/network tests and PII-free telemetry checks passed 33/33. Previous versions and rollback procedure are recorded in the evidence file.
