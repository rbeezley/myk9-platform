...
| User management | `/admin/users` (gap: not in pageDirectory yet) |
| Role assignment | `/admin/permissions` → `/admin/permissions/users` |
| Role requests | `/admin/role-requests` (gap: not in pageDirectory yet) |
| Payout ledger + platform fee | `/admin/payouts` (gap: not in pageDirectory yet) |
