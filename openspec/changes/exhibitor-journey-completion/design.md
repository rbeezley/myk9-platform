...
- nullable `superseded_at`, `superseded_by_grant_id`

The grant table is admin-readable only because its reason, actor, revocation, and supersession fields are internal. The current user receives only a sanitized source/status/start/end projection through the security-definer entitlement-context RPC. Direct authenticated select/insert/update/delete on the table is denied to non-admins. A server-authorized RPC grants or revokes access atomically, verifies `is_site_admin()`, requires a reason, validates the date range, and locks the target person.

Naturally ended grants remain unrevoked and derive status `expired`. The RPC permits a later non-overlapping grant without rewriting that history. It rejects overlapping effective ranges unless the admin explicitly replaces the current grant; replacement records `superseded_at` and the successor id rather than a false revocation. Explicit Revoke alone writes revocation fields. Target-person locking and an overlap check serialize concurrent requests so at most one grant is active at any instant while any number of truthful expired/revoked/superseded history rows may remain.
...
- [Concurrent or overlapping grant requests race] → Lock the target person, reject overlapping effective ranges unless explicitly superseded, and preserve naturally expired rows as unrevoked history.
