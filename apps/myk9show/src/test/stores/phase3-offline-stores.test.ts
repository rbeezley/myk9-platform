...
describe('Phase 3 Offline Store Operations', () => {
  beforeEach(() => {
    localStorage.clear();
  ...
  describe('TrialStore - Offline Trial Management', () => {
    ...
    it('should update trial status', () => {
      ...
      // Verify sync queue
      const syncQueue = useSyncQueue.getState().queue;
      const statusUpdate = syncQueue.find(op => 
        op.operation === 'UPDATE_TRIAL' && 
        op.data?.status === 'in_progress'
      );
  ...
  describe('ClassStore - Offline Class Management', () => {
    ...
    it('should update class status and track state changes', () => {
      ...
      const syncQueue = useSyncQueue.getState().queue;
      const statusUpdates = syncQueue.filter(op => 
        op.operation === 'UPDATE_CLASS' && 
        op.entityId === 'class-1'
      );
  ...
  describe('Error Handling and Edge Cases', () => {
    ...
    it('should handle concurrent modifications', () => {
      ...
      const update1 = { ...entry, registrationData: { ...entry.registrationData, handler: 'Handler A' } };
      const update2 = { ...entry, registrationData: { ...entry.registrationData, handler: 'Handler B' } };
