...
export default function SupportInboxPage() {
  ...
  const ticketDataAvailable = !tickets.isLoading && !hasTicketQueryError;
  const availableTickets = useMemo(
    () => (ticketDataAvailable ? (tickets.data ?? []) : []),
    [ticketDataAvailable, tickets.data]
  );
  const filteredTickets = useMemo(
    () => filterTickets(availableTickets, filter),
      ...
      {hasTicketQueryError ? (
        <Alert variant="destructive">
          <AlertDescription className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <span>{getTicketQueryErrorMessage(tickets.error)}</span>
            <Button
              type="button"
              variant="outline"
            ...
            >
              {tickets.isFetching ? 'Retrying…' : 'Retry'}
            </Button>
          </AlertDescription>
        </Alert>
      ) : (
        <div className="grid min-h-[640px] grid-cols-1 gap-4 lg:grid-cols-[360px_minmax(0,1fr)]">
          <aside className="rounded-md border border-border bg-card">
            ...
            <div className="divide-y divide-border">
              {tickets.isLoading ? (
                <div role="status" className="space-y-2 p-4 animate-pulse">
