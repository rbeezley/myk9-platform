...
test.describe('Browse Shows with Real Authentication', () => {
  ...
  test('should authenticate and browse shows successfully', async ({ page }) => {
    ...
    const hasShowContent =
      (await page.locator('text=/show/i').count()) > 0 ||
    ...
    const viewDetailsButtons = await page
      .locator('text=/view.*details/i, text=/details/i, [href*="show"]')
      .count();
    ...
    if (viewDetailsButtons > 0) {
      ...
      if (!hasDetailsContent) {
        const noShowsMessage = await page
          .locator('text=/no.*show/i, text=/not.*found/i')
          .textContent();
  ...
  test('should handle authentication state properly', async ({ page }) => {
    ...
    const isAuthenticated =
      (await page.locator('text=/sign.*out/i').count()) > 0 ||
