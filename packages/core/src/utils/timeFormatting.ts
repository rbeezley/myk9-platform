 ...
 *
 * This module provides consistent time handling across the myK9 platform.
 * Used for scoresheets, entry lists, and anywhere time values need formatting.
 */
...
export function formatTimeInputToMMSS(value: string, maxMinutes?: number): string {
  ...
  if (cleaned.includes(':')) {
    ...
    let minutes = parseInt(parts[0] ?? '0', 10) || 0;
    let seconds = parseInt(parts[1] ?? '0', 10) || 0;
