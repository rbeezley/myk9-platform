...
- [ ] **Phase 4 — error observability** — Code complete 2026-06-14 in branch `codex/dynamic-qa-error-observability`: Sentry SDK/env wiring, hidden source-map upload gating, PII scrubber, React/app error reporting, and role-surface boundaries for at-show, secretary, judge, exhibitor/register, and admin. Remaining: confirm/create external Sentry project + DSN/env vars, then manually verify a thrown dev error reaches the project.
- [x] ~~**Phase 5 — flaky-test quarantine + suite health**~~ — Measurement complete 2026-06-14 (worktree `festive-ellis-2aa50e`). Exit criteria met: 6 consecutive full-suite default-order runs green (925 files / 9035 tests). **No active timing flakes.** A `--sequence.shuffle` probe exposed 8 files of latent test-isolation debt; fixed the 2 intra-file order-dependence cases (`AKCScentWorkEntryForm.test.tsx` persistent-mock reset; `phase5-support-systems.test.ts` store-state reset + a self-contained-ness bug in the smart-suggestions test), verified across default + 9 shuffle seeds. Timing baseline recorded in the plan (suite is jsdom-environment-bound, not test-bound). Remaining cross-file pollution → owner-task below. Details in [`docs/plan-dynamic-qa-infrastructure.md`](docs/plan-dynamic-qa-infrastructure.md#phase-5--flaky-test-quarantine-and-suite-health).
  - [ ] **[QA] Dynamic QA — test-isolation hardening sweep** — 6 files fail only under shuffled full-suite order (cross-file global-state leakage), passing alone: `ReplicationSyncProvider.authGuard`, `ShowAccessCodesCard`, `phase3-integration`, `phase4-template-system`, `armbandQueries.replication`, `DogDetailsTabs`. Prime suspect: `src/test/setup.ts` module-level `localStorageStore` is never cleared in `beforeEach` while Zustand `persist` writes there. Repro: `npx vitest run --sequence.shuffle --sequence.seed=<n>` from `apps/myk9show`. Do **not** enable `--sequence.shuffle` in CI until resolved (would turn latent debt into red builds).
  - [ ] **[QA] Date time-bomb fixture sweep** — `ReplicatedShowsTable.test.ts` hardcoded a "future" show at `2026-06-15` that turned "active" on 2026-06-15 UTC and reddened CI (fixed in PR [#736](https://github.com/rbeezley/myk9-platform/pull/736) by switching to `Date.now()`-relative dates). ~25 other test files hold hardcoded `2026-0[678]` dates (`grep -rnE "'2026-0[678]-" src --include="*.test.*"`); most are benign timestamps but any used as a "future"/"upcoming" assertion is a latent bomb. Sweep them, and consider a frozen clock (`vi.setSystemTime`) for date-window suites.
...
- [ ] **[DB drift] Security advisor remediation triage** — ERRORs resolved in PR [#722](https://github.com/rbeezley/myk9-platform/pull/722): 8 `security_definer_view` errors fixed (`security_invoker=true` on all views), 4 RBAC anon-executable SECURITY DEFINER functions revoked, `view_myk9q_entries` anon exposure closed. Always-true write-capable RLS policies fixed in PR [#727](https://github.com/rbeezley/myk9-platform/pull/727): `show_templates`/`template_fields` → site-admin writes; `achievements` → dog-owner-scoped writes (it is dog-owned user data, not reference data); `judge_certifications` → site-admin-only writes (closes credential fabrication). **Accepted/intentional WARNs:** 2 public buckets (`images` + `premium-published` require public read by design); `show_passcodes` RLS-enabled/no-policies is intentional (SECURITY DEFINER RPCs only); the 83 anon-callable + 85 authenticated-callable SECURITY DEFINER functions are PostgREST/PostGIS/Supabase internals not safely revokable; 16 mutable search paths are Supabase internals. No further action needed unless Supabase advisor re-categorizes.
- [x] ~~**[DB drift] Fix `entries.entry_status` CHECK regression**~~ — Resolved in PR [#722](https://github.com/rbeezley/myk9-platform/pull/722): migration `20260613090000_fix_entry_status_hyphen_constraint.sql` restored the hyphenated request states (`scratch-requested`, `move-up-requested`) while retaining underscore compatibility. Verified 2026-06-14: linked DB migration history includes the fix, the live `entries_entry_status_check` constraint allows both forms, and `pnpm qa:db-drift:enum` reports no enum/CHECK drift findings.
...
- [x] ~~**[DB drift] Decide repo-only edge functions**~~ — Resolved in PR [#727](https://github.com/rbeezley/myk9-platform/pull/727): `send-results` deployed to staging with `--no-verify-jwt` (handles JWT internally via `handle({ auth: 'jwt' })`); `receive-logs` deleted (dead code, no app callers, superseded by Sentry error observability PR [#723](https://github.com/rbeezley/myk9-platform/pull/723)).
...
- [x] ~~**Phase 3 — secretary journey**~~ — Completed in PR [#716](https://github.com/rbeezley/myk9-platform/pull/716): cold-start setup, Show Desk pressure, Entry Management, reports, Results Control, Submit Results, and evidence screenshots captured in `docs/audits/2026-06-ux-journeys/02-secretary-journey.md`.
  ...
  - [ ] **[UX P1] Legacy `phase=show-desk` redirects to Setup not Show Desk** — Implemented locally in branch `codex/wave4-secretary-closeout`: legacy base URLs with `?phase=show-desk` now route to canonical Show Desk and drop the stale phase query. Pending PR/merge.
  - [ ] **[UX High] Submit Results leaves "Send to AKC" enabled beside a missing-registration warning** — Implemented locally in branch `codex/wave4-secretary-closeout`: missing AKC registration numbers now block `Send to AKC`. Pending PR/merge.
...
- [ ] **Phase 4 — cross-role seams** — Read-only baseline captured in `docs/audits/2026-06-ux-journeys/03-cross-role-seams.md`; true latency/state-agreement walks still need approved shared Supabase seed mutations or local Dynamic QA fixtures. Wave 2 remediation merged in PR [#735](https://github.com/rbeezley/myk9-platform/pull/735); fixture-backed proof remains open.
  ...
  - [x] ~~**[UX P1] Align withdrawn/refunded entry state across roles**~~ — Resolved in PR [#735](https://github.com/rbeezley/myk9-platform/pull/735): exhibitor My Entries status/payment copy now treats withdrawn/refunded entries as terminal states; browser mutation proof remains part of Wave 5 fixtures.
