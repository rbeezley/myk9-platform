...
- [MYK9-211](https://linear.app/myk9-platform/issue/MYK9-211): staging mutation proof with a disposable fixture — grant a scoped role, verify the audit event (actor/target/role/exact scope/timestamp) and the `/admin/permissions` rail, revoke, verify again, prove failed/no-op writes nothing, clean up, record evidence → Done. Can run any time after Batch 0 (independent of Batch 1 code). **[EXPANDED] This one needs explicit approval before it runs**: the issue's own reopen comment calls it an "explicitly approved safe test mutation," and it writes role grants plus permanent `permission_audit_log` rows to the shared staging database. Those audit rows are **not cleanable** — the table is an append-only access trail and deleting from it to tidy up would corrupt the very evidence surface under test. So the fixture must be a disposable _person_, the grant must be scoped to a disposable club or show, and the residue is accepted and named in the closure comment rather than removed. Confirm the fixture identity with Richard before writing anything.
  ...
  **[MUTATION PROOF COMPLETE 2026-08-24 — UI fix awaiting PR/deploy.]** Richard approved actor `testadmin@myk9t.com`, seeded profile-only target Test Club (`club@myk9t.com`), temporary `secretary`, exact Heartland club scope, and permanent audit residue after correcting all fixture credentials/secrets and removing `test.myk9.com` auth identities. GitHub production deployment `6068017957` ties successful status `17261468248` and unique Vercel URL `myk9-platform-myk9show-p7wtfun7v-richard-beezleys-projects.vercel.app` to SHA `7f83dc19bfe4d54e4494e7e0b4a08827a935db42`. Read-only baseline `2026-08-24T19:58:23.586689Z` found zero matching target assignments and zero prior target audit rows. The authenticated local Vite `RoleManager` source at that SHA, connected to staging auth/data, created assignment `00024170-bb22-47a8-bf3c-f9852d5bab84`; invalid person `00000000-0000-0000-0000-000000000211` failed before insert; scoped revoke returned `true`; repeated revoke returned `false`. Staging now holds exactly `role_assigned` event `ea61ab25-36bb-483e-a18c-c83cda658684` (`2026-08-24T20:01:23.271961Z`) and `role_revoked` event `7016a266-22b5-4419-ac34-a64741f780df` (`2026-08-24T20:01:23.98978Z`) with the exact actor, target, role, Heartland `club_id`, and null `show_id`. The assignment is inactive and active matching authority count is zero. The deployed overview at 1440×900 displayed both readable Test Club events (private screenshot SHA-256 `6a43d67bc99dbdce8a63ad4e33da27eab1a49fff0baf159bbd989b6fc7e99093`). The first full-audit replay exposed that revoke Details was blank because the page/export read only `new_value`; its defect screenshot SHA-256 is `60a4fd9733370645aacd42876b9b3c0411194a0778f46099c8d33e139c9681bf`. The branch now falls back to `old_value`, with an assertion-first regression and a passing local live-data replay (private SHA-256 `1a5afb8876f704ca5dbdd73b72ae775d1efb575c0b61d154c510d75d71f6c72d`). Four focused files / 35 tests passed under six shuffled seeds; app-local typecheck/lint, `git diff --check`, and strict OpenSpec validation passed. Closure requires CI, merge, a successful production deployment tied to the merged SHA, and a fresh staging screenshot proving revoke role/scope details.
