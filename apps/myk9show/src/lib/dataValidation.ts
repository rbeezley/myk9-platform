...
export const ValidationSchemas = {
  ...
  dog: z.object({
    ...
    name: z
      ...
      .regex(
        /^[a-zA-Z\s'-]+$/,
        'Dog name can only contain letters, spaces, hyphens, and apostrophes'
      ),
    breed: z
      .string()
    ...
    microchip_number: z
      ...
      .optional()
      .refine(
        val => !val || /^[0-9A-F]{15}$/.test(val),
        'Microchip number must be 15 hexadecimal characters'
      ),
...
export class DataConsistencyChecker {
  ...
  async checkDogOwnerConsistency(): Promise<{ valid: boolean; issues: string[] }> {
    ...
    try {
      const dogs =
        (this.queryClient.getQueryData(queryKeys.dogs) as Array<{
          id: string;
          owner_id: string;
          name: string;
        }>) || [];
      const users =
        (this.queryClient.getQueryData(queryKeys.users.all) as Array<{
          id: string;
          first_name: string;
          last_name: string;
        }>) || [];
      ...
      Object.entries(dogsByOwner).forEach(([ownerId, ownerDogs]) => {
        const nameCount = ownerDogs.reduce(
          (acc, dog) => {
  ...
  async checkShowClubConsistency(): Promise<{ valid: boolean; issues: string[] }> {
    ...
    try {
      const shows =
        (this.queryClient.getQueryData(queryKeys.shows) as Array<{
          id: string;
          club_id: string;
          name: string;
          date: string;
      ...
      const clubs =
        (this.queryClient.getQueryData(queryKeys.clubs) as Array<{
          id: string;
          name: string;
          is_active: boolean;
        }>) || [];
      ...
      orphanedShows.forEach(show => {
        issues.push(
          `Show "${show.name}" (${show.date}) references non-existent club ${show.club_id}`
        );
      });

      // Check for shows with inactive clubs
      const inactiveClubShows = shows.filter(
        show => !activeClubIds.has(show.club_id) && allClubIds.has(show.club_id)
      );
      inactiveClubShows.forEach(show => {
        const club = clubs.find(c => c.id === show.club_id);
        issues.push(
          `Show "${show.name}" (${show.date}) is hosted by inactive club "${club?.name}"`
        );
      });
    } catch (error) {
    ...
    try {
      ...
      Object.entries(emailCount).forEach(([email, usersWithSameEmail]) => {
        if (usersWithSameEmail.length > 1) {
          const userNames = usersWithSameEmail
      ...
      shows.forEach(show => {
        ...
        if (show.entry_deadline) {
          ...
          if (entryDeadline >= showDate) {
            issues.push(
              `Show "${show.name}": Entry deadline (${show.entry_deadline}) must be before show date (${show.date})`
            );
          }
        }
        ...
        if (show.premium_deadline) {
          ...
          if (premiumDeadline >= showDate) {
            issues.push(
              `Show "${show.name}": Premium deadline (${show.premium_deadline}) must be before show date (${show.date})`
            );
          }
          ...
          if (show.entry_deadline) {
            ...
            if (premiumDeadline >= entryDeadline) {
              issues.push(
                `Show "${show.name}": Premium deadline (${show.premium_deadline}) should be before entry deadline (${show.entry_deadline})`
              );
            }
          }
        }
...
export const useDataValidation = () => {
  ...
  return {
    ...
    checkRegistrationUniqueness:
      consistencyChecker.checkRegistrationUniqueness.bind(consistencyChecker),
