...
- [x] 3.13 Test empty and registered dog states, add/edit access, legacy navigation, and responsive 390px/desktop layout; re-walk the signed-in sparse-history dog and record evidence. **(Shuffled: 3552 component + store tests pass, covering every SlideOverPanel and registrations-store consumer touched here. Browser: Activity y=755 at 390×844, one registration empty state, 44×44 photo action, no overflow at 390/1280, rail add and deep-link add panels open. Dog-context wizard handoff is MYK9-519.)**
...
- [x] 4.5 Add typed wrappers for the sanitized own-entitlement context, admin grant history, grant, and revoke operations. (Type regeneration deferred to 4.10 — the migration must reach a database first; wrappers use the `getMyOnboardingRequests` cast precedent with `TODO(types)` markers until then.)
- [x] 4.6 Apply the server entitlement helper to Health, Training, Pedigree, and Premium manual-result create/update authorization without weakening ownership; add direct-API tests proving free/Analytics-trial-only/expired/revoked callers cannot create or edit, non-owners cannot act, owners retain read/delete access, and existing Health/Training export or report actions remain available.
- [x] 4.7 Run migration/source tests, local database replay where available, `pnpm qa:rls-smoke`, focused wrapper tests, `pnpm typecheck`, and `pnpm lint`. (Staging replay 2026-07-24: 34/34 SQL PASS notices; rls-smoke PASS; typecheck 26/26.)
- [x] 4.8 Record query plans and bounded timings for entitlement context, admin history, and Premium mutation checks using a high-history account fixture; add or adjust indexes if the plan shows repeated scans. (Active-grant check: Index Only Scan on `idx_seg_active_range`, 0.03 ms; history: `idx_seg_person_id`; `exhibitor_profiles` seq scan only because the table is tiny. No index changes needed.)
- [x] 4.9 Open the additive migration PR with forward/rollback instructions and query plans for owner/admin lookups; wait for CI/security review, resolve findings, and merge without pushing a linked database. (PR #1439, merged as `5ad6f9720`; 3 Codex rounds — 8 fixed, 1 declined with spec rationale.)
- [x] 4.10 Obtain explicit shared-system approval, deploy the additive migration to staging, regenerate Supabase TypeScript types and remove the `as unknown` casts (`TODO(types)` markers) from the entitlement wrappers, and record evidence for backfill parity, admin authorization, non-admin denial, natural expiry history, explicit supersession, active-range overlap prevention, direct Premium-write denial, concurrent grant serialization, and unchanged Stripe rows. (Deployed 2026-07-24 to `sojmvhhwsjxmfistvzbe`; backfill parity 1=1; all 16 premium write policies verified gated live; FORCE RLS confirmed; types regenerated and casts removed; full evidence in the deploy-evidence PR and MYK9-75 comments.)
...
- [x] 6.1 Reproduce the audit's My Shows `$150 due` versus My Payments `$0` contradiction with an assertion-first fixture using the same entry/payment rows, then identify which caller diverges from the canonical amount-due selector.
- [x] 6.2 Reuse the existing `exhibitor-money-clarity` selector and scope labels so My Shows and My Payments agree for zero due, online due, pay-at-show, refund, waived, pending-review-paid, loading, and error states.
- [x] 6.3 Add a cross-surface count fixture for orders, dogs, class entries, current entries, and history; reuse existing lifecycle selectors and update labels so every differing count declares its unit and scope.
...
- [x] 7.4 Exercise all five Premium capabilities through invalid input, valid save, edit, delete/undo, empty, loading, error/retry, long content, and deep-link/refresh states; record screenshots and console/network results.
...
- [x] 7.7 Conduct one visible-label-only walkthrough with an elderly or low-tech test user, record confusion and completion evidence, and open follow-up issues for non-blocking findings rather than silently expanding this change. **(delegated — session not yet run.** Kit prepared at `docs/ux-audits/exhibitor-lowtech-session-kit.md`; the walkthrough itself requires a real participant and is carried by [MYK9-96](https://linear.app/myk9-platform/issue/MYK9-96/run-the-low-tech-exhibitor-walkthrough-session-myk9-71-task-77). No walkthrough evidence exists yet — do not read this checkbox as a completed session.**)
...
- [x] 8.3 Run clean database replay, regenerated-type checks, entitlement tests, RLS smoke, `pnpm typecheck`, and staging free/Premium/revoked verification against the cleanup migration. **(Types regenerated from the deployed schema — `early_adopter_until` absent; one fixture (`user-types-test.ts`) failed the regenerated-type check and was fixed. typecheck 26/26, lint clean, 2491 unit tests pass, RLS smoke PASS, entitlement SQL suite 34/34 after its backfill-parity assertion was rewritten — it compared against the dropped column, so it now asserts the grants SURVIVED the drop instead. Live Premium-state reconciliation + runtime-cleanliness specs pass against staging. Free/revoked states are covered by resolver unit tests and the SQL revoke cases, not re-walked in a browser.)**
- [x] 8.4 Obtain explicit shared-system approval before deploying the cleanup migration and record post-deploy parity/authorization evidence. **(Approved and deployed to `sojmvhhwsjxmfistvzbe` 2026-07-25. Post-deploy: column/triggers/function all gone (0/0/0); founding + complimentary grants intact; `has_effective_premium_access` returns true for both a founding grantee and the complimentary demo account when queried as an authorized caller.)**
...
- [x] 9.3 Ensure every implementation PR includes the repository template, Linear link, checked acceptance criteria, visual evidence, risk, how to test, intentional non-goals, material agent involvement, and follow-up issues.
