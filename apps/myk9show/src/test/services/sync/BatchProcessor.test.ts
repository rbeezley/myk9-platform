...
import { SyncOperation, SyncPriority, SyncStatus, BatchResult } from '@/types/performance-types';
...
describe('BatchProcessor', () => {
  ...
  describe('Batch Processing', () => {
    it('should process batches and emit events', async () => {
    ...
    it('should respect max parallel batches limit', async () => {
      const operations = Array.from({ length: 200 }, (_, i) => createMockOperation(i.toString()));
  ...
  describe('Adaptive Batch Sizing', () => {
    it('should adjust batch size based on performance', async () => {
      ...
      const operations = Array.from({ length: 50 }, (_, i) => createMockOperation(i.toString()));
    ...
    it('should retry failed batches with exponential backoff', async () => {
      await new Promise<void>(resolve => {
        ...
        batchProcessor.on('batch:retry', event => {
          ...
          if (retryCount === 2) resolve();
        });
        batchProcessor.once('batch:processed', () => {
          if (retryCount === 0) resolve();
        });
        batchProcessor.queueOperations(operations);
      });
    });

    it('should emit final failure after max retries', async () => {
      await new Promise<void>(resolve => {
        ...
        batchProcessor.once('batch:processed', resolve);
        batchProcessor.queueOperations(operations);
      });
    });
  });
  ...
  describe('Compression Integration', () => {
    it('should apply compression when beneficial', async () => {
      const operations = Array.from({ length: 10 }, (_, i) => createMockOperation(i.toString()));
  ...
  describe('Metrics and Monitoring', () => {
    ...
    it('should track processing metrics per entity type', async () => {
      const operations = [createMockOperation('1', 'dog'), createMockOperation('2', 'person')];
  ...
  describe('Edge Cases', () => {
    it('should handle very large batch sizes', async () => {
      ...
      const operations = Array.from({ length: 5000 }, (_, i) => createMockOperation(i.toString()));
    ...
    it('should handle concurrent queue operations', async () => {
      const operations1 = Array.from({ length: 10 }, (_, i) => createMockOperation(`batch1-${i}`));

      const operations2 = Array.from({ length: 10 }, (_, i) => createMockOperation(`batch2-${i}`));
