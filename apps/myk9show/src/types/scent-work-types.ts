...
export function isMultiAreaClass(
  element: ScentWorkClassConfig['element'],
  level: ScentWorkClassConfig['level']
): boolean {
...
export function getAreaTimeLimits(
  element: ScentWorkClassConfig['element'],
  level: ScentWorkClassConfig['level']
): number[] {
  ...
  if (element === 'Interior' && level === 'Masters') {
    const mastersLimits = SCENT_WORK_TIME_LIMITS.Interior.Masters as {
      area1: number;
      area2: number;
      area3: number;
    };
...
export function validateMultiAreaScentWorkResult(
  result: Partial<MultiAreaScentWorkResult>
): boolean {
  ...
  return result.areaResults.every(
    (area: AreaResult) =>
