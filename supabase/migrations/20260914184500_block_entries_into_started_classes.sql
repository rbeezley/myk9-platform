-- =============================================================================
-- MYK9-516: refuse an entry into a class the judge has already started.
--
-- The registration wizard now greys out a class whose `status` is 'in_progress'
-- or 'completed' (ClassSelectionStep.availability.ts, `getClassEntryWindow`).
-- That rule must never be the only guard: the client can be stale, offline, or
...
-- is reverted. The only edits are the `v_class_status` variable, reading
-- `c.status` in the class lookup, and the two guards.
-- =============================================================================
