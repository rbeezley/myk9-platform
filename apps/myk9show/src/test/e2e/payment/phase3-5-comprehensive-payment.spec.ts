...
test.describe('Phase 3.5: Comprehensive Payment Processing', () => {
  ...
  test.describe('Credit Card Processing', () => {
    test('should process successful credit card payment with security validation', async ({
      page,
    }) => {
      ...
      await expect(page.locator('[data-testid="payment-processing-message"]')).toContainText(
      ...
      await expect(page.locator('[data-testid="payment-confirmation-email"]')).toContainText(
        'sent to'
      );
    ...
    test('should handle credit card validation errors', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="card-number-error"]')).toContainText(
        'Invalid card number'
      );
    ...
    test('should handle declined credit card payments with recovery options', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="decline-message"]')).toContainText(
    ...
    test('should handle 3DS authentication flow', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="3ds-message"]')).toContainText(
  ...
  test.describe('Check Payment Processing', () => {
    test('should process check payment with tracking', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="mailing-address"]')).toContainText(
        'Springfield Dog Club'
      );
    ...
    test('should prevent duplicate check numbers', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="duplicate-check-warning"]')).toContainText(
        'already been used'
      );
  ...
  test.describe('Refund Processing', () => {
    test('should calculate and process early refunds', async ({ page }) => {
      ...
      await page.fill(
        '[data-testid="refund-notes-textarea"]',
        'Exhibitor unable to attend due to emergency'
      );
    ...
    test('should calculate and process late refunds with higher fees', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="fee-explanation"]')).toContainText(
    ...
    test('should handle partial refunds for multi-class entries', async ({ page }) => {
      ...
      await page.fill(
        '[data-testid="refund-notes-textarea"]',
        'Classes 2 and 3 cancelled due to judge unavailability'
      );
    ...
    test('should handle refund authorization workflow', async ({ page }) => {
      ...
      await page.fill(
        '[data-testid="refund-notes-textarea"]',
        'Payment processing error - duplicate charge'
      );
      ...
      await page.fill(
        ...
        'Approved - confirmed duplicate processing error'
      );
  ...
  test.describe('Payment Status Tracking and Audit', () => {
    test('should maintain comprehensive payment audit trail', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="audit-entry-1"]')).toContainText(
        'Payment initiated'
      );
      await expect(page.locator('[data-testid="audit-entry-2"]')).toContainText('Card validated');
      await expect(page.locator('[data-testid="audit-entry-3"]')).toContainText(
        'Payment processed'
      );
      await expect(page.locator('[data-testid="audit-entry-4"]')).toContainText(
        'Confirmation sent'
      );
      ...
      await expect(page.locator('[data-testid="audit-details"]')).toContainText(
        'Method: Credit Card'
      );
      await expect(page.locator('[data-testid="audit-details"]')).toContainText(
    ...
    test('should track payment status transitions correctly', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="status-history-1"]')).toContainText(
        'pending → paid_by_check'
      );
    });
  ...
  test.describe('Financial Reporting and Reconciliation', () => {
    ...
    test('should handle payment reconciliation workflow', async ({ page }) => {
      ...
      await page.click(
        '[data-testid="outstanding-payment-1"] [data-testid="mark-received-button"]'
      );
