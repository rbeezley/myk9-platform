...
import { LoadTestDataGenerator, PerformanceMonitor, PERFORMANCE_TARGETS } from './LoadTestFramework';
...
class DatabaseLoadTester {
  ...
  async runCrudLoadTest(config: DatabaseTestConfig): Promise<DatabaseTestResults> {
    ...
    return {
      ...
      averageResponseTime: crudMetrics.totalOps > 0 ? crudMetrics.totalTime / crudMetrics.totalOps : 0,
  ...
  async runComplexQueryTest(config: DatabaseTestConfig): Promise<DatabaseTestResults> {
    ...
    return {
      ...
      averageResponseTime: queryMetrics.totalOps > 0 ? queryMetrics.totalTime / queryMetrics.totalOps : 0,
      p95ResponseTime: Math.max(...Object.values(metrics).map(m => m.p95)),
      throughput: queryMetrics.totalOps / (totalTime / 1000),
      errorRate: queryMetrics.totalOps > 0 ? (queryMetrics.errors / queryMetrics.totalOps) * 100 : 0,
      connectionErrors: 0,
      timeoutErrors: 0
    };
  }
  ...
  private async performAggregateQuery(client: SupabaseClient, metricKey: string): Promise<void> {
    ...
    try {
      ...
      //   const key = `${show.type}_${show.status}`;
      //   acc[key] = (acc[key] || 0) + 1;
      //   return acc;
  ...
  private async performComplexFilterQuery(client: SupabaseClient, metricKey: string): Promise<void> {
    this.performanceMonitor.startMeasurement(metricKey);
