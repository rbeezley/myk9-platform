...
const UpcomingShowsSection: React.FC<UpcomingShowsSectionProps> = ({ showAddDialog, onAddDialogClose }) => {
  ...
  React.useEffect(() => {
    if (competitions.length === 0) {
      mockCompetitions.forEach((comp) => addCompetition(comp));
    }
    ...
    <section className="py-2 text-foreground transition-colors duration-300">
      <div className="w-full">
        <div className="flex flex-col gap-3 w-full px-4">
          {/* Responsive grid for upcoming shows */}
            ...
            {competitions.map((show) => (
              <SectionCard key={show.id}>
                <div className="relative flex flex-col gap-2">
                  ...
                  {/* You may add additional Competition fields here if needed */}
                  <div className="text-xs text-muted-foreground">{show.date ? formatDateMMDDYYYY(show.date) : ''}</div>
          ...
          >
            <div className="text-base text-foreground">
              {selectedCompetition ? (
                <>Are you sure you want to delete <b>"{selectedCompetition.name}"</b>? This action cannot be undone.</>
              ) : null}
            </div>
          </StandardDialog>
          {/* End Responsive grid */}
