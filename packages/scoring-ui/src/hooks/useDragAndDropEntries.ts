 ...
 * - Database sync: Persists order via callback
 ...
 *   onUpdateOrder: (entries) => updateExhibitorOrder(entries),
 * });
 *
 * // In render
 * <DndContext
 ...
 * ```
 */
export function useDragAndDropEntries<T extends BaseEntry>({
  localEntries,
  setLocalEntries,
  currentEntries,
  ...
  const [_internalManualOrder, setInternalManualOrder] = useState<T[]>([]);
