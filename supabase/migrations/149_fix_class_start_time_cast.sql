-- =============================================================================
-- Migration 149: Fix classes.start_time TEXT→TIME cast in create_show_with_children
--
-- Bug: Migration 146 introduced `NULLIF(v_class->>'start_time', '')` without a
-- ::TIME cast. The ->> operator always returns TEXT; Postgres has no implicit
-- cast TEXT→TIME, so even a NULL value of TEXT type triggers:
--   "column start_time is of type time without time zone but expression is of type text"
--
-- Fix: add ::TIME to the start_time expression.
-- All other logic is unchanged from migration 146.
-- =============================================================================
...
COMMENT ON FUNCTION public.create_show_with_children(jsonb, jsonb, jsonb, uuid[]) IS
  ...
  'Fixed in migration 149: added ::TIME cast to classes.start_time to resolve implicit '
  'TEXT→TIME cast failure (Postgres has no such implicit cast). '
