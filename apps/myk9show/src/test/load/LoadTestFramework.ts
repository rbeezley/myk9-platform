...
export class LoadTestDataGenerator {
  ...
  generateUser(role: UserRole = UserRole.EXHIBITOR): User {
    return {
  ...
  generateShow(): Show {
    ...
    return {
      ...
      entryCloseDate: faker.date.future({ years: 1 }).toISOString(),
  ...
  private getRandomRole(): UserRole {
    const weights = [
      { role: UserRole.EXHIBITOR, weight: 0.7 },
      { role: UserRole.SECRETARY, weight: 0.15 },
      { role: UserRole.JUDGE, weight: 0.1 },
