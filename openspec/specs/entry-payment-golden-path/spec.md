# entry-payment-golden-path Specification

## Purpose
Entry start, dog selection, payment, confirmation, and late-entry flows share
the existing registration wizard, cart, checkout, Show Desk, and confirmation
surfaces. Role-specific entry paths route users into the right mode without
duplicating the underlying workflow.
