# Emergency Trial Packet Specification
...
Every page SHALL display `SNAPSHOT — NOT LIVE`, the generation timestamp, show name, trial date, and page number, plus ring and class identity on class-specific pages when that sport uses rings.

#### Scenario: A printed stack is dropped or mixed
- **WHEN** pages are separated from their original order
- **THEN** staff can identify the show, trial day, applicable ring, class, generation, and page sequence needed to reassemble them
...
- **WHEN** another packet is prepared for the same show and trial day
