  ...
  persist(
    (set, get) => ({
      ...
      generateClassesFromTemplate: templateId => {
        ...
        const { fields, maxEntriesDefault, requiresJumpHeight } = template;
        ...
        const matchingPreset = Object.values(CLASS_TEMPLATE_PRESETS).find(
          preset =>
            preset.template.name === template.name &&
            preset.template.organization === template.organization
        );
