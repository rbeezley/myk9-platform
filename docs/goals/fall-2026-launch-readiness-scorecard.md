...
| Exhibitor golden path | An exhibitor can find a show, enter a dog, pay, receive updates, understand show-day status, and view results. | Browser walkthrough; payment/entry test data; real-user test. | Yellow — re-walked steps 1–7 ([`plan-ux-journey-phase6.md`](../plan-ux-journey-phase6.md)). 2026-06-15: find→enter→pay as `exhibitor1` (P1-01 class seed; [#767](https://github.com/rbeezley/myk9-platform/pull/767)/[#768](https://github.com/rbeezley/myk9-platform/pull/768)). 2026-06-16: confirmation/status surface coherent (past entries show historical labels "Review incomplete"/"Payment unresolved", not actionable; check-in + where/when + post-deadline "Message the show team" present) — no defects. **Step 8 (view results) 2026-06-16 — partial**: approved seed scored the whole class so placements computed (exhibitor1 = 2nd place Q); My Entries shows "Q 42.5s" but **omits placement** (P3), and the public class results page shows **stale "Pending/0th"** to the exhibitor (reads replication store, not a direct read — P2, #768-class). P1-01/P1-02/P1-03 cleared. Residual for Green: step-8 findings A (placement on My Entries) + B (class-results staleness), plus **P1-04** (refund/withdrawn state agreement; deferred seam-walk). |
