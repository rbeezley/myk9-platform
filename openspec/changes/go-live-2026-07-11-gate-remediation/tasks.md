...
- [ ] 10.0f [EXPANDED] [SHARED-SYSTEM GATE] After PR review/merge and explicit approval, record the current version of every affected Edge Function, deploy each with `--no-verify-jwt`, then run one controlled transient-failure smoke or provider-supported test plus ordinary auth/registration/alert smokes without sending bulk production mail. Verify retry telemetry contains only attempt/status/delay. Roll back by redeploying the recorded previous versions and repeating ordinary smokes if any function regresses.
