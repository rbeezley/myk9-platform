...
- [ ] 4.1 Make the `My Shows` h1 visible on all viewports (remove `sr-only`), keeping heading order; adjust greeting layout
- [ ] 4.2 "Enter a Show" CTA: raise from `size="sm"` to ≥44px height; sidebar close control to ≥44px
- [ ] 4.3 `PrimaryTabs.tsx`: prevent label illegibility at 390px (min-width per trigger, strip scrolls) and add a visible overflow edge cue; apply the same cue pattern to the dog strip
- [ ] 4.4 Swap the generic EmptyState for `EMPTY_STATE_BY_TAB`-driven rendering; replace error copy with the offline-first phrasing (Retry unchanged)
- [ ] 4.5 `AppHeader.tsx`: at <md widths move theme toggle and AskQ assistant into the existing account dropdown as labeled items; hide their standalone icons on phone only; desktop unchanged; add/update header tests
- [ ] 4.6 Typography pass: bump primary-reading muted copy sizes/contrast per design (component-level classes only, no global token edits); verify both dark and light themes
- [ ] 4.7 Extract empty/error states, tab defs, and section scaffolding from `MyEntriesPage/index.tsx` into sibling modules so the file is under 500 lines with behavior unchanged

## 5. Verification

- [ ] 5.1 `pnpm typecheck` clean (clear stale tsbuildinfo if new files were added)
- [ ] 5.2 `cd apps/myk9show && npx vitest run src/pages/MyEntriesPage src/components/exhibitor src/components/common/PrimaryTabs* src/components/layout` (plus any relocated test paths) — all green
- [ ] 5.3 `pnpm lint` clean (watch react-hooks/set-state-in-effect and max-warnings 0)
- [ ] 5.4 Manual/preview walk at 390px phone, 834px tablet portrait, and desktop, dark AND light mode [EXPANDED]: visible title, count labels, collapsed cards with next action, per-tab empty states, header consolidation, 44px targets (inspect computed heights for CTA, directions, sidebar close)
- [ ] 5.5 Confirm no `// INTENT:` comments were removed or contradicted (index.tsx, CompactStatsRow, FirstRunZeroState) and no new pages/dialogs/menus were added
