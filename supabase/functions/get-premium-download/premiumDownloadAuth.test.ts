...
describe('hasShowManagementAccess', () => {
  ...
  it('does not query manager scope for an invalid bearer token', async () => {
    ...
    await expect(hasShowManagementAccess(showId, 'Bearer invalid-token', auth)).resolves.toBe(
      false
    );
  ...
  it.each([
    ...
    await expect(hasShowManagementAccess(showId, 'Bearer valid-user-jwt', auth)).resolves.toBe(
      true
    );
