...
| 2.10 | **F12** (P2)  | The class Edit dialog lists only judges already attached to the show, so a secretary needing a different judge has no path from the dialog they are in; the judge must be added first via the header **Actions → Show settings… → Judges** tab (it was More show actions → Edit → Judges until MYK9-630 deleted that menu). Link the two halves, or allow adding inline? **Also missed when this plan was written.**        |
