...
test.describe('Show Edit Debug', () => {
  test('debug show edit functionality', async ({ page }) => {
    ...
    const showElements = await page
      .locator('[data-testid*="show"], .show-item, a[href*="/shows/"]')
      .all();
    ...
    if (await firstShow.isVisible()) {
      ...
      if (editButton) {
        ...
        const nameField = page
          .locator(
            'input[value], input[placeholder*="name" i], label:has-text("name") + input, input[id*="name"]'
          )
          .first();
        ...
        if (await nameField.isVisible()) {
          ...
          if (saveButton) {
            ...
            const dialogStillOpen = await page
              .locator('.dialog, [role="dialog"], .modal')
              .isVisible();
