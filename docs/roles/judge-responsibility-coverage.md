...
| --------------------------------------------------------------------------------- | ---------- | -------------------------------------------------------------------------------------- | ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Get into the ringside scoring experience with a show passcode, no account needed. | Required   | `/at-show` SmartSignInPage → `validate-passcode` edge fn → anon session with claim     | Evidence partial | Inventory complete 2026-07-10. **Remediated (R3):** throttle RPCs (`check_login_rate_limit`/`record_login_attempt`) were missing from every migration AND absent on the live DB (confirmed via `git log -S` + catalog query); the edge fn failed open, so the throttle was a silent no-op since launch. Backfilled in migration 20260710140000 (pushed to staging): 5/15min per IP, 30min block. Judge-device cold-session walk still pending.                                          |
...
| Stay scoped to the correct show and survive session expiry gracefully.            | Required   | `RingsideShowBoundary`, `RingsideSessionHeartbeat`, `AtShowAccessGate`                 | Evidence partial | Inventory 2026-07-10; scoping enforced by `AtShowAccessGate` (tested). **Remediated (R2):** `validate-passcode` now stamps `passcode_generation` (from `show_passcodes.created_at`, bumped on every regenerate); both ringside RPCs reject a stale generation with 42501 in the passcode-claim arm only (edge fn deployed + migration 20260710150000 pushed, live-verified). Client surfaces "Access revoked — re-enter code". Live regeneration walk still recommended as final proof. |
...
| Know the search-area parameters for the class (hides, distractions, time). | Required   | Class detail data on ringside pages                             | Evidence partial | **Remediated (R4):** `classes.hides_known`/`distraction_count` (present since migration 033) are now plumbed through `buildClassInfo` → `ClassInfo` → the ringside class-details popover (falsy-safe; adapter tests cover false/0/omitted). Class-aggregate only — per-area granularity remains a deferred schema question, not a fall gap. |
...
| Escalate anything the judge cannot fix to the secretary without app-hopping. | Required   | Same-app: `/at-show` and secretary surfaces in one myK9Show  | Evidence partial | **Remediated (R1):** announcements had zero wiring into `/at-show`; now the existing `announcementStore` + `AnnouncementItem` are mounted read-only inside `AtShowAccessGate`, scoped to the session's show. No RLS change needed (`show_announcements` SELECT already admits the anon passcode session; pinned by a contract test). No composer — escalation stays judge-sees + in-person relay. |
