  ...
  start: (testName: string, category: keyof typeof TIMEOUT_CONFIG) => {
    ...
    return {
        ...
        if (duration > threshold) {
          logger.warn(
            `⚠️ Slow test: ${testName} took ${duration.toFixed(2)}ms (>${threshold.toFixed(2)}ms)`,
            'app',
            {}
          );
        }
  ...
  trackRender: (componentName: string) => {
    ...
    if (newCount > renderingLoopPrevention.maxRenders) {
      const error = new Error(
        `Possible infinite render loop detected in ${componentName} (${newCount} renders)`
      );
...
export const memoryLeakDetection = {
  ...
  checkLeaks: () => {
    ...
    if (leakCount > 50) {
      ...
      logger.warn('Active components:', 'config', {
        data: Array.from(memoryLeakDetection.activeComponents),
      });
    }
    return leakCount;
  },
