...
describe('buildRunSheetEntries', () => {
  ...
  it('prefers call_name over name', () => {
    const [e] = buildRunSheetEntries(
      [makeRow({ dog: { id: 'd1', name: 'Registered Name', call_name: 'Buddy', breed: null, owner: null } })],
      'runOrder'
    );
  ...
  it('falls back to name when call_name is null', () => {
    const [e] = buildRunSheetEntries(
      [makeRow({ dog: { id: 'd1', name: 'Registered Name', call_name: null, breed: null, owner: null } })],
      'runOrder'
    );
  ...
  it('builds a qualified result from a scored row', () => {
    const [e] = buildRunSheetEntries(
      [makeRow({ is_scored: true, result_status: 'qualified', search_time_seconds: 95.5, total_faults: 0, final_placement: 1, judge_notes: 'Good search' })],
      'runOrder'
    );
  ...
  it('sorts by runOrder ascending', () => {
    const rows = [makeRow({ id: 'a', run_order: 3 }), makeRow({ id: 'b', run_order: 1 }), makeRow({ id: 'c', run_order: 2 })];
  ...
  it('sorts by armband ascending (numeric, not lexicographic)', () => {
    const rows = [makeRow({ id: 'a', armband: '10' }), makeRow({ id: 'b', armband: '3' }), makeRow({ id: 'c', armband: '7' })];
  ...
  it('builds ownerName from first + last', () => {
    const [e] = buildRunSheetEntries(
      [makeRow({ dog: { id: 'd1', name: 'Rex', call_name: null, breed: null, owner: { id: 'o1', first_name: 'John', last_name: 'Doe' } } })],
      'runOrder'
    );
  ...
  it('returns empty ownerName when owner is null', () => {
    const [e] = buildRunSheetEntries(
      [makeRow({ dog: { id: 'd1', name: 'Rex', call_name: null, breed: null, owner: null } })],
      'runOrder'
    );
