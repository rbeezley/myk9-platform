...
export class CompressionService {
  ...
  private algorithmStats: Map<
    CompressionAlgorithm,
    { totalSize: number; compressedSize: number; count: number }
  > = new Map();
  ...
  private initializeDictionaries(): void {
    ...
    this.customDictionaries.set('json', [
      'id',
      'name',
      'type',
      'status',
    ...
    this.customDictionaries.set('sync', [
      'version',
      'timestamp',
      'changes',
      'operations',
   ...
   */
  async compress(data: unknown, options: CompressionOptions = {}): Promise<CompressionResult> {
  ...
  private selectByHistoricalPerformance(): CompressionAlgorithm {
    ...
    this.algorithmStats.forEach(
      (
        stats: { totalSize: number; compressedSize: number; count: number },
        algorithm: CompressionAlgorithm
      ) => {
        if (stats.count > 0) {
    ...
    return {
      ...
      overall: {
        ...
        overallCompressionRatio:
          totalOriginalSize > 0 ? 1 - totalCompressedSize / totalOriginalSize : 0,
      },
    };
  }
