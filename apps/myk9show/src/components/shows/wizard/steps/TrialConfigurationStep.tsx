...
export const TrialConfigurationStep: React.FC<TrialConfigurationStepProps> = ({
  ...
  const handleTrialDateTimeChange = (trialId: string, date: Date | undefined) => {
    if (date) {
      ...
      const updates: { dateTime: string; name?: string } = { dateTime: nextDateTime };
      const changedDay = trial?.dateTime.slice(0, 10) !== nextDateTime.slice(0, 10);
      if (trial?.autoGeneratedName && changedDay) {
        const otherTrials = [
          ...existingTrials,
          ...trials
            .filter(item => item.id !== trialId)
            .map(item => ({ name: item.name, trialDate: item.dateTime })),
        ];
        updates.name = getNextTrialName(otherTrials, nextDateTime);
      }
      updateTrial(trialId, updates);
    }
  };
