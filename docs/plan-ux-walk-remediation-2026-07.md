...
**Rough sizing [ADDED]:** Phase 0 ≈ 1–2 spike-days remaining (4 of 6 verdicts closed at planning time). Phase 1 ≈ 6 packages, S–M. Phase 2 ≈ 7 packages, M each (test-heavy by design). Phase 3 ≈ 6 packages, S–M. Phase 4 ≈ 10 packages, mostly M (4.A/4.B are the L items). Phase 5 ≈ 5 packages, S. Phase 6 ≈ one elapsed week of verification. The audits' per-finding Effort columns remain the finer-grained source when splitting packages.
