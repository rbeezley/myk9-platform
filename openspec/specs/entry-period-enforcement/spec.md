...
Ensures a show's entry-close deadline is enforced server-authoritatively on every entry-creation path — the online checkout path (`stripe-checkout`) and the offline submission RPC (`submit_show_entries`) — so a closed show cannot receive new entries regardless of any client-side gate. The boundary is a timezone-anchored calendar day, so no exhibitor is blocked before the end of the local close day, with an official bypass for legitimate late and day-of entries. Introduced by `exhibitor-closed-show-server-guard` to deliver the server-side half of the cart-integrity requirement (the client cart gate shipped in `exhibitor-ux-remediation` / #1217) and to fix a UTC-instant comparison that closed online entry ~a day early for shows west of UTC.
