...
- **5c — Live cold-session walk. ⬜ TODO.** Create one UKC and one ASCA scent trial on staging; verify premium PDF (AKC/UKC only — see 5b), heritage landing, entry blank, confirmation email, and the registration wizard class-selection all show correct levels/elements/variants/legal language.
...
- Update OPEN-TODOS + flip this plan's status to Complete; `git mv` to `docs/archive/` and drop the README row.
