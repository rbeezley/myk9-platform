...
- [ ] **Execute myK9Show + myK9Q unification plan** — Absorb myK9Q's ringside UI into myK9Show as `/at-show` backed by a shared `packages/ringside`, unify the messaging pipeline (inbox + push + suppression), and eventually delete `apps/myk9q`. Phases 0–6 (foundation → at-show route → auto-routing for entered exhibitors → unified messaging → retire duplicated surfaces → branding/copy → app deletion). Full plan in [docs/plans/2026-05-17-unify-myk9show-myk9q.md](docs/plans/2026-05-17-unify-myk9show-myk9q.md). Context in TO-DOS.md § "Platform Unification — myK9Show + myK9Q — 2026-05-25 10:39".
  ...
  - [ ] **Show Desk tools collapse individually inside the side panel** — The Show Desk Tools sheet currently forces secretaries to scroll through every tool to find the one they need. Make each tool/card in the side panel collapsible with a compact header, sensible default-open behavior for actionable/current tools, and persisted per-show open state if the interaction proves useful. Duplication check: refine the existing `ShowDeskToolsSheet`; do not add another tools page, another drawer, or duplicate tool entry points. Include focused `ShowDeskToolsSheet` coverage for expand/collapse, keyboard/accessibility behavior, and no regressions to the existing tool content.
  - [ ] **At-show trial sections collapsible** — On `/at-show/:showId`, make each trial section in the class picker collapsible so judges/stewards can reduce the long trial list and focus on the current ring/day. Source URL: `https://myk9-platform-myk9show-eolf80hbs-richard-beezleys-projects.vercel.app/at-show/18802fc0-1558-4dc3-902d-989edef4df3c`. Duplication check: this refines the existing `/at-show` surface; do not add a new page or duplicate class-list UI. Include focused `AtShowClassListPage` coverage for expand/collapse state and a mobile/desktop smoke check.
