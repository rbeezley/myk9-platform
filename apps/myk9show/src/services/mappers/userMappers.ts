...
export const mapDatabaseToUser = (dbUser: Record<string, unknown>): User => {
  return {
    ...
    // MYK9-664: no dateOfBirth / juniorHandlerNumbers here. They are not
    // `people` columns any more; they live in `people_private`, readable only by
    // the person and site admins (services/database/users/personPrivate.ts).
