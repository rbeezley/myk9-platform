...
| `apps/myk9show/src/test/e2e/registration/handlerNameSpace.spec.ts`          | The handler-name field accepts spaces, hyphens and apostrophes: `Mary-Jane O'Brien` typed key-by-key survives in the field and on the saved handler row (MYK9-567). Secretary-authed, cart and entry writes mocked. Selected by no config array; run it with `pnpm exec playwright test src/test/e2e/registration/handlerNameSpace.spec.ts` from `apps/myk9show`.                                            |
