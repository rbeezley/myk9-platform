...
export const OrganizationSelector: React.FC<OrganizationSelectorProps> = ({
  ...
  const [trialType, setTrialType] = useState<TrialType | undefined>(selectedTrialType);
  ...
  const organizations = Array.from(
    new Set(templates.filter(t => t.isActive).map(t => t.organization))
  ) as Organization[];
  const trialTypes = organization
    ? (Array.from(
        new Set(
          templates
            .filter((t: ClassTemplate) => t.organization === organization && t.isActive)
            .map((t: ClassTemplate) => t.trialType)
        )
      ) as TrialType[])
    : [];
  ...
  const computedTemplates = React.useMemo(() => {
    if (organization && trialType) {
      return templates.filter(
        t => t.isActive && t.organization === organization && t.trialType === trialType
      );
  ...
  }, [organization, trialType, templates]);
  ...
  const [prevComputedKey, setPrevComputedKey] = useState(
    () => `${organization}-${trialType}-${templates.length}`
  );
  const computedKey = `${organization}-${trialType}-${templates.length}`;
  ...
  const handleTrialTypeChange = (value: string) => {
    if (value) {
      setTrialType(value as TrialType);
    }
  };
    ...
    <div className="space-y-6">
            ...
            {organization && (
              <div className="bg-green-50 border border-green-200 rounded-lg p-3">
                ...
                <p className="text-sm text-green-700 mt-1">
                  {
                    templates.filter(
                      (t: ClassTemplate) => t.organization === organization && t.isActive
                    ).length
                  }{' '}
                  templates available
                </p>
      ...
      {organization && (
        <Card>
          ...
          <CardContent>
            <div className="space-y-4">
              <Select value={trialType || ''} onValueChange={handleTrialTypeChange}>
                <SelectTrigger className="w-full">
              ...
              {trialType && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                  <div className="flex items-center gap-2 text-blue-800">
                    <CheckCircle className="h-4 w-4" />
                    <span className="font-medium">{trialType} selected</span>
                  </div>
                  <p className="text-sm text-blue-700 mt-1">
                    {availableTemplates.length} template(s) available for {organization} {trialType}
                  </p>
                </div>
              )}
            </div>
      ...
      {organization && trialType && availableTemplates.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              {selectedTemplate && (
                <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                  ...
                  <p className="text-sm text-green-700">
                    Ready to preview and select classes. This template includes{' '}
                    {selectedTemplate.classDefinitions.length} classes with{' '}
                    {selectedTemplate.fieldSpecifications.length} configurable fields.
                  </p>
                </div>
              )}
            </div>
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-muted-foreground text-center">
              There are no active templates for {organization} {trialType}. Contact your site
              administrator to create templates for this organization and show type.
            </p>
          </CardContent>
        </Card>
      )}
