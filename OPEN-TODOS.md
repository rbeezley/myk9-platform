...
- [ ] **Unify the two uncoordinated `isSyncing` loops** — `useOfflineQueueProcessor` and `offlineQueueStore.startSync` both read/write an `isSyncing` flag (in separate stores — `syncStatusStore.ts:23` and `offlineQueueStore.ts:53`), and `addToQueue` schedules `setTimeout(() => get().startSync(), 100)` at `offlineQueueStore.ts:133` without coordinating with the processor's own `processingRef + isSyncing` guard. Made the processor test fragile because either loop can leave `isSyncing` stuck `true` if the other crashes mid-flight. Architectural cleanup — collapse to a single sync owner. Full context in TO-DOS.md § "Uncoordinated sync loops — 2026-05-25".
