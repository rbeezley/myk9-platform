...
| `apps/myk9show/src/test/e2e/dialogContainsLongContent.spec.ts`         | A long unbreakable string cannot widen a dialog's other children. Appends an unbreakable string and a `w-full` sibling as direct grid items of the popup and measures the sibling against the popup's own width. Parametrized over both grid-based primitives (`DialogContent` and `AlertDialogContent`), so dropping the guard from either goes red. Geometry-asserted, so it guards every dialog carrying no local `min-w-0`, not only `dialog.tsx`. **Data dependency:** exhibitor-authed (`E2E_DEMO_EXHIBITOR_*`); the `DialogContent` case needs the demo exhibitor to have at least one entry on `/exhibitor/entries` rendering an "Add to calendar" button, and fails with a message naming that seed data rather than a locator timeout. The `AlertDialogContent` case needs no seed data beyond the login (`/account?section=data` -> "Clear Cache"). |
...
| `apps/myk9show/src/test/e2e/admin/health-verdict-responsive.spec.ts`        | Admin health verdict geometry at 768/1024/1440, light+dark (MYK9-434).                                                                                                                                                                                                                                                                                                                       |
| `apps/myk9show/src/test/e2e/registration/handlerNameSpace.spec.ts`          | Handler-name field accepts space/hyphen/apostrophe in a real browser (MYK9-567). Asserts in-page state only: every cart and entry write is mocked and `submit_show_entries` is refused, so nothing is persisted. Seed dependency: `LIVE_REGISTRATION_SHOW_ID` (`QA_REGISTRATION_SHOW_ID`, else the Heartland secretary show) with a dog matching "Ranger" enterable in Container / Novice A. |
| `apps/myk9show/src/test/e2e/theme-mode-selection-border.spec.ts`            | MYK9-591: Theme Mode card selection border via computed `border-color` (Base UI Radio.Root's `data-checked`, not Radix's `data-state=checked`). Admin-authed. Mutates and restores the shared e2e-admin's persisted `user_preferences.mode`; Nightly runs are serialized so no concurrent spec observes the flip.                                                                            |
...
- **Call names are not unique.** The MYK9-109 load fixture repeats them (three
  ...
  dog seeded as "Juniper" is `Select Juni`.

### What the sweep still depends on

"Tolerates drift" is not "has no fixture dependency". Three remain, and the
first one is a clock bomb:

- **The sweep expires 76 days after the last reseed.** `applyRegistrationClock`
  defaults to real time precisely because the window is relative — but the
  window is `CURRENT_DATE - 16 .. + 76`, so once staging has gone 77 days
  without a reseed every exhibitor-path spec goes red with exactly the symptom
  MYK9-545 diagnosed: "This show is not accepting online entries yet." Reseed;
  do not reach for the override.
- **`QA_REGISTRATION_TIME` is not a substitute for a reseed.** `page.clock`
  fakes the BROWSER clock only. The server's own entry-window guard still sees
...
- **Strict-mode locators hold only while no debris collides.** `^Select Willow$`
  ...
  only presence take `.first()`; specs that need a specific dog (the
  multi-exhibitor guard in `secretaryExistingUsers`, which needs two dogs with
  DIFFERENT owners) genuinely need the name to resolve to one row.

**Specs that must run only on a freshly reseeded database: none**, in the sense
that none needs a pristine dog roster or the absence of walk debris. Every spec
in the directory does need a reseed within the last 76 days, per the first
bullet above. If a new spec needs more than that, note it here rather than
letting the sweep fail for whoever runs it next.
...
| `apps/myk9show/src/test/e2e/registration/exhibitorSelfRegistration.spec.ts` | **Currently red.** MYK9-545 got it through the whole wizard against the Heartland fixture (the window was never closed; the spec pinned a clock date outside it), but it now fails on its last assertion: `/cart` overflows horizontally at 390px, `scrollWidth` 415 — MYK9-625. It stays a candidate rather than Nightly Active until that lands. |
...
| `apps/myk9show/src/test/e2e/radio-checkbox-indicator-centering.spec.ts`     | Shared radio/checkbox indicator centering: rendered geometry of the Base UI Radio/Checkbox primitives on the dogs table and the change-status dialog. State-neutral by construction.                                                                                                                                                                                                                                                 |
...
| `apps/myk9show/src/test/e2e/club-admin/crossClubClassScope.spec.ts`         | Real cross-club class scope (MYK9-464): a club-scoped viewer keeps lifecycle controls on their own club but not the operational run sheet, and neither on another club's. Club-admin cases need `E2E_CLUB_ADMIN_PASSWORD` and skip without it.                                                                                                                                                                                       |
...
| `apps/myk9show/src/test/e2e/entities/dogsBulkDeleteBlocked.spec.ts`         | Bulk dog delete refused over paid/scored entries: the dialog and toast both report it, the override stays reachable, and no dog is lost from the list.                                                                                                                                                                                                                                                                               |
...
| `apps/myk9show/src/test/e2e/registration/wizardReentry.spec.ts`             | Exhibitor entry survives browser Back and resumes at the saved step.                                                                                                                                                                                                                                                                                                                                                                 |
| `apps/myk9show/src/test/e2e/registration/wizardRehydrate.spec.ts`           | Exhibitor entry survives a browser reload and resumes at the saved step. It never carried a skip; it was simply red on phone-height projects until MYK9-543 fixed the shared `selectFirstDogAndContinue` helper. Selected by no CI project (`playwright.ci.config.ts` lists it in neither `PR_SMOKE_SPECS` nor `REGRESSION_SPECS`), so phone coverage comes from running it locally on `mobile-chrome` / `mobile-safari` / `tablet`. |
| `apps/myk9show/src/test/e2e/registration/checkoutCancelRecovery.spec.ts`    | Cancelled checkout returns to the wizard with the selections intact and amendable. Shares `selectFirstDogAndContinue`, so MYK9-543 turned it green on `mobile-chrome` too. Selected by no CI project.                                                                                                                                                                                                                                |
| `apps/myk9show/src/test/e2e/registration/chipReclick.spec.ts`               | Re-clicking a selected class chip deselects it: cart count 1 → 0 → 1 → 0 with no console error and no duplicate cart line (MYK9-530). Its `width > 0 && width < 768` skip was removed by MYK9-543, which also un-skips `mobile-safari` (390x844); the `tablet` project is exactly 768 wide and was never skipped. Selected by no CI project (`playwright.ci.config.ts` lists it in neither `PR_SMOKE_SPECS` nor `REGRESSION_SPECS`). |
