...
**Date:** 2026-05-22
**Status:** **Tracked in OpenSpec.** PO interview complete (2026-07-12) — all six edge-case rules locked (recommendations accepted as-is), override-marker design point and candidate (A) trigger confirmed. The full plan has been drafted as an OpenSpec change.
> Tracked in openspec change: `class-status-auto-derivation` (`openspec/changes/class-status-auto-derivation/`). This doc remains the design/decision record; the change's `tasks.md` is the sole execution tracker. Exploration corrected the stub's premise — the server-side derivation already shipped in migration `20260525170000`; the change *refines* it (completeness math, override marker, late-entry reopen) rather than building it greenfield.
