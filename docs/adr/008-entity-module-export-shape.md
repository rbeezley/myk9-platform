...
- 2026-06-14: `services/database/judges/` -- migrated from Shape Y to Shape X (Phase 6 of `plan-architecture-deepening.md`). Flattened 5 query objects (`judgeQualificationQueries`, `judgeAnalyticsQueries`, `judgeAvailabilityQueries`, `judgeCertificationQueries`, plus the already-flat assignment helpers) into 23 entity-prefixed named functions. Updated 7 call sites: `hooks/queries/useJudgeDatabase.ts`, `hooks/queries/useJudgeAnalyticsQuery.ts`, `components/panels/edit/JudgeQualificationPanel.tsx`, `components/panels/edit/UserEditPanel.tsx`, `components/users/UserDetails/JudgeAvailabilityCard.tsx`, `components/shows/wizard/steps/ShowDetailsStep.tsx`, `store/userStore.ts`.
- Pending: `clubs/`, `armbands/`, `registrations/`, `show-registrations/`, `users/`, `waitlists/`, `milestones/`, others to be confirmed during ongoing Scope B consolidation of `services/database/queries/`.
