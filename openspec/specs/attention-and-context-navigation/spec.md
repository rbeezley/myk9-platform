# attention-and-context-navigation Specification

## Purpose
Actionable attention summaries and compact related-context navigation across existing show-management surfaces: typed pending-signal destinations built from the canonical entry/class attention classifiers and route helpers, count-to-destination agreement as the acceptance contract, staff-gated rendering with no new inbox or queue surface, and related links (show, trial, class, entry) built only from already-loaded, show-scoped data with offline behavior preserved.
