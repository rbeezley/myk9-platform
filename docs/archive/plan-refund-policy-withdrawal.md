...
- **Show cancellation:** a **bulk "Refund all entries"** action (make-whole — full incl. platform fee) on the show-management surface, with a confirm + progress + per-result detail (partial-failure tolerant). Gated to club-admin/secretary.
  - **[REVISED 2026-06-27] Make-whole needs a NEW server path — it can NOT reuse `stripe-refund-entry`.** That function hard-caps at the entry fee (`validateRefund` → `amount_exceeds_fee`, "platform fee is never refunded"); make-whole must refund the full original charge. New edge fn **`stripe-refund-show`** ( `{ show_id }` ):
    - **Charge model (verified):** separate charges + separate transfers — the PaymentIntent is a plain platform charge (no `application_fee_amount` / `transfer_data` / `on_behalf_of`); the club's share goes out later as a `cron-process-payouts` **transfer**. So a full-intent refund returns the exhibitor's gross **from the platform balance** — no Connect application-fee clawback or `reverse_transfer` needed.
    - **Group entries by `stripe_payment_intent_id`** (one cart charge can cover N entries) and refund **each intent in full, once** — naturally idempotent (Stripe caps over-refunds) and reuse-guarded via `refunds.list` + `metadata.entry_id`/intent, the same pattern as `stripe-refund-entry`. **ASSUMPTION to verify in impl:** an intent is single-show (carts are per-show); if an intent can span shows, refund only this show's share, not the full intent.
    - **Stamp each entry on the intent with `refund_amount = its own entry_fee`** (service-role write, trigger-guarded). `calculateShowPayoutCents` deducts `refund_amount` per entry → `max(0, fee − fee) = 0`, so the club is paid nothing for the cancelled entries; the **platform-fee slice of the refund is absorbed by platform revenue** (correct party on a club cancellation). A stamp failure after a successful Stripe refund → `alertAdmin` (payout-overpay risk), mirroring `stripe-refund-entry`.
    - **Authz:** reuse the three predicates (`is_show_secretary` / `is_club_admin(show.club_id)` / `is_site_admin`). **Payout guard:** read `show_payouts` once — `completed` → `payout_already_sent` (settle with club manually), `processing` → `payout_in_progress`.
    - **Scale:** **bounded concurrency** over intents (small batch, not 200 parallel Stripe calls). **Skip** (report, not fail) entries already refunded / offline-paid (cash/check/comped) / unpaid / no payment-intent. Re-run safe (refunds.list reuse).
    - **Return** a summary `{ refunded: [{intent, amountCents, entryIds}], skipped: [{entryId, reason}], failed: [{intent, error}] }` for per-result UI.
...
- [x] Bulk cancellation: `buildShowRefundPlan` groups by intent + taints intents with an already-refunded member; `showRefundReuse` idempotency; `stamp_show_refund_entries` RPC stamps each intent **atomically** (no partial-stamp half-state); `stripeRefundShow.source` pins authz (3 predicates), **cancelled-show gate**, payout guard (+ per-intent re-check), full-intent (no-amount) make-whole, atomic RPC stamp, paginated cross-show guard, bounded concurrency; `RefundAllEntriesCard` two-step (mark-cancelled → refund) + per-result summary + no-success-on-zero toast. (29 tests; review #974 findings 1/2/3 + Codex P1a cancellation gate / P1b atomic stamp fixed.)
...
> **Shipped status (2026-06-27):** Phases 1–3 + Phase 4 *voluntary withdrawal* (#967/#969/#970/#973) are **merged AND deployed** — 3 migrations pushed (`20260625200000`, `20260626000000`, `20260627000000`; columns/trigger/grants verified live via `has_column_privilege`), `stripe-webhook` (v66) + `stripe-payment-link` (v4) redeployed. Remaining: Phase 4 *show-cancellation make-whole* (`stripe-refund-show`, this PR). **Deploy order:** push migration `20260628000000_stamp_show_refund_entries.sql` FIRST (the edge fn calls the RPC), then `supabase functions deploy stripe-refund-show --workdir apps/myk9show --project-ref sojmvhhwsjxmfistvzbe --no-verify-jwt`.
