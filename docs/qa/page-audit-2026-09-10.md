...
| full class fixture | `dec1a55e-0000-0000-0000-000000000036` | Container Advanced, Sunday Trial 3 — the only class with `max_entries` set (63, matching its seeded entry count); MYK9-515. As of MYK9-529 it is also the only DELIBERATELY full class on this show: `default_judge_day_capacity` was raised 125→200 because the MYK9-109 load fixture (pinned by id, `loadFixture.test.ts`) put Test Judge's Saturday day (`…021`) at 129 confirmed and Sunday (`…022`) at 127, both over 125, which folded classes `…032`–`…035` into `judgeDayFull` for every exhibitor. |
