...
| `apps/myk9show/src/services/entryService.ts` / `markInRing` | P2 | `markInRing` has no direct app caller found; type/schema audit also found it writes `{ status }` to nonexistent `entries.status`. | finder candidate; Phase-2 pending liveness proof | Delete file/function if liveness proof holds; only fix the schema write if a real caller is discovered. | Cross-reference: type-escape/schema-drift audit. |
