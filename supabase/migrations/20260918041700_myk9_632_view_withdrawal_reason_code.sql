-- MYK9-632: project `entries.withdrawal_reason_code` through the authenticated
-- entry-results views, so the exhibitor's Edit Entry sheet can say WHICH
-- withdrawal happened from data it already fetches.
--
-- The badge on a removed class row reads "Withdrawn · Dog in season". Until
-- now the only way to get that code to the client was a direct
-- `supabase.from('entries')` read, which was the single non-replica hop in a
-- dialog whose every other read is replication-backed: offline it silently
-- degraded to a bare "Withdrawn", and on the secretary tier it re-fetched a
-- value Entry Management had already loaded and was rendering on the card
-- behind the sheet. The view already carries the FREE-TEXT sibling
-- `withdrawal_reason`; the enumerated code was simply never added beside it.
--
-- Masking: `can_view_admin` — a show manager, or the entry's own exhibitor —
-- exactly as `withdrawal_reason` and every other payment/admin column on this
-- view. No new reader gains anything; the reason code is strictly less
-- sensitive than the free text already exposed on the same predicate.
--
-- Mechanics this file is obliged to get right:
--
--  * The body is copied from 20260910224500 (MYK9-126), which is the LATEST
--    migration defining this view — NOT from an older canonical-looking one.
--    `grep -l view_authenticated_entry_results supabase/migrations/ | sort`
--    confirms 20260911034500 and 20260912183000 touch the PUBLIC view and the
--    write grants respectively, and neither redefines this body. Everything
--    below is byte-identical to 20260910224500 except the one appended column.
--
...
--
--  * `view_authenticated_entry_results_replication` used `entries.*`, which
--    re-expands on every rebuild. Leaving the star would have put
--    `withdrawal_reason_code` at ordinal 99 — where `show_deleted_at` is today
--    — and CREATE OR REPLACE would have refused the rename. It is replaced
--    here by the explicit 102-column list the LIVE view returns (read from
--    information_schema.columns in ordinal order), with the new column appended
--    after them. Same columns, same order, same types; only the star is gone.
--    Without this the replica would never see the column at all, which is the
...
-- is Richard's to run. Until it lands, `withdrawal_reason_code` simply is not in
-- the view, every client read of it is `undefined`, and the badge falls back to
-- the bare word "Withdrawn" — the pre-MYK9-632 rendering, never "Pulled".
