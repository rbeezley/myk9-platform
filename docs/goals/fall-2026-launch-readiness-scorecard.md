...
| Operational readiness | Deploys, migrations, environment config, monitoring, rollback, and support paths are documented enough for launch. | Deployment checklist; migration history; staging smoke; support/runbook docs. | Yellow — Error observability live in production (Sentry SDK + PII scrubber + role-surface boundaries, [#723](https://github.com/rbeezley/myk9-platform/pull/723); end-to-end verified); launch-milestone QA checklist codified ([`launch-milestone-qa-checklist.md`](../launch-milestone-qa-checklist.md)); monthly dependency-audit cron; migration lineage tracked. July 11 agent-owned remediation is merged and deployed: FORCE-RLS drift is zero, health snapshots/watchdog are live, SA-023/024/025/028/029/030 have focused runtime evidence, advisors are dispositioned, and the post-#1294 migration dry run reports the remote database is up to date. Custom SMTP is live at a 100/hour Auth limit with branded Gmail delivery verified. Bounded Resend retry code is live on all 12 affected functions with version/rollback evidence, fail-closed checks, and a delivered post-deploy auth email; valid registration/operator-alert and controlled transient-failure smokes remain open. **Gaps:** operator Stripe/legal/data/device/browser/Sentry/DNS gates remain; Stripe go-live runbook + treasurer onboarding guide open (Lane 4); CI-gated Vercel deploys + E2E-blocking not yet enabled (Lane 3); rollback path not yet rehearsed. |
