# context-aware-command-menu Specification

## Purpose
A context-aware access layer inside the existing Command Palette: contextual navigation into owner surfaces with normalized prefilled filters, a single allowlisted selected-entry check-in command that reuses the shared MYK9-47 action registry and the page's own dispatch chain, user-namespaced device-local recent searches with account-change clearing, and a single-source keyboard-shortcut registry with pointer/touch parity. The palette is a doorway into existing pages and mutations — never a second CRUD surface, mutation layer, or record index.
## Requirements
