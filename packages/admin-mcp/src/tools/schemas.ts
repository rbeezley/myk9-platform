...
export const searchStringSchema = z.string().trim().min(1, 'must not be empty');
...
export const diagnosePaymentInput = z
  ...
  .refine(
    value =>
      [value.entryId, value.paymentIntentId, value.checkoutSessionId].filter(Boolean).length === 1,
    {
      message: 'provide exactly one of entryId, paymentIntentId, or checkoutSessionId',
    }
  );
