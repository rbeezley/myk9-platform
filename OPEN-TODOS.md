...
- [ ] **LAUNCH BLOCKER — wire the registration wizard's online payment to Stripe** — The wizard's "Online (credit card)" path never reaches Stripe: it submits via `submitShowRegistration` with a literal `MOCK-PAYMENT-REF` and shows a "coming soon" message. The parallel cart path (`createEntryCheckoutSession` → `/cart` → checkout) works end-to-end but the wizard doesn't hand off to it: `ClassSelectionStep`'s `addItem` fails silently and `createCart` raced, creating two empty active carts in the same millisecond (cart `54f3b1e3` walkthrough, only worked after manual seeding + the `loadActiveCart` hydration fix). Proper fix needs a dedicated plan: wizard `credit_card` → Stripe checkout handoff, fix the silent `addItem` failure + the duplicate-`createCart` race, and reconcile webhook-created entries with the wizard's enrollment/`registration_id` records so the two entry-creation paths don't diverge. Files: `apps/myk9show/src/components/shows/RegistrationWorkflow/`, `apps/myk9show/src/store/cartStore.ts`, `apps/myk9show/supabase/functions/stripe-webhook/index.ts`.
- [ ] **Polish — order/confirmation number on checkout success** — `/checkout/success` shows entries + total but no number an exhibitor can write down or quote to support. Surface the payment intent id (it's what refunds, payouts, and the Stripe dashboard all pivot on) on the success page and in the confirmation email. Caveat: the page can render a second or two before the webhook writes `stripe_orders` — pull from the `session_id` already in the URL or render the id once available, don't assume the order row exists instantly. Raised by Richard during the 2026-06-10 walkthrough.
- [ ] **Walkthrough remainder** — Stage 4 refund ($30 via RefundEntryDialog) → Stage 5 manual payout cron run (backdate Headline `end_date`; expect `pending` until club onboards) → Stage 2 club Express onboarding (blocked on Stripe's "verify connected account creation" review clearing; screenshot every screen for the treasurer guide → `docs/guides/club-payment-setup-guide.md`) → real sandbox transfer → Stage 6 premium paywall (monthly + annual + founding member).
- [ ] **Codex (GPT-5) review of PR #625** — required pre-merge gate (payments). Usage limit reset ~7:31 PM 2026-06-10.
