# core-identity-deduplication Specification

## Purpose
Core people and club directory rows are shared by entries, dogs, judges, show
managers, club access requests, Stripe accounts, support diagnostics, and show
creation. The system prevents exact duplicate identities where the identity key
is strong enough, suggests likely matches inside existing create flows, and
keeps duplicate resolution calm without adding standalone duplicate-management
surfaces.
