...
- [ ] 1.7 [ADDED] Create a redacted mutation ledger template covering action owner, approval, precondition, authoritative-state query, verification, previous-setting recovery reference, idempotency/reconciliation rule, and completion state for every external mutation.
...
- [ ] 4.10 [EXPANDED] Design a versioned, fail-closed production-to-staging manifest for database fields and Storage buckets/object paths that preserves troubleshooting-relevant IDs/mappings, relationships, timestamps, permissions, workflow state, and explicitly selected assets; sanitizes contact, Auth, payment-sensitive, private-message, secret, passcode, object metadata, and embedded/visible personal data; and rejects every unclassified schema or Storage path change.
- [ ] 4.11 [EXPANDED] Add tests for deterministic masking, schema/Storage-drift rejection, content-type/size/malware and residual-sensitive-data scanning, referential integrity, designated staging Auth-account replacement, prohibited-field removal, encrypted/access-restricted scratch handling, retention and cleanup after success/failure, versioned-prefix cleanup, idempotent checkpoints/reruns, and zero staging-to-production connectivity.
- [ ] 4.12 [EXPANDED] Implement the on-demand refresh procedure using a transactionally consistent source snapshot, bounded batches, non-sensitive durable checkpoints, idempotent retries, and an encrypted, access-restricted, retention-limited scratch destination. Preflight rows/objects/bytes, scratch space, staging headroom, and duration limits; block staging import until database and versioned Storage candidates pass every check; verifiably delete temporary snapshots, exports, and Storage objects after success or failure; preserve the prior staging dataset on failure; and keep external side effects disabled or redirected. Obtain separate immediate approval before the first real production snapshot refresh.
- [ ] 4.13 [EXPANDED] Before public launch, rehearse representative club and exhibitor troubleshooting—including required fixture/allowlisted Storage assets—using synthetic fixtures and forced timeout/capacity/upload/import failures; prove prior staging remains usable. After real production data exists, validate the first approved sanitized refresh as an operational follow-up rather than a blocker for initial environment separation.
...
- [ ] 5.5 [ADDED] Execute external configuration through the mutation ledger with new side effects disabled/non-public, verify each checkpoint before the next dependency, reconcile authoritative state after ambiguous responses, and stop or restore the previous setting on failure; domain aliasing and production workflow enablement remain final activation steps.
