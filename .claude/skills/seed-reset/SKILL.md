...
1. Run the query above and look at each id's evidence. **If the entry's dog is named `ZZ Walk Dog <run token> #N`, it is exhibitor-walk residue:** clear it with the token-scoped record-then-apply cleanup in `docs/operations/walk-residue-cleanup.md` (MYK9-734), which records the dog, entry, history, order and enrollment before it deletes anything, instead of the hand steps below.
...
Both `stripe_orders` scope FKs are ON DELETE RESTRICT since migration `20260915191700` (MYK9-527), so the seed refuses before the FK would fail with a raw violation. Run `order_stray` as a SELECT with `scope_shows`. Since migration `20260925072300` (MYK9-748) it also names an order on the seed's own enrollment `dededede-...070` even when that enrollment was reassigned to a show outside the seed's set, because the seed deletes `...070` by id wherever it sits.
...
3. Run `seed-demo.sql` with its output captured to a file, stderr included: psql prints `WARNING` and `ERROR` lines to stderr, so `> seed.log` alone captures none of them. Use `psql -X -v ON_ERROR_STOP=1 -f supabase/seed-demo.sql > .logs/seed.log 2>&1`. It is idempotent over its own rows, but it ABORTS by design rather than cascade away an entry, enrollment or Stripe order that carries real money, and it WARNS about rows it removed. Afterwards `grep -i warning .logs/seed.log`, because a warning in 400 lines of output is easy to miss and the rows it names are gone. An abort is the guard working; find its heading above before touching any SQL.
