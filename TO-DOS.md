...
- **Remove "List" view mode and standardize the view switcher UI across all pages** — Pages that offer Grid/List/Table views should be reduced to Grid + Table only, and the switcher component should look and behave consistently everywhere. **Problem:** (1) The People page has three view modes (Grid, List, Table) but List is too similar to Grid — redundant and adds cognitive overhead. (2) The view switcher UI is inconsistent: some pages (e.g. show entries tab) show just two small icon-only toggles, while others (e.g. People page) show labeled buttons — there is no shared component. **Files:** `apps/myk9show/src/pages/PeoplePage.tsx` or `BrowsePeoplePage.tsx` (remove List option, update view switcher), any shared `ViewSwitcher` or `ViewToggle` component if one exists (standardize API to Grid + Table), show entries tab view toggle (update to match new standard). **Solution:** (1) Audit all pages that use a view mode switcher and identify which have a "List" view that can be removed. (2) Build or update a shared `ViewSwitcher` component that renders labeled icon buttons (like the People page) for 2-mode toggles — consistent across all pages. (3) Remove List view from People page and any other pages that have it. (4) Replace ad-hoc icon-only toggles (like the entries tab) with the standardized component.
