...
- [ ] **[DB drift] Security advisor remediation triage** — ERRORs resolved via PR [#722](https://github.com/rbeezley/myk9-platform/pull/722): 8 `security_definer_view` errors fixed (`security_invoker=true` on all views); 4 RBAC function anon grants revoked; `view_myk9q_entries` anon exposure closed. Remaining WARNs: 83 anon-callable SECURITY DEFINER functions (Supabase/PostGIS internals — triage carefully, do not bulk-revoke), 85 authenticated-callable SECURITY DEFINER functions, 9 always-true write-capable RLS policies, 2 public bucket listing policies, 16 mutable function search paths. Route remaining items through `/security-audit`; do not batch-fix blindly.
- [x] ~~**[DB drift] Fix `entries.entry_status` CHECK regression**~~ — Resolved 2026-06-14 via PR [#722](https://github.com/rbeezley/myk9-platform/pull/722): both hyphen (`scratch-requested`, `move-up-requested`) and underscore forms now accepted by the CHECK constraint. `pnpm qa:db-drift:enum` reports no findings.
