...
export const ShowSelector: React.FC<ShowSelectorProps> = ({ shows, onShowSelected, className }) => {
  ...
  const availableYears = useMemo(() => {
    const years = new Set(shows.map(show => new Date(show.startDate).getFullYear().toString()));
    return Array.from(years).sort().reverse();
  }, [shows]);

  const availableTypes = useMemo(() => {
    const types = new Set(shows.map(show => show.organization));
  ...
  const filteredShows = useMemo(() => {
    const filtered = shows.filter(show => {
      ...
      // Type filter
      if (selectedType !== 'all' && show.organization !== selectedType) {
        return false;
      }
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1 space-y-3">
                        ...
                        <div className="flex items-start justify-between">
                          <div>
                            ...
                            <div className="flex items-center gap-3 mt-1">
                              <Badge variant="outline">{show.organization}</Badge>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                          <div className="space-y-2">
                            <div className="flex items-center gap-2">
                              ...
                              <span>
                                {format(parseISO(show.startDate), 'MMM d, yyyy')} -{' '}
                                {format(parseISO(show.endDate), 'MMM d, yyyy')}
                              </span>
                            </div>
                            <div className="flex items-center gap-2">
                        ...
                        <div className="text-xs text-muted-foreground">
                          Entry deadline:{' '}
                          {show.entryDeadline
                            ? format(parseISO(show.entryDeadline), 'MMM d, yyyy')
                            : show.entryCloseDate
                              ? format(parseISO(show.entryCloseDate), 'MMM d, yyyy')
                              : 'TBD'}
                        </div>
                      </div>
                    </div>
