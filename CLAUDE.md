...
- **An ACL audit must cover sequences (`relkind='S'`), not just tables and columns**: a BEFORE INSERT trigger's `nextval()` fires before RLS `WITH CHECK`, so a table INSERT grant dies 42501 on the sequence. Resolve them by `pg_depend` ownership **and** by grepping the migration for `CREATE SEQUENCE`/`nextval(` — a trigger's standalone sequence is tied to its table by neither dependency nor name. (docs/lessons/README.md#sequence-privileges)
