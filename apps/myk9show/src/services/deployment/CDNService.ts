...
export class CDNService {
  ...
  async uploadAsset(
    ...
    try {
      ...
      throw new Error(
        `CDN upload failed: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  private async optimizeAsset(file: File | Blob, options: UploadOptions): Promise<Blob> {
    if (this.compressionWorker && options.compress !== false) {
      return new Promise(resolve => {
        ...
        this.compressionWorker!.onerror = error => {
          logger.warn(
            'Compression worker failed, using original file:',
            'deployment',
            {},
            error as Error
          );
