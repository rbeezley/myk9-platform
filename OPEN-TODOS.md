...
- [ ] **[DB drift] Security advisor remediation triage** — ERRORs resolved in PR [#722](https://github.com/rbeezley/myk9-platform/pull/722): 8 `security_definer_view` errors fixed (security_invoker=true), 4 RBAC anon-executable SECURITY DEFINER functions revoked. Always-true write-capable RLS policies fixed in PR [#727](https://github.com/rbeezley/myk9-platform/pull/727) (4 tables: achievements, show_templates, template_fields, judge_certifications). **Accepted/intentional WARNs:** 2 public buckets (`images` + `premium-published` require public read by design); `show_passcodes` RLS-enabled/no-policies is intentional (SECURITY DEFINER RPCs only); remaining 83 anon-callable functions are PostgREST/PostGIS extension functions not revokable without breaking core functionality; 16 mutable search paths are Supabase internals. No further action needed unless Supabase advisor re-categorizes.
