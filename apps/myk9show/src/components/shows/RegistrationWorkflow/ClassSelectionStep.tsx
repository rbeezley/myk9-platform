...
export const ClassSelectionStep: React.FC<ClassSelectionStepProps> = ({
  ...
  const isInCart = useCallback(
    (dogId: string, classId: string): CartItemWithDetails | undefined => {
  ...
  const getCartItemsForDog = useCallback(
    (dogId: string): CartItemWithDetails[] => {
  ...
  const isClassSelected = (dogId: string, classId: string): boolean => {
    ...
    return (
      !!isInCart(dogId, classId) ||
      getSelectionForDog(dogId).selectedClasses.some(c => c.classId === classId)
    );
  };
                    ...
                    <div className="space-y-6">
                      {showTrials.length === 0 ? (
                        ...
                        <Alert>
                          <Info className="h-4 w-4" />
                          <AlertDescription>
                            No classes available yet. Found {showTrials.length} trial
                            {showTrials.length !== 1 ? 's' : ''} but no classes assigned.
                          </AlertDescription>
                        </Alert>
                      ) : (
                                                        ...
                                                        <Tooltip>
                                                          ...
                                                          <TooltipContent className="max-w-xs">
                                                            <div className="space-y-1">
                                                              ...
                                                              <ul className="list-disc pl-4 text-xs">
                                                                {eligibilityResult?.warnings.map(
                                                                  (warning, idx) => (
                                                                    <li key={idx}>
                                                                      {warning.message}
                                                                    </li>
                                        ...
                                        {isFull && !isAlreadyEntered && (
                                          <div className="mt-2 ml-9 flex items-center gap-2">
                                            ...
                                            <Button
                                              ...
                                              disabled={
                                                joiningWaitlist === `${dogId}-${classData.id}`
                                              }
                                              onClick={async () => {
                                                if (!user?.id) {
                                                  toast.error(
                                                    'You must be logged in to join a waitlist'
                                                  );
                                                  return;
                                                }
                                                ...
                                                try {
                                                  const { error, position } = await joinWaitlist(
                                                    classData.id,
                                                    dogId,
                                                    user.id
                                                  );
        ...
        <div className="mt-4 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
          <div className="flex justify-between items-center">
            ...
            <span className="text-lg font-semibold">
              $
              {selectedDogs
                .reduce((total, dogId) => total + getTotalFeesForDog(dogId), 0)
                .toFixed(2)}
            </span>
          </div>
        </div>
