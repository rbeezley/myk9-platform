 ...
 *
 * Moved from apps/myk9q/src/pages/EntryList/hooks/useResetScore.test.ts in
 * PR E2a alongside the implementation. The hook never imported any app
 * singletons — `handleResetScoreHook` was always injected by the caller —
 * so the move is mostly mock-path rewires.
 *
 * Coverage:
 * - Menu open/close + position calculation from click event
 * - Reset confirm dialog show/cancel
 * - confirmResetScore optimistic update — clears scoring fields, switches
 *   tab, closes dialog, then awaits background sync
 * - Silent-failure contract: background `handleResetScoreHook` rejection
 *   does NOT throw, does NOT revert optimistic update (offline-first)
 *
 * Regression class: PR #320 had a silent data-loss bug where undo paths
 * filtered `null` out of optional fields. The optimistic update here
 * clears five scoring fields plus `placement: undefined`; if anyone later
 * replaces the inline object with a merge helper, these assertions catch
 * dropped fields.
 */
