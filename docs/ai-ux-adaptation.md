...
3. **Prompt-injection treated as a real threat.** User input (`{{SEARCH_QUERY}}`, `{{USER_GOAL}}`, hover text, free-text fields) is _data_, not _instructions_. Wrap user content in delimited blocks, instruct the model to ignore instructions inside those blocks, and reject responses that don't match the JSON schema.
...
## Shared infrastructure to build before _any_ AI UX feature
...
- Filter the searchable corpus by role _before_ it reaches the model.
...
## Recommended order _if_ we decide to build
