...
test.describe('Phase 2: Show Management Workflow', () => {
  ...
  test.describe('Test 2.1: Create Show with Basic Information', () => {
    test('should create an all-breed conformation show with complete details', async () => {
      ...
      await expect(showTestHelper.page.locator('[data-testid="pre-entry-fee-input"]')).toHaveValue(
        showData.preEntryFee.toString()
      );
      await expect(
        showTestHelper.page.locator('[data-testid="day-of-show-fee-input"]')
      ).toHaveValue(showData.dayOfShowFee.toString());

      // Verify staff assignments
      await expect(showTestHelper.page.locator('[data-testid="chairman-input"]')).toHaveValue(
        showData.chairman
      );
      await expect(showTestHelper.page.locator('[data-testid="secretary-input"]')).toHaveValue(
        showData.secretary
      );
    ...
    test('should create a specialty breed show with specific requirements', async () => {
      ...
      await expect(
        showTestHelper.page.locator('[data-testid="breed-specific-options"]')
      ).toBeVisible();
    });
    ...
    test('should validate required fields and show appropriate error messages', async () => {
      ...
      await expect(showTestHelper.page.locator('[data-testid="error-show-name"]')).toContainText(
        'required'
      );
      await expect(showTestHelper.page.locator('[data-testid="error-show-type"]')).toContainText(
        'required'
      );
      await expect(showTestHelper.page.locator('[data-testid="error-location"]')).toContainText(
        'required'
      );
      await expect(showTestHelper.page.locator('[data-testid="error-club"]')).toContainText(
        'required'
      );
      ...
      await expect(
        showTestHelper.page.locator('[data-testid="error-show-name"]')
      ).not.toBeVisible();
    });
    ...
    test('should validate date logic and prevent invalid date combinations', async () => {
      ...
      await expect(showTestHelper.page.locator('[data-testid="error-date-range"]')).toContainText(
      ...
      await expect(showTestHelper.page.locator('[data-testid="error-entry-dates"]')).toContainText(
        'Entry close date must be before show start date'
      );
    });
  });
  ...
  test.describe('Test 2.2: Add Trials Using Show Creation Wizard', () => {
    test('should add conformation trial with breed classes', async () => {
      ...
      await expect(
        showTestHelper.page.locator(`[data-testid="trial-item-${conformationTrial.id}"]`)
      ).toBeVisible();
      await expect(
        showTestHelper.page.locator(`[data-testid="trial-name-${conformationTrial.id}"]`)
      ).toContainText('Conformation');

      // Verify breed classes are available for selection
      await showTestHelper.page.click('[data-testid="next-step-button"]');
      await expect(
        showTestHelper.page.locator('[data-testid="conformation-classes-section"]')
      ).toBeVisible();
    });
    ...
    test('should add obedience trial with all levels', async () => {
      ...
      await expect(
        showTestHelper.page.locator(`[data-testid="trial-type-${obedienceTrial.id}"]`)
      ).toContainText('Obedience');
    ...
    test('should detect and warn about trial scheduling conflicts', async () => {
      ...
      await expect(
        showTestHelper.page.locator('[data-testid="schedule-conflict-warning"]')
      ).toBeVisible();
      await expect(showTestHelper.page.locator('[data-testid="conflict-details"]')).toContainText(
        'overlapping'
      );
    });
    ...
    test('should allow editing and removing trials', async () => {
      ...
      await expect(
        showTestHelper.page.locator(`[data-testid="trial-name-${trial.id}"]`)
  ...
  test.describe('Test 2.3: Class Creation Using Templates', () => {
    test('should use AKC Conformation template to create classes', async () => {
      ...
      await expect(
        showTestHelper.page.locator('[data-testid="selected-classes-count"]')
    ...
    test('should use Scent Work class templates with element variations', async () => {
      ...
      await expect(
        showTestHelper.page.locator('[data-testid="class-Container-Novice-A"]')
      ).toBeVisible();
      await expect(
        showTestHelper.page.locator('[data-testid="class-Interior-Advanced-A"]')
      ).toBeVisible();
      await expect(
        showTestHelper.page.locator('[data-testid="class-Buried-Excellent-A"]')
      ).toBeVisible();
    ...
    test('should customize class parameters and validate constraints', async () => {
      ...
      await expect(showTestHelper.page.locator('[data-testid="error-max-entries"]')).toContainText(
        'must be positive'
      );
    ...
    test('should set entry limits and validate capacity constraints', async () => {
      ...
      await expect(showTestHelper.page.locator('[data-testid="capacity-warning"]')).toContainText(
        'exceeds total show capacity'
      );
    });
    ...
    test('should configure age and eligibility requirements', async () => {
      ...
      await expect(
        showTestHelper.page.locator('[data-testid="eligibility-saved-message"]')
      ).toBeVisible();
    });
  });
  ...
  test.describe('Test 2.4: Judging Assignments and Scheduling', () => {
      ...
      for (const assignment of assignments) {
        ...
        await expect(
          showTestHelper.page.locator(
            `[data-testid="class-${assignment.className}"] [data-testid="assigned-judge"]`
          )
        ).toContainText(judge.name);
        await expect(
          showTestHelper.page.locator(
            `[data-testid="class-${assignment.className}"] [data-testid="judging-time"]`
          )
    ...
    test('should create and optimize judging schedule', async () => {
      ...
      await showTestHelper.page.selectOption(
      ...
      await expect(showTestHelper.page.locator('[data-testid="schedule-metrics"]')).toContainText(
        'Total judging time'
      );
      ...
      await expect(
        showTestHelper.page.locator('[data-testid="schedule-applied-message"]')
      ).toBeVisible();
    });
    ...
    test('should handle judge conflicts and provide alternatives', async () => {
      ...
      await showTestHelper.page.click(
        `[data-testid="class-Open-Dog"] [data-testid="assign-judge-button"]`
      );
      await showTestHelper.page.selectOption(
        '[data-testid="judge-select"]',
        workflowData.judges[0].id
      );
      ...
      // Should show conflict warning
      await expect(
        showTestHelper.page.locator('[data-testid="judge-conflict-warning"]')
      ).toBeVisible();
    ...
    test('should generate judge contracts and agreements', async () => {
      ...
      await expect(
        showTestHelper.page.locator('[data-testid="contracts-generated"]')
      ).toBeVisible();

      // Download contract
      const downloadPromise = showTestHelper.page.waitForEvent('download');
      await showTestHelper.page.click(
        `[data-testid="download-contract-${workflowData.judges[0].id}"]`
      );
    ...
    test('should handle last-minute judge changes and notifications', async () => {
      ...
      const originalAssignment = {
        className: 'Puppy Dog',
        judgeId: workflowData.judges[0].id,
        time: '09:00',
      };
      ...
      await showTestHelper.page.click(
        `[data-testid="class-Puppy-Dog"] [data-testid="change-judge-button"]`
      );
      await showTestHelper.page.selectOption(
        '[data-testid="replacement-judge-select"]',
        workflowData.judges[1].id
      );
      await showTestHelper.page.fill(
        '[data-testid="change-reason"]',
        'Original judge had emergency'
      );
      ...
      await expect(
        showTestHelper.page.locator(
          `[data-testid="class-Puppy-Dog"] [data-testid="assigned-judge"]`
        )
      ).toContainText(newJudge.name);

      // Verify notification was sent
      await expect(
        showTestHelper.page.locator('[data-testid="change-notification-sent"]')
      ).toBeVisible();
    });
  });
  ...
  test.describe('Test 2.5: Show Publication and Entry Opening', () => {
    test('should publish show and make entries accessible', async () => {
      ...
      await expect(
        showTestHelper.page.locator('[data-testid="accepting-entries-indicator"]')
      ).toBeVisible();
    ...
    test('should enforce entry deadlines and prevent late entries', async () => {
      ...
      await expect(showTestHelper.page.locator('[data-testid="entry-status"]')).toContainText(
        'Closed'
      );
      await expect(showTestHelper.page.locator('[data-testid="enter-show-button"]')).toBeDisabled();
      await expect(
        showTestHelper.page.locator('[data-testid="entry-closed-message"]')
      ).toContainText('Entry deadline has passed');
    });
    ...
    test('should handle show modifications after publication with restrictions', async () => {
      ...
      await showTestHelper.page.click('[data-testid="edit-classes-button"]');
      await expect(
        showTestHelper.page.locator('[data-testid="class-modification-form"]')
      ).toBeVisible();

      // Test entry management
      await showTestHelper.page.click('[data-testid="manage-entries-button"]');
      await expect(
        showTestHelper.page.locator('[data-testid="entry-management-interface"]')
      ).toBeVisible();
    });
    ...
    test('should handle entry withdrawals and deadline enforcement', async () => {
      ...
      await showTestHelper.page.click(
        '[data-testid="entry-123"] [data-testid="withdraw-entry-button"]'
      );
      ...
      await expect(
        showTestHelper.page.locator('[data-testid="withdrawal-processed"]')
      ).toBeVisible();
      ...
      await showTestHelper.page.click(
        '[data-testid="entry-456"] [data-testid="withdraw-entry-button"]'
      );

      // Should show deadline warning
      await expect(
        showTestHelper.page.locator('[data-testid="withdrawal-deadline-warning"]')
      ).toBeVisible();
      await expect(showTestHelper.page.locator('[data-testid="refund-amount"]')).toHaveValue(
        'none'
      ); // No refund past deadline
    });
    ...
    test('should generate and distribute show documentation', async () => {
      ...
      await expect(
        showTestHelper.page.locator('[data-testid="premium-list-preview"]')
      ).toBeVisible();

      // Test show confirmation generation
      await showTestHelper.page.click('[data-testid="generate-confirmation-button"]');
      await expect(
        showTestHelper.page.locator('[data-testid="confirmation-preview"]')
      ).toBeVisible();
    ...
    test('should cancel show and process refunds appropriately', async () => {
      ...
      await expect(
        showTestHelper.page.locator('[data-testid="cancellation-reason"]')
      ).toContainText('Venue unavailable');
      ...
      await expect(
        showTestHelper.page.locator('[data-testid="cancellation-notifications-sent"]')
      ).toBeVisible();
    });
  });
  ...
  test.describe('Integration Tests: Complete Show Management Workflow', () => {
    test('should complete entire show creation workflow from start to publication', async () => {
      ...
      await expect(showTestHelper.page.locator('[data-testid="total-trials"]')).toContainText(
        workflowData.trials.length.toString()
      );

      const totalClasses = workflowData.trials.reduce(
        (sum, trial) => sum + trial.classes.length,
        0
      );
      await expect(showTestHelper.page.locator('[data-testid="total-classes"]')).toContainText(
    ...
    test('should handle complex multi-day show with multiple trials per day', async () => {
      ...
      const trials = [
        ...
        ShowTestDataFactory.createTrialData({
          name: 'Day 1 Morning Obedience',
          dateTime: '2024-12-15T09:00:00.000Z',
          type: 'Obedience',
        }),
        ShowTestDataFactory.createTrialData({
          name: 'Day 1 Afternoon Agility',
          dateTime: '2024-12-15T13:00:00.000Z',
          type: 'Agility',
        }),
        // Day 2
        ShowTestDataFactory.createTrialData({
          name: 'Day 2 Morning Conformation',
          dateTime: '2024-12-16T09:00:00.000Z',
          type: 'Conformation',
        }),
        ShowTestDataFactory.createTrialData({
          name: 'Day 2 Afternoon Scent Work',
          dateTime: '2024-12-16T13:00:00.000Z',
          type: 'Scent Work',
        }),
        // Day 3
        ShowTestDataFactory.createTrialData({
          name: 'Day 3 Rally',
          dateTime: '2024-12-17T09:00:00.000Z',
          type: 'Rally',
        }),
      ];
      ...
      await expect(
        showTestHelper.page.locator('[data-testid="schedule-conflicts"]')
      ).not.toBeVisible();

      // Test entry workflow for multi-day show
      await showTestHelper.page.click('[data-testid="enter-show-button"]');
      await expect(
        showTestHelper.page.locator('[data-testid="multi-day-entry-form"]')
      ).toBeVisible();
