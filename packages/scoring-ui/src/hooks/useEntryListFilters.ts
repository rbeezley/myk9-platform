...
type SortComparator<T extends BaseEntry> = (a: T, b: T, context?: SortContext) => number;
 ...
 */
function createSortComparators<T extends BaseEntry>(): Record<SortType, SortComparator<T>> {
  return {
    manual: (a, b, context) => {
    ...
    run: (a, b) => {
      ...
      const aOrder = a.exhibitorOrder || a.armband || 0;
      const bOrder = b.exhibitorOrder || b.armband || 0;
 ...
 */
function getPriorityDiff<T extends BaseEntry>(
  a: T,
  b: T,
 ...
 * ```
 */
export function useEntryListFilters<T extends BaseEntry>({
  entries,
  ...
  // Create comparators once
  const sortComparators = useMemo(() => createSortComparators<T>(), []);
  ...
  const filterByTab = useCallback((entry: T, tab: TabType): boolean => {
