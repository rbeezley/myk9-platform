...
- [ ] **Add Analytics entry point for exhibitors** — `/exhibitor/analytics` (lifetime stats, qualification trends, per-dog breakdown, fastest times) is subscription-gated and exists but has no nav link. Add a "My Stats" sidebar item or prominent link from My Shows. Files: `apps/myk9show/src/components/layout/sidebar/unifiedSidebarConfig.ts`, `apps/myk9show/src/pages/MyEntriesPage/`. Full context in TO-DOS.md § "Analytics unreachable for exhibitors".
- [ ] **Add Messages entry point from entry cards** — `/messages/:showId` (exhibitor ↔ show organizer chat) requires knowing a showId and has no fallback discovery path. Confirm a link exists on entry cards or show detail; add one if missing. Files: `apps/myk9show/src/pages/MyEntriesPage/modules/MyEntryCard.tsx`, `apps/myk9show/src/pages/ShowDetailsPage.tsx`. Full context in TO-DOS.md § "Messages has no exhibitor entry point".
...
- [ ] **Make secretary dashboard Needs Attention collapsible** — On `/secretary/dashboard` (e.g. https://myk9show.com/secretary/dashboard), the Needs Attention list can dominate the first viewport when many shows have pending entries. Make this existing section collapsible with a sensible default/persisted state so secretaries can get back to the multi-show overview without losing the attention queue. Duplication check: this does not add a new surface; it tightens the existing dashboard section in place.

- [ ] **Brainstorm the secretary dashboard's distinct job** — Re-evaluate what belongs on `/secretary/dashboard` now that Entry Management owns entry review/approval and Show Desk owns single-show setup/day-of/wrap-up work. The brainstorm should answer: what secretary decisions still need a multi-show dashboard, what should deep-link into Entry Management or Show Desk instead of being reimplemented, and whether any current dashboard modules should be simplified, collapsed, or deleted. Duplication check: treat new dashboard UI as unjustified unless it summarizes across shows or routes to an existing concern-owned page with context.
