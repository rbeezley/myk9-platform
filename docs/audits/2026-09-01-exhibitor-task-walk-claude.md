...
| `Heartland Scent Work Classic` | 2026-09-01  | **Yes** — closes at end of _today_      |
...
_silence after payment is the scariest state_; this is that state, with the terminal message
...
_"reliably reaches the … state on a real phone without manual action, including after a
transient network failure during the initial poll window"_, and its fix added slow
...
one level up: _truncated_ presented as _complete_.
...
That reads the **entry's** soft-delete flag. Every downstream string says _show_:
...
_(The 4th pending row is `promotion-expired`, which per the recorded product decision stays
pending deliberately; it is excluded from the $90.)_
...
2. **The confirm dialog promises the opposite:** _"You are about to delete ZZ Walk Dog
   2026-09-01 **and 2 entries**. This action cannot be undone."_ It counts the very entries
...
_(Aside: `components/exhibitor/TitleProgressCard.tsx` has the same unsliced per-dog pattern
and **no importers** — dead code.)_
...
_"Whoever picks this up should get those URLs first."_ It was closed today (16:49) by PR #1934,
...
2. **A 251-request burst is exactly what would fail an idle-window check before a route
   transition** — and it explains the part MYK9-289 flags as the puzzle: _"Only the exhibitor
   role fails. Public, secretary, admin and judge pass."_ Only the exhibitor lands on a page
...
Wizard step 3 is titled _"Payment — Review fees and payment"_ and carries a _"Payment
Summary — Amount Due: $30.00"_. The exhibitor ticks the entry agreement and clicks
**"Submit & pay"** against that number. The 7% service fee appears only on the next screen.

This is the shape MYK9-265 was filed for — _a quote lower than the charge_.
...
The underlying value is `upcomingClassCount`, so the _count_ is defensible — Heartland is in
...
Only then: _"This club's payment account is not set up to receive online entry fees."_
...
a show at _"That took 30 seconds"_.
...
1. **"Seven requests never settle on all five MYK9-289 routes."** They were the _same seven_
...
2. **"`/shows`, `/account`, `/notifications` render almost nothing"** (471, 702, 322 chars vs
   ...
   `bodyChars` metric was measuring a page that is genuinely short. `/` also _redirects_ to
...
**Deviation from the draft plan, recorded deliberately.** Draft D1 proposed _re-opening_ MYK9-207.
E12 was filed as a new issue (MYK9-294) linked to MYK9-207 and MYK9-98 instead. MYK9-207 was a
paid-then-_refunded_ cart parked by a suspected network blip, and its fix (PR #1686) did ship —
the "Payment Not Found Yet" state I hit is that fix's own copy behaving correctly. E12 is a
paid-and-_succeeded_ order whose row provably exists and is unreadable for ~30 s. Re-opening
...
- **MYK9-289** — post E9's evidence and consider re-opening: the five routes _do_ settle in a
...
| E9, E13, E16, E20, E21, E22 | **High**                                   | Reproduced on local _and_ staging                                               |
