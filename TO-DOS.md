...
- [x] **Print armband labels on Avery sheet labels** — Done (PR #47). Reusable 3-layer label engine (templates, layout, renderers) with 3 Avery sizes (18262, 18163, 8387). ArmbandLabelsReport registered in report registry with inline config panel (label size, entry filters, content toggles, skip count, vertical pitch adjustment). VenueWifiCard on Show Settings. Migration 127 adds `venue_wifi_network`, `venue_wifi_password` on shows and `is_day_of_show` on entries. 71 tests. Deploy: `supabase db push` for migration 127, regenerate types.
...
- [x] **Port "Print Show Flyer" to myK9Show** — Done. `ShowFlyerReport` registered in report registry as show-scoped report. 2-page printable: page 1 has QR code + exhibitor passcode, page 2 has getting-started guide. Uses `getExhibitorLoginUrl` from `@/utils/passcodes`. 9 tests.
...
- [x] **Add terms & rules agreement checkbox before registration submission** — Done. Implemented `organization_agreements` table (migrations 122–123) with RLS, seeded AKC Scent Work agreement. Added `useOrganizationAgreement()` React Query hook with static cache, `EntryAgreementSection` component (collapsible agreement + required checkbox with loading/error states), and integrated into `PaymentStep` with controlled state and `canProceed()` gate. 18 tests added across hook, component, and integration layers.

---

## Auto-Populate Organization Entry Forms - 2026-04-07 07:50

- [x] **Build standard org entry form generation for trial secretaries** — Done. AKC Scent Work entry form registered in report engine. Secretary selects show from Reports page, picks "AKC Scent Work Entry Form", gets one pre-filled form per dog with class grid, dog info, owner details, and AKC agreement text. Optional filtering by trial or individual dog via dog picker. HTML-to-Print approach (~90% visual fidelity to official AKC form). Files: `AKCScentWorkEntryForm.tsx`, `useEntryFormData.ts`, `entryFormTypes.ts`, `entryFormUtils.ts`. Dog picker added to `ReportControlsBar`. 24 new tests. Future orgs (UKC, NACSW) follow same pattern — new component + registry entry.
...
- [x] **Build electronic results export and submission system** — In progress (parallel agent). Trial secretaries need to submit show results electronically to AKC (and eventually other organizations). AKC Scent Work is the first target. The current workflow in the user's Microsoft Access application generates an XML file from the show data and emails it to AKC. The platform needs to replicate this, starting with AKC Scent Work and designed as an extensible foundation for other sport types and organizations. **Problem:** No electronic results submission exists in myK9Show. Secretaries must currently export data manually and use external tools (Access/Excel) to format and send results to sanctioning bodies. This is the final step in the trial lifecycle and blocks full platform adoption. **Files:** `apps/myk9show/src/features/secretary/` (secretary tools — add "Submit Results" action), `apps/myk9show/src/pages/secretary/ReportsPage.tsx` (or a dedicated results submission page), `packages/secretary/src/` (shared secretary logic — XML generation belongs here). **Solution:** (1) Build an extensible results export engine with a registry pattern (similar to the report engine): each org+sport combination registers a formatter that maps show/trial/class/entry data to the org's required XML schema. (2) Start with `AKCScentWorkFormatter` — user will provide VBA code and example XML output to define the schema. (3) Secretary selects show → "Submit Results to AKC" → system generates XML, previews it, and either downloads for manual email or sends via edge function. (4) Record submission status (`results_submitted_at`, `submitted_to_org`) on the show/trial for audit. May need a `result_submissions` table tracking what was sent, when, and to whom. Design the formatter interface so adding new orgs (UKC, NACSW, etc.) and sport types is just a new registry entry.

---

## Full Exhibitor Onboarding Wizard - 2026-04-07 12:17

- [x] **Build a multi-step onboarding wizard for new exhibitors** — Done. Full-page `/onboarding` route with 5 steps: Profile, Dogs (AddDogPanel), Address, Notifications, Welcome. `onboarding_completed_at` tracked on `exhibitor_profiles`. ExhibitorOnboardingChecker redirects new users to wizard. Replace the current single-field modal (`ExhibitorOnboardingModal`) with a proper wizard that guides a new exhibitor through everything they need before registering for a show. **Problem:** The existing onboarding is a bare-minimum profile gate (first name / last name / phone). New exhibitors have no guided path to set up their account, add their dogs, learn the platform, or understand the registration process — they land on an empty dashboard with no direction. **Files:** `apps/myk9show/src/components/exhibitor/ExhibitorOnboardingChecker.tsx` (trigger logic), `apps/myk9show/src/components/exhibitor/ExhibitorOnboardingModal.tsx` (current single-step modal — replace or extend), `apps/myk9show/src/hooks/useExhibitorProfile.ts` (profile detection + creation), `apps/myk9show/src/pages/Home.tsx` (exhibitor dashboard — should reflect onboarding completion state). **Solution:** Multi-step wizard (dialog or full-page route like `/onboarding`) with steps: (1) Profile — first name, last name, phone (pre-fill from auth metadata); (2) Add your dogs — inline dog creation (name, breed, AKC/org reg number, call name) with skip option; (3) Emergency contact or mailing address (optional but encouraged); (4) Notification preferences (push/email opt-in); (5) Welcome screen / "you're ready to register" CTA. Track completion with an `onboarding_completed_at` timestamp on `exhibitor_profiles` so the wizard never re-shows once dismissed. May need a migration to add that column. Consider making the wizard a dedicated `/onboarding` route (not a modal) for better mobile UX and shareable URL.
...
## Avery Labels Follow-up — Post-Migration Cleanup - 2026-04-08 14:23

- [ ] **Regenerate Supabase types after migration 127** — Run `npx supabase gen types typescript --project-id sojmvhhwsjxmfistvzbe > apps/myk9show/src/types/supabase.ts` to pick up `venue_wifi_network`, `venue_wifi_password` on shows and `is_day_of_show` on entries. **Problem:** Two `@ts-ignore` comments in `useArmbandLabelData.ts` suppress type errors for columns the TypeScript types don't know about yet. These will silently hide real type errors. **Files:** `apps/myk9show/src/hooks/queries/useArmbandLabelData.ts:48-51,67-70`, `apps/myk9show/src/types/supabase.ts`. **Solution:** Regenerate types, then remove the two `@ts-ignore` lines and their TODO comments. Verify `showWifi?.venue_wifi_network` resolves without error.

- [ ] **Wire VenueWifiCard save mutation** — Connect the VenueWifiCard to a Supabase update mutation so secretaries can actually save venue WiFi info. **Problem:** Card currently shows "Coming soon" with disabled inputs because `onSave` is omitted — no data persists. **Files:** `apps/myk9show/src/components/secretary/VenueWifiCard.tsx`, `apps/myk9show/src/pages/secretary/ShowSettingsPage/index.tsx:113-116`. **Solution:** Create a mutation that updates `venue_wifi_network` and `venue_wifi_password` on the shows table (requires regenerated types from task above). Pass as `onSave` prop in ShowSettingsPage. Load current WiFi values from the show record to pre-populate the card inputs (currently hardcoded to empty strings).
