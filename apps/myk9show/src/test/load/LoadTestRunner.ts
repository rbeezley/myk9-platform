...
export class LoadTestRunner {
  ...
  async initialize(browser: Browser): Promise<void> {
    ...
    logger.debug(
      `Generated ${this.testData.users.length} users, ${this.testData.dogs.length} dogs, ${this.testData.shows.length} shows`,
      'app',
      {}
    );
  }
  ...
  async runScenario(scenario: LoadTestScenario): Promise<void> {
    ...
    try {
      ...
      logger.debug('Generated report:', 'test', {
  ...
  private async executeUserWorkflows(
    ...
    while (Date.now() < testEndTime) {
      ...
      try {
        ...
        logger.error(
          `❌ Workflow error for user ${userContext.userId}:`,
          'app',
          {},
          error as Error
        );
  ...
  private async validatePerformanceTargets(scenario: LoadTestScenario): Promise<void> {
    ...
    for (const [operation, operationMetrics] of Object.entries(metrics)) {
      ...
      logger.debug(
        `  Response Time (P95): ${operationMetrics.p95}ms ${responseTimeOk ? '✅' : '❌'}`,
        'app',
        {}
      );
      logger.debug(
        `  Error Rate: ${operationMetrics.errorRate.toFixed(1)}% ${errorRateOk ? '✅' : '❌'}`,
        'app',
        {}
      );
    }
