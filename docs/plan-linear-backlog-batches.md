...
> **Status:** Active — Batches 0–3 complete except MYK9-204 (operator-blocked on Richard). **Batch 3.5 and Batch 4 are complete:** MYK9-233, MYK9-236, MYK9-211, MYK9-245, MYK9-235, MYK9-234, MYK9-237, MYK9-238, MYK9-239, MYK9-240, MYK9-241, and MYK9-242 are Done and deployed/merged as of 2026-08-25. **Read "[HANDOFF 2026-08-24]" under Batch 3.5 before starting anything** — it records what was already claimed by another agent, three issues filed after the last registry refresh, and the verification traps this batch has already hit. Next is Batch 5.
...
PRIMARY:MYK9-235=completed-batch-3.5-lane-b-step-2
PRIMARY:MYK9-234=completed-batch-3.5-lane-b-step-3
PRIMARY:MYK9-237=completed-batch-3.5-lane-c
PRIMARY:MYK9-238=completed-batch-3.5-lane-c
PRIMARY:MYK9-239=completed-batch-3.5-lane-c
PRIMARY:MYK9-240=completed-batch-3.5-lane-d-step-1
PRIMARY:MYK9-241=completed-batch-3.5-lane-d-step-2
PRIMARY:MYK9-242=completed-batch-3.5-lane-d-step-3
PRIMARY:MYK9-88=openspec-umbrella-exhibitor-ux-remediation
PRIMARY:MYK9-109=operator-gated-load-harness-dispatch
PRIMARY:MYK9-198=completed-operator-accepted-2026-08-25
-->
...
| D3  | **Option 1 + 2** — sticky Name column _and_ drop Breed/Sex below `md`                                                                                |
...
MYK9-245 carries a **do-not-do** instruction that will look wrong to a fresh reader: it argues _against_ gating `entries_select`'s handler/owner arms. Those arms are the only route by which an exhibitor could ever be shown their own dropped-show entry. Read it before "hardening" anything there.
...
- **Lane 3.5A — data exposure and ACL contract (do this first).** [MYK9-233](https://linear.app/myk9-platform/issue/MYK9-233) is the highest-severity open item on the board: `manageable_show_ids()` is `SECURITY DEFINER` and never restates the `deleted_at IS NULL` filter that `shows_select` applies **outside** its whole OR group. A site admin therefore sees the entries of a soft-deleted show while the show row itself is invisible, and `buildLedgerRows` maps over `shows` rather than entries — so every dropped show's collected, refunded and net cents vanish from the admin ledger **and** from both summary totals, with nothing on screen. **Verified against staging: 0 soft-deleted shows, so it is reachable but not currently firing.** That is exactly why it should land now rather than after launch. Step 1 is a product decision, not code — _should_ a site admin see soft-deleted shows? The two answers have different blast radii. Then [MYK9-236](https://linear.app/myk9-platform/issue/MYK9-236) (the grant contract's `service_role` column is unenforced against live; 129 of 130 tables hold more than it declares). Both are ACL-contract work and share the verify-against-the-applied-database discipline.
  ...
  **What caught what — the useful part of this one.** Codex pass 1 found a false contract in the migration/test headers plus a fixture that concealed it (the person named "Unrelated Exhibitor" was the owner _and_ handler of both entries and was the identity the over-opening guard impersonated), but not the premise error. CI found the premise error — and found that the behavioural SQL test had **never once executed**, because there is no container runtime on this machine, so CI is always its first real run. Local mutation checks confirmed the wrong line was load-bearing; they cannot say a line should not exist. Codex pass 2 on the corrected diff: no findings.

  **Follow-up filed:** [MYK9-245](https://linear.app/myk9-platform/issue/MYK9-245) — a dropped show hides the exhibitor's entry but keeps their Stripe charge. It argues _against_ gating `entries_select`'s handler/owner arms, because those are the only route by which an exhibitor could be shown that entry.
...
- **Lane 3.5B — payment and payout reconciliation surfaces.** **[DONE 2026-08-25] MYK9-245 — PR #1783, merge `7c85c7670`.** The selected product decision keeps an exhibitor's own paid entry visible after a show soft-delete, marks it cancelled, preserves the existing entry/Payments surfaces, makes charge/refund state legible (including partial refunds), and suppresses payment/check-in next actions. The migration and behavioral SQL contract preserve owner-only tombstone visibility; all CI, build, A11y, E2E, and Vercel gates passed. **[DONE 2026-08-25] MYK9-235 — PR #1787, merge `a087215d3`.** The live database had zero anomalous rows, but the state remains reachable through the service-role/manual reconciliation path; the ledger now surfaces its count and amount without changing Collected/Refunds/Net owed arithmetic. Focused tests, typecheck, focused lint, full CI, build, A11y, E2E, and Vercel gates passed. **[DONE 2026-08-25] MYK9-234 — PR #1789, merge `77f0f3a`.** The payout ledger now uses one responsive tree with preserved table semantics and Playwright geometry coverage at 375px and 1280px; all required CI, A11y, E2E, build, SQL, and package gates passed. Lane 3.5B is complete. 3.5B is the **symptom** side of 3.5A's ledger defect and does not substitute for it: making dropped rows visible on one page leaves the policy asymmetry intact for every other surface built on `manageable_show_ids()`.
- **Lane 3.5C — Batch 3 follow-ups.** **[DONE 2026-08-25] MYK9-237 — PR #1785, merge `b6a91537e`.** A canonical `rosterIsOwnDogsOnly` predicate now drives dog fetching, page title, sidebar label, and owner-column behavior, with intentional role chrome kept separate and a full 128-role-combination matrix. **[DONE 2026-08-25] MYK9-238 — PR #1786, merge `5cefb1a5`.** Semantic CSS-variable colors now emit valid opacity utilities, with a durable build guard that fails when configured colors lack an alpha-capable form; production build, focused tests, typecheck, lint, A11y, E2E, and Vercel gates passed. **[DONE 2026-08-25] MYK9-239 — PR #1788, merge `c27056f1`.** The unreachable 586-line `RegistrationConfirmationNotifications` component was deleted after static, lazy, registry, barrel, string-keyed, and fixture reachability checks; typecheck, lint, focused tests, A11y, E2E, and Vercel gates passed.
- **Lane 3.5D — Reports information architecture.** **[DONE 2026-08-25] MYK9-240 — PR #1790, merge `a2d55e390`.** Reports now opens on after-show report selection, with the existing emergency-packet tool owned by Show Desk and reached through one deep link; packet data preserves authoritative labels/instructions and the normal replication-shaped row contract. Required CI, A11y, E2E, build, SQL, and package gates passed; the Vercel preview was non-required and rate-limited. **[DONE 2026-08-25] MYK9-241 — PR #1791, merge `56f2dd5fc`.** Print state now has one shared writer, Reports reads actor/timestamp/stale state from replication, and loading/sync failure gates duplicate confirmation; 62 focused tests and all required CI, build, A11y, E2E, SQL, and package gates passed. **[DONE 2026-08-25] MYK9-242 — PR #1794, merge `41e7732d9`.** Report selection now scopes registry-specific forms to the selected trial registry while preserving generic, Coming Soon, malformed-data fallback, and deep-link behavior; 80 focused tests and all required CI, build, A11y, E2E, SQL, and package gates passed. Lane 3.5D is complete. Read MYK9-241 against MYK9-198 before starting: three surfaces write print state to `replicatedPaperworkPrintsTable` with three vocabularies, and Reports is the one that shows nothing — so a secretary re-confirms a print she already recorded from the Show Desk an hour earlier, and the twice-daily print reminder reads that record. This is the consolidate-don't-duplicate rule with a live example; the fix deletes the inline writer in `ReportsPage/index.tsx` rather than adding a fourth UI.

### Three issues that no batch originally covered
...
- **[DONE 2026-08-25] [MYK9-198](https://linear.app/myk9-platform/issue/MYK9-198).** The emergency packet, scent-work class parameters, automated per-day generation/delivery (MYK9-228), snapshot/reassembly labels, and paper-results recovery guidance shipped. Richard confirmed live email receipt plus PDF opening and printing, then explicitly accepted closure. The stricter signed-out/clean-device check and mock paper-day rehearsal were waived as residual pre-live UAT rather than claimed as completed evidence.
...
4. **Batch 3.5 is complete.** Proceed to Batch 5 after resolving any operator-gated prerequisites.
