...
3. Remove the retry action for this permanent authorization failure while preserving existing retry/discard behavior for other sync failures.
...
2. Add a provider listener regression test proving an authorization-classified score failure shows the specific message and omits Retry.
