...
- **Pay with a throwaway dog, never a seeded one.** Every dog this walk creates is named `ZZ Walk Dog <run token> #N`, N unique across the run regardless of account, so each row anchors to exactly one locator (assert the match count is 1 before any delete) and the walk's orders never contaminate the fixtures the secretary walk and CI depend on. Mint one run token at start — `<YYYY-MM-DD HHMM>` — print it at the top of the report and use it in every dog name, so a same-day re-run never matches an earlier run's rows. Record each account's dog and entry counts before the walk. **Submitted entries cannot be undone by an exhibitor** — self-service withdraw is deferred post-fall (`docs/roles/exhibitor.md`) — so every entry this run submits persists on `Heartland UKC Nosework Trial` (`dededede-0000-0000-0000-000000000011`); record each entry id and the new entry count in the report so the accumulation stays visible. That is accepted residue, not a finding, and an entry-count difference equal to what this run submitted is expected; a larger one is a finding. At the end soft-delete every dog this run created that holds NO submitted entry. Match by this run's full name, never by the `ZZ Walk` prefix. An earlier run's residue is reported, not deleted. **The dog that paid is expected residue: do not attempt to delete it.** `soft_delete_dog` correctly refuses a dog with a paid entry (`MK002`), and withdraw/refund are out of scope. Payment and full teardown cannot both be satisfied through the UI, and that is accepted, not a failure. **Its sink is the walk-residue cleanup (MYK9-734), which an operator runs, never this walk:** `supabase/ops/walk-residue-cleanup.sql`, keyed on this run's EXACT token, records every row it would remove (the dogs, their entries and status history, the Stripe order and any enrollment left with nothing else on it) before it deletes anything, and refuses to delete a state it did not record. How to run it: `docs/operations/walk-residue-cleanup.md`. Put the line `WALK RESIDUE TOKEN <run token>` in the report, with the paid dog's name and entry id, so the operator has the exact key. Until it runs, this residue also makes the next reseed abort on its money guard, because the target show is one the seed deletes. Then assert the dog count equals the recorded start count plus the dogs this run left for that reason, and name each one. Also report the running total of `ZZ Walk Dog %` rows on `exhibitor@`, so the accumulation stays visible. Count assertions are report-and-stop checks: a mismatch is a finding, never corrected by deleting a row this run did not create. That teardown runs before finishing for ANY reason, including a run that stops after one persona pass.
...
**The show to enter is `Heartland UKC Nosework Trial`** (`dededede-0000-0000-0000-000000000011`), a lean-seed show: entry window `CURRENT_DATE - 16 .. + 76` relative to the last reseed, so it is always open, zero seeded entries, and payable through the Heartland club's sandbox Stripe account. It is UKC, so the throwaway dog needs a UKC registration number before a class can be selected — add one in task 1. (Until MYK9-558 this walk entered a MYK9-109 load show, which a plain reseed now removes.) **Its paid residue blocks the next reseed:** the show is one the seed deletes and re-creates, so a paid entry or Stripe order this walk leaves on it makes `seed-demo.sql` abort on its money guard until an operator clears it with the walk-residue cleanup (MYK9-734, `docs/operations/walk-residue-cleanup.md`). Record every paid entry id and the run token in the report so the operator has the exact key.
...
- The `ZZ Audit - *` and `[E2E MYK9-336] Past Due` shows are genuinely closed. **Walk what a CLOSED show offers an exhibitor every run.** A prior walk found the Enter CTA letting the user begin an impossible task (E24 / MYK9-336), and the 2026-09-13 walk skipped it with the fixture available. That omission is not repeated.
...
Read the prior exhibitor audits in `docs/audits/`: `2026-07-02-exhibitor-elderly-ux-audit-claude.md` (E1–E8), `2026-07-05-exhibitor-entries-scanability-ux-audit.md`, `2026-07-06-exhibitor-elderly-browser-ux-audit.md`, `2026-09-01-exhibitor-task-walk-claude.md` (E9–E23), `2026-09-04-…` (E24–E31), `2026-09-06-…` (E32–E37), `2026-09-13-…` (E38–E43), plus any later `docs/audits/*-exhibitor-*.md`. Re-walk E1–E8 and every finding marked fixed, and confirm it holds **in the browser**. A finding that regressed is a P1 regardless of its original severity. Explicitly list which prior findings you re-verified and which you could not reach.
