...
const ThreeDotMenu: React.FC<ThreeDotMenuProps> = ({ 
  ...
  <DropdownMenu>
    ...
    <DropdownMenuContent align="end">
      {onView && (
        <DropdownMenuItem onSelect={() => { onView(); }} className="min-h-[44px]"><Eye size={18} className="mr-2" />{viewLabel}</DropdownMenuItem>
      )}
      {onEdit && (
        <DropdownMenuItem onSelect={() => { onEdit(); }} className="min-h-[44px]"><Pencil size={18} className="mr-2" />{editLabel}</DropdownMenuItem>
      )}
      {onEditPhoto && (
        <DropdownMenuItem onSelect={() => { onEditPhoto(); }} className="min-h-[44px]">
      ...
      {showManageQualifications && onManageQualifications && (
        <>
          ...
          <DropdownMenuItem onSelect={() => { onManageQualifications(); }} className="min-h-[44px]">
      ...
      {onDelete && (
        <DropdownMenuItem onSelect={() => { onDelete(); }} className="text-red-600 min-h-[44px]"><Trash2 size={18} className="mr-2" />Delete</DropdownMenuItem>
      )}
    </DropdownMenuContent>
  </DropdownMenu>
);
