...
describe('send-access-request-email', () => {
  ...
  it("notifies the club's admins of a membership request", async () => {
    ...
    await run(
      fetchImpl,
      { kind: 'membership', requestId: REQUEST_ID, event: 'submitted' },
      REQUESTER_AUTH
    );
  ...
  it('tells an approved new-club requester that club-admin access is available', async () => {
    ...
    await run(
      fetchImpl,
      { kind: 'new_club', requestId: REQUEST_ID, event: 'decision' },
      REVIEWER_AUTH
    );
  ...
  it('rejects an unknown kind or a malformed id before reading anything', async () => {
    await expect(
      run(
        okFetch(),
        { kind: 'club_admin', requestId: REQUEST_ID, event: 'submitted' },
        REQUESTER_AUTH
      )
    ...
    await expect(
      run(
        okFetch(),
        { kind: 'new_club', requestId: 'not-a-uuid', event: 'submitted' },
        REQUESTER_AUTH
      )
    ).rejects.toMatchObject({ status: 400 });
  });
