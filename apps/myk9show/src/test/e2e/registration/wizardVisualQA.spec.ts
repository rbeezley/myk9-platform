...
test('dark-mode muted captions clear WCAG AA on the composited wizard surfaces', async ({
  ...
  // Known answers first, through the same page function, printed every run:
  // if the measurement itself breaks, this test must fail loudly rather than
  // certify whatever it computed (MYK9-784).
  await assertContrastHarness(page);
  ...
  console.log(
    'WORST_MUTED_DETAIL ' +
      JSON.stringify({
        ...muted!.details,
        awaitedAnimations: muted!.awaitedAnimations,
        initialMinOpacity: muted!.initialMinOpacity,
      })
  );
  // A reading taken while a finite animation still runs is a frame of that
