...
async function globalTeardown(config: FullConfig) {
  ...
  try {
    ...
    try {
      ...
      const summary = {
        total:
          parsedTestData.suites?.reduce(
            (acc: number, suite: TestSpec) => acc + (suite.specs?.length || 0),
            0
          ) || 0,
        passed:
          parsedTestData.suites?.reduce(
            (acc: number, suite: TestSpec) =>
              acc +
              (
                suite.specs?.filter(spec =>
                  spec.tests?.every((test: Test) =>
                    test.results?.every((result: TestResult) => result.status === 'passed')
                  )
                ) || []
              ).length,
            0
              ...
              (
                suite.specs?.filter(spec =>
                  spec.tests?.some((test: Test) =>
                    test.results?.some((result: TestResult) => result.status === 'failed')
                  )
                ) || []
              ).length,
            0
  ...
  try {
    ...
    if (isWindows) {
      ...
      const { stdout: gitStdout } = await execAsync(
        'tasklist /fi "imagename eq git.exe" /fo csv | find /c /v ""'
      );
      ...
      try {
        const { stdout: bashStdout } = await execAsync(
          'tasklist /fi "imagename eq bash.exe" /fo csv 2>nul | find /c "bash.exe"'
        );
