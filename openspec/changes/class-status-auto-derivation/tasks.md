...
- [ ] 7.7 **FOLLOW-UP (from review):** resend intent preservation — `rebuildUpdatePayload` currently strips ALL server-owned keys (status/status_source/is_scoring_finalized/reopened_after_closeout_at) on resend, so a queued-unsent manual Mark Complete/Started caught by a token-advance reconcile is dropped and must be re-issued. Proper fix: persist the original mutation's explicit key set alongside the `PendingMutation` so a resend re-asserts exactly what the mutation intended. Narrow/self-correcting; not launch-blocking.
