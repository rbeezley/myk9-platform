# exhibitor-show-day-check-in Specification

## Purpose

Defines self check-in from the exhibitor's My Shows dog card: a single day-gated control that checks a dog into every eligible class for the current trial day in the trial's timezone, never for a later day, looping the existing per-class write path; plus per-class links that keep individual check-in state changeable through the existing status dialog.
