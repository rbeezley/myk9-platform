...
- **Implement judge availability persistence** — Persist availability data (dates, capacity, travel radius) to a dedicated `judge_availability` table so it can power judge-to-show matching queries. **Problem:** `AvailabilitySection.tsx` collects startDate, endDate, maxShowsPerMonth, travelRadius, and `JudgeInfo.availability` type exists, but no DB table or queries exist — data is never persisted. **Files:** `supabase/migrations/` (new 050), `apps/myk9show/src/services/database/queries/judgeQueries.ts`, `apps/myk9show/src/types/database-mappings.ts`, `apps/myk9show/src/components/panels/entities/JudgeCreationPanel/index.tsx`, `apps/myk9show/src/components/panels/edit/UserEditPanel.tsx`, `apps/myk9show/src/store/userStore.ts`. **Solution:** Full plan saved to `docs/plans/availability-persistence-plan.md` — dedicated table with UNIQUE on person_id, upsert pattern, 7 implementation steps, 8 files changed. Implement when ready to build judge-to-show matching.

## Judge Analytics Dashboard - 2026-03-06 12:00

- **Build judge analytics for admins and judges** — Two views: admin/secretary view ("which judges should I invite?") with roster summary, utilization stats, acceptance rates, fee totals, qualification alerts; and judge personal view ("how's my season?") with entries judged, result distribution, scoring pace, upcoming assignments. **Problem:** No analytics exist for judge activity despite rich data in `judge_assignments`, `judge_qualifications`, `entries` (scoring timestamps, results). Secretaries have no data-driven way to choose judges for shows; judges have no personal performance summary. **Files:** `apps/myk9show/src/services/database/queries/judgeQueries.ts` (add analyticsQueries), `apps/myk9show/src/hooks/queries/useJudgeAnalyticsQuery.ts` (new), `apps/myk9show/src/pages/admin/JudgeAnalyticsPage.tsx` (new), `apps/myk9show/src/routes/adminRoutes.tsx`, `apps/myk9show/src/pages/JudgeDashboard.tsx`, `apps/myk9show/src/features/pipeline/print/print-templates.tsx`. **Solution:** Full plan saved to `docs/plans/judge-analytics-plan.md` — read-only over existing tables (no new migrations), recharts visualizations reusing dog stats patterns, CSV export via existing `lib/export.ts`, 6 files changed. Implement after judge assignments are actively flowing.

## Role-Based Dashboards - 2026-03-06 13:30

- **Build sidebar-driven dashboards for Judge and Exhibitor roles** — Extend the Secretary Mission Control pattern to give each role a dedicated command center. **Problem:** Judge has no layout wrapper or sidebar (standalone pages); Exhibitor layout exists but dashboard content is basic (stats cards + entry list). Secretary's Mission Control proves the sidebar pattern works. **Files:** `apps/myk9show/src/components/judge/JudgeLayout.tsx` (new), `apps/myk9show/src/components/judge/JudgeSidebar.tsx` (new), `apps/myk9show/src/routes/judgeRoutes.tsx`, `apps/myk9show/src/pages/JudgeDashboard.tsx`, `apps/myk9show/src/components/exhibitor/ExhibitorSidebar.tsx`, `apps/myk9show/src/components/exhibitor/ExhibitorLayout.tsx`, `apps/myk9show/src/pages/ExhibitorDashboard.tsx`. **Solution:** Full plan saved to `docs/plans/role-dashboards-plan.md` — 3 phases: (1) Judge dashboard with JudgeLayout + JudgeSidebar + route restructuring, (2) Exhibitor dashboard enrichment with dog cards + entry timeline + collapsible sidebar, (3) Optional shared component extraction. Scoring pages explicitly excluded from sidebar. Phases 1-2 are independent and parallelizable.
