...
//   3. chargeVerification  FeeBreakdown / NoFeeBreakdown + pending-net counts
...
export interface FinancialSummaryInput extends FinancialScopeArgs {
   ...
   * snapshot to point at and resolve to NoFeeBreakdown — recorded, with no Stripe row behind them.
   */
...
export function deriveChargeVerification(
  ...
  for (const line of entryAccounting.lines) {
    ...
    if (state === 'FeeBreakdown') result.feeBreakdownCount += 1;
    else result.noFeeBreakdownCount += 1;
  }
