...
| [plan-email-fn-authz.md](plan-email-fn-authz.md)           | SA-004 (send-email), SA-005 (send-auth-email), SA-013 (send-waitlist-invite) | MED×2 + LOW   | M      | No       | **DONE + DEPLOYED + VERIFIED LIVE 2026-07-04** — code merged [#1111](https://github.com/rbeezley/myk9-platform/pull/1111); deploys: `send-waitlist-invite` v28 (403 on missing/wrong secret; Vault+env `waitlist_invite_secret` provisioned), `send-email` v58 (401 unauthenticated), `send-auth-email` v45 (live password-reset logged `200` signature-verified + `resend-webhook 200`). SA-005 env secret matched the already-enabled dashboard hook (no dashboard write). Change archived; 3 specs promoted. |
...
| [plan-people-overfetch.md](plan-people-overfetch.md)       | SA-008 (`select('*')` on people at login)                                    | MEDIUM        | M      | No       | DONE — `openspec/changes/security-people-overfetch/` (fetch gated to mgmt roles + column-allowlist on getAllUsers; LazyDogCard repointed to `dog.ownerName`)     |
| [plan-passcode-throttle.md](plan-passcode-throttle.md)     | SA-011 (`upsert_ringside_session` no throttle)                               | LOW           | S–M    | No       | **DONE + DEPLOYED + VERIFIED LIVE 2026-07-04** ([#1130](https://github.com/rbeezley/myk9-platform/pull/1130)) — RPC now authorizes on the forge-proof `app_metadata` claim (no inline `validate_passcode`), explicit `REVOKE ... FROM anon` (Codex-caught). Migration `20260704190000` pushed; live checks: anon EXECUTE=false, `validate_passcode` absent, `app_metadata` read; rolled-back psql proof confirms ALLOW (valid claim upserts) + DENY (raw passcode ignored → rejected). Change archived. |
