...
test.describe('Show Creation Wizard - Detailed Testing', () => {
  ...
  test.describe('Wizard Navigation and State Management', () => {
    ...
    test('should navigate through all wizard steps correctly', async () => {
      ...
      await expect(showTestHelper.page.locator('[data-testid="step-indicator"]')).toContainText('Show Details');
      await expect(showTestHelper.page.locator('[data-testid="progress-bar"]')).toHaveAttribute('aria-valuenow', '0');
      ...
      // Verify step 2
      await expect(showTestHelper.page.locator('[data-testid="step-indicator"]')).toContainText('Trials');
      await expect(showTestHelper.page.locator('[data-testid="progress-bar"]')).toHaveAttribute('aria-valuenow', '25');
      ...
      // Verify step 3
      await expect(showTestHelper.page.locator('[data-testid="step-indicator"]')).toContainText('Classes');
      await expect(showTestHelper.page.locator('[data-testid="progress-bar"]')).toHaveAttribute('aria-valuenow', '50');
      ...
      // Verify step 4
      await expect(showTestHelper.page.locator('[data-testid="step-indicator"]')).toContainText('Review');
      await expect(showTestHelper.page.locator('[data-testid="progress-bar"]')).toHaveAttribute('aria-valuenow', '75');
    });
    ...
    test('should support backward navigation and preserve state', async () => {
      ...
      await expect(showTestHelper.page.locator('[data-testid="class-checkbox-Novice-A"]')).toBeChecked();
      
      await showTestHelper.page.click('[data-testid="previous-step-button"]');
      await expect(showTestHelper.page.locator(`[data-testid="trial-item-${trial.id}"]`)).toBeVisible();
    ...
    test('should save and restore draft progress', async () => {
      ...
      await expect(showTestHelper.page.locator('[data-testid="draft-saved-message"]')).toBeVisible();
      ...
      await expect(showTestHelper.page.locator('[data-testid="draft-restored-message"]')).toBeVisible();
      await expect(showTestHelper.page.locator('[data-testid="show-name-input"]')).toHaveValue(showData.name);
    });
    ...
    test('should warn about unsaved changes before closing', async () => {
      ...
      await expect(showTestHelper.page.locator('[data-testid="unsaved-changes-dialog"]')).toBeVisible();
      await expect(showTestHelper.page.locator('[data-testid="dialog-title"]')).toContainText('Unsaved Changes');
  ...
  test.describe('Step Validation and Error Handling', () => {
    ...
    test('should prevent progression with invalid step data', async () => {
      ...
      await expect(showTestHelper.page.locator('[data-testid="validation-messages"]')).toContainText('Show name is required');
      ...
      await expect(showTestHelper.page.locator('[data-testid="validation-messages"]')).not.toContainText('Show name is required');
      await expect(showTestHelper.page.locator('[data-testid="validation-messages"]')).toContainText('Show type is required');
    });
    ...
    test('should validate complex business rules', async () => {
      ...
      await expect(showTestHelper.page.locator('[data-testid="validation-messages"]')).toContainText('End date must be after start date');
      ...
      await expect(showTestHelper.page.locator('[data-testid="validation-messages"]')).toContainText('Entry close date must be before show start date');
    });
    ...
    test('should validate fee constraints', async () => {
      ...
      await expect(showTestHelper.page.locator('[data-testid="validation-messages"]')).toContainText('Day-of-show fee should typically be higher than pre-entry fee');
    });
    ...
    test('should handle network errors gracefully', async () => {
      ...
      await expect(showTestHelper.page.locator('[data-testid="club-select"] option')).toHaveCount(2); // Default + test club
    });
  });
  ...
  test.describe('Trial Management Within Wizard', () => {
    ...
    test('should support adding multiple trials of different types', async () => {
      ...
      // Verify trial count
      await expect(showTestHelper.page.locator('[data-testid="trial-count"]')).toContainText('3 trials');
    });
    ...
    test('should validate trial scheduling and prevent conflicts', async () => {
      ...
      await expect(showTestHelper.page.locator('[data-testid="scheduling-conflict-warning"]')).toBeVisible();
      await expect(showTestHelper.page.locator('[data-testid="conflict-suggestion"]')).toContainText('Consider scheduling at');
      
      // Accept suggestion
      await showTestHelper.page.click('[data-testid="accept-suggestion-button"]');
      
      // Should update time automatically
      await expect(showTestHelper.page.locator('[data-testid="trial-time-input"]')).not.toHaveValue('09:15');
    });
    ...
    test('should support trial reordering and time optimization', async () => {
      ...
      const trials = [
        ShowTestDataFactory.createTrialData({ name: 'Afternoon Trial', dateTime: '2024-12-15T14:00:00.000Z', type: 'Agility' }),
        ShowTestDataFactory.createTrialData({ name: 'Morning Trial', dateTime: '2024-12-15T09:00:00.000Z', type: 'Obedience' }),
        ShowTestDataFactory.createTrialData({ name: 'Evening Trial', dateTime: '2024-12-15T17:00:00.000Z', type: 'Rally' }),
      ];
      ...
      const firstTrialName = await trialElements[0].locator('[data-testid="trial-name"]').textContent();
      expect(firstTrialName).toContain('Morning');
      
      const lastTrialName = await trialElements[2].locator('[data-testid="trial-name"]').textContent();
    ...
    test('should support bulk trial operations', async () => {
      ...
      await expect(showTestHelper.page.locator(`[data-testid="trial-date-${trials[0].id}"]`)).toContainText('2024-12-16');
      await expect(showTestHelper.page.locator(`[data-testid="trial-date-${trials[1].id}"]`)).toContainText('2024-12-16');
      
      // Third trial should be unchanged
      await expect(showTestHelper.page.locator(`[data-testid="trial-date-${trials[2].id}"]`)).not.toContainText('2024-12-16');
    });
  });
  ...
  test.describe('Class Selection and Template Integration', () => {
    ...
    test('should filter classes by multiple criteria', async () => {
      ...
      const containerClasses = await showTestHelper.page.locator('[data-testid^="class-card-Container"]').count();
      expect(containerClasses).toBeGreaterThan(0);
      
      // Verify non-container classes are hidden
      await expect(showTestHelper.page.locator('[data-testid^="class-card-Interior"]')).not.toBeVisible();
      
      // Test level filter combination
      await showTestHelper.page.selectOption('[data-testid="level-filter"]', 'Novice');
      
      const containerNoviceClasses = await showTestHelper.page.locator('[data-testid*="Container"][data-testid*="Novice"]').count();
    ...
    test('should support class customization before adding', async () => {
      ...
      await showTestHelper.page.click('[data-testid="class-card-Container-Novice-A"] [data-testid="customize-button"]');
      ...
      await expect(showTestHelper.page.locator('[data-testid="class-card-Container-Novice-A"] [data-testid="customized-indicator"]')).toBeVisible();
      await expect(showTestHelper.page.locator('[data-testid="class-card-Container-Novice-A"] [data-testid="max-entries-display"]')).toContainText('25');
    });
    ...
    test('should validate class combinations and show warnings', async () => {
      ...
      await expect(showTestHelper.page.locator('[data-testid="scheduling-suggestion"]')).toBeVisible();
      await expect(showTestHelper.page.locator('[data-testid="suggestion-text"]')).toContainText('running order');
      ...
      await expect(showTestHelper.page.locator('[data-testid="running-order-display"]')).toBeVisible();
    });
    ...
    test('should support preset class packages', async () => {
      ...
      await expect(showTestHelper.page.locator('[data-testid="class-checkbox-Novice-A"]')).toBeChecked();
      await expect(showTestHelper.page.locator('[data-testid="class-checkbox-Novice-B"]')).toBeChecked();
      await expect(showTestHelper.page.locator('[data-testid="class-checkbox-Open-A"]')).toBeChecked();
      await expect(showTestHelper.page.locator('[data-testid="class-checkbox-Open-B"]')).toBeChecked();
      await expect(showTestHelper.page.locator('[data-testid="class-checkbox-Utility-A"]')).toBeChecked();
      await expect(showTestHelper.page.locator('[data-testid="class-checkbox-Utility-B"]')).toBeChecked();
      ...
      await expect(showTestHelper.page.locator('[data-testid="class-checkbox-Novice-A"]')).toBeChecked();
      await expect(showTestHelper.page.locator('[data-testid="class-checkbox-Novice-B"]')).toBeChecked();
      await expect(showTestHelper.page.locator('[data-testid="class-checkbox-Open-A"]')).not.toBeChecked();
      await expect(showTestHelper.page.locator('[data-testid="class-checkbox-Utility-A"]')).not.toBeChecked();
    });
  });
  ...
  test.describe('Review Step and Show Creation', () => {
    ...
    test('should display comprehensive show summary', async () => {
      ...
      // Add multiple trials
      for (const trial of workflowData.trials.slice(0, 2)) { // Just first 2 for test speed
        await showTestHelper.addTrial(trial);
      }
      await showTestHelper.page.click('[data-testid="next-step-button"]');
      ...
      await expect(showTestHelper.page.locator('[data-testid="review-show-name"]')).toContainText(workflowData.show.name);
      await expect(showTestHelper.page.locator('[data-testid="review-show-type"]')).toContainText(workflowData.show.type);
      await expect(showTestHelper.page.locator('[data-testid="review-location"]')).toContainText(workflowData.show.location);
      
      // Trial summary
      await expect(showTestHelper.page.locator('[data-testid="review-trial-count"]')).toContainText('2 trials');
      
      // Class summary
      await expect(showTestHelper.page.locator('[data-testid="review-class-count"]')).toContainText('2 classes');
      
      // Fee summary
      await expect(showTestHelper.page.locator('[data-testid="review-pre-entry-fee"]')).toContainText(workflowData.show.preEntryFee.toString());
      
      // Judge summary
      await expect(showTestHelper.page.locator('[data-testid="review-judge-count"]')).toContainText(workflowData.show.judgeIds.length.toString());
    });
    ...
    test('should support different creation options', async () => {
      ...
      await expect(showTestHelper.page.locator('[data-testid="create-and-publish-button"]')).toBeVisible();
      ...
      // Verify show status
      await expect(showTestHelper.page.locator('[data-testid="show-status"]')).toContainText('Draft');
    });
    ...
    test('should validate required data before creation', async () => {
      ...
      await expect(showTestHelper.page.locator('[data-testid="validation-banner"]')).toContainText('At least one trial is required');
    });
    ...
    test('should handle creation errors gracefully', async () => {
      ...
      await expect(showTestHelper.page.locator('[data-testid="error-message"]')).toContainText('Database connection failed');
      
      // Should offer retry option
      await expect(showTestHelper.page.locator('[data-testid="retry-creation-button"]')).toBeVisible();
