...
**Fix:** `user_roles_select` is restored to own rows OR `is_site_admin()`; club show managers cannot enumerate raw grants, while site admins retain the complete assignment ledger needed by audit and reactivation workflows. Live person screens use `get_visible_person_roles(uuid[])`, which returns only deduplicated, active, unexpired role names for live people; non-site-admin managers may resolve only judge labels and receive no grant metadata. Role-filtered people reads first call `get_visible_person_ids_by_role(text)` so they do not load the entire people directory. `permission_audit_log` SELECT remains site-admin-only.
