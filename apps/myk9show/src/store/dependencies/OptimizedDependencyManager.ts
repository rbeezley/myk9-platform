...
export class OptimizedDependencyManager {
  ...
  async loadStoreDependencies(
    ...
    if (dependency.requiredStores.length > 0) {
      await Promise.all(
        dependency.requiredStores.map(async depStore => {
  ...
  generateOptimizedLoadReport(): string {
    ...
    this.loadGroups.forEach(group => {
      lines.push(
        `  Priority ${group.priority} - ${group.groupName} (${group.stores.length} stores)`
      );
