...
| P2 | Type escapes / schema drift | `entryService.ts` writes `{ status }` to `entries`, which has no `status` column. | Generated entries type has `entry_status`, `check_in_status`, `is_in_ring`; `markInRing` has no direct app caller found. | Verify liveness first; delete if dead; only fix the write path if live. | Finder-confirmed; Phase-2 pending; cross-referenced for Wave A dead-code verification. |
...
| P2 | Dead code / schema drift | `entryService.ts` `markInRing` appears uncalled but writes nonexistent `entries.status`. | Source grep found no direct app caller; type audit confirmed no `entries.status` column. | Prove liveness absence; if dead, delete in Wave A instead of fixing schema drift. |
