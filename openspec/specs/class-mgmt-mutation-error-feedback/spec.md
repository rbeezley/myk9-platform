...
The system SHALL show a `toast.error` for each bulk status-change or bulk-delete
`.mutate()` call that fails, rather than silently clearing the selection as if
all calls succeeded.
