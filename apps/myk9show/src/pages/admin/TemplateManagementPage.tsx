...
import { Plus, Filter, MoreVertical, Edit, TestTube, Copy, Download, Trash2, FileText } from 'lucide-react';
      ...
      <div className="apple-template-container">
        ...
        <div className="apple-template-header">
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="apple-template-subtitle">Create and manage class templates for different organizations and show types</p>
            </div>
            <div className="flex gap-2">{/* Save button temporarily removed */}
              ...
              <Button 
                onClick={async () => {
                  if (confirm('This will clear ALL templates and reload defaults. Are you sure?')) {
        ...
        <div className="apple-filter-section">
          ...
          <div className="apple-filter-grid">
            ...
            {/* Organization */}
            <Select value={filter.organization} onValueChange={(value) => handleFilterChange('organization', value as Organization)}>
            ...
            {/* Show Type */}
            <Select value={filter.showType} onValueChange={(value) => handleFilterChange('showType', value as ShowType)}>
              <SelectTrigger className="apple-filter-input">
            ...
            {/* Template Type */}
            <Select value={filter.isOfficial === undefined ? 'all' : filter.isOfficial ? 'true' : 'false'} onValueChange={(value) => handleFilterChange('isOfficial', value === 'all' ? undefined : value === 'true')}>
              <SelectTrigger className="apple-filter-input">
            ...
            {/* Active Status */}
            <Select value={filter.isActive === undefined ? 'all' : filter.isActive ? 'true' : 'false'} onValueChange={(value) => handleFilterChange('isActive', value === 'all' ? undefined : value === 'true')}>
              <SelectTrigger className="apple-filter-input">
          ...
          {/* Active Filters */}
          {(filter.searchTerm || filter.organization || filter.showType || filter.isActive !== undefined || filter.isOfficial !== undefined) && (
            <div className="flex gap-2 mt-4 flex-wrap">
              <span className="text-sm text-muted-foreground">Active filters:</span>
              {filter.searchTerm && (
                <Badge variant="secondary" className="flex items-center gap-1">
                  Search: "{filter.searchTerm}"
                  <button onClick={() => handleFilterChange('searchTerm', '')} className="ml-1 hover:bg-red-100 rounded-full">
                    ×
                  </button>
                </Badge>
              )}
              {filter.organization && (
                <Badge variant="secondary" className="flex items-center gap-1">
                  ...
                  <button onClick={() => handleFilterChange('organization', undefined)} className="ml-1 hover:bg-red-100 rounded-full">
                    ×
                  </button>
                </Badge>
              )}
              {filter.showType && (
                <Badge variant="secondary" className="flex items-center gap-1">
                  ...
                  <button onClick={() => handleFilterChange('showType', undefined)} className="ml-1 hover:bg-red-100 rounded-full">
                    ×
                  </button>
                </Badge>
              )}
              {filter.isActive !== undefined && (
                <Badge variant="secondary" className="flex items-center gap-1">
                  ...
                  <button onClick={() => handleFilterChange('isActive', undefined)} className="ml-1 hover:bg-red-100 rounded-full">
                    ×
                  </button>
                </Badge>
              )}
              {filter.isOfficial !== undefined && (
                <Badge variant="secondary" className="flex items-center gap-1">
                  ...
                  <button onClick={() => handleFilterChange('isOfficial', undefined)} className="ml-1 hover:bg-red-100 rounded-full">
                    ×
                  </button>
                </Badge>
              )}
            ...
            <div className="apple-templates-grid">
              ...
              <div key={template.id} className="apple-template-card">
                ...
                <div className="apple-template-card-content">
                  ...
                  <p className="apple-template-card-showtype">{template.showType.replace('_', ' ')}</p>
                  ...
                  <div className="apple-template-card-badges">
                    <span className={`apple-template-badge ${template.isActive ? 'active' : 'inactive'}`}>
                      {template.isActive ? 'Active' : 'Inactive'}
                    </span>
                    <span className={`apple-template-badge ${template.isOfficial ? 'official' : 'custom'}`}>
              ...
              {filteredTemplates.length === 0 && (
                <div className="apple-template-empty">
                  <div className="apple-template-empty-icon">
                    📋
                  </div>
              ...
              <div className="flex">
                ...
                <div className="ml-3">
                  ...
                  <p className="mt-2 text-sm text-red-700">
                    This will permanently delete the template and all its configurations. 
                    Any shows or classes using this template will not be affected.
                  </p>
                </div>
              </div>
            </div>
