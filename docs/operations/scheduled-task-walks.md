...
- MYK9-558 (2026-09-23) changed the secretary walk's PDF-calibration hint and the exhibitor walk's
  target show, account table and scale mechanic, because the 63-entry classes, the load shows and
  the load dogs now exist only while `supabase/seed-load-fixture.sql` is applied. Copy both fenced blocks into the installed
  `SKILL.md` files and rerun `pnpm qa:prompt-parity` after the change merges.
...
| --- | --- | --- |
| `exhibitor@myk9t.com` | **5 seeded dogs, 12 seeded entries** (plus earlier walks' residue) | The established account: paid, pending, refunded, move-up, scored and full-class history on the Heartland show. With the opt-in MYK9-109 load fixture applied it also carries ~250 load dogs and ~1,200 entries; a plain reseed removes them. |
