...
function requireHttpUrl(env: NodeJS.ProcessEnv, key: string): string {
  ...
  try {
    ...
    throw new AdminMcpConfigError(`${key} must be an absolute http(s) URL, received "${raw}"`);
  }
  if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
    throw new AdminMcpConfigError(`${key} must use the http or https scheme, received "${raw}"`);
  }
  return raw;
}

function parsePositiveIntLimit(env: NodeJS.ProcessEnv, key: string, fallback: number): number {
  const raw = env[key];
  if (raw == null || raw.trim() === '') {
    return fallback;
  }
  ...
  if (!Number.isInteger(parsed) || parsed <= 0) {
    throw new AdminMcpConfigError(`${key} must be a positive integer, received "${raw}"`);
  }
  return parsed;
}
...
export function loadAdminMcpConfig(env: NodeJS.ProcessEnv): AdminMcpConfig {
  ...
  const supabaseServiceRoleKey = requireString(env, 'MYK9_MCP_SUPABASE_SERVICE_ROLE_KEY');
  ...
  const configuredMax = parsePositiveIntLimit(env, 'MYK9_MCP_MAX_LIMIT', FALLBACK_MAX_LIMIT);
