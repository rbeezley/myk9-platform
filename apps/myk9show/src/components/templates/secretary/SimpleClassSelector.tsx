...
export const SimpleClassSelector: React.FC<SimpleClassSelectorProps> = ({
  ...
  const levels = useMemo(() => {
    const uniqueLevels = Array.from(new Set(
      classes.map(c => c.level).filter((level): level is string => Boolean(level))
    ));
  ...
  const selectableClasses = filteredClasses.filter(cls => !isClassExisting(cls));
  const allSelected = selectableClasses.length > 0 && 
    selectableClasses.every(cls => isClassSelected(cls));
          ...
          <div className="apple-class-search">
            <div className="apple-class-search-wrapper relative">
              ...
              {selectedClasses.length > 0 && (
                <div className="absolute right-3 top-1/2 transform -translate-y-1/2 bg-blue-500 text-white text-xs px-2 py-1 rounded-full font-medium">
      ...
      <div className="space-y-5">
        {Object.keys(classesByElement).length > 0 ? (
          Object.entries(classesByElement).map(([element, elementClasses]) => {
            ...
            const allElementClassesSelected = selectableElementClasses.length > 0 && 
              selectableElementClasses.every(cls => isClassSelected(cls));
            const someElementClassesSelected = selectableElementClasses.some(cls => isClassSelected(cls));
            
            const toggleAllElementClasses = () => {
              if (allElementClassesSelected) {
                // Deselect all classes in this element
                const updatedSelected = selectedClasses.filter(selected => 
                  !elementClasses.some(elementClass => 
                ...
                const elementClassesToAdd = elementClasses.filter(cls => 
                  !isClassSelected(cls) && !isClassExisting(cls)
                );
            ...
            return (
              <div key={element} className="apple-class-element-section">
                  ...
                  <div className="apple-class-element-info flex-wrap">
                    <div 
                      className={`apple-class-element-checkbox ${
                        allElementClassesSelected ? 'checked' : 
                        someElementClassesSelected ? 'indeterminate' : ''
                      }`}
                    ...
                    />
                    <h3 
                      className="apple-class-element-title"
                      onClick={toggleAllElementClasses}
                    >
                      {element}
                    </h3>
                    <div className="apple-class-element-count">
                      {elementClasses.length} classes
                    </div>
                ...
                <div className={`apple-class-cards-grid ${availableJudges.length > 0 ? 'judge-mode' : ''}`}>
                        ...
                        <div className="apple-class-card-content">
                          <div className={`apple-class-card-level ${isExisting ? 'disabled' : ''}`}>
                            ...
                            {classDefinition.section && (
                              <span className={`apple-class-card-section-inline ${isExisting ? 'disabled' : ''}`}>
                                {classDefinition.section}
                              </span>
                            )}
                          </div>
                          ...
                          {judgeAssignments[classDefinition.className] && !isExisting && (
                            <div className="mt-1 text-[9px] text-muted-foreground flex items-center gap-1">
                              ...
                              <span className="truncate">{getJudgeForClass(classDefinition.className)?.judgeName || 'Unknown Judge'}</span>
                            </div>
                          )}
                          ...
                          {isExisting && (
                            <div className="apple-class-card-existing-badge">
                              Already in trial
                            </div>
                          )}
                        </div>
