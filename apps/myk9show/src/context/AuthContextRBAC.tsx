...
interface AuthContextType {
  ...
  switchUserRole: (email: string) => void; // For development testing
}
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  // Get user with roles using proper RBAC
  const userWithRoles = useMemo((): UserWithRoles | null => {
    if (!auth.user) return null;
    
    // If we've set a specific mock user for testing, use that
    if (currentMockUser && MOCK_USERS[currentMockUser]) {
      return MOCK_USERS[currentMockUser];
    }
    
    // In development, try to match by email to mock users (for testing)
    const mockUser = Object.values(MOCK_USERS).find(u => u.email === auth.user?.email);
    if (mockUser) {
      return mockUser;
    }
    
    // Use RBAC system for roles
    if (userRoles.length > 0) {
      ...
      return {
        ...
        scopes: [] // TODO: Implement scopes when needed
      } as UserWithRoles;
    }
    
    // Default to exhibitor role for users without RBAC data
    return {
      ...auth.user,
  ...
  }, [auth.user, currentMockUser, userRoles]);
