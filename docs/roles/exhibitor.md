...
1. **Manage dog records** — add and maintain dogs owned or handled by this exhibitor.
2. **Find and enter shows** — discover upcoming trials and enter dogs into specific classes.
3. **Pay entry fees online** — pay at entry time via Stripe. Mail-in exhibitors send a check and the secretary marks it paid manually.
4. **View entry status** — see whether each entry is accepted, waitlisted, or (for mail-ins) pending payment. No silent limbo.
5. **View the published running order and ring assignments** — once the secretary posts it, see when each entered class runs and which ring it's in.
6. **Receive announcements from the secretary** — read confirmations, waitlist notifications, and schedule-change alerts in a read-only inbox.
7. **Check in on show day** — self-check-in in either myK9Show or myK9Q, whichever the exhibitor prefers.
8. **Review results** — see results for each class entered, once the secretary posts them.
...
- Which app to use on show day — both myK9Show and myK9Q work for check-in.
