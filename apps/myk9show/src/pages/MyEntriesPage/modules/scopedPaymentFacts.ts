...
} from '@/features/payments/orderRefundReconciliation';
...
export function buildScopedPaymentFacts(order: EntryReceiptOrder): ScopedPaymentFacts {
  ...
  // Shared with the receipt DIALOG and the My Payments ledger — see
  // `orderRefundReconciliation`. Keeping a second copy here is what let the two
  // surfaces state different refunds for the same order (MYK9-428).
  ...
  return {
    ...
    statusLabel: orderRefundStatusLabel(order),
    rows,
