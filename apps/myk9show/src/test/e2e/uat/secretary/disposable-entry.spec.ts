...
test.describe('Phase 1 UAT - Secretary disposable entry management', () => {
  ...
  test('secretary can find, assign armband, accept, and check in a disposable entry', async ({
    ...
    await clickMenuItemWhenStable(
      entryStatusButton,
      page.getByRole('menuitem', { name: 'Accepted', exact: true })
    );
    ...
    await clickMenuItemWhenStable(
      checkInStatusButton,
      page.getByRole('menuitem', { name: 'Checked-in' })
    );
...
async function clickMenuItemWhenStable(trigger: Locator, menuItem: Locator) {
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    await trigger.click();
