...
- [ ] **Late-entry / day-of additions workflow** — Walk-in handling: secretary path to add a new exhibitor + dog + entry + payment at the desk on show day. Phase D plan: [`docs/plan-phase-d-late-entry-workflow.md`](docs/plan-phase-d-late-entry-workflow.md). PR #232 brought the existing-dog day-of entry flow into the Today workbench; current slice fills the new exhibitor/dog gap inside that dialog.
