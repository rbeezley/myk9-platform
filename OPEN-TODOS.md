...
- [ ] **Guided next-action surfaces (cross-cutting)** — Four pieces that compose, not a single feature:
  ...
  - [ ] **Searchable "What do I do if…" AskQ entry points** — Implemented in the Phase C AskQ entry-points branch: reuse the existing AskQ slide-out help surface for the unpredictable input flow, add secretary show-day prompts, and open AskQ with a prefilled prompt plus show context instead of creating a second help panel.
