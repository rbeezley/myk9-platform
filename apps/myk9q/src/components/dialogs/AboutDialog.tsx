...
import { X, Info, Globe, Key, Smartphone, RefreshCw, CheckCircle, Download } from 'lucide-react';
...
export const AboutDialog: React.FC<AboutDialogProps> = ({ isOpen, onClose, licenseKey }) => {
  ...
  const checkForUpdates = useCallback(async () => {
    ...
    try {
      ...
      // Check if there's already a waiting SW (update previously downloaded)
      if (registration.waiting) {
        setUpdateStatus('update-available');
        return;
      }

      // Listen for a new update
      ...
      // Give time for updatefound event to fire
      const checkTimer = setTimeout(() => {
        registration.removeEventListener('updatefound', onUpdateFound);
        if (!updateFound) {
          setUpdateStatus('up-to-date');
          const resetTimer = setTimeout(() => setUpdateStatus('idle'), 4000);
