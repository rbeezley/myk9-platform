...
--   First run on 2026-06-17 after a one-time hard wipe of all shows/trials/
--   classes/entries/dogs/clubs and non-protected people. That wipe is not part
--   of a reseed and must not be repeated now that clubs run their own UAT shows
--   on staging (MYK9-558); see DESTRUCTIVE SCOPE below.
--
--   The MYK9-109 load fixture (63 load dogs + 504 entries on the demo show,
--   three load clubs/shows with 189 dogs) is NOT part of this file any more
--   (MYK9-558 Part B). It lives in supabase/seed-load-fixture.sql and is applied
--   AFTER this file only for a load rehearsal or PDF calibration. Section 0
--   below still deletes every load-fixture id range, so running this file alone
--   removes a previously applied load fixture and leaves the lean set.
--
-- DESTRUCTIVE SCOPE
--   Every DELETE in this file is scoped to ids this seed (or the load fixture)
--   mints: the fixed 'dededede-'/'dec1a55e-' ids (seeded shows, trials,
--   classes, dogs, the two demo clubs), the myk9_109 'a1090000-' ranges and the
--   'ee110000-' email-history range. A show created by hand under its own club
--   -- a club's UAT show -- is outside that scope and survives a reseed. Never
--   widen it; seedDemoSelfCleaningRelationshipDeleteContract.test.ts pins it.
--
...
--   load armband a1090000-0000-0000-0003-*   load show    a1090000-0000-0000-0010-*
--   load trial   a1090000-0000-0000-0011-*   load class   a1090000-0000-0000-0012-*
--   load club    a1090000-0000-0000-0013-*
...
-- people).
--
-- Extracted to public.seed_demo_assert_no_paid_strays() by migration
-- 20260916213500 (MYK9-538). It was an anonymous DO $$ block here, which meant
-- the only automated coverage was the source-text contract test that reads this
-- file — and ten arm-neutering mutations run inside it left eight of them green.
-- A guard whose whole job is to refuse to destroy a money row has to be
-- provably live, so the logic now lives in a function that
-- supabase/tests/seed_demo_paid_stray_guard_test.sql calls directly. What only
-- the source-text test can still see stays there: this call's PLACEMENT ahead
-- of every parent delete, and the drift check that the function names every
-- show and dog id-space this section deletes.
SELECT public.seed_demo_assert_no_paid_strays();
...
-- expired row a walker produced is seed residue too.
...
-- trial_packet_snapshots.show_id is RESTRICT on purpose (private Storage PDFs
-- must go through the Storage API first); same preflight as the demo show below.
DO $$
BEGIN
  IF EXISTS (
    ...
    RAISE EXCEPTION 'seed-demo preflight: remove load-show trial packet objects through the Storage API before reseeding';
  END IF;
END $$;
...
-- Unscoped by status on purpose: seed-owned dogs, and the seed's own §15 waiting
-- row for ...043 is re-inserted below.
...
BEGIN
  IF EXISTS (
    SELECT 1 FROM public.trial_packet_snapshots
    WHERE show_id IN ('dededede-0000-0000-0000-000000000011','dededede-0000-0000-0000-000000000012')
  ) THEN
    RAISE EXCEPTION 'seed-demo preflight: remove sibling-show trial packet objects through the Storage API before reseeding';
  END IF;
END $$;
...
ON CONFLICT (club_id, person_id) DO UPDATE
  ...
  -- the reseed reported success. The load-club membership insert in
  -- seed-load-fixture.sql keeps DO NOTHING: those clubs ARE still deleted in the
  -- idempotency block.
...
--    The MYK9-109 load fixture (supabase/seed-load-fixture.sql, applied only for
--    a load rehearsal) pins 63 confirmed entries into each
...
--    layered on top. The lean set alone is far below 125; the value is kept at
--    200 so the show behaves the same with or without the load fixture applied.
--    Classes ...036-...039 (trials ...023/...024) carry no
...
--
--    MYK9-610: names carry NO weekday. Dates are CURRENT_DATE + N, so the
--    weekday moves with every reseed; a hardcoded 'Saturday Trial' was wrong on
--    six days out of seven. Trials are numbered in date/start-time order, and
--    name = trial_number = category = 'Trial N' is exactly the shape the show
--    creation wizard writes (buildCreateShowPayload.ts), so the fixture matches
--    what a real secretary-created show carries.
-- ---------------------------------------------------------------------------
...
VALUES
  ('dededede-0000-0000-0000-000000000021', 'dededede-0000-0000-0000-000000000010',
   'Trial 1', (CURRENT_DATE + 45), 'Trial 1', 'upcoming',
  ...
  ('dededede-0000-0000-0000-000000000022', 'dededede-0000-0000-0000-000000000010',
   'Trial 2', (CURRENT_DATE + 46), 'Trial 2', 'upcoming',
   '8:00 AM', false, 'scent_work', 1, 2, 'Trial 2', 'AKC', 'America/Chicago', 1),
  ...
  ('dededede-0000-0000-0000-000000000023', 'dededede-0000-0000-0000-000000000010',
   'Trial 3', (CURRENT_DATE + 46), 'Trial 3', 'upcoming',
   '1:00 PM', false, 'scent_work', 1, 3, 'Trial 3', 'AKC', 'America/Chicago', 1),
  ('dededede-0000-0000-0000-000000000024', 'dededede-0000-0000-0000-000000000010',
   'Trial 4', (CURRENT_DATE + 46), 'Trial 4', 'upcoming',
   '2:00 PM', false, 'scent_work', 1, 4, 'Trial 4', 'AKC', 'America/Chicago', 1);
...
-- ---------------------------------------------------------------------------
-- 17. MYK9-109 LOAD FIXTURE -- moved to supabase/seed-load-fixture.sql
--     (MYK9-558 Part B). The 63-dog / 504-entry demo-show load set, the three
--     load clubs and shows, and the MYK9-515 full-class cap on ...036 are
--     opt-in: apply that file AFTER this one for a load rehearsal or PDF
--     calibration. Section 0 above still deletes every myk9_109 id range, so
--     rerunning this file alone removes the fixture and leaves the lean set.
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  IF v_load_rows <> 0 THEN
    RAISE EXCEPTION 'seed-demo expected no MYK9-109 load-fixture rows after a reseed, found %', v_load_rows;
  END IF;
END $$;
