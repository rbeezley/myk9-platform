...
- `/admin/health` shows a `class_results_push` check that reads the source of truth, not just the rows. It fails, naming the classes, when a class has been due for 20+ minutes and is not sent (no row at all, or still `pending`), when a row is `failed`, or when `class-results-push-retry` is not scheduled, not active, last failed, or has not succeeded for 15 minutes.
