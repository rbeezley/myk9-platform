...
- [x] **Step 3:** Surfaced in `EntryListCard` (card view) and table view (`EntryRowActionMenu` + `EntriesTableView`). The table path reuses the same `RequestPaymentDialog` / `RefundEntryDialog` surfaces as the card view; no new payment surface.
...
- [x] **Step 1 — capacity-config audit:** Confirm `WaitListSettingsCard`/`useJudgeDayCapacity` read/write the 114 columns correctly and that fixed + percentage reservation compute as expected. Completed locally: settings card already reads/writes all 114 settings; fixed/percentage reservation tests exist. Review follow-up removed the speculative `classes.overflow_policy` column because it had no live reader and duplicated the plan's own "build on 114" correction.
- [ ] **[ADDED] Step 1a — deadline strategy & auto-release in the gate:** Two 114 config modes the enforcement gate must honor: `mail_in_strategy='deadline'` reserves 0 but gives mail-in priority before `mail_in_deadline` (define what "priority" means in the gate — e.g. online blocked from the last N spots until the date, or purely informational); and `mail_in_auto_release`/`mail_in_release_date` must drop the effective reserve to 0 once the release date passes so online can use the held spots. Auto-release is completed locally in the capacity readers; deadline-priority semantics still need the server-side gate.
...
- [x] **Step 3 — wire split checkout:** Replaced `CartPage`'s flat checkout with `checkoutWithWaitlist`: open → Stripe; full+waitlist → `add_to_waitlist`; full+deny → blocked pre-checkout. Checkout success now shows a correlated, single-use split summary so stale same-show/same-count data cannot render. Confirmed `add_to_waitlist` ownership/RLS via existing source coverage.
- [x] **Step 4 — server-side atomic enforcement (CRITICAL):** Added a service-role-only `create_online_paid_entry(...)` RPC that uses `get_judge_day_capacity` semantics under judge-day advisory locks. Online overflow follows existing `classes.allow_waitlist`: waitlist rows are created for waitlist-enabled classes; deny/no-service lines are auto-refunded with prorated platform-fee share. `stripe_orders.entry_ids` now contains only paid entries, with explicit overflow/refund metadata. Residual verification gap: source/pure tests cover the behavior, but the RPC has not been DB-executed locally or pushed.
...
- [~] **Step 5 — tests:** Source/pure/client tests now cover split routing, confirmation display, deny-vs-waitlist overflow routing, prorated overflow refunds, paid-only `stripe_orders.entry_ids`, service-role-only RPC grants, and the advisory-lock + `get_judge_day_capacity` contract. Remaining DB-execution gap: fixed/percentage reserve, deadline strategy, concurrent-overbook race, lowering-limit grandfather, and config write-authz still need database-backed verification before final rollout.
...
- [x] **[ADDED] Step 5 — mail-in promotions use the OFFLINE path (fairness, owner-decided 2026-06-20):** The pay-to-claim online-link + 48h clock works AGAINST the elderly mail-in population the reservation exists to protect. So a waitlister who joined by mail-in must NOT be auto-expired on the online clock. Record how each waitlister joined (`waitlist_entries.joined_via` = `online | mail_in`, set through `add_to_waitlist` via optional `p_joined_via`, defaulting online for existing cart/exhibitor flows). On promotion of a `mail_in` waitlister: the secretary records check/phone payment (reuse the offline "Secretary Payment (Already Received)" path) under a **manual/longer hold**, and the cron does NOT drop it on `waitlist_payment_deadline_hours`. Online waitlisters keep the link + deadline. The secretary may still send a link to a mail-in person who wants to pay online. Completed locally: adds `waitlist_entries.joined_via`, marks current exhibitor/cart waitlist joins `online`, exposes the mail-in channel through the RPC, and skips mail-in auto-expiry/payment-link generation in cron and secretary offer orchestration.
...
- [x] **Step 1:** `EntryRowActionMenu` imports `isStripeRefundable` / `isPaymentRequestable` and gates Refund + Request-payment actions, while `EntriesTableView` owns the shared dialogs.
- [x] **Step 2:** `EntryRowActionMenu` tests assert Refund renders for stripe-refundable entries and hides when not eligible or when no handler is supplied; request-payment render-gate coverage is present in the split/table wiring.
