...
- **Replace raw `<button>` with `<Button>` variants in scoresheet** — Add new CVA variants to the shared Button component, then use them in the live scoresheet. **Problem:** `AKCScentWorkLiveScoresheet.tsx` uses raw `<button>` elements with 8-12 inline Tailwind classes each, bypassing the `@myk9/ui` design system. When the design system evolves (accessibility, theme changes), these buttons will be left behind. **Files:** `packages/scoring-ui/src/components/scoresheets/AKC/AKCScentWorkLiveScoresheet.tsx` (timer buttons, result chips, submit/cancel buttons), `packages/ui/src/components/Button/Button.tsx` (shared Button with CVA variants). **Solution:** Add variants like `variant="gradient"`, `size="scoresheet-lg"` to the shared Button, then swap raw buttons in the scoresheet.

- **Recompose scoresheet around existing `ResultChoiceChips` and `TimerDisplay`** — Extract and reuse existing scoring components instead of reimplementing them inline. **Problem:** The scoresheet reimplements choice chips (result selection, NQ reasons, fault counter) and timer (gradient card, start/stop/reset, progress ring) that already exist as standalone components with haptic feedback, sport-specific validation, and proper styling. **Files:** `packages/scoring-ui/src/components/scoresheets/AKC/AKCScentWorkLiveScoresheet.tsx` (inline reimplementations), `apps/myk9show/src/components/scoring/ResultChoiceChips.tsx` (choice chips with haptic feedback + sport-specific NQ/EX reasons + fault counter), `apps/myk9q/src/components/scoring/ResultChoiceChips.tsx` (duplicate copy), `packages/ui/src/components/TimerDisplay/TimerDisplay.tsx` (complete timer component). **Solution:** Extract `ResultChoiceChips` to `@myk9/scoring-ui` package (single source of truth), then compose both `ResultChoiceChips` and `TimerDisplay` into `AKCScentWorkLiveScoresheet.tsx`.

- **Add clear button to shared `SearchBar` component** — Enhance SearchBar, then use it in ClassResultsTable. **Problem:** `ClassResultsTable/index.tsx` has an inline search input with a clear (X) button because the shared `SearchBar` component lacks clear functionality. This means the search pattern isn't reusable. **Files:** `apps/myk9show/src/components/common/SearchBar.tsx` (shared component — has Search icon but no clear button), `apps/myk9show/src/components/classes/ClassResultsTable/index.tsx:476-497` (inline search input with clear button). **Solution:** Add optional `onClear` prop (or auto-clear when value is controlled) to `SearchBar`, then replace the inline input in ClassResultsTable with `<SearchBar>`.
