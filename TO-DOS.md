...
- [x] **Dog notification pipeline** — Done: `useNotificationMonitor` hook mounted in App.tsx subscribes to Supabase realtime changes on entries/classes. Four alert types: `your_turn` (entry → in-ring, alerts next N dogs by armband order with cross-class conflict detection), `class_starting` (class → In Progress), `check_in_reminder` (per unchecked-in dog when class starts), `results_posted` (when scoring finalized). Dual delivery: in-app via `useNotificationDelivery` + push via `send-push-notification` Edge Function (JWT self-send auth). Service worker navigates to `/classes/:classId` on notification tap. Catch-up pass fires missed alerts when app opens mid-show. Run order by armband ascending (configurable sort is future enhancement). Conflict rendering in NotificationCenter (amber warning lines). 28 tests across 4 files. Hardened: NaN armband handling, subscription churn prevention, dedup. Spec: `docs/superpowers/specs/2026-03-30-dog-notification-pipeline-design.md`. Plan: `docs/superpowers/plans/2026-03-30-dog-notification-pipeline.md`. PR #34.
...
- [x] **Secretary show-level statistics** — Done: Added public "Show Stats" sub-tab within the Results tab on ShowDetailsPage. `useShowStats(showId)` hook fetches all entries from `view_entry_with_results` (no dog/user filter), with trial metadata from `classes` table (two-query merge — PostgREST can't join through views). Reuses all four shared analytics components (StatsSummaryCards, ResultDistributionChart, DogBreakdownCards, FastestTimesTable). Sub-tab appears when scored entries exist. 3 tests. Design: `docs/superpowers/specs/2026-03-30-show-judge-stats-design.md`.
- [x] **Judge class-level statistics** — Done: Added public "Judge Stats" sub-tab within the Results tab on ShowDetailsPage. Judge dropdown selector (auto-selects first judge) powered by `useShowJudges(showId)`. `useJudgeShowStats(judgeId, showId)` fetches entries for the selected judge's assigned classes via `judge_assignments` join. Includes all shared analytics components plus new `ClassBreakdownTable` showing per-class Q rate with progress bar, trial date/number, best/avg times. New `computeClassBreakdown()` utility in analytics-utils. Sub-tab appears when judge assignments exist. 11 tests across 4 new test files. Plan: `docs/superpowers/plans/2026-03-30-show-judge-stats.md`.
- [x] **Add analytics section usage tracking** — Done: `analytics_events` Supabase table (migration 096) with RLS (insert=authenticated, select=site_admin). `useTrackSectionView` hook uses IntersectionObserver (50% threshold) to fire one insert per section per page session (module-level Set dedup, reset on navigation). 4 tracking refs wired to AnalyticsPage: lifetime page container, QualificationTrendChart, DogBreakdownCards, FastestTimesTable. 8 unit tests. Spec: `docs/superpowers/specs/2026-03-30-analytics-section-tracking-design.md`. Plan: `docs/superpowers/plans/2026-03-30-analytics-section-tracking.md`.
...
- [x] **Add standard toolbar to Pending/Completed/All entry sub-tabs** — Done: SearchBar (with clear button, size="sm") already sits between SubTabs and EntryCardGrid/DataTable in ClassResultsTable. Filters by dog name, handler name, or armband across whichever tab is active. Added as part of the SearchBar clear button work (commit d3c22669).

---

## Simplified Scoring Data Flow — 2026-03-27

- [x] **Collapse scoring state to single source of truth** — Done: Rewrote `useClassResults` (388 lines → 247 lines) as simple edit-buffer-over-raw-DB-rows pattern. Raw DB rows are the source of truth. User edits stored in `Map<entryId, ScoringEdit>`. Display merges raw + edits. Submit writes directly to `replicatedEntriesTable.updateEntry()`. `justScoredIds` set handles immediate Pending→Completed tab update for offline. Per-row clear button. Placements calculated on submit. Validation on submit only (no live Invalid badges). Eliminated: `BulkEntryData`, `ResultsSummary`, dual-source init useEffect, version key tracking, init guards. Specs: `docs/superpowers/specs/2026-03-27-simple-scoring-table-design.md`. Plan: `docs/superpowers/plans/2026-03-27-simple-scoring-table.md`.

---

## Polish Live Scoresheet UI — 2026-03-26 21:48

- [x] **Polish live scoresheet to match myK9Q layout** — Done: Aligned timer (gradient card, pill buttons, repositioned progress ring, reset button), dog info (gradient armband badge), result chips (pill-shaped, myK9Q color scheme), fault counter (circular +/- buttons), NQ reasons (choice chips replacing dropdown), submit buttons (taller rounded). Code review fixes: extracted shared class constants, replaced hex colors with Tailwind semantic classes, added result label lookup map. Commit `cf7e3e8b`.

---

## Scoresheet & Search Component Reuse — 2026-03-29 08:19

- [x] **Replace raw `<button>` with `<Button>` variants in scoresheet** — Done: Added `variant="danger"` (red, for stop/abort) and `size="xl"` (h-14, for action bars) to shared buttonVariants. Replaced all 5 raw `<button>` elements in AKCScentWorkLiveScoresheet with `<Button>`: reset (ghost+icon), timer start/stop/resume (danger/default+className), area-time clear (ghost+icon), cancel (secondary+xl), save (default+xl). Removed `timerButtonBase` and `actionButtonBase` constants. Consolidated Start/Resume into single branch. 278 tests passing.
