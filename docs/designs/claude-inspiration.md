> **Historical inspiration document.** This describes Claude.ai's marketing-site design system, which inspired myK9's warm palette. It is NOT the project's design spec — the canonical, code-derived spec lives at the repo root: [`DESIGN.md`](../../DESIGN.md) (+ `DESIGN.json` sidecar). Kept for palette provenance only.
