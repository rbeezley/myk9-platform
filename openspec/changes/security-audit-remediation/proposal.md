...
- **SA-009** — Client subscribes to `user_roles` changes for the current `auth_user_id` (or piggybacks the existing 60s `userProfile` poll) to refresh permissions and sign out on suspension without requiring a reload.
...
- Client: `apps/myk9show/src/pages/admin/OperatorAlertsSection.tsx`, `apps/myk9show/src/context/AuthContext.tsx` (+ RBAC hook wiring for the realtime/poll refresh).
