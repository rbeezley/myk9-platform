...
- **Severity:** high
- **Role:** secretary | admin
- **Surface:** exact Phase 2 active Nightly Playwright command from `docs/qa/e2e-suite-map.md`; `registration/singleDogSingleClass.spec.ts`, `route-health-by-role.spec.ts` admin group, and `uat/secretary/disposable-entry.spec.ts`.
- **Suite category:** nightly
- **Pattern:** test-flake
- **Detected by:** Playwright
- **Evidence:** 2026-06-20 isolated Nightly on `origin/main` `9805fb9af` passed Phase 1 Vitest (`18/18`) but Phase 2 failed after exceeding the global budget: `42 passed, 5 failed, 2 skipped, 1 did not run (52.3m, --retries=0)`. Two low-risk stale assertion fixes were proven in this run: `basic/registrationSmoke.spec.ts` now waits for root navigation commit instead of `domcontentloaded`, and `uat/secretary/critical-path.spec.ts` now asserts the current Entry Management `Entries`/`Waitlist` + work-mode UI rather than the retired `All (n)` status tab. Remaining durable failures after focused repair: (1) `registration/singleDogSingleClass.spec.ts` reached `/secretary/register/:showId` but the page stayed on duplicate `Loading...` paragraphs and never rendered `Register for Show`; evidence path `apps/myk9show/test-results/registration-singleDogSing-28b55--dog-and-one-selected-class-chromium/error-context.md`. (2) `route-health-by-role.spec.ts` admin group timed out inside `sweepRoutes`; public, exhibitor, secretary, and judge route groups passed, club-admin skipped, admin failed; evidence path `apps/myk9show/test-results/route-health-by-role-Route-b0cad-n-admin-routes-render-clean-chromium/error-context.md`. (3) `uat/secretary/disposable-entry.spec.ts` is order-dependent: it passed alone after moving the workflow to Cards view (`1 passed, 9.1s`) but failed when run after the patched registration-smoke and critical-path subset because Entry Management rendered its shell and tab header but left the Entries tab body on a spinner with no `Search entries` textbox; evidence path `apps/myk9show/test-results/uat-secretary-disposable-e-b1800-check-in-a-disposable-entry-chromium/error-context.md`.
- **User impact:** Nightly cannot currently prove the secretary registration/disposable-entry baseline or the admin route-health baseline. The failures include a possible page-load/data-load stall, so the suite is not yet distinguishing a real route regression from harness order-dependence.
- **Intent check:** Harms secretary "That was easy" and site-admin "platform is healthy" confidence because core proof routes can stall without a clear user-facing state.
- **Fix owner:** active Playwright registration/UAT specs and the Entry Management/admin route loading paths they exercise.
- **Proof required:** Focus and fix the remaining three surfaces without suppressing browser-health failures: run `singleDogSingleClass.spec.ts` alone and in sequence after registration smoke; run `route-health-by-role.spec.ts --grep "Route health: admin"`; run `uat/secretary/disposable-entry.spec.ts` alone and after the patched critical-path subset. Then rerun the exact Phase 2 active Nightly command with `--retries=0`; proof target is all locally available active specs passing without any file exceeding the Nightly wall-clock discipline.
- **Notes:** Do not close from the standalone disposable-entry pass alone; the same spec failed in a three-test sequence, which is the actionable signal. Do not re-run full Phase 2 blindly until the focused order-dependence and admin timeout are bounded.
