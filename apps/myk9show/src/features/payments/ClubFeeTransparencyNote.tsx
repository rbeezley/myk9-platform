...
import { formatCartCurrency, formatPlatformFeeLabel } from '@/store/cartStore.helpers';
    ...
    <section className={className}>
      <div className="space-y-2 rounded-lg border bg-muted/30 p-4">
        <p className="text-sm font-medium">
          Your club receives 100% of entry fees. The service fee is never deducted from your payout.
        </p>
        <p className="text-sm text-muted-foreground">
          ...
          covers running the platform. On {formatCartCurrency(EXAMPLE_SUBTOTAL_CENTS)} of entries
          the fee is {formatCartCurrency(split.feeCents)}, of which roughly{' '}
          {formatCartCurrency(split.cardProcessingCents)} is card processing — an approximate
          figure, because Stripe&rsquo;s exact fee depends on the card and is only known after the
          payment settles.
        </p>
