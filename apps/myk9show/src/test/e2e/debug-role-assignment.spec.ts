...
test('Debug role assignment for testsecretary@example.com', async ({ page }) => {
  ...
  if (currentUrl.includes('/sign-in')) {
    ...
    const errorMessages = await page
      .locator('[class*="error"], [data-testid*="error"], .text-red-500, .text-destructive')
      .count();
    if (errorMessages > 0) {
  ...
  const secretaryLinks = await page
    .locator('a[href*="secretary"], button:has-text("Secretary")')
    .count();
  ...
  const userProfile = await page
    .locator('[data-testid*="user"], [data-testid*="profile"], button:has-text("@")')
    .count();
