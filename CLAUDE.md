...
- **What ships today:** nothing deploys on merge. `apps/myk9show/vercel.json` sets `git.deploymentEnabled: false`, so Vercel builds neither `main` nor PR branches for myK9Show. Production (myk9show.com and myk9-platform-myk9show.vercel.app) moves only when someone runs the **Deploy myK9Show** workflow (`gh workflow run deploy-myk9show.yml`), which builds the newest `main` commit with a green CI run in Actions and uploads it prebuilt. Running it is a shared-system write: only when the user asks. `main` routinely runs ahead of production; the run summary names the deployed commit.
...
3. **Before merging**: scrutiny scales with risk — `pnpm qa:review-tier --base origin/main` reports the review floor and optional-review eligibility; authoritative rules are in `scripts/qa/review-tier.ts` and `docs/PLAYBOOK.md` § 4. `scripts/qa/post-review-gate.sh` is the only writer of a `Review gate:` line. **Optional review** — docs-only, bounded small app-source changes (at most 3 files/100 changed lines; tests may only be added, never edited), and dependency-only changes labeled `dependencies` whose manifests change only dependency fields can pass without review evidence; CI remains required and voluntary reviews are welcome. **`independent`** — guardrails/security-sensitive paths require the OTHER harness (Codex for Claude-authored, Claude for Codex-authored; same-harness never suffices), via `pnpm qa:codex-review`; the verdict is pinned to the exact head SHA, and a later push requires a fresh review. **`adversarial`** — 2+ same-harness lenses told to find bugs, not approve; verdict `<N> lenses, all findings addressed` (`<N>` >= 2). (#1536: two clean subagent rounds missed a P1 Codex caught) — weaker than `independent`. **`none`** — docs-only paths retain the explicit `low-risk paths, CI green` attestation option. **`owner` override** — harness unavailable: an OWNER/MEMBER may defer scrutiny with verdict `override, floor was independent`/`override, floor was adversarial` and a deferred issue; see [`docs/PLAYBOOK.md`](docs/PLAYBOOK.md) § 4. **Convergence — when to stop patching.** Stop and restructure at the SECOND finding on the same path across rounds, or when findings describe code the PREVIOUS fix introduced. Both mean the design is wrong: ask whether to delete the feature before guarding it (LESSONS `discriminator-branches`). Read severity, not count (#2180, #2210). Orchestrated changes ship one PR per batch and review after the structural batch, not only at the end ([`docs/lessons/README.md#orchestrated-change-rounds`](docs/lessons/README.md#orchestrated-change-rounds)). On stopping, post the restructure proposal instead of another patch.
...
5. **After merge**: a merge is not a deploy. Migrations need `supabase db push`, edge functions need `supabase functions deploy` (see the `deploy` skill), and the frontend changes only on the next Deploy myK9Show run. An issue whose acceptance criteria need production evidence stays In Progress until a run's summary names a commit at or after the merge; any other issue closes on merge.
...
One to three lines per lesson: the rule, the mechanism, and a pointer to the incident in [`docs/lessons/README.md`](docs/lessons/README.md). A lesson whose trap is now caught by a program is a one-line pointer at that program. `apps/myk9show/src/test/ci/instructionFileBudget.test.ts` enforces the shape and the word ceilings; the budget and retirement rules are [`docs/lessons/README.md`](docs/lessons/README.md) § Budget.
...
- **`git branch -D`/`-d`, `git update-ref -d` and `git checkout -- <path>` are denied here**; in an unattended run that is a silent stall. Discard with `git restore`, merge with `gh pr merge --squash` without `--delete-branch`, leave local branches for `branch-janitor`. (docs/lessons/README.md#git-branch-delete-denied)
...
- **`SlideOverPanel`'s `size` prop is honored except between 640 and 767px**, where `sm:max-w-none` overrides it and every size renders full width. Below 640 the unprefixed `max-w-*` governs, from 768 the `md:` one. Per-caller widths go through `className`. (docs/lessons/README.md#slideover-size-band)
...
- **A mutation test you don't confirm actually mutated is a green light you trust too much** — as is a guard test whose setup never reaches the condition. Match headings exactly, never `index()`; print WHERE the change landed; break the implementation and watch THAT test go red. (docs/lessons/README.md#mutation-actually-mutated)
...
- **A Vercel check whose `targetUrl` ends `?upgradeToPro=build-rate-limit` is an account quota, not a verdict**; GitHub leaves the PR `MERGEABLE`/`UNSTABLE`. Merge on the Actions jobs and say which check you ignored. Production follows the Deploy myK9Show workflow, not `main`: before calling a fix live, read a run summary naming a commit at or after yours. (docs/lessons/README.md#vercel-rate-limit)
...
- **A forked audit session that meets housekeeping noise should file it, not fix it** — `pnpm qa:inflight` (Gates step 1) only sees paths, not the urge to fix what you tripped over. (docs/lessons/README.md#inflight-check)
...
- **Once a review reports no P0/P1 and a core that holds, route every finding against the base commit**: fix the regressions, FILE what the change only exposed. Shared-component work splits out. (docs/lessons/README.md#review-scope-boundary)
- **A rule derived from free-text database columns must be measured against the real values before shipping.** A class-label rule tested on invented fixtures rewrote 14 of 24 live labels, 13 harmfully (`Advanced Load 2 Class 1` on a public premium), while passing red-green, a shuffled suite, typecheck and lint. (docs/lessons/README.md#label-rule-vs-real-columns)
- **A docs-only `git push origin HEAD:main` pushes whatever HEAD is, not the docs commit alone.** A sub-agent's feature branch was checked out inside the orchestrator's worktree (shared index); a ledger commit swept the agent's staged deletion, and the push carried the agent's whole feature commit to `main` with no review gate (PR auto-closed as merged). Before any such push, one command must assert the session branch, a clean pre-edit tree, staged files limited to the intended docs, and exactly one commit ahead of `origin/main` — abort otherwise; after dispatching an implementer, confirm its own worktree with `git worktree list`. (docs/lessons/README.md#docs-push-head-main)
