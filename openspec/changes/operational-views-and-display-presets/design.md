...
Entry Management continues to use `normalizeEntryManagementSearchParams` and `useSearchParams`. New preset helpers build parameters through the existing normalizer rather than callers concatenating strings. Class Management gets a small adapter for its current search/status/element state; if it later becomes URL-backed, it uses the same contract.
...
- Which four or five curated presets are most useful to secretaries in the first browser walk?
- Should Class Management URL state be introduced in this change or remain local until its broader page consolidation is ready?
- Should personal saved views be called “Saved views” or “My presets” to make device-local scope clear?
