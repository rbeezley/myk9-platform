...
| 2. Configure club, registry, trials, classes, rings, judges, and run order. | Show Setup inside the single-show workbench; legacy run-order route redirects into setup | `/shows/:showId/setup`; `/secretary/run-order` redirects to show setup | Static route evidence; `SHOW_MANAGEMENT_SECTIONS` exposes `setup` as the canonical show section | Phase 3 setup pass should verify the canonical `/shows/:showId/setup` surface covers configuration end to end. |
