...
- [x] **Wire the toast pipeline for scoring failures** — Shipped via the H2 fix on `fix/scoring-sync-failed-dispatch`. Picked option 2: `useOptimisticScoring`'s `onError` now dispatches `window.dispatchEvent(new CustomEvent('replication:sync-failed', { detail: { message, failedTables: ['entries'], errors } }))` when `submitScore()` rejects while online. Existing `SyncFailureToast.tsx:142` and `syncStatusStore.ts:170` listeners both consume the event. Regression tests in `useOptimisticScoring.test.ts` assert the dispatch on online failure AND its absence on offline failure.
...
- [ ] **Clear the dirty bit after a successful `submitScore`** — Follow-up to the H2 fix above. The online happy-path leaves `_syncStatus:'pending'` set in IndexedDB because nothing calls back into the replication layer after `submitScore()` succeeds. The next pull then takes `mergeDirtyRow` instead of normal `resolveConflict` — functional but wasteful. Cannot be fixed with `set(id, data, false)` because the dirty-row guard at `packages/replication/src/core/ReplicatedTable.ts:253` would skip the write. Needs a new `markAsSynced(id)` helper on the base `ReplicatedTable` that explicitly bypasses the guard. Once added, call it from the `onSuccess` branch in `useOptimisticScoring.ts`. Bundle with the option-1 MutationManager wiring if/when that's revisited.
