...
| --------------------------------------------------------------------------------- | ---------- | ------------------------------------------------------------------------------------ | ---------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Get into the ringside scoring experience with a show passcode, no account needed. | Required   | `/at-show` SmartSignInPage → `validate-passcode` edge fn → anon session with claim   | Evidence partial | Passcode generation/display/cold sign-in verified from the secretary side (S4.5, 2026-07-09) with 45 tests. Needs a judge-device cold-session walk on the judge code specifically.                |
...
| Stay scoped to the correct show and survive session expiry gracefully.            | Required   | `RingsideShowBoundary`, `RingsideSessionHeartbeat`, `AtShowAccessGate`               | Evidence partial | Components and tests exist. Needs live expiry/regenerated-passcode behavior walk (secretary regenerates codes mid-show).                                                                          |
...
| Know the search-area parameters for the class (hides, distractions, time). | Required   | Class detail data on ringside pages                                | Evidence partial | Clone-fidelity work (2026-07-09) showed hides/distractions/items are not persisted to DB anywhere — verify what ringside actually displays and whether judges need these fields live.             |
...
| Responsibility                                                             | Fall scope | Current myK9 coverage                                        | Status                 | Evidence / verification needed                                                                                                            |
| ----------------------------------------------------------------------------- | ---------- | -------------------------------------------------------------- | ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| Correct a scoring mistake at the ring before the class closes.             | Required   | Re-select scored entry, re-save or clear (paper + ringside)   | Evidence partial       | Clear/re-score verified from Results Control (S6.5); placements auto-recalculate. Needs the same walk from the ringside judge device.       |
...
| Escalate anything the judge cannot fix to the secretary without app-hopping. | Required   | Same-app: `/at-show` and secretary surfaces in one myK9Show   | Evidence partial       | Unified app shipped; needs a "judge flags problem → secretary resolves" rehearsal path check (announcements/messages reach ringside).      |
...
| View own assignments, get schedule-change notifications, track history. | Post-fall  | Not fall scope                                                   | Deferred         | Explicit role-doc deferral.                                                                                                                 |
