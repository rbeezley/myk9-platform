...
- [ ] 4.11 [EXPANDED] Add tests for deterministic masking, schema/Storage-drift rejection, content-type/size/malware and residual-sensitive-data scanning, referential integrity, designated staging Auth-account replacement, prohibited-field removal, versioned-prefix cleanup, idempotent checkpoints/reruns, and zero staging-to-production connectivity.
- [ ] 4.12 [EXPANDED] Implement the on-demand refresh procedure using a transactionally consistent source snapshot, bounded batches, non-sensitive durable checkpoints, idempotent retries, and an isolated scratch destination. Preflight rows/objects/bytes, scratch space, staging headroom, and duration limits; block staging import until database and versioned Storage candidates pass every check; preserve the prior staging dataset on failure; and keep external side effects disabled or redirected. Obtain separate immediate approval before the first real production snapshot refresh.
