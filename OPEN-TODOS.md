...
- [ ] **Late-entry / day-of additions workflow** — Walk-in handling: secretary path to add a new exhibitor + dog + entry + payment at the desk on show day. Phase D plan: [`docs/plan-phase-d-late-entry-workflow.md`](docs/plan-phase-d-late-entry-workflow.md). PR #232 brought the existing-dog day-of entry flow into the Today workbench; PR #233 filled the new exhibitor/dog gap; PR #234 added payment-method capture and Wrap-up reconciliation totals; current slice adds the maintained show-day regression walk.
...
- [ ] **End-of-day reconciliation** — Totals view: entries, no-shows, refunds, fees collected. Natural landing screen for the Wrap-up tab. Phase D PR 3 adds the first late-entry fee reconciliation strip; no-show/refund totals remain.
