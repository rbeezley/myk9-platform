...
import { storageAdapterFactory, getStorageAdapterInfo } from '@/services/database/storage-adapter-factory';
...
export const PerformanceModeToggle: React.FC = () => {
  ...
  const handleToggleChange = (key: keyof Omit<PerformanceSettings, 'storageMode'>, value: boolean) => {
  ...
  const applySettings = async () => {
    ...
    try {
      ...
      storageAdapterFactory.setConfig({
        type: settings.storageMode === 'development' ? 'development' : 
              settings.storageMode === 'production' ? 'basic' : 'optimized',
      ...
      <div className="space-y-6">
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground text-lg">
            Manage storage adapter modes and performance settings for optimal application performance
          </p>
        </div>
              ...
              <div className="grid gap-6 md:grid-cols-3">
                <div className="group relative overflow-hidden p-6 bg-muted/50 rounded-2xl border border-border transition-all duration-500 hover:shadow-lg hover:-translate-y-1">
                  ...
                  <div className="relative">
                    <h4 className="font-medium text-sm text-muted-foreground uppercase tracking-wide mb-2">Storage Adapter</h4>
                    <p className="text-2xl font-bold group-hover:text-primary transition-colors duration-300">{stats.storage.type as string}</p>
                ...
                <div className="group relative overflow-hidden p-6 bg-muted/50 rounded-2xl border border-border transition-all duration-500 hover:shadow-lg hover:-translate-y-1">
                  ...
                  <div className="relative">
                    <h4 className="font-medium text-sm text-muted-foreground uppercase tracking-wide mb-2">Connections</h4>
                    <p className="text-2xl font-bold group-hover:text-primary transition-colors duration-300">{stats.connection.totalConnections}</p>
                    <p className="text-sm text-muted-foreground mt-1">
                ...
                <div className="group relative overflow-hidden p-6 bg-muted/50 rounded-2xl border border-border transition-all duration-500 hover:shadow-lg hover:-translate-y-1">
                  ...
                  <div className="relative">
                    <h4 className="font-medium text-sm text-muted-foreground uppercase tracking-wide mb-2">Compression</h4>
            ...
            <div className="grid gap-4">
              {['development', 'production', 'optimized'].map((mode) => (
                <div
                  ...
                  onClick={() => handleModeChange(mode as 'development' | 'production' | 'optimized')}
                >
                  ...
                  <div className="relative flex items-center gap-4">
                    <div className="flex-shrink-0">
                      <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all duration-300 group-hover:scale-110 ${
                        mode === 'development' ? 'bg-gradient-to-br from-blue-500/20 to-blue-400/10 shadow-lg' :
                        mode === 'production' ? 'bg-gradient-to-br from-green-500/20 to-green-400/10 shadow-lg' :
                        'bg-gradient-to-br from-purple-500/20 to-purple-400/10 shadow-lg'
                      }`}>
                        {mode === 'development' && <Zap className="h-6 w-6 text-blue-500" />}
                        {mode === 'production' && <CheckCircle2 className="h-6 w-6 text-green-500" />}
            ...
            <div className="grid gap-6">
              ...
              <div className="flex items-center justify-between p-4 rounded-xl bg-muted/20 border border-border">
                ...
                <Switch
                  ...
                  onCheckedChange={(checked) => handleToggleChange('enableConnectionPooling', checked)}
                />
              </div>
            </div>
          </div>
          ...
          {!import.meta.env.DEV && settings.storageMode === 'development' && (
            <Alert className="border-amber-200 bg-amber-50 dark:border-amber-800 dark:bg-amber-950/20">
              ...
              <AlertDescription className="text-amber-800 dark:text-amber-200">
                <strong>Warning:</strong> Development mode is active in production. 
                This may impact performance and data persistence. Use for testing only.
              </AlertDescription>
            </Alert>
          )}
