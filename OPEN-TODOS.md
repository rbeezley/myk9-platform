...
- [ ] **Unify the two `isSyncing` flags + decide if `syncManager.processOfflineQueue` is dead code** — Partial progress: the wedge half shipped via [PR #358](https://github.com/rbeezley/myk9-platform/pull/358) (2026-05-25), removing `addToQueue`'s `setTimeout(() => get().startSync(), 100)` that latched `isSyncing=true` after the processor finished draining, with no caller of `syncComplete` in the steady-state online path. Two regression tests in `apps/myk9q/src/stores/__tests__/offlineQueueStore.test.ts` pin the contract. Remaining architectural cleanup: (a) `isSyncing` lives in BOTH `syncStatusStore.ts` and `offlineQueueStore.ts` — collapse one into a selector over the other; (b) `syncManager.processOfflineQueue` is a parallel processor only invoked on offline→online transitions while `useOfflineQueueProcessor` is the steady-state worker — decide if syncManager's queue path is still load-bearing or dead code, and either delete it or document why both exist. Full context in TO-DOS.md § "Uncoordinated sync loops — 2026-05-25".
