...
/**
 * MYK9-535: the withdraw RPC rides the SAME seam as the ringside RPC, so the
 * argument object `executeMutation` builds is the real contract between
 * `ReplicatedEntriesTable.withdrawOwnEntry` and the Postgres function
 * `withdraw_own_entry(p_entry_id uuid, p_fields jsonb, p_expected_version integer)`.
 * A unit test on the queued descriptor alone cannot see a mis-named argument.
 */
describe('executeMutation — withdraw_own_entry argument mapping', () => {
  function withdrawMutation(overrides: Partial<PendingMutation> = {}): PendingMutation {
    return makeMutation({
      operation: 'UPDATE',
      data: { id: 'entry-1', entry_status: 'withdrawn' },
      rpc: { name: 'withdraw_own_entry', fields: { entry_status: 'withdrawn' } },
      ...overrides,
    });
  }

  it('calls the RPC with p_entry_id / p_fields / p_expected_version', async () => {
    const rpc = vi.fn(() => Promise.resolve({ data: 7, error: null }));
    const client = { rpc } as unknown as SupabaseClient;

    await executeMutation(client, makeLogger(), withdrawMutation({ serverVersion: 6 }));

    expect(rpc).toHaveBeenCalledWith('withdraw_own_entry', {
      p_entry_id: 'entry-1',
  ...
  it('sends a null expected version when the row has never been synced', async () => {
    const rpc = vi.fn(() => Promise.resolve({ data: 1, error: null }));
    const client = { rpc } as unknown as SupabaseClient;

    await executeMutation(client, makeLogger(), withdrawMutation());
  ...
  it('never falls through to a direct table UPDATE the RLS policy would deny', async () => {
