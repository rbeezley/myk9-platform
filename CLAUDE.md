...
1. **Before starting** work that will produce a PR: `pnpm qa:inflight` (exit 1 = an open PR, another worktree, or a recent unmerged branch already touches these paths; stale local-branch inventory is counted and shown in the summary, with `--verbose` for details; exit 2 = could not decide, not a pass), then the two checks a shell cannot make: Linear issues **In Progress** naming the paths, and (Claude Code) `list_sessions`. Set the Linear issue to In Progress and keep it there through review.
...
- **"Check for in-flight work first" is worth nothing unless a program runs it**: `pnpm qa:inflight` fails when an open PR, another worktree (including uncommitted files) or an unmerged branch overlaps the paths on the current branch; Linear In Progress and `list_sessions` stay manual. A forked audit session that meets housekeeping noise should file it, not fix it. (docs/lessons/README.md#inflight-check)
