...
const ShowMainCard: React.FC<ShowMainCardProps> = ({
  showData,
  handleEditShow,
  setShowDeleteDialog,
}) => {
    ...
    <div className="relative">
      ...
      <div className="flex items-center justify-between mb-6 w-full">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <Badge
            variant="outline"
            className="ml-2 capitalize px-3 py-1 text-sm font-medium dark:bg-zinc-800 dark:text-gray-100 dark:border-zinc-600"
          >
            {showData.status}
          </Badge>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        ...
        <div>
          ...
          <div className="text-gray-900 dark:text-gray-100 font-medium">
            {showData.clubName || 'National Kennel Club'}
          </div>
          <div className="text-xs text-gray-500 dark:text-gray-400 mt-2">
            {showData.clubAddress || '123 Kennel Drive, Dogtown, CA 94123'}
          </div>
          <a
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        ...
        <div className="p-4">
          ...
          <div className="font-semibold text-gray-900 dark:text-gray-100">
            {resolvePersonName(showData.chiefSteward)}
          </div>
          <a
                ...
                <div key={judge.judgeId} className="p-4 border rounded-lg dark:border-zinc-600">
                  ...
                  {judge.assignedClasses && judge.assignedClasses.length > 0 && (
                    <div className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                      Assigned to {judge.assignedClasses.length} class
                      {judge.assignedClasses.length !== 1 ? 'es' : ''}
                    </div>
                  )}
                </div>
              ))}
