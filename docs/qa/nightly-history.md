...
- **Route sweep:** partial; public route group clean, secretary surface exposed a high-severity people-query timeout
- **Active specs:** Vitest 18/18; Playwright 32/44 passed, 8 failed, 4 did not run (33.8m vs documented 2.4m)
- **Failures:** Wave 1 Nightly Playwright command from `docs/qa/e2e-suite-map.md` failed in `cross-role-workflows.spec.ts` (secretary command center), `registration/entryCreationCore.spec.ts` (status workflow), `registration/secretaryExistingUsers.spec.ts` (page.goto networkidle 30s timeout), `secretary/show-wizard-officials.spec.ts` (chairman picker empty + Secretary "You" badge missing), `secretary-entry-walk.spec.ts` (waitForResponse 15s timeout), `uat/secretary/critical-path.spec.ts` (dashboard command center), and `uat/secretary/disposable-entry.spec.ts` (seeded dog not found). Isolated re-runs of `cross-role-workflows` and `critical-path` passed in seconds, but `show-wizard-officials` failed durably with `Suggested || All People` and `You` assertions never satisfied because the underlying people query returns Postgres `57014` (statement timeout). Findings opened: `QA-NETWORK-ERROR-010` (root cause), `QA-TEST-FLAKE-011` (cascading flake umbrella).
- **Fixes made:** none — root cause is a database-side performance regression (`GET /rest/v1/people?...user_roles!user_roles_user_id_fkey(role:roles(name)),judge_qualifications(...)` consistently returns `500`/`57014`), which is shared-system and out-of-scope for nightly auto-fix. Documented in `docs/qa/findings.md` and this entry.
- **Demotions/promotions:** none. Do not adjust suite-map promotions while `QA-NETWORK-ERROR-010` is open.
- **Notes:** Ran from clean synced `main`, then created local branch `codex/nightly-qa-2026-05-26` before edits. Proofs: promoted Vitest Nightly (`18 passed`), full Wave 1 Nightly Playwright command (`32 passed`, `8 failed`, `4 did not run`, 33.8m), focused isolated re-runs (`cross-role-workflows --grep "secretary can land"` `1 passed`, `critical-path --grep "dashboard shows"` + `show-wizard-officials` `1 passed / 2 failed`). Browser route sweep covered public (`/`, `/shows`, `/clubs`, `/sign-in`, `/registration`, `/terms`, `/privacy`) and a partial authenticated walk of `/secretary/dashboard` + `/secretary/create-show/wizard` from the existing dev-server session; protected role sweeps for exhibitor/judge/club-admin/admin were not exercised separately because the cascading people-query timeout would contaminate any walk that hits people-backed surfaces — defer that sweep until `QA-NETWORK-ERROR-010` is fixed.
