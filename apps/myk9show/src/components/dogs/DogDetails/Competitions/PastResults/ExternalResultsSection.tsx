...
export default function ExternalResultsSection({
  results,
  addResult,
  editResult,
  deleteResult,
      ...
      {(results || []).map((result, idx) => (
        ...
        >
          ...
          <div className="text-xs text-muted-foreground">
            Class: {result.className} | Result: {result.result}
          </div>
          <div className="flex gap-2 mb-1">
            {result.tags &&
              result.tags.map((tag: string) => (
                <span
                  key={tag}
                  className="px-2 py-0.5 rounded bg-green-200 text-xs font-medium text-green-900"
                >
                  {tag}
                </span>
              ))}
