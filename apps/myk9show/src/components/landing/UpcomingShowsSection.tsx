    ...
    <section className="py-16 bg-background text-foreground transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-end mb-4">
          <Button className="bg-blue-600 hover:bg-blue-700 text-primary-foreground" onClick={onAddShow}>
            + Enter New Show
          </Button>
        </div>
        ...
        >
          {shows.map((show) => (
            ...
            >
              {/* Date badge */}
              <span className="absolute top-4 left-4 bg-blue-600 text-white text-xs font-semibold px-3 py-1 rounded-md z-10 shadow dark:bg-blue-600 dark:text-white">
                {show.date}
              </span>
              <CardHeader className="pb-0">
                <img src={show.imageUrl} alt={show.title} className="rounded-lg w-full h-40 object-cover mb-4" />
              ...
              <CardContent>
                <button className="mt-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors w-full dark:bg-blue-600 dark:hover:bg-blue-700 dark:text-white">
                  View Details
                </button>
              </CardContent>
            </Card>
        ...
        <div className="flex justify-center mt-4 gap-2">
          {shows.map((_, idx) => (
            <span
              key={idx}
              className={`w-2 h-2 rounded-full transition-all duration-200 ${scrollIndex === idx ? 'bg-blue-600' : 'bg-muted'}`}
            />
          ))}
        </div>
      </div>
