...
describe('status icon grammar source ownership', () => {
  ...
  it('scans every in-scope source root for legacy entry/class presentation owners', () => {
    ...
    const forbidden =
      /\bCHECKIN_STATUS\b|\bgetCheckinStatusConfig\b|\bCheckInStatusConfig\b|\bCLASS_STATUS_DISPLAY\b|\bgetClassStatusDisplay\b|\bgetClassStatusBadgeClasses\b|\bCLASS_DISPLAY_STATUS_LABELS\b|\bgetClassDisplayStatusLabel\b|\bgetFormattedStatus\b|\bgetClassStatusColor\b|\bgetFormattedClassStatus\b|\bBASE_STATUSES\b|\bRING_MANAGEMENT_STATUSES\b|\bSTATUS_PILL_BG\b|function\s+getStatusDisplay\s*\(\s*entry\s*:\s*ScoringEntry|myk9-entry-status-dot|myk9-judge-progress-dot|\.myk9-entry-card\.(?:pending|in-progress|completed)|\.myk9-entry-status-text\.(?:pending|in-progress)|const\s+statusColors\s*:\s*Record<ClassStatus|const\s+statusBadgeColors\s*:\s*Record<ClassStatus|function\s+getStatusColor\s*\(\s*status:\s*(?:CheckInStatus|ClassEntry)/;
