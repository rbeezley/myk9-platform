...
- Fee totals already exist on the class step (`DogCartSummary`, `OverallCartSummary` in `ClassSelectionStep.components.tsx`, fed by `getTotalFeesForDog` over `cartItems`) and on payment (`PaymentSummaryCard` with `calculatePlatformFeeCents` + `PlatformFeeSplitLines`).
...
7. **Cart expiry:** removed from this change after six review rounds on the cart-lifecycle interaction; filed as a follow-up.
...
- [Sticky is inert at desktop] → the app shell's `main` (`SidebarLayout`) is `flex-1 overflow-auto` inside a `min-h-screen` flex row, so it is a scroll container that never scrolls (the document does) and every `position: sticky` inside it, the wizard header included, is inert. **Resolved by giving the wizard its own scroll context** rather than by changing the shell: when it is full-page, `RegistrationWizardShell`'s root is `h-[calc(100dvh-var(--app-top-inset,3rem))] min-h-0 overflow-y-auto`, so the sticky header (`top-0`) and the sticky entries-panel card both have a real scrollport. `--app-top-inset`, not `--app-header-height`, because it covers the whole fixed chrome above this subtree (header, plus the PWA banner when one is showing). Consequences: the step-change reset scrolls the root (`scrollTo`) as well as calling `scrollIntoView` for the embedded /secretary case, where the sidebar's pane is the scroller and the wizard is not; the phone bar stays `position: fixed` to the viewport and is still cleared by `--registration-bottom-bar-height`; and `wizardVisualQA.spec.ts` reads the root's `scrollTop` rather than `window.scrollY`, asserting at 1440 that the root scrolled, the document did not, and the panel card and header did not move, and at 390 that the header stays at the scrollport top. SidebarLayout's inert scroll container is unchanged and still makes `position: sticky` inert for every OTHER page; that remains a separate follow-up.
