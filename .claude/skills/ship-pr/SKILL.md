...
For this workaround, use Codex `exec_command` with escalated permissions, the **verified PR worktree as `workdir`**, and `yield_time_ms: 1000`. Resolve the PR number inside that command; variables from an earlier tool call do not persist. Start the wrapper and immediately wait on the **same attempt in the same shell**, using its existing `--wait 1500` option (25 minutes), rather than ending the shell after `--detach`. Poll the returned `session_id` with short `write_stdin` calls. The initial yield returns control without terminating the process. Use this only on a harness that supports a live session for at least 30 minutes without a hard command timeout; otherwise use Richard's terminal. Do not add an unbounded shell loop or an automatic restart.

**A replacement needs coordination first.** Before any new detach, identify the original attempt's PR, owning task/operator, and worktree. Poll it from that **original worktree** with the current wrapper's `--wait 1`; another worktree's missing state files prove nothing. Exit 3 means keep waiting, and exit 0/1 is a verdict to handle. Exit 2 is no verdict, not permission to restart. Have the original owner confirm the child has ended before starting a replacement; if process checks are denied or uncertain, stop and ask Richard to inspect it. A successful `kill -0` with empty/denied `ps` output is not evidence of death. A fresh detach overwrites the attempt's files, and an old live child could write its verdict into the replacement's state.
