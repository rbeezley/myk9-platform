...
- Begin with no cart shell and submitted unpaid entries. The fee-card URL carries only the three owed IDs; other entries exist in the transport fixture, so the test proves the clicked target and rendered cart stay scoped to those IDs. Existing cart-store tests cover ownership filtering and stale paid-line reconciliation. Derive the fee-card target through existing payment logic.
