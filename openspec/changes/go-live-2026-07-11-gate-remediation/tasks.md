...
- [ ] 10.0f [EXPANDED] [SHARED-SYSTEM GATE] After PR review/merge and explicit approval, record the current version of every affected Edge Function, deploy each with `--no-verify-jwt`, then run one controlled transient-failure smoke or provider-supported test plus ordinary auth/registration/alert smokes without sending bulk production mail. Verify retry telemetry contains only attempt/status/delay. Roll back by redeploying the recorded previous versions and repeating ordinary smokes if any function regresses. Partial 2026-07-12: all 12 affected functions deployed ACTIVE; fail-closed startup/auth smokes passed; one post-deploy password reset was delivered; deterministic 429/503/network and PII-free telemetry tests passed 33/33. Still required: a controlled/provider-supported transient-failure check and valid-path registration-email and operator-alert smokes using approved fixtures/recipients. Exact prior-source rollback commands and versions are recorded in the evidence file.
