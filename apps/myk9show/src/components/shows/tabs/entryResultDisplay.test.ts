...
import { getPendingResultLabel, getRemovedStateLabel, isPastTrialDate } from './entryResultDisplay';
import type { EntryStatus } from '@/types/entry-lifecycle';

describe('getRemovedStateLabel', () => {
  function entry(entryStatus: EntryStatus, paymentStatus: 'pending' | 'paid' | 'refunded') {
  ...
  it('labels a moved (source) entry "Moved"', () => {
    expect(getRemovedStateLabel(entry('moved', 'paid'))).toBe('Moved');
  });
