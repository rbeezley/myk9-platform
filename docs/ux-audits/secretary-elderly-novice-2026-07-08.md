...
**Multi-viewport re-walk — the 2026-07-10 attempt was blocked** by a shared staging Supabase pooler outage (`Timed out acquiring connection from connection pool`; RBAC, entry sync, and table sync all failed — unrelated to this change).

**Resolution (2026-07-16):** the re-walk was subsequently completed and tracked as [MYK9-8](https://linear.app/myk9-platform/issue/MYK9-8) (Done 2026-07-15). All four "Top 5 to fix first" trust breaks in the Findings table above are fixed in code with assertion-first tests: Show Desk entry-count divergence (task 2.1 — one canonical `queryKeys.showEntries` source, guarded by `showDeskEntriesCanonicalKey.source.test.ts`), the `1 result pending closeout` dead-end (2.3 — now deep-links to `/results-control`), the zero-entry `Print Check-In Sheet` recommendation (2.4 — `recommended` gated on `count > 0`), and the premium-readiness contradiction (3.1–3.3 — shared `classifyPremiumPublishState` + `usePublishInfo`, per the note above). No open code items remain from this audit.
