...
- [ ] **Make the bell the global Message Center** — The top-bar bell should be the universal inbox for every role, using the existing slide-out panel pattern. Order the center as Notifications, Announcements, Messages; include unread counts from all three sources; and route message threads to the existing role-appropriate message surfaces instead of adding entry-card-only links. Design: [`docs/superpowers/specs/2026-06-04-message-center-inbox-design.md`](docs/superpowers/specs/2026-06-04-message-center-inbox-design.md). Full context in TO-DOS.md § "Bell should be the global Message Center".
