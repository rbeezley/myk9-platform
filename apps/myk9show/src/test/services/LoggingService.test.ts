...
describe('LoggingService', () => {
  ...
  describe('Error Handling', () => {
    it('should handle transport failures gracefully', async () => {
      ...
      expect(errorSpy).toHaveBeenCalledWith('Transport failing failed:', expect.any(Error));
    });
    ...
    it('should generate consistent fingerprints', () => {
      ...
      const fingerprint1 = (
        logger as unknown as {
          generateFingerprint: (msg: string, cat: string, level: number) => string;
        }
      ).generateFingerprint(message1, category, LogLevel.ERROR);
      const fingerprint2 = (
        logger as unknown as {
          generateFingerprint: (msg: string, cat: string, level: number) => string;
        }
      ).generateFingerprint(message2, category, LogLevel.ERROR);
