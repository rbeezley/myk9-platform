...
- **A mutation test you don't confirm actually mutated is a green light you trust too much** — as is a guard test whose setup never reaches the condition it guards. Match headings exactly, never `index()`; print WHERE the change landed; break the implementation and watch THAT test go red. (docs/lessons/README.md#mutation-actually-mutated, [#vacuous-guard-tests](docs/lessons/README.md#vacuous-guard-tests))
...
- **Once a review reports no P0/P1 and a core that holds, route every finding**: fix what this change introduced, FILE what it only exposed, with the reviewer's file:line. Shared-component work splits out. (docs/lessons/README.md#review-scope-boundary)
