...
### QA-CONSOLE-ERROR-012

- **Status:** open
- **Severity:** high
- **Role:** exhibitor | secretary | judge | admin
- **Surface:** `apps/myk9show/src/services/rbac/PermissionChecker.ts`, `apps/myk9show/src/context/AuthContext.tsx`, and `apps/myk9show/src/hooks/useRBAC.ts`.
- **Suite category:** none (surfaced by route-health sweep)
- **Pattern:** console-error
- **Detected by:** audit-pages
- **Evidence:** 2026-05-29 strict route-health sweep after the ringside package-resolution fix passed public and club-admin route groups at desktop plus 375px mobile (`4/12` clean total) but failed exhibitor, secretary, judge, and admin route groups on RBAC console errors. Representative messages:
  - `[ERROR] [rbac] Failed to get user permissions: Error: Failed to get user roles: TypeError: Failed to fetch at PermissionChecker.getUserPermissions (...PermissionChecker.ts:67)`
  - `[ERROR] [rbac] Failed to check permission: Error: Failed to get effective permissions: TypeError: Failed to fetch at PermissionChecker.hasPermission (...PermissionChecker.ts:73)`
  - `[ERROR] [app] Failed to load RBAC data: Error: Failed to get user roles: TypeError: Failed to fetch at loadRbacData (...AuthContext.tsx:143)`
  - `[ERROR] [useRBAC] Error checking permission: Error: Failed to get user roles: TypeError: Failed to fetch at check (...useRBAC.ts:36)`
- **User impact:** Authenticated pages may render while role permissions fail to load. Role-gated actions can appear unavailable, stale, or inconsistent without a clear user-facing error.
- **Intent check:** Harms the secretary and admin confidence target; permission state must feel predictable before show-day workflows can feel calm.
- **Fix owner:** RBAC fetch path and auth-context error handling in myK9Show.
- **Proof required:** Re-run the route-health sweep across public, exhibitor, secretary, judge, club-admin, and admin route groups at desktop plus 375px mobile and confirm all authenticated role groups are console-error-free. Also add a focused RBAC regression proof if the fix changes fallback/error behavior.
- **Notes:** Distinct from `QA-CONSOLE-ERROR-011`, which captured replication/database fetch failures on 2026-05-28. Both use `TypeError: Failed to fetch`, so investigation should confirm whether they share an environment/network root cause before changing product behavior.

## Closed Findings

### QA-BROKEN-NAVIGATION-013

- **Status:** fixed
- **Severity:** high
- **Role:** all
- **Surface:** `apps/myk9show/vite.config.ts`, `packages/ringside/package.json`, `apps/myk9show/src/main.tsx`, and `apps/myk9show/src/features/at-show/slots/atShowLayoutSlots.tsx`.
- **Suite category:** nightly
- **Pattern:** broken-navigation
- **Detected by:** Playwright | audit-pages
- **Evidence:** 2026-05-29 active Playwright Nightly emitted a Vite dependency-scan error before the run stabilized: `No matching export in "../../packages/ringside/dist/index.js" for import "DogCard"` from `apps/myk9show/src/features/at-show/slots/atShowLayoutSlots.tsx`. The follow-up route sweep then failed app startup on a clean local install because Vite could not resolve `@myk9/ringside/styles` from `apps/myk9show/src/main.tsx`; `packages/ringside/dist` is gitignored and should not be required for app boot in this workspace.
- **User impact:** Clean or freshly repaired local environments could fail to boot myK9Show routes because the app depended on ignored ringside `dist` output instead of tracked source.
- **Intent check:** Blocks every role before any role-specific intent can be delivered.
- **Fix owner:** ringside package exports and myK9Show Vite aliasing.
- **Proof required:** Passed on 2026-05-29: route sweep no longer failed on ringside import resolution after `packages/ringside` exported styles from tracked source and myK9Show aliased bare `@myk9/ringside` imports to `packages/ringside/src/index.ts`.
- **Notes:** This is a local/workspace package-resolution fix, not a product-flow change.
