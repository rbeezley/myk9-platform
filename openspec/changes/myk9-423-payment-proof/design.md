...
- Sign in to the hosted exhibitor account in a unique owned browser session. Read entries and payments after the already-completed payment. Capture sanitized visible balance evidence and compare the original entry/payment identities where available.

## Risks / Trade-offs

- Database mocks cannot prove RLS/webhooks → use already-recorded successful payment and fresh hosted balance readbacks; do not represent local integration as backend replay.
...
- Payment/PII exposure → no checkout submission; persist only necessary sanitized evidence.
...
No migration or deployment. Test-only changes go through a PR, required CI and independent review before merge/archive. A revert removes only verification artifacts.
