...
interface UseEntryListHandlersParams {
  ...
  handleStatusChangeHook: (entryId: number, status: 'no-status' | 'checked-in' | 'conflict' | 'pulled' | 'at-gate' | 'come-to-gate') => Promise<void>;
...
export function useEntryListHandlers({
  ...
  }, [hasPermission, setDogInRingStatus, getScoreSheetRoute, setLocalEntries, navigate, classInfo, showContext?.org, classId, refresh, setMaxTimeRequiredWarning, setMaxTimeDialogOpen]);
  ...
  }, [handleMarkInRing, handleMarkCompleted, handleStatusChangeHook, localEntries, setActiveStatusPopup, setLocalEntries]);
  ...
  }, [resetConfirmDialog.entry, handleResetScoreHook, setActiveTab, setLocalEntries, setResetConfirmDialog]);
  ...
  }, [localEntries, refresh, setSortOrder, setLocalEntries, setRunOrderDialogOpen, setShowSuccessMessage]);
