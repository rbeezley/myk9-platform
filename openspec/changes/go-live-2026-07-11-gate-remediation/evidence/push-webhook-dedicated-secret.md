...
GREEN: 17 focused tests prove the exact dedicated secret succeeds, missing configuration returns
503, missing/wrong/wrong-length/service-role-only bearers fail, all five handlers authenticate
before reading payloads, and support preserves only its legitimate downstream bearer.
...
review and merge, recheck both digests, then deploy all five functions with `--no-verify-jwt` and
project ref `sojmvhhwsjxmfistvzbe`. Do not deploy only the three edited handlers: class-status and
scoring bundle the changed shared helper and also require a new revision.
...
Rollback stops triggering writes, restores the prior dedicated secret to both Edge and the existing
Vault row from the operator's secure recovery record, confirms matching redacted digests, and
redeploys the last-good source revision if code rollback is also required. Re-run the same inert
dedicated-secret and service-role-rejection smokes before lifting the maintenance window. Never
restore the service-role fallback.
...
No critical, warning, or suggestion findings were identified for this slice. The parent change is
not archive-ready because its remaining deployment, advisor, and operator tasks are intentionally
open.
