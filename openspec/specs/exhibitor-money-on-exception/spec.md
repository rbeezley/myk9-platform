# exhibitor-money-on-exception Specification

## Purpose

Defines when money may speak on the exhibitor's My Shows page. A settled show says "Paid" in its meta line and nothing more; only a balance the exhibitor can act on earns a strip, a refund is a note on the dog it touched, a recent payment is confirmed once per show and then retires, and no payment chip renders anywhere. Amounts always come from the same source the cart will charge, never from a re-sum of fees.
