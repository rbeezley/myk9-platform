...
- [ ] **Premium-style wizard dropdown lists 8 options but the type declares 6 — root-cause confirmed** — Re-walking the wizard, the Premium List Style dropdown's listbox renders eight options in this exact order: `Monogram (default), Banner, Headline, Magazine, Poster, Gazette, Field Guide, Heritage`. The `PremiumStyle` type in [`premium-types.ts`](apps/myk9show/src/types/premium-types.ts) only allows six (`monogram, headline, magazine, poster, gazette, heritage`). So a user can pick `Banner` or `Field Guide` from the wizard and the value will fail type validation downstream — and the orphaned `Banner` / `Field Guide` test shows we found in browse data are direct evidence the bad path has already been taken. Two paths to fix: (a) remove `Banner` and `Field Guide` from the dropdown source so users can't pick them, or (b) restore both values to `PremiumStyle` and add their cover renderers. The wizard's dropdown source is somewhere different from `getPremiumStyleOptions()` — find and align it. Files: `apps/myk9show/src/types/premium-types.ts`, `apps/myk9show/src/pages/secretary/ShowCreationWizardPage.tsx` (or whichever step component renders the style dropdown).
...
- [ ] **Show cards on `/shows` are `<div>` with onClick instead of `<a href>`** — Card root is a clickable `<div>` (confirmed by inspecting card DOM from `BrowseShowsPage`/`ShowsList`). Breaks middle-click "open in new tab", right-click "open in new window", keyboard tab-traversal, and screen-reader role announcement. Replace the click-handler-on-div pattern with `<a href="/shows/:id">` whose default anchor behavior is preserved. Files: `apps/myk9show/src/components/shows/browse/`, `apps/myk9show/src/pages/BrowseShowsPage.tsx`.
...
- [ ] **Raw UUID leaks into Tasks panel UI** — Dashboard Tasks list renders `4584f257-19b5-4016-aae6-5e7827b769cb` as visible text alongside a task title. The UUID should be `data-id` only, never shown to humans. File: secretary dashboard tasks component.

### Show detail / delete-show UX

- [ ] **"More actions" dropdown contains only "Delete Show"** — On a Draft show detail page, the `More actions` button opens a menu with one item. For a single-action menu, either inline the Delete button (with the existing confirm dialog) or expand the menu with secretary actions: Duplicate Show, Archive Show, Cancel Show, Export Premium PDF, Copy Public Link. Worth confirming whether Published shows surface different items before redesigning. File: `apps/myk9show/src/pages/shows/ShowDetailPage` or whichever component owns the toolbar.
- [ ] **No success toast after deleting a show** — After clicking Delete in the confirmation dialog, the page navigates to `/shows` with no toast or banner confirming what was deleted. Add a success toast like `"Heritage deleted"` after the redirect. The deletion already works; this is feedback only. File: same destructive-action confirm dialog used by `MoreActions`.
- [ ] **Delete confirm shows trial times as "12:00 AM"** — The Delete confirmation dialog enumerates the trials being deleted and renders each as e.g. `Friday Trial 1 - 06/12/2026 12:00 AM`. The midnight time is almost certainly a default-time bug at trial creation (no time set → defaults to 00:00) rather than a delete-dialog bug, but the dialog is where it surfaces. Either set a sensible default (e.g. 8:00 AM) at trial creation, or hide the time when only the date is meaningful. Investigate `trials.start_time` defaults in the trial-creation flow.
...
- [ ] **`Create New Club` toggles host-club section into inline-create mode but offers no way back** — Clicking the "Create New Club" button beside the Host Club picker hides the picker entirely and replaces it with `Club name *` + `Email *` text fields plus an `Add Club` button. There's no "Cancel / pick existing club instead" affordance to return to the picker without losing your typed values. Worse, the transition has no visual acknowledgement (no toast, no inline note saying "Creating new club inline"), so a user who clicked it accidentally is stuck. Add a "Use existing club instead" link/button next to `Add Club`. Files: show-creation wizard step 1 host-club section component.
...
- [ ] **`Failed to create show: not authorized to create shows for club <id>` when secretary creates a show on a club they just created** — On the Review step of the wizard, all three submit paths (`Save as Draft`, `Create Show (Unpublished)`, `Create & Publish Show`) returned the same toast: `Failed to create show: not authorized to create shows for club 51eef8bf-6d7a-45d...`. The blocking detail: the secretary had just created that club seconds earlier via the wizard's inline `Create New Club` form. So the club-create RPC succeeded, but it didn't establish the creator as a club admin / club member with show-create privileges. Combined with F14 (no clubs visible to start with), this is a **complete flow-stopper** for fresh secretaries — they literally cannot complete show creation without site-admin help. Likely fix: club-create RPC should `INSERT INTO club_admins (club_id, user_id) VALUES (new.id, auth.uid())` (or whatever the membership table is) inside the same transaction. Investigate `supabase/migrations/` for the most recent club-related migration and the `clubs` RLS policies. Files: club-create RPC / migration, show-create RLS check.
...
- [ ] **Trial Type dropdown displays the raw enum value (`scent_work`) instead of the human label (`Scent Work`)** — The Trial Type field's listbox shows two options labeled `Scent Work` and `Other`. After selection, the trigger button renders the underlying value (`scent_work`) instead of the label (`Scent Work`). Confirmed across all 4 trials in this walk. The label-to-value map exists (the listbox uses it to render the option text); the trigger just isn't using it. Files: trial-type select component in step 2.
...
- [ ] **Trial date picker doesn't auto-navigate to the show's date range** — When opening the Trial Date & Time picker for the first time, the calendar opens on the _current month_ (May 2026) with the show's actual dates (Jun 12-14, 2026) hidden one click forward. Each trial requires the user to click "Next Month" to reach an enabled day. With four trials this becomes four extra clicks. Default the picker to the first valid in-range date (the show's start date), or keep its initial view aligned with Show Dates. Files: trial-date picker component.
- [ ] **`Event Number*` is required but not visually emphasized in the trial form** — Each trial requires an Event Number (placeholder `e.g. 2026123456`) before the wizard's Next button enables. The field only shows up after the first trial is added, the asterisk is small, and there's no inline error explaining _why_ Next is disabled when Name + Type + Date are all filled but Event Number is empty. Surface inline validation (e.g. "Trial 1: Event Number required") below or next to the Next button. Files: wizard step 2 trial form.

### Wizard step 3 — class selector

- [ ] **`Select All` checkbox at the element-section level is a `<div role="checkbox">` not a labeled button — and its label is empty in the accessibility tree** — Each element section (Container / Interior / etc.) has an indeterminate-state element-level checkbox that selects/deselects all 5 levels under it. In the accessibility tree this checkbox renders with `label: ''` (empty). Screen-reader users hear an unlabeled checkbox with state `indeterminate`. Add an `aria-label="Select all Container classes"` (or equivalent per element). Files: class-selector element-section component.
- [ ] **Class-card click handler appears to occasionally drop clicks under rapid sequential interaction** — During automation, clicking 5 unchecked level cards in sequence sometimes resulted in only 4 of the 5 ending up checked (e.g. Excellent silently skipped). A naïve "loop until aria-checked='true'" was needed to top up the missing one. Could be a non-issue for human-pace clicking, but worth checking if there's a state race when the indeterminate parent re-renders mid-click. Reproducer: programmatically dispatch onClick on each card with no wait between calls. Files: class-card component, element-section selection hook.
...
- [ ] **Secretary registration dog picker defaults to empty + "your dogs" filter** — `/secretary/register/:showId` step 1 shows "No recently active dogs found. Clear the filter to see all **your** dogs" — but secretaries register on behalf of others. A specific-name search (e.g. "Ace") does work; single-character search returns 0; default view is empty. Fix: either default to all dogs (with owner column visible) or change the empty-state copy to clearly say "Search above to find a dog by any owner". Files: secretary registration step 1.
...
- [ ] **Class tab header reads "Classes (0)" while viewing "All Classes (40)"** — The active filter shows 40 rows but the heading counter stays at 0. The header is hardcoded to "My Classes" count instead of the active tab's count. Sync the heading to whichever tab is selected. Files: show detail Classes tab heading.
...
- [ ] **Premium narrative generation can fail silently with no actionable detail** — Banner "Couldn't generate narrative sections - edit the fields below before downloading" appeared. The user has to manually fill the narrative fields. Either surface the upstream error (timeout? edge function 5xx? rate limit?) or include a Retry button. Files: premium narrative generation step on the trial pipeline page.
...
- [ ] **Class edit form requires Judge but wizard creates classes without one** — The class edit dialog labels Judge as `*(required)` and refuses to save until set. But the show-creation wizard creates classes with judge = TBD. So a class can exist in a state that the edit form considers invalid. Either make the wizard collect a judge per class, or relax the class-edit requirement. Files: class edit form validation, show-creation wizard class step.
- [ ] **Class edit form requires Start Time but wizard creates classes without one** — Same pattern as the judge inconsistency. Wizard creates classes with no `start_time`. Class edit dialog says "Please enter a start time" on save attempts with the field blank. Either wizard should set a default start time per class (e.g. inherited from the trial's planned_start_time) or class edit should accept empty start_time.
...
- [ ] **F42 — Class judge dropdown renders "Liz Beezley( - )" when the qualification suffix is empty** — The Judge dropdown shows names with a trailing parenthesized qualification like "Liz Beezley(AKC #1234)". When the judge has no qualification data, it renders as "Liz Beezley( - )" — the dash and parens shouldn't appear. Hide the suffix when empty.
