...
function locate(
  ...
  if (s.testid) loc = page.getByTestId(s.testid);
    ...
    loc = page.getByRole(
      s.role as 'button',
      s.name ? { name: new RegExp(s.name, 'i') } : undefined
    );
...
test('secretary walk', async ({ page }) => {
  ...
  page.on('request', r => {
    const u = r.url();
    if (/supabase|rest\/v1|auth\/v1|functions\/v1/.test(u))
      requests.push(`${r.method()} ${u.slice(0, 160)}`);
    // Audit aid: capture the body of show writes so a dropped field is visible.
    if (
      /rpc\/create_show_with_children/.test(u) ||
      (/rest\/v1\/shows/.test(u) && r.method() !== 'GET')
    ) {
      const body = r.postData() ?? '';
      requests.push(
        `  BODY ${r.method()} ${u.replace(/^.*rest\/v1\//, '')} :: ${body.slice(0, 700)}`
      );
    }
  });
  page.on('response', r => {
    ...
    if (/rest\/v1|functions\/v1/.test(u) && r.status() >= 400) {
      ...
      void r
        .text()
        .then(b => {
          consoleErrors.push(`  BODY(${r.status()}): ${b.slice(0, 400)}`);
        })
        .catch(() => undefined);
    }
    ...
    if (/rest\/v1\/entries/.test(u) && r.request().method() === 'PATCH') {
      void r
        ...
        .then(b => {
          consoleErrors.push(
            `  PATCH ${r.status()} rows=${b.trim() === '' ? '(empty body)' : b.slice(0, 200)}`
          );
        })
        .catch(() => undefined);
    }
  ...
  const dump = async (i: number, label: string) => {
    ...
    const snap = await page
      .locator('body')
      .ariaSnapshot()
      .catch(e => 'SNAPSHOT FAILED: ' + String(e));
      ...
      if (s.action === 'goto') await page.goto(s.url);
        ...
        const ok = await page.evaluate((needle: string) => {
          ...
          const target = (el.closest(
            '[role="option"],button,li,[data-value],div[class*="cursor"]'
          ) ??
            el.parentElement ??
            el) as HTMLElement;
          target.click();
        ...
        fs.appendFileSync(
          path.join(OUT_DIR, 'eval.txt'),
          `${label}: ${JSON.stringify(v, null, 2)}\n`
        );
      } else if (s.action === 'select')
        await locate(page, s).selectOption(s.value, { timeout: 15000 });
      await page.waitForTimeout(600);
      await dump(i + 1, label);
    } catch (e) {
      await dump(i + 1, `FAILED-${label}`);
      fs.appendFileSync(
        path.join(OUT_DIR, 'errors.txt'),
        `step ${i + 1} (${label}): ${String(e).slice(0, 400)}\n`
      );
      break;
    }
  }
