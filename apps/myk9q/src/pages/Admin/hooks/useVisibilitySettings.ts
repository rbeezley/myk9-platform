...
export function useVisibilitySettings(): UseVisibilitySettingsReturn {
  ...
  const [trialVisibilitySettings, setTrialVisibilitySettings] = useState<
    Map<number, VisibilityPreset>
  >(new Map());
   ...
   */
  const handleSetShowVisibility = useCallback(
    async (
      preset: VisibilityPreset,
      adminName: string,
      licenseKey: string
   ...
   */
  const handleSetTrialVisibility = useCallback(
    async (
      trialId: number,
      preset: VisibilityPreset,
      adminName: string,
   ...
   */
  const handleRemoveTrialVisibility = useCallback(
    async (trialId: number, _trialLabel: string): Promise<{ success: boolean; error?: string }> => {
      try {
