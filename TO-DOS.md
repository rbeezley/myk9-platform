...
- **Standardize "Add X" button icons to Plus** — All "Add" action buttons should use the Lucide `Plus` icon for consistency. Some currently use other icons (e.g., Add Classes uses a document/sheet icon instead of Plus). **Problem:** Inconsistent iconography — "Add Trial" has a Plus icon, "Add Classes" has a paper icon. Users expect the same visual pattern for the same semantic action. **Files:** `apps/myk9show/src/components/trials/TrialDetail/TrialClassesTable.tsx` (Add Classes button), plus ~40 files with "Add X" buttons — audit all for consistent Plus icon. **Solution:** Find all "Add X" buttons, replace non-Plus icons with `<Plus className="h-4 w-4" />`. The Plus icon is the universal "create/add" affordance.

- **Standardize view toggle to icon-only pattern** — Replace text-labeled "View: Cards | Table" toggles with compact icon-only toggle buttons (list/grid icons) across all pages. **Problem:** Two patterns coexist for the same action — icon-only toggle on TrialClassesTable, text+icon toggle on ClassesTab/TrialsTab/MyEntriesTab. Inconsistency hurts muscle memory. **Files:** `apps/myk9show/src/components/shows/tabs/ClassesTab.tsx` (text+icon ViewToggle), `apps/myk9show/src/components/shows/tabs/TrialsTab.tsx` (text+icon ViewToggle), `apps/myk9show/src/pages/MyEntriesPage/index.tsx` (text+icon ViewToggle), `apps/myk9show/src/components/trials/TrialDetail/TrialClassesTable.tsx` (icon-only — the target pattern). **Solution:** Standardize on the compact icon-only toggle (TrialClassesTable pattern). Apply to all view toggle locations.

- **Indicate why Next is disabled on handler assignment step** — "Not assigned" text should be visually prominent (red/warning color) when it's blocking the Next button, so users understand why they can't proceed. **Problem:** When a dog has no owner/handler, the handler assignment step shows "Not assigned" in muted grey text and disables the Next button, but there's no visual cue connecting the two — users don't understand why they're stuck. **Files:** `apps/myk9show/src/components/shows/RegistrationWorkflow/HandlerAssignmentStep.tsx:181` ("Not assigned" text), `apps/myk9show/src/components/shows/RegistrationWorkflow/InlineHandlerSection.tsx:120` (inline variant). **Solution:** Change "Not assigned" to red/destructive styling when it blocks progression. Optionally add a validation message near the Next button ("Assign handlers to all classes to continue").

- **Add card/table view toggle to ClassResultsTable** — Add myK9Q-style entry cards alongside the existing table view on the Class Details page. **Problem:** The Class Details "Entries & Results" section is table-only. myK9Q has a polished card-based entry display (see `SortableEntryCard`) with Pending/Completed tabs, armband badge (left), dog name + breed + handler (center), and status badge (top-right: "Come to Gate", "Checked-in", "At Gate", "No Status"). Secretaries and judges benefit from at-a-glance card layouts, especially on tablets. **Files:** `apps/myk9show/src/components/classes/ClassResultsTable/index.tsx:271-364` (current table-only render — add view toggle here), `apps/myk9q/src/pages/EntryList/SortableEntryCard.tsx` (myK9Q entry card — design reference), `apps/myk9q/src/pages/EntryList/SortableEntryCardComponents.tsx` (card sub-components), `apps/myk9q/src/pages/EntryList/EntryList.tsx` (Pending/Completed tab pattern), `apps/myk9show/src/hooks/useViewPreference.ts` (existing localStorage view preference hook), `apps/myk9show/src/components/trials/TrialDetail/TrialClassesTable.tsx` (icon-only table/cards toggle pattern to reuse). **Solution:** (1) Create `ResultsCardView.tsx` in `ClassResultsTable/` with entry cards matching myK9Q's `SortableEntryCard` design: armband badge (teal circle), dog name + breed, handler name, status badge. (2) Add Pending/Completed filter tabs above the card grid. (3) Wire `useViewPreference('class-results')` for persistent toggle. (4) Use the icon-only toggle pattern from TrialClassesTable. Cards are read-only display — inline scoring stays in table mode.

---

## Move Show Quick Info Below Hero Section - 2026-03-26 16:54

- **Move QuickInfoCards from Overview tab to below hero section** — Relocate the date/entry fee/location/host club info strip from inside the Overview tab to directly below the DetailHero, matching the ClassDetailsPage metadata strip pattern. Remove duplicate date and location info from the hero section's metadata row. **Problem:** The show details page displays date and location twice — once in the hero section metadata row and again in the QuickInfoCards grid inside the Overview tab. The info strip (date, entry fee, location, host club) is useful context regardless of which tab is active, but it disappears when switching to Trials/Classes/Entries/Results tabs. The ClassDetailsPage already has the right pattern: a compact metadata strip attached below the hero that persists across all content. **Files:** `apps/myk9show/src/pages/ShowDetailsPage.tsx:314-332` (hero section + tab content — move QuickInfoCards render between DetailHero and PrimaryTabs), `apps/myk9show/src/components/shows/overview/QuickInfoCards.tsx:67-90` (the info cards component to relocate), `apps/myk9show/src/components/shows/tabs/ShowOverviewTab.tsx:32` (currently renders QuickInfoCards — remove from here), `apps/myk9show/src/components/common/DetailHero.tsx:63-72` (hero metadata with date/location icons — remove duplicate fields after move), `apps/myk9show/src/components/classes/ClassCompactHeader.tsx:99-130` (reference pattern for metadata strip below hero). **Solution:** (1) Move `<QuickInfoCards>` render from ShowOverviewTab to ShowDetailsPage, positioned between `<DetailHero>` and the tab bar. (2) Remove the date and location metadata items from the DetailHero metadata prop (keep the hero clean — just title, subtitle/club, org badge, and Register button). (3) QuickInfoCards already has the right visual design; it just needs to be repositioned. Consider whether to keep the 4-column card grid or switch to the ClassCompactHeader inline strip style for consistency.

---

## Consistent Edit Button Placement Across Detail Pages - 2026-03-26 16:57

- **Standardize Edit button and three-dot menu placement on detail pages** — Show and Trial detail pages place Edit + three-dot menu in the breadcrumb row (via PageHeader actions), while ClassDetailsPage places them inside the hero/compact header. Pick one pattern and apply everywhere. **Problem:** Inconsistent placement of admin actions across the three main detail pages hurts muscle memory and visual consistency. ShowDetailsPage and TrialDetailsPage render Edit + MoreVertical in PageHeader's `actions` prop (breadcrumb row, outside hero). ClassDetailsPage renders them inside ClassCompactHeader's `actions` prop (inside the hero area). The ClassDetailsPage pattern (inside hero) is arguably better — it groups the entity title with its actions, keeps the breadcrumb row clean for navigation only, and matches common SaaS detail page conventions. **Files:** `apps/myk9show/src/pages/ShowDetailsPage.tsx:291-307` (Edit + ThreeDotMenu in PageHeader actions), `apps/myk9show/src/pages/TrialDetailsPage.tsx:323-339` (Edit + DropdownMenu in PageHeader actions), `apps/myk9show/src/pages/ClassDetailsPage/index.tsx:214-230` (Edit + DropdownMenu in ClassCompactHeader actions, line 252 PageHeader has no actions), `apps/myk9show/src/components/common/PageHeader.tsx:47` (actions slot in breadcrumb row), `apps/myk9show/src/components/common/DetailHero.tsx` (hero component — could accept actions prop), `apps/myk9show/src/components/classes/ClassCompactHeader.tsx:119` (actions slot inside header). **Solution:** Decide which pattern is better (recommendation: inside-hero like ClassDetailsPage — cleaner breadcrumb row, entity-grouped actions). Then move ShowDetailsPage and TrialDetailsPage Edit/menu from PageHeader actions into their respective hero sections (DetailHero would need an `actions` prop, or use a wrapper div). Remove actions from PageHeader on those pages.
