...
Migration `20260711150000_force_rls_go_live_gap.sql` forces RLS on those four tables only and includes manual `NO FORCE` rollback SQL without changing RLS enablement, policies, or grants.

`forceRlsInvariant.test.ts` replays the ordered migration corpus dynamically. Its pre-migration RED reported exactly the four extant gaps; after the remediation migration, all six focused tests pass. The checker uses no table allowlist and covers create, drop, enable, disable, force, and no-force transitions.
