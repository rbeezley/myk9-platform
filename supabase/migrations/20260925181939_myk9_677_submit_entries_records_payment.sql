-- =============================================================================
-- MYK9-677: a secretary-received payment is recorded INSIDE submit_show_entries.
--
-- Codex rounds 4 and 7 on #2441: the wizard created entries in one call and
-- recorded the received cash/check in a second, and every exit between the two
-- (a failed enrollment write, an unmounted page, a lost response) left entries
-- with no payment and nothing that would ever retry it. Owner-approved
-- restructure: the payment is an optional argument here, recorded in the same
-- transaction after the entries are written, by the same private core
-- (private.record_enrollment_payment_core, 20260925181937) the Entry Management
-- RPC calls. A failure anywhere rolls the entries back too.
--
-- p_payment: {"method": "cash"|"check", "received_on": "YYYY-MM-DD" (optional,
-- default today on the show's calendar), "reference": text (optional)}. The
-- amount is NOT taken from the client: it is the sum of the server fees of the
-- entries this call creates.
--
-- Absent (NULL, the default), the function behaves exactly as before for every
-- other caller. The 5-argument signature is dropped and recreated with the
-- defaulted sixth, so a named-argument call without p_payment resolves here.
--
...
DROP FUNCTION public.submit_show_entries(uuid, uuid, jsonb, uuid, text);

CREATE FUNCTION public.submit_show_entries(p_show_id uuid, p_registration_id uuid, p_entries jsonb, p_submission_id uuid, p_payment_method text, p_payment jsonb DEFAULT NULL)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
