...
test.describe('Phase 5: Simple Integration Testing', () => {
  ...
  test('should access all Phase 5 components and interfaces', async ({ page }) => {
    ...
    const hasDeviceCompatibility = await page
      .locator('text=Mobile Optimized')
      .isVisible({ timeout: 5000 })
      .catch(() => false);
    const hasClassSelection = await page
      .locator('text=Select Class to Judge')
      .isVisible({ timeout: 5000 })
      .catch(() => false);
    ...
    const hasJudgeContent = await page
      .locator('text=Judge')
      .first()
      .isVisible({ timeout: 5000 })
      .catch(() => false);
    ...
    for (const navTest of navigationTests) {
      try {
        ...
        const hasExpectedContent = await page
          .locator(`text=${navTest.expectedText}`)
          .first()
          .isVisible({ timeout: 5000 })
          .catch(() => false);
