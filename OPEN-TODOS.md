...
- [ ] **Wave C — extractions** (multi-concern oversized files only) — Replication-core slice 1 extracts mutation ordering, queue capacity, and localStorage backup helpers with direct tests; slice 2 extracts retry-state and OCC empty-update classification helpers with direct tests; slice 3 extracts conflict row-state helpers with direct tests; slice 4 extracts provider status/result helpers with direct tests. Remaining Wave C work still includes deeper table/provider helpers, scoring, and Show Map extractions.
