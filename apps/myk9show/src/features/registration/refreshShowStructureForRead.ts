...
/**
 * Pull one show's structure (the show row, its trials, and their classes) onto
 * this device, waiting at most a few seconds. Forced full syncs: a cold scope
...
export async function refreshShowStructureForRead(
  showId: string,
  options: { includeShow: boolean }
): Promise<void> {
  const forced = { forceFullSync: true };
  const refresh = (async () => {
    await Promise.allSettled([
      replicatedTrialsTable.sync(showId, forced),
      options.includeShow ? replicatedShowsTable.sync('', forced) : Promise.resolve(),
    ]);
