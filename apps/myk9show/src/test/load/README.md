...
`12|0|0` after success or failure. Before either reseed, the workflow
...
eight non-finalized demo classes: 504 load entries on top of the 12 hand-authored
...
the show total is exactly 516. Before an approved rehearsal run both files and
verify `516|504|0`; afterwards run `seed-demo.sql` alone and verify `12|0|0`.
...
target with the canonical reseed (`seed-demo.sql` alone) and verify `12|0|0`
