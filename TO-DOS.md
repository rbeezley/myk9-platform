...
- [ ] **Fix `is_trial_secretary` null `club_id` RLS bypass** — The RLS helper function `is_trial_secretary` in `supabase/migrations/074_fix_secretary_role_in_rls_functions.sql:24` has predicate `ur.club_id IS NULL` which means any `user_roles` row inserted without a `club_id` acts as a platform-wide secretary grant. **Problem:** A secretary role without `club_id` can modify visibility settings, release results, and manage volunteers for any show in the database — not just their assigned shows. **Files:** `supabase/migrations/074_fix_secretary_role_in_rls_functions.sql`. **Solution:** Remove the `ur.club_id IS NULL` fallback from `is_trial_secretary`. Audit all code paths that create secretary roles to ensure `club_id` is always set. Add a CHECK constraint or trigger on `user_roles` requiring `club_id IS NOT NULL` for secretary/club_admin roles.
