...
// TODO: Move to separate hooks file to fix react-refresh warning
// Temporarily disabled to fix lint error
/* const useUserFriendlyErrors = () => {
  const { networkStatus } = useOfflineFirst();
  ...
  const showError = (
    ...
    if (showToast) {
      ErrorToast({ error: structuredError, duration, showRecovery });
    }
