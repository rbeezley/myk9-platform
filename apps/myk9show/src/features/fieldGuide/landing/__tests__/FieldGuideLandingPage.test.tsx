/**
 * MYK9-633: Field Guide repeated the entry CTA twice (TopStrip /
 * FinalCtaSection) — the same shape MYK9-565 fixed on Monogram. Exactly one
 * entry CTA at desktop width (the top strip), plus exactly one more — a
 * mobile-only sticky bottom bar reusing the same copy — below 640px.
 */
import { afterEach, describe, expect, it, vi } from 'vitest';
import { render } from '@/test/utils/testUtils';
import { FieldGuideLandingPage } from '../FieldGuideLandingPage';
import type { FieldGuideLandingData } from '../types';
import { mockViewportWidth } from '@/test/utils/mockViewportWidth';

vi.mock('../../fonts', () => ({
  ensureFieldGuideFontsLoaded: vi.fn(),
  FIELD_GUIDE_DISPLAY_FAMILY: "'Space Grotesk', sans-serif",
  FIELD_GUIDE_BODY_FAMILY: "'IBM Plex Sans', sans-serif",
  FIELD_GUIDE_MONO_FAMILY: "'IBM Plex Mono', monospace",
}));

const baseData: FieldGuideLandingData = {
  clubName: 'Field Guide Kennel Club',
  showName: 'Heartland Scent Work Classic',
  showSubtitle: 'FIELD GUIDE · AKC-2026 · REV 01',
  welcomeText: null,
  trialChairName: null,
...
vi.mock('../useFieldGuideLandingData', () => ({
  useFieldGuideLandingData: () => baseData,
}));

describe('FieldGuideLandingPage — entry CTA count (MYK9-633)', () => {
  afterEach(() => {
