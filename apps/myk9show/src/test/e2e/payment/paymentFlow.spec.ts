...
test.describe('Payment Flow', () => {
  ...
  test('should handle PayPal payment integration', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="paypal-transaction-id"]')).toContainText(
      'PAYID-123456'
    );
  });
  ...
  test('should process refunds correctly', async ({ page }) => {
    ...
    await page.fill(
      ...
      'Exhibitor requested withdrawal due to dog injury'
    );
  ...
  test('should handle partial refunds for multi-class entries', async ({ page }) => {
    ...
    await page.fill(
      '[data-testid="partial-refund-reason"]',
      'Classes cancelled due to judge unavailability'
    );
  ...
  test('should handle payment disputes and chargebacks', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="chargeback-reason"]')).toContainText(
    ...
    await page.setInputFiles(
      ...
      'test-files/registration-confirmation.pdf'
    );
    await page.fill(
      '[data-testid="evidence-description"]',
      'Registration confirmation with digital signature'
    );

    // Add transaction details
    await page.fill(
      '[data-testid="transaction-notes"]',
      'Valid registration completed online with email confirmation sent'
    );
