...
- **Route sweep:** fail on long-settle background health
- **Active specs:** Vitest 18/18; Playwright 42/42 with `--retries=0`
- **Failures:** Initial active Playwright run failed stale/test-drift and entry-loading checks. Fixed and proved: secretary dashboard label drift, stale browser-side audit-trail E2E coverage, secretary registration auth/network waits, show-wizard officials picker assumptions, entry-walk confirmation copy/RPC wait drift, and `/secretary/entries/:showId` loading failure from an optional enrollment join timing out. The final active Nightly command passed `42/42`. The long-settle route sweep failed `18/24` authenticated route/viewport checks due to late owned `500` responses from broad `people` directory loads and full `entries` replication sync; tracked as `QA-NETWORK-ERROR-010` and `QA-NETWORK-ERROR-011`.
- **Fixes made:** `apps/myk9show/src/services/database/entries/secretary.ts`, `apps/myk9show/src/components/shows/wizard/steps/OfficialPicker.tsx`, `apps/myk9show/src/test/unit/entryStore.test.ts`, `apps/myk9show/src/test/components/wizard/OfficialPicker.test.tsx`, `apps/myk9show/src/test/e2e/cross-role-workflows.spec.ts`, `apps/myk9show/src/test/e2e/registration/entryCreationCore.spec.ts`, `apps/myk9show/src/test/e2e/registration/secretaryExistingUsers.spec.ts`, `apps/myk9show/src/test/e2e/secretary-entry-walk.spec.ts`, `apps/myk9show/src/test/e2e/secretary/show-wizard-officials.spec.ts`, `apps/myk9show/src/test/e2e/uat/secretary/critical-path.spec.ts`, `apps/myk9show/src/test/e2e/uat/shared/secretaryData.ts`, `docs/qa/findings.md`, `docs/qa/nightly-history.md`.
- **Demotions/promotions:** none. Active Nightly remains green after the repairs; route-health findings stay open for separate shared data/RLS work.
- **Notes:** Ran from clean synced `main`, then used local branch `codex/nightly-qa-2026-05-25` before editing. Proofs passed: promoted Vitest Nightly (`18 passed`), focused unit review proof (`27 passed`), focused repaired Playwright proof (`7 passed` plus disposable-entry seed proof `1 passed`), full active Playwright Nightly (`42 passed`, retries disabled), short route sweep (`24/24`). The stricter long-settle route sweep intentionally waited 9s after render and exposed the late Supabase `57014` failures recorded above. Temporary route-sweep spec was removed.
