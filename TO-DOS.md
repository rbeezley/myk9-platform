...
- **Fix entries not syncing to Supabase** — Entries created via the registration wizard exist only in IndexedDB (local replication layer), not in Supabase. `dbCount: 0` in React Query results. The replication layer's `queueMutation('INSERT', ...)` queues the mutation but it never executes or fails silently. This means the Supabase dog join (which would also resolve dog names) never fires. **Files:** `apps/myk9show/src/services/replication/ReplicatedEntriesTable.ts:388-407` (`createEntry` — queues INSERT), `packages/replication/src/` (MutationManager — processes queued mutations). **Solution:** Investigate why queued INSERT mutations aren't being processed. Check MutationManager initialization, error handling, and whether mutations are stuck in the queue.
