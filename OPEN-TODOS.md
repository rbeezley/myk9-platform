...
- [ ] **1. Code-Quality Audit** — remove static debt first; every later step audits or builds on the post-cleanup surfaces. Plan: [`docs/plan-code-quality-audit.md`](docs/plan-code-quality-audit.md). _In progress — Wave A, Wave B type-file unification, P1 launch gates, email parity, read-shape parity, Wave C replication-core slices 1-5, and Wave C scoring slice 1 are implemented._
...
- [ ] **Wave C — extractions** (multi-concern oversized files only) — Replication-core slice 1 extracts mutation ordering, queue capacity, and localStorage backup helpers with direct tests; slice 2 extracts retry-state and OCC empty-update classification helpers with direct tests; slice 3 extracts conflict row-state helpers with direct tests; slice 4 extracts provider status/result helpers with direct tests; slice 5 extracts replicated table row-state/stale-row helpers with direct tests; scoring slice 1 extracts offline scoring service helpers with direct tests. Remaining Wave C work still includes Show Map extraction.
