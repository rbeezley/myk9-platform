...
export class ConnectionRecoveryManager {
  ...
  private async retryFailedQueries(): Promise<void> {
    ...
    const failedQueries = queries.filter(
      (query: unknown) => (query as { state: { error: unknown } }).state.error
    ) as Array<{ fetch: () => Promise<unknown>; queryHash: string }>;
  ...
  private async processRecoveryQueue(): Promise<void> {
    ...
    for (const item of sortedQueue) {
      try {
        await this.retryEngine.executeWithRetry(item.operation, item.config, item.id);
  ...
  queueForRecovery(
    ...
    if (this.recoveryQueue.length > 100) {
      this.recoveryQueue = this.recoveryQueue
        ...
        .concat(this.recoveryQueue.filter(item => item.priority !== 'high').slice(-50));
    }
...
export const useConnectionRecovery = () => {
  ...
  const recoveryManager = useMemo(() => new ConnectionRecoveryManager(queryClient), [queryClient]);
