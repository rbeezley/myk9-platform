...
| --- | --- | --- |
| Exhibitor | Yellow (was Red) | **Phase 6 re-walk 2026-06-15:** `UX-P1-01` resolved — staging seed added 32 classes to the accepting show; walked as `exhibitor1@myk9t.com` through class selection → Payment (Container Novice A $30, Total Due, Card method; not submitted). Discovery (P2-05 Browse shows) confirmed desktop + mobile, logged-out. Green pending the confirmation/results leg and new finding `UX-P2-04-EXP` (styled-landing class fit). |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| UX-P1-01 | P1 → RESOLVED | Exhibitor | Accepting show `Monogram` has no classes assigned, so exhibitors cannot enter. | Exhibitor step 3: Enter a dog in the right classes. | "This respects my time" is broken by a registration dead end. | [`01-exhibitor-journey.md`](01-exhibitor-journey.md) | Fix seed/setup data or class inventory gating so accepting shows cannot route to unusable registration; then re-score entry/payment. | No new surface; tighten existing Show Details and Registration Wizard gates. | **RESOLVED 2026-06-15 (Phase 6 re-walk):** test-data gap (Monogram had 4 trials, 0 classes, yet was the only show with an open entry window — verified 0 via anon + auth). User-approved staging seed added 32 classes; walked enter→Payment as `exhibitor1`. Defense-in-depth empty-state recovery shipped in [PR #767](https://github.com/rbeezley/myk9-platform/pull/767). (Prior "already has 32 classes" note was a mis-query.) |
| UX-P2-04-EXP | P2 | Exhibitor | Published-experience shows route entrants to a styled landing (8 styles) that omits the class/level summary the P2-04 "See classes" fix added to `ShowDetailsPage`. | Exhibitor step 2: Understand whether the show fits their dog. | Cannot judge fit before "Enter your dog". | [`01-exhibitor-journey.md`](01-exhibitor-journey.md) (Phase 6 re-walk) | Thread the already-computed class inventory into `StyledLandingProps`; render a shared class summary / See-classes deep-link in each styled landing. | No new surface; reuse class inventory + existing wizard/classes route. Own PR (8-style themed change). | Browser check that class/level detail or a See classes link is visible pre-registration on each styled landing. |
