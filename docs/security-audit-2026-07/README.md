...
| [plan-role-map-disclosure.md](plan-role-map-disclosure.md) | SA-006 (user_roles/permission_audit_log SELECT)                              | MEDIUM        | M      | No       | IMPLEMENTED — PR pending. Mig `20260703180000` scopes the 5 SELECT policies (user_roles→self-or-admin, audit_log→admin-only, catalog→authenticated) + adds `get_show_officials`/`get_club_show_manager_ids` SECURITY DEFINER RPCs; 3 client reads repointed. `openspec/changes/security-role-map-disclosure/` |
