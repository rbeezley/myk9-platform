...
- [ ] **Late-entry / day-of additions workflow** — Walk-in handling: secretary path to add a new exhibitor + dog + entry + payment at the desk on show day. Phase D plan: [`docs/plan-phase-d-late-entry-workflow.md`](docs/plan-phase-d-late-entry-workflow.md). Existing `DayOfEntryDialog` covers existing-dog day-of entries; next slices bring it into the workbench and fill the new exhibitor/dog gap.
