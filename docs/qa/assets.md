...
| Isolated Nightly run          | Preparing and running trusted scheduled Nightly runs | `pnpm qa:nightly:run`                                                                                                           | Detached `origin/main` worktree plus `.qa-nightly.env` with unique Playwright port | Every scheduled Nightly                    | Runs the health phases and removes the temporary worktree on exit                         |
...
| Role-journey visual QA matrix | Cross-role layout and interaction coverage           | `pnpm qa:role-journey`                                                                                                          | Phone/tablet/desktop + light/dark screenshots and browser-health assertions        | Cross-role UX or visual-token changes      | Requires env-backed E2E credentials; feature-audit coverage, not PR smoke                 |
...
Scheduled Nightly runs should use the managed command, which starts from an
isolated detached `origin/main` worktree and removes it after the run. This
prevents concurrent local agents or dirty WIP from contaminating the trusted
baseline without leaving a per-run local branch.
...
For a manual multi-phase run, use `pnpm qa:nightly:prepare`; it prints the
cleanup command to run when finished. Never create a dated report branch for a
Nightly run.
