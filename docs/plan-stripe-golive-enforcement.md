...
**Implementation status (2026-07-13):** merged in [#1314](https://github.com/rbeezley/myk9-platform/pull/1314), migration deployed, functions live, and controlled Stripe test-mode smoke passed. `20260713110000_waitlist_offer_payment_guard.sql` is applied; `stripe-payment-link` is ACTIVE v12 and `decline-waitlist-offer` ACTIVE v1 (15:12 UTC). Both reject unauthenticated no-op POSTs with 401. A controlled `cs_test_` Checkout payment reconciled entry/link/offer/order and a separate controlled decline closed its link without an order or payment intent; only the paid claim consumed judge-day capacity. A narrow dynamic-payment-method fix is locally verified on `codex/stripe-phase-b-smoke-evidence`, pending review, merge, approved `stripe-payment-link` redeployment, and a fresh test-mode Checkout check.
