...
export function useOptimisticRegistration(
  ...
  const updateDogSelection = useCallback(async (selectedDogs: string[]) => {
  ...
  const updateClassSelections = useCallback(async (classSelections: ClassSelectionData[]) => {
    const optimisticState = {
      ...registrationState,
      classSelections
    };
    ...
    return executeOptimisticUpdate(
      ...
      async () => {
        ...
        const totalFees = classSelections.reduce((total, selection) => 
          total + (selection.selectedClasses.length * 25), 0
        );
        
        return { success: true, classSelections, totalFees };
      },
      {
        onSuccess: (result: unknown) => {
          const typedResult = result as { totalFees: number; [key: string]: unknown };
          // Keep class selection toast as it's useful for fee information
          toast.success('Class selections saved', {
            description: `Total fees: $${typedResult.totalFees}`
          });
        }
      }
    );
  ...
  const updateHandlerAssignments = useCallback(async (handlerAssignments: Record<string, HandlerInfo>) => {
    const optimisticState = {
      ...registrationState,
      handlerAssignments
    };
    ...
    return executeOptimisticUpdate(
      ...
      {
        onSuccess: (result: unknown) => {
          const typedResult = result as { validationResults: Array<{ valid: boolean; [key: string]: unknown }>; [key: string]: unknown };
          const validCount = typedResult.validationResults.filter((r: { valid: boolean }) => r.valid).length;
  ...
  const updatePaymentStatusOptimistic = useCallback(async (
    ...
    return executeOptimisticUpdate(
      ...
      {
        onSuccess: (result: unknown) => {
          const typedResult = result as { paymentStatus: PaymentStatus; paymentReference?: string; [key: string]: unknown };
  ...
  const updateEntryStatusOptimistic = useCallback(async (
    registrationId: string,
    ...
    return executeOptimisticUpdate(
      ...
      {
        onSuccess: (result: unknown) => {
          const typedResult = result as { entryStatus: EntryStatus; reason?: string; [key: string]: unknown };
  ...
  const submitRegistrationOptimistic = useCallback(async (registrationId: string) => {
    return executeOptimisticUpdate(
      'Submit Registration',
  ...
  const createRegistrationOptimistic = useCallback(async (userId: string, createdByUserId?: string) => {
    return executeOptimisticUpdate(
      'Create Registration',
      registrationState,
      async () => {
      ...
      {
        onSuccess: (result: unknown) => {
          const typedResult = result as { registrationNumber?: string; id: string; [key: string]: unknown };
  ...
  const batchUpdate = useCallback(async (updates: Partial<RegistrationState>) => {
    const optimisticState = {
      ...registrationState,
      ...updates
    };
