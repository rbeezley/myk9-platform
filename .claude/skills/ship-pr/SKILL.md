...
For this workaround, use Codex `exec_command` with escalated permissions, the **verified PR worktree as `workdir`**, and `yield_time_ms: 1000`. Resolve the PR number inside that command; variables from an earlier tool call do not persist. Use **two separate wrapper invocations in that one shell**: first `bash scripts/qa/claude-review.sh --detach --post "$PR_NUMBER"`; only if it succeeds, run `bash scripts/qa/claude-review.sh --wait 1500 "$PR_NUMBER"` (25 minutes). Never combine `--detach` and `--wait` in one invocation: the wait branch takes precedence and would not start a review. Do not end the shell between the two invocations. Poll the returned `session_id` with short `write_stdin` calls. The initial yield returns control without terminating the process. Use this only on a harness that supports a live session for at least 30 minutes without a hard command timeout; otherwise use Richard's terminal. Do not add an unbounded shell loop or an automatic restart.
