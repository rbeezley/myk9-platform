...
export const CommonDialog: React.FC<CommonDialogProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
      <div
        ref={dialogRef}
        className={`dialog-content w-full ${maxWidth}`}
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        <div className="flex justify-between items-start mb-4">
          <div>
            {title && (
              <h2 id="dialog-title" className="text-xl font-semibold text-foreground flex items-center gap-3">
        ...
        {/* Content */}
        <div className={`px-10 py-6 text-sm leading-normal ${footer ? "mb-2" : "mb-0"}`}>
          {children}
        </div>
        {/* Footer (optional) */}
        {footer && (
          <div className="px-10 py-4 border-t border-border/50">
            {footer}
          </div>
        )}
      </div>
