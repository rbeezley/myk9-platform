...
3. RPC hashes the input and selects from `show_passcodes` where `passcode_hash = $hash`. If exactly one row matches → return `{show_id, role}`. If no row matches → return null (the smart input shows the generic enumeration-resistant error).
...
1. **Migration: `show_passcodes` table.** Create `show_passcodes(id uuid PK, show_id uuid FK shows ON DELETE CASCADE, role text CHECK role IN ('admin','judge','steward','exhibitor'), passcode_hash text NOT NULL, created_at timestamptz DEFAULT now(), UNIQUE(show_id, role))`. Index on `passcode_hash` for lookup. RLS: deny all client roles; service role + site admins only.
2. **Generation logic.** Add the passcode generator to `@myk9/core`: produces a 5-char code with role-letter prefix and 4 random lowercase alphanumeric chars (uses `crypto.getRandomValues`, no Math.random). Wire into the show-creation flow so 4 rows are inserted into `show_passcodes` whenever a show is created. Plaintext codes are returned to the show-creation response exactly once for the secretary's access card; thereafter, only hashes exist server-side. Apply the same logic to existing shows in a backfill migration (each existing show gets 4 fresh random codes; old derived codes stop working).
3. **`validate_passcode(code text)` RPC.** `SECURITY DEFINER`, returns `{show_id uuid, role text}` or null. Hashes input with the same algorithm used at generation, selects from `show_passcodes`. Rate-limited per the Security section.
...
8. **Tests:**
   - Passcode generator unit tests: shape is `[a|j|s|e][a-z0-9]{4}`, output uses CSPRNG, 4 codes per show are distinct, collisions across shows are vanishingly improbable (statistical test on N=10000 generations).
   - `validate_passcode` tests: matching code returns `{show_id, role}`; non-matching returns null; case-insensitive on input; rate-limited.
   ...
   - Show-creation backfill test: creating a new show inserts 4 `show_passcodes` rows; deleting a show cascades them away.
