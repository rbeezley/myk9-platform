...
It turns the long-term goal into observable criteria, evidence, and follow-up work. **Linear (`MYK9-*`) is the task backlog** — `OPEN-TODOS.md` was retired 2026-07-16, so any reference to it below is stale. This document is the measurement frame for deciding what belongs in that backlog and what should be prioritized next; the ordered execution sequence lives in [`operations/go-live-runbook.md`](../operations/go-live-runbook.md).
...
- **a named club has committed to run a named real show on a named date** — see below
...
### On the commercial gate

Every other criterion above can be satisfied alone at a keyboard. That is the
failure mode this first bullet exists to close: a definition made entirely of
internal quality can be met indefinitely without ever testing whether anyone wants
the product, and the result is not shipping something bad — it is shipping
something excellent that nobody was waiting for.

It is deliberately not "we have talked to clubs" or "a club is interested." The
gate is a specific club, a specific show, and a date on a calendar, because only
that version can fail. Reasoning:
[`business-model-assessment.md`](business-model-assessment.md) § 3.

Note the distinction from "2-3 real non-technical users complete the primary
journeys unassisted" above. That is a usability gate and it is a good one. This is
a demand gate. Passing one says nothing about the other.
...
- no Red dimensions
- **no Unknown dimensions** — an unmeasured dimension is not a passing one (clarified 2026-07-26)
...
| Show-day reliability       | Scoring, class status, run order, check-in, scratches, move-ups, late entries, announcements, and wrap-up work under real show pressure.                                     | End-to-end show-day rehearsal; targeted regression tests; no P0/P1 show-day bugs.                                                                                     | Yellow — Show-day walk 2026-06-17 ([`05-showday-walk`](../audits/2026-06-ux-journeys/05-showday-walk-2026-06-17.md)): **secretary half coherent** (status, scratches/pull, move-ups, announcements walked). S1 public-results deep-link fixed + S2/S3 ringside (judge assignment / passcode) seeds added ([#819](https://github.com/rbeezley/myk9-platform/pull/819)). The 2026-07-20 Class Operations Cockpit rehearsal added two-secretary shared-staging proof for manual Class status, Actual Start, Revised Expected Start, already-open `/at-show` convergence, and offline expected-start replay ([evidence](../../openspec/changes/secretary-class-operations-cockpit/verification-evidence.md)). **Gaps:** ringside judge/steward scoring phases are not yet fully re-walked end-to-end; S4 (withdrawn entry counted live: 9 vs 8 / "2/3 complete") + S5 (Saturday trial badge contradiction) remain minor findings; the rehearsal covered cockpit timing/status rather than the entire show-day scoring and closeout loop.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| Offline-first behavior     | Core ringside and secretary workflows either work offline or fail with clear recovery. No lost scores, duplicate placements, or silent sync failures.                        | Offline/reconnect tests; replication queue inspection; staging rehearsal.                                                                                             | Yellow — Strong code+test evidence: replication-core extracted with direct tests (Wave C — mutation ordering, queue capacity, localStorage backup, OCC empty-update, conflict row-state), replication-conflict mutation baseline 77.24% ([`plan-dynamic-qa-infrastructure.md`](../archive/plan-dynamic-qa-infrastructure.md)), failed-mutation persistence + synchronous backup hardened (pre-launch audit). On 2026-07-20 an offline secretary Revised Expected Start change saved locally and converged to a second secretary and already-open `/at-show` after reconnect; append-only Paperwork Print offline/replay and concurrent-confirmation paths have focused tests ([evidence](../../openspec/changes/secretary-class-operations-cockpit/verification-evidence.md)). **Gap:** the full ringside scoring → reconnect → results/closeout staging rehearsal remains unverified.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| Operational readiness      | Deploys, migrations, environment config, monitoring, rollback, and support paths are documented enough for launch.                                                           | Deployment checklist; migration history; staging smoke; support/runbook docs.                                                                                         | Yellow — Error observability live in production (Sentry SDK + PII scrubber + role-surface boundaries, [#723](https://github.com/rbeezley/myk9-platform/pull/723); end-to-end verified); launch-milestone QA checklist codified ([`launch-milestone-qa-checklist.md`](../launch-milestone-qa-checklist.md)); monthly dependency-audit cron; migration lineage tracked. July 11 agent-owned remediation is merged and deployed: FORCE-RLS drift is zero, health snapshots/watchdog are live, SA-023/024/025/028/029/030 have focused runtime evidence, advisors are dispositioned, and the post-#1294 migration dry run reports the remote database is up to date. Custom SMTP is live at a 100/hour Auth limit with branded Gmail delivery verified. Bounded Resend retry runtime acceptance is complete across all 12 affected functions: version/rollback evidence, fail-closed checks, delivered auth/registration logs, a created/resolved and mailbox-confirmed durable alert, and a provider-safe real-429 recovery proof. **Stripe enforcement is complete:** shared capacity, waitlist notification dispatch, in-place offer payment/decline, and sandbox reconciliation are merged, deployed, and archived. Phase 0.4 Edge-function drift recovery is complete: deployed-ahead `stripe-upgrade-subscription` source recovery ([#1313](https://github.com/rbeezley/myk9-platform/pull/1313)) is deployed and bundle-verified, and the four-function helper batch is deployed and smoke-verified. Legacy `send-notification` was retired after a zero-event 30-day log review. **Gaps:** operator Stripe/legal/data/device/browser/Sentry/DNS gates remain; Stripe go-live runbook + treasurer onboarding guide open (Lane 4); CI-gated Vercel deploys + E2E-blocking not yet enabled (Lane 3); rollback path not yet rehearsed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| Performance & capacity     | The platform serves a realistic show weekend — ~300–800 entries with 50+ concurrent ringside devices — within the load harness' documented targets, on a known compute tier. | Recorded run of `apps/myk9show/src/test/load/` against a ~500-entry fixture; peak CPU; peak connections vs the 60 cap; p95 scoring-write latency; `40001` retry rate. | **Unknown** — dimension added 2026-07-26 ([`launch/go-live-2026-07-26.md`](../launch/go-live-2026-07-26.md) § 8). A full harness exists (`k6-load-tests.js`, `artillery-config.yml`, `playwright-load-tests.spec.ts`, `DatabaseLoadTests.ts`, `LoadTestFramework.ts`, `LoadTestRunner.ts`) with a README specifying 100+ concurrent users, p95 <200 ms API, <3 s page loads, <5% error rate, >100 req/s — but **no rehearsal result has ever been recorded** and it runs in no workflow, so it may be stale (predates the ringside/`at-show` consolidation). Supporting signals on 2026-07-26: **309** performance advisors (73 `auth_rls_initplan`, 72 `multiple_permissive_policies`, 58 unindexed FKs, 103 unused indexes, 1 connection-headroom); `max_connections` **60** with **17** used at idle; on a 50 MB / 33-entry DB, `judge_assignments` at **650:1** and `show_passcodes` at **~30,000:1** seq-to-index scans (root cause tracked as MYK9-114, _not_ the `auth_rls_initplan` fix). Per the scoring model, treat Unknown as Yellow. **Green requires a rehearsal that PASSES the thresholds for the scenario named below, not merely one that was recorded** — a run missing them is Yellow, and Yellow does not close gate **G9**. The harness defines _per-scenario_ thresholds (`LoadTestFramework.ts:32` `PERFORMANCE_TARGETS`), so a flat number is ambiguous: **Normal** 100 users / ≤5% errors, **Peak** 250 users / ≤10%, **Stress** 500 users / ≤25%, with p95 ≤200 ms API, ≤300 ms search, ≤500 ms realtime, ≤3 s page. **G9 closes on the Normal scenario (100 concurrent users, ≤5% errors, the p95 targets above, plus its declared `throughputMin: 50` req/s and `availability: 99.5`%)** — all four dimensions, not latency alone. **Critically, the stock `normal_load` scenario does not by itself satisfy this dimension's Ready Means:** its 100 users are 70 exhibitors / 15 secretaries / **10 judges** / 3 admins / 2 anon with browse-heavy workflows (`LoadTestFramework.ts:477-482`), so it never exercises 50+ concurrent ringside devices. The gate scenario must be refreshed to carry ≥50 concurrent ringside scoring sessions, or replaced by a dedicated show-day scenario. Peak and Stress runs are informational and judged against their own looser thresholds; they neither close nor block G9. (Caveat for whoever runs it: `LoadTestFramework.ts:448` hardcodes `errorRate.normal` when generating its report, so the harness grades every scenario against 5% regardless — read Peak/Stress numbers yourself rather than trusting its ✅/❌.) Note also that the harness' Playwright entry points are currently broken: `test:load:playwright` / `:quick` target `src/test/load/` while `playwright.config.ts:14` sets `testDir: './src/test/e2e'`, so Playwright exits **1** with "No tests found" without running a single load scenario (verified 2026-07-26). It fails loudly rather than faking a pass, but this half of the harness has never actually been exercised; repairing it is step (a) of the work. Gate **G9**; tracked as MYK9-109. |
| Data durability            | Backups are verified enabled, a restore has actually been performed, RPO/RTO are stated, and a single-show partial recovery is documented.                                   | Dashboard confirmation of PITR + retention; a performed restore with elapsed time; written RPO/RTO; partial-recovery procedure.                                       | **Red** — dimension added 2026-07-26. Red per this document's own definition ("risks data loss"): we cannot currently demonstrate that we can recover data. The only recovery guidance in the whole live docs tree is one cell of the runbook's rollback appendix — _"use PITR via Supabase support as last resort"_ — and PITR has **not been confirmed enabled** on `sojmvhhwsjxmfistvzbe`; on some plans it is a paid add-on, so that sentence may be false rather than merely vague. No RPO/RTO, no tested restore, and no procedure for the realistic incident (one show's entries/scores destroyed mid-weekend, recovered without rolling back every other show). After launch this covers real money and irreplaceable results. Flips to Yellow once the posture is merely confirmed; **Green requires a restore actually performed and verified within the agreed RTO** — elapsed time is the observed recovery time, not a redefinition of the objective. Yellow does not close gate **G8**, which blocks the Stripe cutover. Gate **G8**; tracked as MYK9-110.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| Test and CI health         | Relevant tests pass consistently, `main` is deployable, and known flaky/hanging tests are documented.                                                                        | `pnpm typecheck`; focused unit/e2e tests; CI status; known-issues list.                                                                                               | **Yellow** (was Green until 2026-07-26 — see ‡ below) — Dynamic QA Phase 7 final regression green on a clean worktree (typecheck ✅, lint ✅, packages 11/11, app suite ~927 files / ~9061 tests, 0 fail); test-isolation campaign complete and `--sequence.shuffle` enabled in CI ([#749](https://github.com/rbeezley/myk9-platform/pull/749)/[#752](https://github.com/rbeezley/myk9-platform/pull/752)/[#755](https://github.com/rbeezley/myk9-platform/pull/755)/[#761](https://github.com/rbeezley/myk9-platform/pull/761), 18-seed sweep SHUFFLE-CLEAN); bundle-budget + a11y-smoke CI gates ([#738](https://github.com/rbeezley/myk9-platform/pull/738)/[#750](https://github.com/rbeezley/myk9-platform/pull/750), A11y smoke now a required check); code-quality ratchet baselines. Known watch-item: ~30 wall-clock perf asserts (reactive only).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| Status      | Dimensions                                                                                                                                         |
| ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Green**   | Secretary golden path*, Exhibitor golden path*, Admin minimum†                                                                                     |
| **Yellow**  | Show-day reliability, Offline-first behavior, Data correctness, Reports and official forms, UX clarity, Operational readiness, Test and CI health‡ |
| **Unknown** | **Performance & capacity** (added 2026-07-26)                                                                                                      |
| **Red**     | **Data durability** (added 2026-07-26)                                                                                                             |
...
‡Test and CI health dropped **Green → Yellow on 2026-07-26**: `main` is deployable and the unit suite
is green (1,594 files / 14,590 tests, 0 failures, verified that day), but the scheduled
`nightly-health` workflow had failed **six consecutive nights** (missing shared-package build →
`@myk9/core` unresolvable → every route-health spec fails; MYK9-106) and the stateful Playwright
Regression suite has had no green run since 2026-07-18 (MYK9-107). Returns to Green on one green
scheduled nightly.

**Status revised 2026-07-26 — this section previously read "no Red dimensions, no Unknown dimensions,
and no open P0/P1."** That was true of the eight dimensions then listed, but two dimensions were
**missing from the scorecard entirely**, so the two hardest launch blockers had nowhere to appear:

- **Data durability** is now **Red** — we cannot demonstrate we can recover data (G8 / MYK9-110).
- **Performance & capacity** is now **Unknown** — no load rehearsal has ever been recorded
  (G9 / MYK9-109).

The launch gate (all Primary Green + no Red + no Unknown + no P0/P1 + real-user testing done) is
therefore **not met, and further from met than the previous revision implied**. Ordering by risk:
**data durability first** (it is the only Red, and it gates the Stripe cutover — do not take real
money before a restore is proven), then **capacity**, then the Yellow gaps, with **real-user testing
(Lane 1.7)** last against a frozen product. Live tracking is Linear `MYK9-*`; `OPEN-TODOS.md` was
retired 2026-07-16 and references to it here are stale.
