...
- Phase 4 sunset happens after the pilot show plus one more clean show. As of the 2026-06-04 Android real-device retest, the push-tap acceptance gate is closed; the live Phase 4B sunset/redirect flip still requires explicit approval before changing the standalone myK9Q deployment or origin migration behavior.
...
- **2026-06-04 (Phase 3 acceptance retest)** — User confirmed the post-fix real-device push tap acceptance test was run on an Android phone. This closes the remaining Phase 3 push-tap gate: account-keyed targeted message notifications now open the exhibitor inbox thread (`/messages/:showId`) instead of the ringside `/at-show` surface. The earlier 2026-05-31 synthetic pilot remains the evidence for inbox persistence, account/passcode fanout, fresh `/at-show` suppression, stale subscription cleanup, and response counts.
