...
| `apps/myk9show/src/test/e2e/registration/wizardRehydrate.spec.ts`           | Exhibitor entry survives a browser reload and resumes at the saved step. Runs on `mobile-chrome` as well as the desktop projects since MYK9-543.                                                                                               |
...
| `apps/myk9show/src/test/e2e/registration/chipReclick.spec.ts`               | Re-clicking a selected class chip deselects it: cart count 1 → 0 → 1 → 0 with no console error and no duplicate cart line (MYK9-530). Its phone-width skip was removed by MYK9-543; it now runs on `mobile-chrome` too.                        |
