...
const BrowseShowsPage: React.FC = () => {
  ...
  // Exhibitor discovery aid: when the general browse list has shows but none
  // are currently open for entry, say so plainly instead of leaving a novice
  // clicking into closed shows one by one. Scoped to the "all" tab so
  // management and judge tabs never surface it, and hidden when the list is
  // empty (the EmptyState already covers that) or any show can be entered.
  const noneEnterableNotice = useMemo(() => {
    if (selectedTab !== 'all' || enhancedShows.length === 0) return null;
    if (enhancedShows.some(show => getEntryStatus(show).canEnter)) return null;
    return (
      <Alert className="mt-4">
        <CalendarClock className="h-4 w-4" />
        <AlertTitle>No shows are open for entries right now</AlertTitle>
        <AlertDescription>
          These shows aren’t accepting online entries yet. Open any show to see its schedule and
          when its entry window opens.
        </AlertDescription>
      </Alert>
    );
  }, [selectedTab, enhancedShows]);
