...
const COLUMNS: ColumnDef<EnhancedShow>[] = [
  {
    key: 'name',
    label: 'Name',
    className: 'w-[220px]',
...
function renderCell(show: EnhancedShow, column: ColumnDef<EnhancedShow>) {
  switch (column.key) {
    case 'name':
      return (
        <td className="px-4 py-3">
          <div className="min-w-0">
            ...
            {show.events.length > 0 && (
              <div className="text-xs text-muted-foreground truncate">{show.events.join(', ')}</div>
            )}
          </div>
        </td>
      );
...
export const ShowsTableView: React.FC<ShowsTableViewProps> = ({ shows }) => {
