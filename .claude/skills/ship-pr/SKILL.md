...
bash scripts/qa/claude-review.sh --wait 240 $PR_NUMBER           # ordinary detached polling; the supervised workaround (references/codex-runs-claude-review.md) has a 25-minute limit
#   0 clean and evidence posted · 1 findings posted · 2 did not run (not a verdict) · 3 still running, call again
```

**Codex: read `references/codex-runs-claude-review.md` before the first attempt.** The review needs the macOS Keychain and the network, which `workspace-write` denies; that file has the escalation, the supervised `--detach`/`--wait` workaround, and the rules for replacing a dead or stale attempt. Never substitute a same-harness subagent review.
