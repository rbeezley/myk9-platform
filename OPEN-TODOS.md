...
- [ ] **Show-map tree extensions for show-day use** — Discrete enhancements to `ShowMapTab` so it can serve as the spine of the Today and Wrap-up tabs. Each sub-item is independently shippable.
  ...
  - [x] **Inline row actions v1 — three-dot + right-click + keyboard, with smart "Recommended" section** — Single `<RowActionsMenu>` component invoked by three triggers: visible three-dot icon, native `contextmenu` right-click, and `Enter`/`Space` on a focused row. All three open the same menu instance — guard against divergence in code review. Per-node-type shipped mix: entry rows get Scratch, Move Up, Mark Checked-In, Message Handler; class rows get Open Class, Open Scoring, Print Check-In Sheet; trial rows get Open Schedule, Print Trial Reports.
    ...
    **Phase F closeout progress:** [x] Keyboard trigger + focus contract — shipped via PR #278; focused `ShowMapStructureTable` tests and myK9Show typecheck passed. [x] Class/trial destination audit — shipped via PR #279 with PR #280 CI assertion hotfix; class/trial row actions now use verified secretary/report destinations with focused action and ReportsPage tests. [x] Recommended-section hardening — shipped via PR #281; Recommended now uses an explicit two-action cap, deterministic shared ordering, shared why-lines, and keeps disabled navigation actions in the full list with plain-English reasons. [x] Final trigger/action regression walk — implemented 2026-05-21; focused regression coverage walks three-dot, right-click, keyboard, navigate, mutation, dialog, and disabled navigation behavior.

  - [ ] **Show-map ARIA tree roving focus** — Follow-up accessibility pass for the tree widget itself: replace per-row tab stops with roving `tabIndex`, add Arrow key movement, and keep `Enter` / `Space` opening row actions from the focused treeitem. Phase F v1 closed keyboard access to row actions, but full ARIA tree navigation remains explicit follow-up work.
  - [ ] **Class status quick actions** — Add row actions for class lifecycle changes such as Mark Class Started / Complete only after choosing the offline-safe class-status mutation path and the confirmation/error behavior. Phase F v1 deliberately kept class row actions to existing route/report destinations rather than adding a new mutation workflow.
