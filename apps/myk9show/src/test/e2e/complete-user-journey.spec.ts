...
test.describe('Complete User Journey - Real World Flow', () => {
  test('should sign up, authenticate, browse shows, and view show details', async ({ page }) => {
    ...
    const showElements = await page
      .locator('[data-testid="show-card"], .show-card, .card, [class*="show"], [class*="card"]')
      .count();
    ...
    if (!hasShowContent) {
      ...
      const errorElements = await page
        .locator('text=/error/i, text=/no.*found/i, text=/loading/i')
        .all();
    ...
    const detailsButtons = await page
      .locator(
        'text=/view.*details/i, text=/details/i, a[href*="show"], button:has-text("Details")'
      )
      .all();
    ...
    if (detailsButtons.length > 0) {
      ...
      const hasDetailsContent =
        detailsPageText && !detailsPageText.toLowerCase().includes('no shows available');
      ...
      if (!hasDetailsContent) {
        ...
        const noShowsMessage = await page
          .locator('text=/no.*show/i, text=/not.*found/i, text=/no.*available/i')
          .first()
          .textContent();
  ...
  test('should handle navigation between Browse Shows and Show Details', async ({ page }) => {
    ...
    if (showLinks.length > 0) {
      ...
      if (isShowDetailsPage) {
        ...
        console.log(
          '📄 Page contains "no shows available":',
          bodyText?.includes('no shows available')
        );
      }
    }
  });
