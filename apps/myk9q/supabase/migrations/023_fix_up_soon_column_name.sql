-- =====================================================
-- Migration 023: Fix column name in up_soon trigger
-- =====================================================
-- Problem: notify_up_soon() references e.call_name but column is e.dog_call_name
-- Solution: Update query to use correct column name
-- =====================================================
  ...
  IF NEW.is_scored = true AND (OLD.is_scored IS NULL OR OLD.is_scored = false) THEN
    ...
    FOR next_entry IN
      SELECT
        e.id,
        e.armband_number,
        e.dog_call_name,  -- FIXED: was e.call_name
        e.handler,
        e.class_id,
        COALESCE(NULLIF(e.exhibitor_order, 0), e.armband_number) AS effective_run_order
      FROM entries e
    ...
    LOOP
      ...
      v_payload := jsonb_build_object(
        ...
        'body', next_entry.dog_call_name || ' (#' || next_entry.armband_number || ') is ' || v_dogs_ahead || ' dogs away in ' || v_class_name,  -- FIXED: was next_entry.call_name
        'url', '/entries',
        'armband_number', next_entry.armband_number,
        'dog_name', next_entry.dog_call_name,  -- FIXED: was next_entry.call_name
...
Fixed: Uses dog_call_name instead of call_name (correct column name).';
