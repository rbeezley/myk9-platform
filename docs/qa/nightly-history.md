...
- **Route sweep:** pass
- **Active specs:** Vitest 18/18; Playwright 44/44; route sweep 12/12 role+viewport checks
- **Failures:** Initial active Playwright run failed `33/44` with 7 failed and 4 skipped. Failures were a mix of stale secretary dashboard assertions, a show-wizard page object waiting on `networkidle`, shared-write assumptions in `secretary-entry-walk.spec.ts`, and a durable PostgREST `57014` timeout from the people query used by the Show Creation Wizard official pickers. Findings closed in this run: `QA-NETWORK-ERROR-010` and `QA-TEST-FLAKE-011`.
- **Fixes made:** `apps/myk9show/src/services/database/users/reads.ts`, `apps/myk9show/src/services/database/entries/secretary.ts`, `apps/myk9show/src/hooks/useEntryManagementData.ts`, `apps/myk9show/src/test/e2e/cross-role-workflows.spec.ts`, `apps/myk9show/src/test/e2e/page-objects/ShowCreationWizardPage.ts`, `apps/myk9show/src/test/e2e/secretary-entry-walk.spec.ts`, `apps/myk9show/src/test/e2e/secretary/show-wizard-officials.spec.ts`, `apps/myk9show/src/test/e2e/uat/secretary/critical-path.spec.ts`, `docs/qa/findings.md`, `docs/qa/nightly-history.md`
- **Demotions/promotions:** none
- **Notes:** Ran from clean synced `main`, then created local branch `codex/nightly-qa-2026-05-26` before edits. Proofs passed: promoted Vitest Nightly (`18 passed`; initial transient timeout in `useInfiniteScroll.performanceCaching.test.ts` passed in focused and full reruns), `pnpm --filter @myk9/show typecheck`, `pnpm qa:e2e-map:check`, focused secretary replay (`10 passed`), full active Nightly Playwright command (`44 passed`, retries disabled), and route sweep across public, exhibitor, secretary, judge, club-admin, and admin route groups at desktop plus 375px mobile (`12/12`). Route sweep hard-gated app-owned 4xx/5xx responses, page errors, permission-denied states, and unresolved busy regions; generic browser-side `TypeError: Failed to fetch` noise from Supabase-backed background sync/auth reads was treated as environment noise rather than a durable finding.
