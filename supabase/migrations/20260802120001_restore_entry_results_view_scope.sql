-- MYK9-169: recreate the entry-results view after adding show-scoped
-- secretary IDs to private.entry_results_caller_context().
--
-- The preceding migration drops this dependent view before changing the
-- private function's composite return type. Keeping the view replacement in
-- its own migration keeps each migration reviewable while preserving the
