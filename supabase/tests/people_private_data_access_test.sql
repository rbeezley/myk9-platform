...
-- The fixture is transaction-local and proves the caller boundary: self and
-- site admin may read; all managers and exhibitors are denied absent consent.
...
VALUES
  ...
  ('00000000-0000-0000-0000-000000664022', 'Deleted', 'Entry Target', '00000000-0000-0000-0000-000000664110'),
  ('00000000-0000-0000-0000-000000664023', 'Unlinked', 'Target', NULL),
...
UPDATE public.people SET email = 'Handler@Example.com'
WHERE id = '00000000-0000-0000-0000-000000664014';
...
-- The stale handler FK points at Handler, but the printed name resolves to the
-- owner. This legacy relationship is not trusted private-read provenance.
...
INSERT INTO public.people_private (person_id, date_of_birth, junior_handler_numbers)
VALUES (
  '00000000-0000-0000-0000-000000664022', DATE '2015-04-02', '{"AKC":"664-DELETED-ENTRY"}'::jsonb
);
INSERT INTO public.people_private (person_id, date_of_birth, junior_handler_numbers)
VALUES (
  '00000000-0000-0000-0000-000000664023', DATE '2015-04-03', '{"AKC":"664-UNLINKED"}'::jsonb
);
INSERT INTO public.entries (id, show_id, handler_id, entry_status, payment_status)
VALUES (
...
INSERT INTO public.entries (id, show_id, handler_id, entry_status, payment_status, deleted_at)
VALUES (
  '00000000-0000-0000-0000-000000664022',
  '00000000-0000-0000-0000-000000664003',
  '00000000-0000-0000-0000-000000664022',
...
DECLARE
  ...
  deleted_entry_target uuid := '00000000-0000-0000-0000-000000664022';
  unlinked_target uuid := '00000000-0000-0000-0000-000000664023';
...
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE c.oid = 'public.people_private'::regclass
      AND acl.grantee = 0
  ) THEN
    RAISE EXCEPTION 'FAIL PUBLIC retained a people_private table privilege';
  END IF;
  ...
  IF EXISTS (
    ...
    WHERE p.oid IN (
      'public.get_people_private(uuid[])'::regprocedure,
      'public.update_person_with_private(uuid,jsonb,jsonb)'::regprocedure,
      'public.can_read_people_private(uuid)'::regprocedure,
      'public.can_write_people_private(uuid)'::regprocedure,
      'public.sync_people_private_from_legacy()'::regprocedure
    )
      AND acl.grantee = 0
      AND acl.privilege_type = 'EXECUTE'
  ) THEN
    RAISE EXCEPTION 'FAIL PUBLIC retained private people function EXECUTE';
  END IF;
  ...
  END IF;
  IF NOT has_table_privilege('service_role', 'public.people_private', 'SELECT')
     OR NOT has_table_privilege('service_role', 'public.people_private', 'INSERT')
     OR NOT has_table_privilege('service_role', 'public.people_private', 'UPDATE')
     OR NOT has_table_privilege('service_role', 'public.people_private', 'DELETE') THEN
    RAISE EXCEPTION 'FAIL service_role lacks required people_private privileges';
  END IF;
  IF EXISTS (
    SELECT 1
    FROM pg_attribute a,
         LATERAL aclexplode(a.attacl) acl
    WHERE a.attrelid = 'public.people_private'::regclass
      AND a.attnum > 0
      AND acl.grantee IN (0, (SELECT oid FROM pg_roles WHERE rolname = 'anon'))
  ) THEN
    RAISE EXCEPTION 'FAIL PUBLIC or anon retained people_private column privileges';
  END IF;
  ...
  SELECT count(*) INTO visible_rows FROM public.people_private WHERE person_id = handler_id;
  IF visible_rows <> 1 THEN RAISE EXCEPTION 'FAIL self cannot read private row through RLS'; END IF;
  ...
  PERFORM public.update_person_with_private(
    handler_id,
    '{"phone":"self-save","email":"handler@example.com"}'::jsonb,
    '{"date_of_birth":"2012-04-03","junior_handler_numbers":{"AKC":"664-JR-2"}}'::jsonb
  );
  SELECT phone INTO public_phone FROM public.people WHERE id = handler_id;
  SELECT date_of_birth, junior_handler_numbers
  INTO private_dob, private_numbers
  FROM public.people_private WHERE person_id = handler_id;
  ...
  BEGIN
    PERFORM public.update_person_with_private(
      handler_id,
      '{"email":"different@example.com"}'::jsonb,
      '{}'::jsonb
    );
  ...
  IF NOT writes_denied THEN
    RAISE EXCEPTION 'FAIL linked person changed sign-in email through profile RPC';
  END IF;
  ...
  -- A show manager may still update public fields but cannot read private data.
  ...
  SELECT count(*) INTO visible_rows FROM public.get_people_private(ARRAY[handler_id]);
  IF visible_rows <> 0 THEN RAISE EXCEPTION 'FAIL related show manager can read private profile'; END IF;
  SELECT count(*) INTO visible_rows FROM public.people_private WHERE person_id = handler_id;
  IF visible_rows <> 0 THEN RAISE EXCEPTION 'FAIL related show manager can read private row through RLS'; END IF;
  SELECT count(*) INTO visible_rows FROM public.get_people_private(ARRAY['00000000-0000-0000-0000-000000664017'::uuid]);
  IF visible_rows <> 0 THEN RAISE EXCEPTION 'FAIL related show manager can read an owner's private profile'; END IF;
  SELECT count(*) INTO visible_rows FROM public.get_people_private(ARRAY[empty_person]);
  IF visible_rows <> 0 THEN RAISE EXCEPTION 'FAIL manager can read an empty private profile'; END IF;
  ...
  SELECT count(*) INTO visible_rows FROM public.get_people_private(ARRAY[deleted_entry_target]);
  IF visible_rows <> 0 THEN RAISE EXCEPTION 'FAIL deleted entry still authorizes private profile read'; END IF;
  ...
  SELECT date_of_birth, junior_handler_numbers INTO private_dob, private_numbers
  FROM public.get_people_private(ARRAY[handler_id]);
  IF private_dob IS DISTINCT FROM DATE '2012-04-03'
     OR private_numbers IS DISTINCT FROM '{}'::jsonb THEN
    RAISE EXCEPTION 'FAIL manager private patch changed private fields';
  END IF;
  PERFORM set_config('request.jwt.claim.sub', related_manager::text, true);
  PERFORM set_config('request.jwt.claims', json_build_object('sub', related_manager, 'role', 'authenticated')::text, true);

  -- Adversarial reassignment: manager can change handler_id on an entry in
  -- their show under current entry RLS. That mutable field must not authorize
  -- private reads. If a future policy blocks the reassignment, that is also a
  -- safe outcome; if it succeeds, the private RPC must still deny the target.
  writes_denied := false;
  BEGIN
    UPDATE public.entries
    SET handler_id = unlinked_target
    WHERE id = '00000000-0000-0000-0000-000000664004';
    GET DIAGNOSTICS affected_rows = ROW_COUNT;
  EXCEPTION WHEN insufficient_privilege THEN
    writes_denied := true;
  END;
  IF NOT writes_denied THEN
    IF affected_rows NOT IN (0, 1) THEN
      RAISE EXCEPTION 'FAIL unexpected row count from entry handler reassignment: %', affected_rows;
    END IF;
    SELECT count(*) INTO visible_rows FROM public.get_people_private(ARRAY[unlinked_target]);
    IF visible_rows <> 0 THEN
      RAISE EXCEPTION 'FAIL manager reassigned handler_id and obtained unrelated private data';
    END IF;
    UPDATE public.entries
    SET handler_id = '00000000-0000-0000-0000-000000664014'
    WHERE id = '00000000-0000-0000-0000-000000664004';
  END IF;
  ...
  BEGIN
    PERFORM public.update_person_with_private(
      '00000000-0000-0000-0000-000000664023',
      '{"phone":"unrelated-save"}'::jsonb,
      '{}'::jsonb
    );
  EXCEPTION WHEN insufficient_privilege THEN
    writes_denied := true;
  END;
  IF NOT writes_denied THEN
    RAISE EXCEPTION 'FAIL unrelated manager updated unlinked person';
  END IF;
  ...
  SELECT count(*) INTO visible_rows FROM public.people_private WHERE person_id = handler_id;
  IF visible_rows <> 0 THEN RAISE EXCEPTION 'FAIL unrelated manager can read private row through RLS'; END IF;
  ...
  SELECT count(*) INTO visible_rows FROM public.people_private WHERE person_id = handler_id;
  IF visible_rows <> 0 THEN RAISE EXCEPTION 'FAIL exhibitor can read private row through RLS'; END IF;
  ...
  SELECT count(*) INTO visible_rows FROM public.get_people_private(ARRAY[deleted_handler]);
  IF visible_rows <> 0 THEN RAISE EXCEPTION 'FAIL site admin can read deleted-person private details'; END IF;
  PERFORM public.update_person_with_private(
    handler_id,
    '{}'::jsonb,
