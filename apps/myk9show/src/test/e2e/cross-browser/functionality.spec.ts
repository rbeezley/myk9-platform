...
test.describe('Cross-Browser Functionality Tests', () => {
  ...
  test.describe('Template System Workflows', () => {
    ...
    test('sport rules render read-only', async ({ page, browserName }) => {
      // Was 'template creation workflow' against /admin/template-management — a route
      // that never existed, so the test could only ever have passed vacuously. The
      // authoring surface is gone; /admin/templates is now read-only.
      ...
      console.log(`✅ ${browserName}: Sport rules read-only view rendered`);
    });
