...
| **D** | Move ringside domain stores | low | **Shipped 2026-05-26 (PR #383).** Audit revealed only `entryStore` was a live extraction candidate: apps/myk9q's `timerStore.ts` was dead duplicate code (live version already in `@myk9/scoring`); `scoringStore.ts` was already a 2-line shim. Moved `entryStore` to ringside with factory pattern matching `@myk9/scoring`'s `createTimerStore()`; deleted dead `timerStore.ts`; left `scoringStore` shim alone. Net −305 LOC. The Q5 table-subclass DI question did NOT surface in PR D (entryStore is pure Zustand, no replication deps) — it gets confronted properly in PR E0.5 below. |
| **E0** | Move ClassList pure helpers | low | `ClassList.helpers.ts`, `utils/noviceClassGrouping`, `utils/statusFormatting`. Well-tested (~400 LOC including tests). Establishes "page-internal pure helpers" as a ringside surface category. Real consumer = apps/myk9q's ClassList re-imports from `@myk9/ringside`. |
| **E0.5** | Design + ship `RingsideProvider` context (Q5 DI surface) | medium | Confront the typed-table dependency injection question from Q5. Define `RingsideProvider` props (replicated tables, auth context, permission hook). Ship with a stubbed micro-consumer in ringside to prove the API shape before moving 6700 LOC of page code. ~300 LOC. **Highest-leverage decision in Phase 0** — gets its own PR so review focuses on API design, not file moves. |
| **E1** | Move ClassList page | medium | ~26 files, ~6700 prod LOC + 2079 test LOC. DI pattern from E0.5 in place; helpers already moved in E0. Largest individual page move. Diff stays manageable because most files are pure `git mv` + import-path rewires. |
| **E1.5** | Backfill EntryList smoke/integration tests | medium | EntryList currently has **21 LOC of test coverage across 7956 LOC of prod code**. Untenable to move that surface without coverage. Minimum scope: `useEntryListData`, `useDragAndDropEntries`, `useResetScore` — the three hooks whose silent behavior changes would be hardest to catch in code review. ~300–400 LOC of new tests. |
| **E2** | Move EntryList page | medium-high | ~32 files, ~8000 prod LOC. Same DI pattern as E1, plus 10 dialog dependencies — some may need to move alongside (transitive necessity) or be injected via slots. Gated on E1.5 landing. |
...
| **G** | Move remaining dialogs + ringside hooks | low | Mop-up. Some dialogs may already be in `@myk9/ringside` by this point if E2 had to pull them transitively; G covers whatever's left. **DONE (see [`phase-0-pr-g-remaining-dialogs-hooks.md`](phase-0-pr-g-remaining-dialogs-hooks.md)):** discovery found nothing left to move — dialogs are slot-injected (host owns the components), remaining ringside-candidate hooks are already-moved / host-coupled (stay app-side per §3) / Nationals (deferred), and `useClassFilters`+`useClassSelection` were **dead** (zero importers) so they were deleted, not moved. **This closes Phase 0.** |
