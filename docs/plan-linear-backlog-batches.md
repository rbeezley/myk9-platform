...
> **Status:** Active — Batches 0–3 complete except MYK9-204 (operator-blocked on Richard). **Batch 3.5 Lane A and Batch 4 are complete:** MYK9-233, MYK9-236, and MYK9-211 are Done and deployed as of 2026-08-24. **Read "[HANDOFF 2026-08-24]" under Batch 3.5 before starting anything** — it records what was already claimed by another agent, three issues filed after the last registry refresh, and the verification traps this batch has already hit. Next: Batch 3.5 Lane B begins with MYK9-245, while Lanes C–D may run in parallel, then Batch 5.
...
- **Lane 3.5B — payment and payout reconciliation surfaces.** Start with High-priority [MYK9-245](https://linear.app/myk9-platform/issue/MYK9-245): decide whether a paid entry in a soft-deleted show remains visible to its exhibitor as cancelled (recommended) or disappears and therefore requires tightening the `entries_select` handler/owner arms. If it remains visible, reconcile the entry against its existing My Payments charge and make refund state legible on the existing entry surface; do not add a new page or duplicate payment UI. Update the pinning assertion in `admin_soft_deleted_show_visibility_test.sql` either way. Then [MYK9-235](https://linear.app/myk9-platform/issue/MYK9-235) (a refund against money never collected is invisible on every reconciliation surface), then [MYK9-234](https://linear.app/myk9-platform/issue/MYK9-234) (the ledger renders every row twice — the responsive two-copy antipattern; one node that reflows, not two CSS-hidden copies). 3.5B is the **symptom** side of 3.5A's ledger defect and does not substitute for it: making dropped rows visible on one page leaves the policy asymmetry intact for every other surface built on `manageable_show_ids()`.
