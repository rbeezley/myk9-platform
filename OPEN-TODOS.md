...
- [ ] **Phase 6 — bundle budget + a11y smoke + dependency cadence** — **6a DONE** 2026-06-15 (worktree `qa-phase6a-budgets`): consolidated two dead budget scripts into `scripts/check-bundle-budget.js` (parses `dist/index.html` for the real initial-load payload), wired into the CI `build` job, budgets baselined ratchet-down-only (initialJs 3300 / CSS 360 / maxChunk 1550 KB), 9 unit tests; `@axe-core/playwright` a11y smoke on 5 public landing pages (`test:a11y`), gating serious/critical. **6b PENDING user go-ahead** (shared-system): dependency cadence (monthly `pnpm audit`/`outdated` + review the `@supabase/supabase-js` override pin) and the weekly scheduled drift run — both `.github/**` scheduled jobs. Details in [`docs/plan-dynamic-qa-infrastructure.md`](docs/plan-dynamic-qa-infrastructure.md#phase-6--budgets-and-cadence).
