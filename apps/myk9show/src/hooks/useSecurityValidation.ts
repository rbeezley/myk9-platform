...
export function useSecurityValidation() {
   ...
   */
  const validateSecureForm = useCallback(
    <T extends Record<string, unknown>>(data: T, schema: z.ZodSchema<T>, csrfToken?: string) => {
      ...
      const suspiciousField = stringFields.find(
        ([, value]) => !SecurityValidation.isSafeInput(value as string)
      );
   ...
   */
  const validateSecureFile = useCallback((file: File, allowedTypes: string[], maxSize: number) => {
  ...
  const validateSecureEmail = useCallback((email: string) => {
    ...
    if (!SecurityValidation.isSafeInput(email)) {
      validator.custom(email, () => false, 'email', 'Email address contains invalid characters');
    }
...
export function useAuthSecurity() {
  ...
  const validateRegistration = useCallback(
    (data: {
      ...
      // Name validation
      const validator = new InputValidator();
      validator.safeString(data.firstName, 'firstName').safeString(data.lastName, 'lastName');

      if (!validator.isValid()) {
        const errors = validator.getErrors().reduce(
          (acc, err) => {
