...
| `apps/myk9show/src/test/e2e/tabsTriggerSelectedStyleGuard.spec.ts`          | MYK9-612: Edit Dog panel active-tab computed style pins the primitive's own opaque `bg-background` pill (the former Radix data-state overrides were dead and are deleted, not repointed). Exhibitor-authed, read-only, never saves.                                                                                                                                                          |
...
| `apps/myk9show/src/test/e2e/registration/wizardRehydrate.spec.ts`           | Exhibitor entry survives a browser reload and resumes at the saved step.                                                                                                                                                                       |
| `apps/myk9show/src/test/e2e/registration/checkoutCancelRecovery.spec.ts`    | Cancelled checkout returns to the wizard with the selections intact and amendable.                                                                                                                                                             |
| `apps/myk9show/src/test/e2e/registration/chipReclick.spec.ts`               | Re-clicking a selected class chip deselects it: cart count 1 → 0 → 1 → 0 with no console error and no duplicate cart line (MYK9-530).                                                                                                          |
